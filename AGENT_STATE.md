...
Update for the current slice: frame `move_ref_from` replacement coverage now has broader build-checked fuzz invariants matching `frame_move_ref_from_moves_references_and_unrefs_source`. The deterministic `avutil_core_models` fixture now covers replacing a nonempty destination frame, releasing the destination's old external plane owner, unreferencing the source frame, preserving moved PTS/data, and delaying source external owner release until the moved destination drops. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- `cargo test -p avutil frame_move_ref_from_moves_references_and_unrefs_source`
...
- No remaining failing commands in the latest frame `move_ref_from` replacement fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens frame `move_ref_from` replacement fuzz coverage. `Frame` already has a unit test proving `move_ref_from` releases an existing destination frame's external plane owner, moves PTS/data, unrefs the source frame, and keeps the moved source plane owner alive until the destination drops. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened frame `move_ref_from` replacement fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_move_ref_from_moves_references_and_unrefs_source`: `Frame::move_ref_from` releases an existing destination frame's external plane owner, moves PTS/data, leaves the source frame empty, and delays the moved source plane owner's release until the destination drops. Docs and the ledger now call out these `move_ref_from` ownership invariants explicitly. Validation passed with `cargo test -p avutil frame_move_ref_from_moves_references_and_unrefs_source`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
