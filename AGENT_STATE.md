...
Latest current-loop `avutil-frame` update: View ID side-data fuzz coverage now mirrors and strengthens the focused unit contracts for `AV_FRAME_DATA_VIEW_ID`. The deterministic `avutil_core_models` fixture preserves native signed `int` payloads for positive, negative, `i32::MIN`, and `i32::MAX` view identifiers, and rejects empty, short, and long payloads through raw parser and deferred side-data accessor paths. This remains build-checked fuzz coverage only; pinned-oracle differential tests, upstream FATE media parity, and actual local fuzz execution are still blocked.
...
This slice strengthens View ID side-data fuzz coverage. `FrameViewId` already had unit coverage for signed native-int parsing, malformed lengths, and non-view-id lookup absence, and now the unit test and `avutil_core_models` fixture both explicitly preserve positive, negative, `i32::MIN`, and `i32::MAX` payloads while rejecting empty, short, and long payloads through raw parser and deferred side-data accessor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
