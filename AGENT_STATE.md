...
Update for the current slice: `FrameExif::common_tags()` root TIFF `Orientation` and `ResolutionUnit` handling now has dedicated validation coverage as typed SHORT enums alongside the existing root descriptive/image-layout tags. Unit tests cover valid inline parsing plus malformed Orientation type, out-of-range Orientation value, malformed ResolutionUnit count, and out-of-range ResolutionUnit value, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid Orientation/ResolutionUnit paths plus enum-range accessor invariants. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` already exposed `Orientation` and `ResolutionUnit`; they now have dedicated root-tag validation coverage as typed SHORT enums alongside the existing root image-layout, document/page, camera-identity, software, date/time, artist, host-computer, predictor, and copyright tags. Unit tests cover valid inline parsing plus malformed Orientation type, out-of-range Orientation value, malformed ResolutionUnit count, and out-of-range ResolutionUnit value, and the `avutil_core_models` build-checked fuzz harness asserts deterministic valid and invalid paths plus enum-range accessor invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
