...
`fftools-ffprobe-mov-language-tag` is the current focus; the latest slice parses constrained MOV/MP4 `mdia/mdhd` packed ISO-639 language codes and renders them as `language` stream tags in `ffprobe-rs -show_streams` output.
...
Latest slice: add constrained MOV/MP4 language stream tags. The MOV demuxer now decodes valid packed ISO-639 `mdia/mdhd` language values, stores them as `language` track metadata, leaves zero or unknown encodings absent, and `ffprobe-rs` renders that metadata in JSON stream tags alongside `handler_name` for local MOV/MP4 fixtures. This still lacks pinned-oracle differential/FATE/fuzz coverage.
