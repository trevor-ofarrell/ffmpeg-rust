...
`avutil-buffer` is the current focus for this slice. It is implemented with unit tests, callback-owned release hooks, readonly owned/callback-owned storage, borrowed static readonly storage, shared Arc-slice readonly storage, external opaque readonly storage, visible versus padded byte storage, fallible resize/reallocation, exact-shape manual non-readonly buffer-pool recycling, automatic pool-owned storage return when the last shared reference drops, custom pool allocation/release callbacks, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because hardware-frame references, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVBuffer/AVBufferRef ABI/lifetime semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-buffer` toward the next AVBufferRef lifetime gap, likely hardware-frame reference semantics or deeper ref/data ownership probes, with invalid-input and sharing tests.
...
Latest slice: added external opaque readonly `BufferRef` storage over shared byte slices. Unit tests now cover release after the last original reference, unique detach release on `make_mut()`, resize detach waiting for shared originals, pointer retention before detach, invalid visible-length rejection without invoking release, and source immutability; `avutil_core_models` now build-checks external opaque release timing, non-writability, resize detach, prefix preservation, zeroed growth/padding, and shared-reference invariants. The final validation pass ran formatting, focused buffer tests, fuzz-target check/clippy, local `avutil-buffer` mapping, changed-component dry-run and execution, workspace clippy, workspace tests, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
