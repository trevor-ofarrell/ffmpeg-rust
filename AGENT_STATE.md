...
Latest `avutil-packet` differential slice: added `crates/avutil/tests/packet_oracle.rs`, an ignored pinned libavcodec/libavutil `AVPacket` oracle harness that validates `av_packet_alloc` defaults, `av_packet_rescale_ts` with known and unknown timestamps, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `AV_PKT_FLAG_KEY|CORRUPT`, one `AV_PKT_DATA_NEW_EXTRADATA` side-data payload, `opaque` address copying, payload preservation, and packet `time_base` fields against the Rust `Packet` model. `tests/differential/mappings.txt` now exposes `avutil-packet|oracle-libavcodec-packet-core`, and `fate-runner` changed-path selection maps the oracle harness back to `avutil-packet`. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because broader AVPacket side-data API calls, packet grow/shrink/from-data/make-refcounted/make-writable behavior, full side-data oracle vectors, upstream FATE coverage or a documented FATE exception, and broader differential media integration remain pending.
...
Latest validation commands: `cargo test -p avutil packet`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, `cargo test -p fate-runner changed_selection_maps_packet_oracle_test_to_component`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo clippy -p fate-runner --all-targets --all-features -- -D warnings`, and WSL `cargo fuzz run avutil_core_models -- -runs=1` pass for the packet differential slice. A mistaken multi-filter `cargo test -p avutil ...` invocation failed before running tests; it was corrected with the passing `cargo test -p avutil packet` command.
