...
Latest slice: added shared Arc-slice readonly `BufferRef` storage. Unit tests now cover shared pointer retention before detach, Arc ownership counts, detach-on-mutation, detach-on-resize with zeroed padding, shared readonly references remaining untouched, invalid visible-length rejection without taking the caller Arc, and source immutability; `avutil_core_models` now build-checks shared Arc-slice pointer retention, non-writability, Arc ownership, resize detach, prefix preservation, zeroed growth/padding, and shared-reference invariants. The final validation pass ran formatting, local `avutil-buffer` mapping, changed-component mappings, workspace clippy, workspace tests, fuzz-target check/clippy, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
