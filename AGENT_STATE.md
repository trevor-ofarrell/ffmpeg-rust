...
Update for the current slice: `FrameExif::common_tags()` now validates SubSecTime, SubSecTimeOriginal, and SubSecTimeDigitized as digit-only fractional-second strings after the existing single-string ASCII checks. Unit tests and the `avutil_core_models` build-checked fuzz fixture cover non-digit inline and out-of-line SubSecTime values, and the fuzz harness asserts accepted generated common-tag SubSecTime strings contain only digits. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` SubSecTime validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count, ASCII-shape, coordinate, timestamp, date/time range, and direction enforcement and additionally rejects SubSecTime, SubSecTimeOriginal, and SubSecTimeDigitized strings containing non-digit fractional-second characters. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: tightened `FrameExif::common_tags()` SubSecTime validation so SubSecTime, SubSecTimeOriginal, and SubSecTimeDigitized accepted strings must contain only ASCII digits after the existing single-string ASCII checks. Unit tests and the `avutil_core_models` fuzz fixture now cover invalid inline SubSecTime, out-of-line SubSecTimeOriginal, and inline SubSecTimeDigitized values, while the random fuzz path asserts accepted generated SubSecTime strings are digit-only. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_version_timing_comment_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
