...
Latest `avutil-buffer` reallocatable-origin slice: `BufferRef::realloc(&mut Option<BufferRef>, len)` now models FFmpeg's `BUFFER_FLAG_REALLOCATABLE` split. Buffers allocated by nullable realloc become reallocatable for later unique/full-offset in-place realloc, while ordinary `av_buffer_alloc`/`av_buffer_create`-style storage takes the allocate-copy-replace path and releases custom owners. The pinned `buffer_oracle` harness now validates ordinary alloc-origin replacement, repeated nullable-realloc status, shared same-size no-op, and existing prefix preservation while treating newly allocated/grown bytes as unspecified like FFmpeg's `av_realloc`. `avutil_core_models` fuzz-smokes custom-owner release on realloc and no longer asserts zero-filled C-API realloc growth. `avutil-buffer` remains `differential_pass`, not `complete`, because broader ABI/lifetime closure, FATE disposition, and final completion documentation remain pending.

Latest validation commands for the `avutil-buffer` reallocatable-origin slice: `cargo fmt --all`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --lib buffer_ref_realloc_handles_nullable_c_api_shape`, WSL `CARGO_TARGET_DIR=target-wsl cargo test -p avutil --test buffer_oracle -- --ignored --nocapture`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --mappings tests\\differential\\mappings.txt --component avutil-buffer --target oracle-libavutil-buffer --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `CARGO_TARGET_DIR=target-codex cargo check -p avutil`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo check --manifest-path fuzz\\Cargo.toml`, `CARGO_TARGET_DIR=target-codex cargo clippy -p avutil -p fate-runner --all-targets --all-features -- -D warnings`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo clippy --manifest-path fuzz\\Cargo.toml -- -D warnings`, WSL `CARGO_TARGET_DIR=target-wsl cargo run -p fate-runner -- run --component avutil-buffer`, `cargo fmt --all -- --check`, `git diff --check`, and WSL `CARGO_TARGET_DIR=target-wsl-fuzz cargo fuzz run avutil_core_models -- -runs=1` passed. The default Windows test target and one direct `target-codex` ignored oracle execution were blocked by Windows Application Control `os error 4551`; the mapped `target-codex` oracle command and WSL equivalents passed.
