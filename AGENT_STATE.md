...
`avformat-hash-muxer` now supports MD5 packet-data hashing in addition to Adler-32 and IEEE CRC-32. `HashMuxerReport` carries a variable-width `HashDigest`, formats MD5 digest lines as lowercase hex, preserves packet/byte accounting, and the `avformat_packet_muxers` fuzz target now build-checks MD5 digest stability alongside Adler-32/CRC-32. The ledger still keeps `avformat-hash-muxer` at `implemented`, not `complete`, because CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned-oracle differential tests, FATE coverage, SHA variants, and actual local fuzz execution are still absent.
...
`avutil-hash` is the latest coherent slice. SHA-256 one-shot/streaming helpers have been added beside Adler-32, IEEE CRC-32, and MD5, with standard vectors, block-boundary streaming tests, and build-checked `avutil_core_models` coverage. Pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, additional SHA variants, and full FFmpeg hash/framehash behavior are still absent.

## Next 3 Concrete Actions

1. Continue hash parity by wiring SHA-256 into the internal hash muxer or adding the next missing SHA digest variant with tests.
...
Latest slice: strengthened `avutil::hash` with Rust-native SHA-256 one-shot and streaming helpers, standard-vector/block-boundary tests, fuzz-target coverage, and docs/ledger updates while keeping `avutil-hash` below `complete`.
