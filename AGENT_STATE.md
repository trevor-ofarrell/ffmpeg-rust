...
Update for the current slice: EXIF linked-IFD parsing now has broader build-checked fuzz invariants matching `frame_side_data_parses_exif_linked_ifds`. The deterministic `avutil_core_models` linked-IFD fixture now preserves root entry counts, linked EXIF/GPS/interoperability IFD ordering, parent offsets, source tags, target offsets, nested interoperability pointer offsets, GPS version type/count/value bytes, interoperability index type/value bytes, and missing tag/pointer lookup behavior. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked-IFD fuzz coverage. `FrameExif` already parsed root IFD pointers into linked EXIF, GPS, and interoperability directories, and the unit test already asserted their order, source pointer metadata, target offsets, nested interoperability pointer offset, GPS version entry shape, interoperability index shape, and missing lookup behavior. The `avutil_core_models` build-checked fuzz harness now mirrors those deterministic linked-IFD invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF linked-IFD fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_parses_exif_linked_ifds`: root entry counts, linked EXIF/GPS/interoperability ordering, parent offsets, source tags, target offsets, nested interoperability pointer offsets, GPS version type/count/value bytes, interoperability index type/value bytes, missing root-tag lookup, and invalid pointer-tag lookup. Docs and the ledger now call out these linked-IFD metadata shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_exif_linked_ifds`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
