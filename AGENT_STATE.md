...
- Current `avutil-channel-layout` explicit ambisonic layout slice:
...
1. Continue `avutil-channel-layout` with the next source-checked parser gap: full `av_opt_get_key_value` escaping/quoting behavior for custom lists or broader native/custom/ambisonic retyping.
...
3. Keep `avutil-channel-layout` below `complete` until full parsing, broad retyping, full ambisonic order semantics, oracle inventory, upstream FATE, and fuzz parity are proven.

## Known Blockers

- Latest `avutil-channel-layout` parser coverage now stores bounded `ambisonic <order>` no-extra and native-extra forms as explicit `AmbisonicChannelLayout` values, while named/custom extras still use Custom-backed AMBI maps. Broader retyping, exact FFmpeg edge behavior for very large orders, full `av_opt_get_key_value` escaping/quoting parity, full ambisonic comparison/index-string semantics, full `ffmpeg -layouts` inventory comparison, upstream FATE parity, and actual fuzz execution remain blockers.
