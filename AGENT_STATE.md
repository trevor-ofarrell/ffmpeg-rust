...
Latest parity status: the 10% strict-parity milestone has been reached. Strict completion is now 10/96 components (about `10%`). `avutil-error`, `avutil-rational`, `avutil-timebase`, `avutil-byteio`, `avutil-bitreader`, `repo-runtime-guard`, `oracle-inventory`, `avutil-hash`, `avutil-sample-format`, and `avutil-dict` are the current `complete` rows for the default-native profile. Intermediate measurable parity still includes 2/96 non-complete status rows: `fftools-version` and `avutil-channel-layout` are `differential_pass`/validated evidence rows. Current ledger status counts are 83 `implemented`, 1 `scaffolded`, 2 `differential_pass`, and 10 `complete`.
...
Latest validation commands: `cargo test -p avutil bitreader`, `cargo test -p avutil --test bitreader_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-bitreader --target oracle-libavcodec-get-bits --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-bitreader`, WSL `cargo fuzz run avutil_bitreader -- -runs=1`, `cargo test -p fate-runner changed_selection_maps_bitreader_oracle_test_to_component`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --changed`, `cargo clippy -p fate-runner --all-targets --all-features -- -D warnings`, `cargo fmt --all -- --check`, and `git diff --check` all pass for the bitreader completion slice. `git diff --check` reports CRLF conversion warnings only.
