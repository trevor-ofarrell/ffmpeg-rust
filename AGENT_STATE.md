...
`fftools-ffprobe-stream-field-order` is the current focus; the latest slice adds `field_order=unknown` to `ffprobe-rs -show_streams` default and JSON output for constrained MOV/MP4 and AVI video streams.
...
Latest slice: add `ffprobe-rs` stream field-order reporting. Constrained MOV/MP4 and AVI video streams now render `field_order=unknown` in default output and `"field_order": "unknown"` in JSON because the Rust demuxers do not yet parse interlace/progressive field-order metadata. This still lacks pinned-oracle differential/FATE/fuzz coverage.
