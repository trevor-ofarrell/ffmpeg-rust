...
Latest `avutil-buffer` same-size realloc slice: `BufferRef::realloc(&mut Option<BufferRef>, len)` now models FFmpeg's early `av_buffer_realloc()` no-op when a non-NULL ref already has the requested visible size. This preserves shared storage and readonly/callback-owned storage instead of detaching through the generic resize path. The pinned `buffer_oracle` harness now validates the shared-ref same-size no-op row, and `avutil_core_models` fuzz-smokes same-size storage preservation. `avutil-buffer` remains `differential_pass`, not `complete`, because broader ABI/lifetime closure, FATE disposition, and final completion documentation remain pending.

Latest validation commands for the `avutil-buffer` same-size realloc slice: `cargo fmt --all`, `cargo test -p avutil --lib buffer_ref_realloc_handles_nullable_c_api_shape`, `cargo test -p avutil --test buffer_oracle -- --ignored --nocapture`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --mappings tests\\differential\\mappings.txt --component avutil-buffer --target oracle-libavutil-buffer --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `CARGO_TARGET_DIR=target-codex cargo check -p avutil`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo check --manifest-path fuzz\\Cargo.toml`, `CARGO_TARGET_DIR=target-codex cargo clippy -p avutil -p fate-runner --all-targets --all-features -- -D warnings`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo clippy --manifest-path fuzz\\Cargo.toml -- -D warnings`, WSL `CARGO_TARGET_DIR=target-wsl cargo run -p fate-runner -- run --component avutil-buffer`, `cargo fmt --all -- --check`, `git diff --check`, and WSL `CARGO_TARGET_DIR=target-wsl-fuzz cargo fuzz run avutil_core_models -- -runs=1` passed. Windows `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --component avutil-buffer` ran the selected buffer tests successfully, then failed when Windows Application Control blocked a rebuilt unrelated `bitwriter_oracle` test binary with `os error 4551`; the equivalent WSL component mapping passed.

Previous `avutil-buffer` nullable realloc slice: added `BufferRef::realloc(&mut Option<BufferRef>, len)` so Rust can model `av_buffer_realloc(&buf, size)` when `buf` is `NULL`, while preserving no-mutation failure behavior. The pinned `buffer_oracle` harness validates the `av_buffer_realloc(NULL, 4)` return/status row, and `avutil_core_models` fuzz-smokes nullable realloc allocation, prefix preservation, zero-fill growth, and failed NULL realloc invariants.
