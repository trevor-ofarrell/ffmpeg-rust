...
- Current high-bit semi-planar P-family `avutil-pixel-format` / rawvideo slice:
  ...
  - `cargo test -p fftools runs_rawvideo_p010le_to_null_stdout --target-dir target-codex`
  - `cargo fmt --all -- --check`
...
`avutil-pixel-format` remains the current focus, with linked rawvideo decoder, demuxer, muxer, constrained `ffmpeg-rs` input parsing, and fuzz-harness invariant coverage. The latest concrete change adds FFmpeg's high-bit semi-planar P-family names `p010le`, `p010be`, `p012le`, `p012be`, `p016le`, `p016be`, `p210le`, `p210be`, `p212le`, `p212be`, `p216le`, `p216be`, `p410le`, `p410be`, `p412le`, `p412be`, `p416le`, and `p416be` as two payload-plane formats with pinned descriptor bpp, endian-specific names, 4:2:0/4:2:2/4:4:4 chroma geometry, and matching rawvideo packet sizing. It does not claim conversion support, full per-component `AVPixFmtDescriptor` parity, full `ffmpeg -pix_fmts` inventory, pinned oracle parity, upstream FATE media parity, hardware formats, or actual fuzz execution.
...
1. Continue priority-1 format-model work by choosing the next rawvideo-facing FFmpeg 8.1.1 pixel-format family from pinned inventory/source notes, likely the remaining rawvideo-facing RGB/YUV/gray/pal/pixel-format gaps now that the high-bit P semi-planar family is modeled.
2. Add pinned-oracle differential coverage for constrained pixel-format inventory and rawvideo packet sizing once the FFmpeg 8.1.1 oracle binary is available.
3. Keep local FATE-runner changed-path coverage aligned with ledger selections, and document/standardize the accepted target-directory workaround for Windows Application Control blocks without treating policy-blocked fresh executable launches as Rust test failures.
...
Latest slice: added FFmpeg 8.1.1 high-bit semi-planar P-family support to the current shared pixel/rawvideo model. `PixelFormat` now reports `p010le`/`p010be`, `p012le`/`p012be`, `p016le`/`p016be`, `p210le`/`p210be`, `p212le`/`p212be`, `p216le`/`p216be`, `p410le`/`p410be`, `p412le`/`p412be`, and `p416le`/`p416be` in the inventory with YUV descriptor metadata, two payload planes, 10/12/16-bit component depths, 15/18/20/24/30/32/36/48 average descriptor bpp, two stored bytes per component sample, and matching 4:2:0, 4:2:2, or 4:4:4 chroma validation. `VideoFrame`, `RawVideoDecoder`, `RawVideoDemuxer`, `RawVideoMuxer`, constrained `ffmpeg-rs` rawvideo-to-null execution, and affected fuzz harnesses now exercise representative `p010`, `p212`, and `p416` paths. Validation passed with focused avutil/avcodec/avformat/fftools tests, fuzz package check/clippy, workspace format check, workspace clippy, workspace library tests, local changed-path FATE mappings through `target-codex`, and `git diff --check` with CRLF warnings only. The affected components remain `implemented`, not `complete`, because oracle differentials, upstream FATE media coverage, full pixel inventory coverage, full `AVPixFmtDescriptor` parity, pixel conversion, hardware formats, and actual fuzz execution are still absent.
