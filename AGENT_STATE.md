...
Current avutil-frame coverage also now includes deterministic build-checked public `VideoFrame::aligned_line_sizes` and `AudioFrame::aligned_line_sizes` overflow rejection, matching the focused frame-alignment unit coverage without relying on the private helper directly.

Update for the current slice: frame alignment overflow coverage now has a build-checked fuzz invariant matching `frame_alignment_rejects_zero_alignment_and_overflow` through public video/audio line-size APIs. The deterministic `avutil_core_models` fixture now asserts that very large gray8 video widths and u8 audio sample counts reject nonzero alignment overflow with typed `InvalidArgument` errors. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens frame alignment overflow fuzz coverage. `VideoFrame` and `AudioFrame` already have unit coverage proving zero-alignment rejection and private-helper overflow rejection. The `avutil_core_models` build-checked fuzz harness now mirrors the overflow failure class through public `VideoFrame::aligned_line_sizes` and `AudioFrame::aligned_line_sizes` calls, using very large gray8 video widths and u8 audio sample counts that reject nonzero alignment overflow with typed `InvalidArgument` errors. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened frame alignment overflow fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_alignment_rejects_zero_alignment_and_overflow` through the public `VideoFrame::aligned_line_sizes` and `AudioFrame::aligned_line_sizes` APIs, proving very large gray8 video widths and u8 audio sample counts reject nonzero alignment overflow with typed `InvalidArgument` errors. Docs and the ledger now call out this public alignment-overflow invariant explicitly. Validation passed with `cargo test -p avutil frame_alignment_rejects_zero_alignment_and_overflow`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
