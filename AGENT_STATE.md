...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern "AV_CHAN_TOP_BACK|AV_CH_TOP_BACK|AV_CH_LAYOUT_OCTAGONAL|AV_CH_LAYOUT_CUBE|AV_CHAN_TOP_FRONT"` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern '"5\.1\.2|"octagonal|"cube|TOP_BACK|top back|TBL|TBR'` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `TBL`/`TBR`, `octagonal`, and `cube`; the focused channel-layout unit test passed with `--target-dir target-codex`.
  - Full `ffmpeg -layouts` oracle inventory comparison, broader height-channel layout coverage after `cube`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `TBL`/`TBR` native mask bits plus the `octagonal` and `cube` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving the upstream naming-to-mask mapping, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entries after `cube`, starting with `5.1.4`, `7.1.2`, or the smallest source-backed subset that requires only a few missing height/LFE2 channel bits.
