...
`fftools-ffprobe-stream-codec-long-name` is the current focus; the latest slice reports constrained `codec_long_name` values alongside existing `codec_name` values in `ffprobe-rs -show_streams` output.
...
Latest slice: add constrained `codec_long_name` reporting to `ffprobe-rs`. The stream report now maps the existing known FourCC codec names to long names for rawvideo, H.264/AVC, HEVC, and MPEG-4 Part 2; fixtures prove raw MOV, H.264 MOV, and raw AVI JSON/default output.
