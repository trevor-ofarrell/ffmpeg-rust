...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `7.1(wide)` and `7.1(wide-side)` layouts alongside the prior `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, mono/stereo/quad/5.1/5.1(side)/7.1 subset. The implementation preserves FFmpeg's map shape where `7.1(wide)` uses the back-speaker wide mask and `7.1(wide-side)` uses the side-speaker wide mask. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new layouts, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, height-channel layouts, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures were observed while adding `7.1(wide)` and `7.1(wide-side)`; the focused channel-layout unit test passed inside the sandbox with `--target-dir target-codex`.
  - Full `ffmpeg -layouts` oracle inventory comparison, height-channel layout coverage after the wide 7.1 entries, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `7.1(wide)` and `7.1(wide-side)` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving the upstream naming-to-mask mapping, extends exact name/mask/parser tests, and expands the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, height-channel layouts, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entries after `7.1(wide-side)`, starting with height-channel layouts such as `5.1.2` and `5.1.2(back)` and the needed top-front channel mask bits.
2. Implement one source-backed follow-up layout slice with unit tests, fuzz-generator coverage, ledger updates, and local FATE mapping validation.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, and `BC`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, and `7.1(wide-side)` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no height-channel layout coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added the next source-checked wide 7.1 native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CH_LAYOUT_7POINT1`, `AV_CH_LAYOUT_7POINT1_WIDE`, `AV_CH_LAYOUT_7POINT1_WIDE_BACK`, the public layout names `7.1`, `7.1(wide)`, and `7.1(wide-side)`, and the upstream map detail that `7.1(wide)` uses `AV_CH_LAYOUT_7POINT1_WIDE_BACK` while `7.1(wide-side)` uses `AV_CH_LAYOUT_7POINT1_WIDE`. The Rust model now exposes those named layouts while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. The component remains `implemented`, not `complete`, because full oracle inventory parity, height-channel layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
