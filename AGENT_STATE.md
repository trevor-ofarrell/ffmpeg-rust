...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed, explicit custom, and checked aligned line-size metadata, descriptor-style sample-format metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane and side-data ownership, the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_*` side-data inventory, descriptor/property metadata, remove-by-property helpers, explicit set-style side-data replacement/coalescing, typed `AV_FRAME_DATA_DISPLAYMATRIX`, `AV_FRAME_DATA_MATRIXENCODING`, `AV_FRAME_DATA_DOWNMIX_INFO`, `AV_FRAME_DATA_REPLAYGAIN`, `AV_FRAME_DATA_AFD`, `AV_FRAME_DATA_MOTION_VECTORS`, `AV_FRAME_DATA_SKIP_SAMPLES`, `AV_FRAME_DATA_AUDIO_SERVICE_TYPE`, `AV_FRAME_DATA_MASTERING_DISPLAY_METADATA`, `AV_FRAME_DATA_GOP_TIMECODE`, `AV_FRAME_DATA_SPHERICAL`, `AV_FRAME_DATA_CONTENT_LIGHT_LEVEL`, `AV_FRAME_DATA_ICC_PROFILE`, `AV_FRAME_DATA_S12M_TIMECODE`, `AV_FRAME_DATA_DYNAMIC_HDR_PLUS`, `AV_FRAME_DATA_REGIONS_OF_INTEREST`, `AV_FRAME_DATA_VIDEO_ENC_PARAMS`, `AV_FRAME_DATA_FILM_GRAIN_PARAMS`, `AV_FRAME_DATA_DETECTION_BBOXES`, and `AV_FRAME_DATA_SEI_UNREGISTERED` payload parsers, per-record metadata, side-data/hw-context writable checks and copy-on-write detachment helpers, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage. It remains below `complete` because typed FFmpeg side-data payload schemas beyond display matrix, matrix encoding, downmix info, ReplayGain, AFD, motion vectors, skip samples, audio service type, mastering display metadata, GOP timecode, spherical mapping, content light metadata, ICC profile, S12M timecode, dynamic HDR10+ envelope, regions of interest, video encoding parameters, film grain parameters, detection bounding boxes, and SEI unregistered, deeper HDR10+ color transform semantics, ROI encoder constraint handling, codec-specific video encoding parameter quantizer semantics, film grain synthesis/selection behavior, detection-model semantics beyond the native envelope, complete FFmpeg alignment policy, sample-data conversion routines, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

This slice adds `AV_FRAME_DATA_DETECTION_BBOXES` to the typed `avutil-frame` parser set as the pinned FFmpeg 8.1.1 native `AVDetectionBBoxHeader` plus `AVDetectionBBox` record array envelope. The focus component remains below `complete` because remaining typed side-data payload schemas, detection-model semantics beyond the native envelope, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: added `FrameDetectionBbox`, `FrameDetectionBboxes`, `FrameSideData::new_detection_bboxes`, and `FrameSideData::detection_bboxes()` for the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_DETECTION_BBOXES` native payload shape. The parser validates exact native header sizing, `bboxes_offset`, `bbox_size`, count-derived total length, and per-record `classify_count`, preserves source/label fixed byte arrays, geometry fields, detection/classification confidences, and returns `None` for non-detection side data. Validation passed with `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, and `git diff --check`.
