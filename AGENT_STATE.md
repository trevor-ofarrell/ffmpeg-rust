...
Current avutil-frame ownership coverage also now includes deterministic build-checked direct `VideoFrame::make_writable` and `AudioFrame::make_writable` readonly plane detachment, matching the focused typed-frame unit coverage.

Update for the current slice: direct typed video/audio frame `make_writable` coverage now has broader build-checked fuzz invariants matching `video_frame_make_writable_detaches_readonly_plane_storage` and `audio_frame_make_writable_detaches_readonly_plane_storage`. The deterministic `avutil_core_models` fixture now covers detaching readonly gray8 video and packed s16 audio plane storage through the direct `VideoFrame` and `AudioFrame` APIs while preserving visible bytes and the original readonly source buffers. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens direct typed frame `make_writable` fuzz coverage. `VideoFrame` and `AudioFrame` already have unit tests proving direct `make_writable` detaches readonly plane storage, preserves visible bytes, and leaves the original readonly source buffers unchanged. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions for gray8 video and packed s16 audio. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened direct typed frame `make_writable` fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `video_frame_make_writable_detaches_readonly_plane_storage` and `audio_frame_make_writable_detaches_readonly_plane_storage`: direct `VideoFrame::make_writable` and `AudioFrame::make_writable` detach readonly plane storage, preserve visible bytes, and leave the original readonly source buffers unchanged. Docs and the ledger now call out this direct typed-frame detachment invariant explicitly. Validation passed with `cargo test -p avutil video_frame_make_writable_detaches_readonly_plane_storage`, `cargo test -p avutil audio_frame_make_writable_detaches_readonly_plane_storage`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
