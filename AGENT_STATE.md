...
This slice extends EXIF typed-entry value coverage. `FrameExifEntry` typed accessors for SIGNED BYTE, FLOAT, and DOUBLE are now covered by the EXIF value semantics fixture, including inline signed byte/float values and out-of-line double value decoding. The focused unit test checks exact signed byte values and float/double bit patterns, and the `avutil_core_models` build-checked fuzz harness asserts the accessor type/length invariants for these TIFF entry kinds. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended EXIF typed-entry value coverage for SIGNED BYTE, FLOAT, and DOUBLE TIFF entries. The EXIF value semantics fixture now covers inline signed-byte values, inline `f32` bit patterns, and out-of-line `f64` bit patterns; `frame_side_data_decodes_exif_entry_values` checks those values, and `avutil_core_models` now build-checks type/length invariants for the new typed accessors. Validation passed with escalated `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_decodes_exif_entry_values` after the sandboxed launch was blocked by Windows Application Control, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, escalated `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
