...
Update for the current slice: `FrameExif::common_tags()` now validates digit-coded EXIF version byte tags and component configuration bytes before returning the selected common-tag view. `ExifVersion` and `FlashpixVersion` must be fixed four-byte digit-coded UNDEFINED fields, `ComponentsConfiguration` must be a fixed four-byte UNDEFINED field with component identifiers in `0..=6`, and `GPSVersionID` fixed-count rejection is covered alongside the previous DateTime, OffsetTime, GPSDateStamp, and GPS reference checks. Unit tests and the `avutil_core_models` build-checked fuzz target cover invalid ExifVersion count/value, invalid FlashpixVersion count/value, invalid ComponentsConfiguration count/value, and invalid GPSVersionID count. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` version and component validation in the selected common-tag view. `FrameExif::common_tags()` now requires `ExifVersion` and `FlashpixVersion` to be fixed four-byte digit-coded UNDEFINED fields, keeps `ComponentsConfiguration` fixed at four UNDEFINED bytes, rejects component identifiers outside `0..=6`, and adds explicit fixed-count coverage for `GPSVersionID`. DateTime, OffsetTime, GPSDateStamp, and GPS one-character reference fixed-count validation remains in place. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: tightened `FrameExif::common_tags()` version and component validation so `ExifVersion` and `FlashpixVersion` require fixed four-byte digit-coded UNDEFINED payloads, `ComponentsConfiguration` rejects component IDs outside `0..=6`, and `GPSVersionID` fixed-count rejection is explicitly covered. Unit tests and the `avutil_core_models` fuzz fixture now cover invalid ExifVersion count/value, invalid FlashpixVersion count/value, invalid ComponentsConfiguration count/value, and invalid GPSVersionID count. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_common_exif_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_version_timing_comment_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
