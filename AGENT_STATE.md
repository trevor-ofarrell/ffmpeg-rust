...
1. Continue priority-1 primitive work by tightening the next unblocked avutil surface, likely logging edge parity, deeper bit I/O vectors, or option API/CLI integration, unless a pinned-oracle scaffold becomes available first.
2. Add pinned-oracle differential coverage for AVOption parsing/ranges, direct/rational rescale, rational reduction/double-conversion, error strings/codes, and constrained hash/framehash/streamhash CLI paths once the FFmpeg 8.1.1 oracle binary is available.
