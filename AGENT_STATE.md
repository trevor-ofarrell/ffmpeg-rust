...
`repo-runtime-guard` is the current focus for this slice. It is implemented as `cargo run -p xtask -- guard-runtime`, with unit coverage for allowed local project dependencies, forbidden dependency names, forbidden `package =` renames, forbidden resolved `Cargo.lock` wrapper packages, forbidden runtime wrapper imports, forbidden literal runtime `ffmpeg` process spawns, and allowed `ffmpeg-rs`/`ffprobe-rs` binary-name strings. The guard is also wired into `xtask quick`, `xtask changed`, `xtask full`, `tests/fate/mappings.txt`, and `fate-runner` changed-path selection for workspace manifests, crate manifests, fuzz/xtask manifests, `Cargo.lock`, and `xtask/` edits.
...
Latest slice: strengthened the runtime FFmpeg linkage guard and changed-test selection. `xtask guard-runtime` now rejects known FFmpeg wrapper dependencies, package-renamed FFmpeg wrapper crates, resolved `Cargo.lock` wrapper packages, runtime wrapper import tokens, and literal `ffmpeg`/`ffprobe`/`ffplay` process spawns from implementation crates, while allowing oracle/FATE/test/fuzz/xtask tooling to invoke configured oracle binaries. The guard runs from `xtask quick`, `xtask changed`, `xtask full`, and the local FATE-runner mapping `repo-runtime-guard|local-runtime-guard`; `fate-runner` changed-path selection now maps workspace manifests, crate manifests, fuzz/xtask manifests, and `Cargo.lock` to `repo-runtime-guard` while selecting affected component families for crate manifests. Validation passed with `cargo fmt --all`, `cargo test -p xtask lockfile_guard_rejects_forbidden_transitive_packages`, `cargo test -p fate-runner changed_selection_maps_dependency_manifests_to_runtime_guard`, `cargo test -p fate-runner`, `cargo test -p xtask`, `cargo run -p xtask -- guard-runtime`, `cargo fmt --all -- --check`, `cargo clippy -p xtask -p fate-runner --all-targets -- -D warnings`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo run -p xtask -- quick`, and `git diff --check`.
