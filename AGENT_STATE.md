...
Update for the current slice: `FrameExif::common_tags()` offset-time coverage now has build-checked fuzz invariants matching the existing unit test for malformed UTC-offset tags. The deterministic `avutil_core_models` EXIF fixture now covers valid OffsetTime, OffsetTimeOriginal, and OffsetTimeDigitized parsing plus malformed OffsetTime count, sign/layout, and hour cases, malformed OffsetTimeOriginal type and minute cases, and malformed OffsetTimeDigitized ASCII terminator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- No remaining failing commands in the latest EXIF offset-time fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF offset-time common-tag fuzz coverage. `FrameExif::common_tags()` already exposed OffsetTime, OffsetTimeOriginal, and OffsetTimeDigitized with unit coverage for malformed count, type, layout, hour, and minute shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: OffsetTime wrong count, OffsetTime bad sign/layout, OffsetTime hour 24, OffsetTimeOriginal wrong type, OffsetTimeOriginal minute 60, and OffsetTimeDigitized bad ASCII terminator. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF offset-time common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_offset_time_tags`: malformed OffsetTime count, malformed OffsetTime sign/layout, malformed OffsetTime hour, malformed OffsetTimeOriginal type, malformed OffsetTimeOriginal minute, and malformed OffsetTimeDigitized ASCII terminator. Docs and the ledger now call out the offset-time fixed-count, ASCII type, layout, hour, and minute validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_offset_time_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
