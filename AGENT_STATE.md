...
Update for the current slice: `FrameExif::common_tags()` root TIFF strip-position coverage now has build-checked fuzz invariants matching the existing unit test for malformed `RowsPerStrip` and `XPosition` shapes. The deterministic `avutil_core_models` EXIF root fixtures now cover valid parsing, malformed RowsPerStrip type/value cases, malformed XPosition count, and malformed YPosition denominator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_root_strip_position_tags`
...
- No remaining failing commands in the latest EXIF root strip-position fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF root strip-position common-tag fuzz coverage. `FrameExif::common_tags()` already exposed `RowsPerStrip` as a positive SHORT/LONG value and `XPosition`/`YPosition` as single unsigned rationals, with unit coverage for malformed type/count/value/denominator shapes. The `avutil_core_models` build-checked fuzz harness now asserts the remaining deterministic invalid paths too: RowsPerStrip wrong type, RowsPerStrip zero, XPosition wrong count, and YPosition zero denominator. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
