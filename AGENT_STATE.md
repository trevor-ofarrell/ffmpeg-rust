...
Latest slice: added `VideoFrame::new_with_line_sizes` and `VideoFrame::new_with_buffer_refs_and_line_sizes` for explicit per-plane video line sizes in the current pixel-format subset. Strided frame storage is retained through `plane_buffers()`, while `planes()` continues to expose packed visible rows with row padding stripped. Unit tests now prove gray and yuv420p strided storage, packed visible snapshots, raw strided buffer retention, invalid line-size count rejection, line-size-too-small rejection, wrong storage-length rejection, and yuv420p geometry rejection; `avutil_core_models` now build-checks custom video line sizes and row-padding exclusion alongside existing frame plane ownership, side-data, and hw context invariants. Validation passed with formatting, focused `cargo test -p avutil frame`, `cargo test -p avutil --lib --no-run`, fuzz-target check/clippy, workspace clippy, local `avutil-frame` FATE-runner execution, and FATE-runner changed dry-run. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
