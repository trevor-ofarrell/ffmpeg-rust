...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `TSL`/top-side-left, `TSR`/top-side-right, and `9.1.6` from `channel_layout_map`, mapped to `AV_CH_LAYOUT_9POINT1POINT6` as `9.1.4` plus those top-side channels. This builds on prior `9.1.4`, `TBC`/top-back-center, `LFE2`/low-frequency-2, `7.2.3`, `5.1.4`, `7.1.2`, `7.1.4`, `TBL`/top-back-left, `TBR`/top-back-right, `octagonal`, `cube`, `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `TFL`/top-front-left, `TFR`/top-front-right, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new channels/layout, canonical `FL+FR`-style string, mask/name/channel-expression round trips, default lookups, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, `hexadecagonal` and broader height-channel layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CH_LAYOUT_9POINT1POINT4_BACK|AV_CHANNEL_LAYOUT_9POINT1POINT4_BACK|AV_CH_LAYOUT_9POINT1POINT6|AV_CH_TOP_SIDE_LEFT|AV_CH_TOP_SIDE_RIGHT|AV_CHAN_TOP_SIDE_LEFT|AV_CHAN_TOP_SIDE_RIGHT|AV_CH_LAYOUT_HEXADECAGONAL|AV_CH_LAYOUT_BINAURAL|AV_CH_LAYOUT_STEREO_DOWNMIX' -Context 2,3` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern '"9\.1\.4"|"9\.1\.6"|"hexadecagonal"|"binaural"|"stereo downmix"|TOP_SIDE|TSL|TSR|WIDE_LEFT|BINAURAL' -Context 2,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `TSL`, `TSR`, and `9.1.6`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, and `avformat` audio regression passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, `hexadecagonal` and broader height-channel layout coverage after `9.1.6`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `TSL`/top-side-left, `TSR`/top-side-right, and `9.1.6` from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving its `AV_CH_LAYOUT_9POINT1POINT6` mapping as `9.1.4` plus those top-side channels, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, `hexadecagonal`/broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entry after `9.1.6`, starting with `hexadecagonal` and its required `WL`/`WR`/top-center channel families.
2. Source-check whether `binaural` or stereo-downmix require additional channel bits or non-native order semantics, then choose the smallest unblocked follow-up layout slice.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFR`, `TBL`, `TBC`, `TBR`, `LFE2`, `TSL`, and `TSR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, `7.1.4`, `7.2.3`, `9.1.4`, and `9.1.6` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no `hexadecagonal` and later height layout coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added source-checked `TSL`/`TSR` channel bits and `9.1.6` native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CHAN_TOP_SIDE_LEFT`, `AV_CHAN_TOP_SIDE_RIGHT`, `AV_CH_TOP_SIDE_LEFT`, `AV_CH_TOP_SIDE_RIGHT`, `AV_CH_LAYOUT_9POINT1POINT6`, `AV_CHANNEL_LAYOUT_9POINT1POINT6`, the public short names `TSL`/`TSR`, and the `channel_layout_map` name `9.1.6`. The Rust model now exposes `9.1.6` as `9.1.4` plus top-side left/right, while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover channel names/masks, layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local FATE mapping, changed-path local mappings, `avformat` audio tests, format checks, and `git diff --check` passed. The component remains `implemented`, not `complete`, because full oracle inventory parity, `hexadecagonal` and broader height-channel layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
