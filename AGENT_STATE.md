...
Latest `avutil-frame` update: `FrameSeiUnregistered` now parses `AV_FRAME_DATA_SEI_UNREGISTERED` side-data payloads as a 16-byte UUID plus trailing user data, rejects short payloads with `InvalidData`, and is covered by focused unit tests plus the build-checked `avutil_core_models` fuzz target. Other typed FFmpeg frame side-data payload schemas remain pending.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed, explicit custom, and checked aligned line-size metadata, descriptor-style sample-format metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane and side-data ownership, the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_*` side-data inventory, descriptor/property metadata, remove-by-property helpers, explicit set-style side-data replacement/coalescing, the first typed `AV_FRAME_DATA_SEI_UNREGISTERED` UUID/user-data payload parser, per-record metadata, side-data/hw-context writable checks and copy-on-write detachment helpers, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage. It remains below `complete` because typed FFmpeg side-data payload schemas beyond the initial SEI unregistered parser, complete FFmpeg alignment policy, sample-data conversion routines, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: added `FrameSeiUnregistered`, `FrameSideData::new_sei_unregistered`, and `FrameSideData::sei_unregistered()` for the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_SEI_UNREGISTERED` payload shape: the first 16 bytes are UUID and the remaining bytes are user data. Malformed SEI payloads shorter than the UUID are rejected with `InvalidData`, non-SEI side data returns `None`, and the `avutil_core_models` fuzz harness now build-checks valid, invalid, and random SEI parsing invariants. Validation passed with `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, and `git diff --check`.
