...
Latest slice: added readonly `BufferRef` storage constructors and readonly-aware mutation semantics. Unit tests now cover readonly visible-length validation, non-writable unique readonly buffers, copy-before-write with padding preservation, callback-owned readonly release timing for unique and shared references, and rejecting readonly buffers from mutable pool recycling; `avutil_core_models` now build-checks readonly detach and callback-release invariants. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
