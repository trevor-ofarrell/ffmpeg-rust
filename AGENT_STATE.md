...
Latest slice: added `VideoFrame` and `AudioFrame` writability checks, explicit copy-on-write detachment, and visible-plane mutation. Unit tests now prove shared and readonly plane buffers detach before mutation, cloned frames remain unchanged, strided video row padding is preserved when visible rows are rewritten, packed visible snapshots stay synchronized with retained buffers, and invalid plane index/visible-size writes are typed errors. `avutil_core_models` now build-checks writable plane detachment and mutation alongside custom video line sizes, frame plane ownership, side-data, and hw context invariants. Validation passed with formatting, focused `cargo test -p avutil frame`, `cargo test -p avutil --lib --no-run`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, focused fuzz-target clippy, workspace clippy, local `avutil-frame` FATE-runner smoke execution, FATE-runner changed dry-run, format checks, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
