...
Update for the current slice: `FrameExif::common_tags()` root TIFF threshold/fill-order coverage now has build-checked fuzz invariants matching the existing unit tests for malformed `Thresholding` and `FillOrder` shapes. The deterministic `avutil_core_models` EXIF root fixtures now cover valid parsing, malformed Thresholding type/count/value cases, and malformed FillOrder type/count/value cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_root_thresholding_tags`
- `cargo test -p avutil frame_side_data_interprets_exif_root_fill_order_tags`
...
- No remaining failing commands in the latest EXIF root threshold/fill-order fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF root threshold/fill-order common-tag fuzz coverage. `FrameExif::common_tags()` already exposed `Thresholding` as a typed 1/2/3 enum and `FillOrder` as a typed 1/2 enum, with unit coverage for malformed type/count/value shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: Thresholding wrong type, wrong count, bad value, and FillOrder wrong type, wrong count, bad value. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root threshold/fill-order common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_root_thresholding_tags` and `frame_side_data_interprets_exif_root_fill_order_tags`: malformed Thresholding type, malformed Thresholding count, invalid Thresholding value, malformed FillOrder type, malformed FillOrder count, and invalid FillOrder value. Docs and the ledger now call out the threshold/fill-order type/count/value validation shape explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_root_thresholding_tags`, `cargo test -p avutil frame_side_data_interprets_exif_root_fill_order_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
