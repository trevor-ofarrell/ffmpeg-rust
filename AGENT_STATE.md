...
`avutil-packet` is the current focus for this slice. The concrete change is packet side-data lifecycle support: side-data records can be queried by kind, shrunk with no mutation on oversize errors, taken or removed by first matching kind while duplicate order is preserved, and cleared as a group.
