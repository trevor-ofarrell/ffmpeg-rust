...
2. Extend MOV sample-entry parsing into additional codec-specific audio extension atoms beyond direct `esds`/`btrt`/`damr`/`dac3`/`dec3`/`dOps`/`dfLa`/`alac`/`wave`/`chan` and remaining subtitle/data sample-entry variants such as WebVTT/text-subtitle details.
...
Latest slice: parsed MOV `stpp` XML subtitle sample entries into `MovXmlSubtitleSampleEntry`, preserving namespace, schema-location, auxiliary-MIME-types, and optional child boxes; added malformed `stpp` rejection coverage; updated MOV compatibility notes; and kept the component below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
