...
Latest `avutil-packet` differential slice: `crates/avutil/tests/packet_oracle.rs` now validates pinned libavcodec/libavutil payload-layout rows for FFmpeg C struct/enum packet side data plus fixed/comment-defined/raw side-data payloads: H.263 MB info, quality stats, fallback track, skip samples, parameter change, JP dual mono, strings metadata, subtitle position, Matroska BlockAdditional, WebVTT identifier/settings, metadata update, MPEGTS stream ID, A53 CC, AFD, opaque ICC profile bytes, S12M timecode, frame cropping, and LCEVC. The same oracle continues to validate `AV_PKT_DATA_NB`, every public `AV_PKT_DATA_*` numeric enum value, all 41 public `av_packet_side_data_name()` display strings, core lifecycle/property behavior, payload API behavior, and side-data API behavior: `av_packet_alloc` defaults, `av_packet_rescale_ts` with known and unknown timestamps, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `av_packet_from_data`, `av_grow_packet`, `av_shrink_packet`, `av_packet_make_refcounted`, `av_packet_make_writable`, `AV_PKT_FLAG_KEY|CORRUPT`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data` replacement/append behavior, duplicate `av_packet_new_side_data` replacement behavior, missing side-data lookup size reset, `opaque` address copying, payload preservation, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
