...
Update for the current slice: `FrameExif::common_tags()` camera-lens coverage now has build-checked fuzz invariants matching the invalid-input paths from the existing unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid ImageUniqueID, CameraOwnerName, BodySerialNumber, LensSpecification, LensMake, LensModel, and LensSerialNumber parsing plus malformed ImageUniqueID count, LensSpecification count, LensSpecification type, and LensModel ASCII byte cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_camera_lens_tags`
...
- No remaining failing commands in the latest EXIF camera-lens fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF camera-lens common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ImageUniqueID, CameraOwnerName, BodySerialNumber, LensSpecification, LensMake, LensModel, and LensSerialNumber with unit coverage for malformed unique-ID counts, LensSpecification count/type shapes, and LensModel ASCII validation. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too while preserving the valid identity strings and four-rational lens specification fixture. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` through the next selected EXIF common-tag fuzz gap, likely gamma-composite invalid-input coverage.
...
Latest slice: strengthened EXIF camera-lens common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching `frame_side_data_interprets_exif_camera_lens_tags`: malformed ImageUniqueID count, malformed LensSpecification count, malformed LensSpecification TIFF type, and malformed LensModel ASCII data, while preserving valid ImageUniqueID, camera owner, body serial, LensSpecification, lens make/model, and lens serial values. Docs and the ledger now call out the camera-lens validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_camera_lens_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
