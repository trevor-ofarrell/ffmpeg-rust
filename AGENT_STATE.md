...
`fftools-ffprobe-count-packets` is the current focus; the latest slice accepts `-count_packets` and renders `nb_read_packets` in constrained MOV/MP4 and AVI `ffprobe-rs -show_streams` output.
