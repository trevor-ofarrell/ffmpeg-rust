...
`fftools-ffprobe-mov-raw-bits-per-raw-sample` is the current focus; the latest slice preserves full MOV raw VisualSampleEntry metadata and reports raw sample-entry depth as `bits_per_raw_sample` in constrained `ffprobe-rs -show_streams` output.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as MOV sample-entry compressor-name-adjacent fields or more AVI stream header fields.
...
Latest slice: add MOV raw `bits_per_raw_sample` reporting to `ffprobe-rs`. `MovDemuxer` now preserves full `raw ` VisualSampleEntry metadata when present, and `ffprobe-rs` reports raw sample-entry depth; the current fixture proves depth 24 as `bits_per_raw_sample=24` in JSON output.
