...
`avutil-error` is the latest completed slice. The shared error model now has common constructors, IO error classification, optional preserved `io::ErrorKind` metadata, an EOF predicate, AVIO propagation, unit coverage, and build-checked `avutil_core_models` fuzz coverage, but it remains below `complete` because no full parity proof exists.
...
Latest slice: strengthened `avutil::AvError` with common constructors, stable IO error classification, preserved optional `io::ErrorKind` metadata, and an EOF predicate; wired AVIO to the shared IO conversion; added unit and build-checked fuzz coverage; and updated the ledger/docs while keeping `avutil-error` below `complete`.
