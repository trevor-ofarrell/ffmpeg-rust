...
`avutil-buffer` is the current focus for this slice. It is implemented with unit tests, callback-owned release hooks, visible versus padded byte storage, exact-shape manual buffer-pool recycling, automatic pool-owned storage return when the last shared reference drops, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because custom AVBufferPool allocation/free callbacks, hardware-frame references, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVBuffer/AVBufferRef ABI/lifetime semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-buffer` toward custom AVBufferPool allocation/free callbacks with invalid-input, ownership-transfer, and sharing tests.
...
Latest slice: added callback-owned `avutil::BufferRef` storage constructors with release callbacks and visible-length validation. Unit tests now cover release after the final shared reference, padded callback-owned storage, invalid visible-length rejection without invoking the callback, and copy-on-write behavior that releases only the original storage; `avutil_core_models` now build-checks callback release timing, invalid visible-length rejection, and callback-owned copy-on-write invariants. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
