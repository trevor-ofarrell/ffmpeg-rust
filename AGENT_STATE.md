...
Update for the current slice: `FrameExif::common_tags()` now also reports ComponentsConfiguration, MakerNote, UserComment, SubSecTime, SubSecTimeOriginal, SubSecTimeDigitized, FlashpixVersion, and RelatedSoundFile with strict UNDEFINED/ASCII type and count validation where the selected tags require fixed byte counts. `avutil_core_models` build-checks the new EXIF version/timing/comment fixture alongside the optics/subject and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF version, timing, comment, and related-file metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports ComponentsConfiguration, MakerNote, UserComment, SubSecTime, SubSecTimeOriginal, SubSecTimeDigitized, FlashpixVersion, and RelatedSoundFile while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
