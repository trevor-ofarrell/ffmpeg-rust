...
Current avutil-frame coverage also now includes deterministic build-checked `FrameVideoEncParams` valid zero-block, malformed-length, invalid type, invalid block-offset, invalid block-size, invalid block-dimension, and invalid block-constructor coverage, matching the focused video encoding parameters unit coverage for raw and deferred side-data rejection paths.

Update for the current slice: Video encoding parameters coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_video_enc_params_payload`. The deterministic `avutil_core_models` fixture now asserts valid zero-block parsing, empty/short/long/truncated payload rejection, invalid type rejection, invalid block offset and block size rejection, invalid raw block width/height rejection through raw and deferred side-data paths, invalid block constructor rejection, and non-video-enc lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens video encoding parameters fuzz coverage. `FrameVideoEncParams` already has unit coverage for valid multi-block and zero-block payloads, raw `qp`/`delta_qp`/block field preservation, malformed header/long/truncated payloads, invalid type values, invalid block offset and block size fields, invalid block dimensions, invalid block constructor dimensions, and non-video-enc lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser and deferred side-data accessor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened video encoding parameters fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the video-enc-params malformed-input unit branches for valid zero-block parsing, empty/short/long/truncated payload rejection, invalid type rejection, invalid block offset and block size rejection, invalid raw block width/height rejection through raw and deferred side-data paths, and invalid block constructor rejection. Docs and the ledger now call out these video encoding parameters malformed length/type/block-offset/block-size/dimension invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_video_enc_params_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_video_enc_params_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
