...
Latest `avutil-packet` differential slice: extended the pinned libavcodec `AVPacket` oracle rows to validate `AVPacket.opaque_ref` data, size, and `av_buffer_is_writable` state across allocation defaults, rescale, copy-props, ref, clone, move-ref, and unref rows. The Rust expected model now seeds common packets with a `BufferRef` opaque reference and compares FFmpeg's shared-reference writability transitions against `Packet`'s `opaque_ref` storage model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
