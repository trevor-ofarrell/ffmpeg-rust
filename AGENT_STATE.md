...
Latest `avutil-buffer` differential slice: added `crates/avutil/tests/buffer_oracle.rs`, an ignored pinned libavutil `AVBufferRef` oracle harness that compiles a small C helper against the local FFmpeg 8.1.1 `libavutil.a` and validates allocation status, zeroed allocation, ref sharing, refcount/writability, make-writable copy-on-write, readonly external opaque release behavior, realloc grow/shrink/shared detach prefix preservation, replace sharing, and unref nulling against the Rust `BufferRef` model. `tests/differential/mappings.txt` now exposes `avutil-buffer|oracle-libavutil-buffer`, and `fate-runner` changed-path selection maps the oracle harness back to `avutil-buffer`. `PORTING_LEDGER.toml` moves `avutil-buffer` to `differential_pass`, not `complete`, because AVBufferPool public API oracle/FATE disposition, broader ABI/lifetime closure, and strict completion documentation remain pending. Two early oracle test runs timed out because the Rust expectation builder double-locked the same mutex after readonly release; binding one lock guard fixed the harness deadlock, and the final oracle run passed.
...
`avutil-buffer` is the current focus. The latest slice adds pinned libavutil `AVBufferRef` oracle coverage for allocation status, zeroed allocation, ref sharing, refcount/writability, make-writable copy-on-write, readonly external opaque release behavior, realloc grow/shrink/shared detach prefix preservation, replace sharing, and unref nulling. The component is now `differential_pass`, not `complete`, because AVBufferPool public API oracle coverage/FATE disposition, broader ABI/lifetime parity, and completion documentation still need closure.
...
1. Continue `avutil-buffer` toward strict completion by adding or documenting `AVBufferPool` oracle/FATE disposition and closing broader AVBuffer ABI/lifetime gaps.
...
- `avutil-buffer` now has pinned libavutil `AVBufferRef` differential evidence and is marked `differential_pass`, but not complete. Remaining blockers are `AVBufferPool` public API oracle coverage or FATE disposition, broader ABI/lifetime closure, and final completion documentation.
