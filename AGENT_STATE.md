...
Update for the current slice: `FrameExif::common_tags()` now exposes root TIFF BitsPerSample through `FrameExifBitsPerSample`, validating SHORT type, nonempty count, nonzero bit depths, and count matching with SamplesPerPixel when that tag is present. The crate root re-exports the helper type. Unit tests cover valid RGB-style `[8, 8, 8]` parsing plus malformed type, zero count, zero bit depth, and SamplesPerPixel mismatch rejection, and the `avutil_core_models` build-checked fuzz fixture covers the deterministic valid and invalid BitsPerSample path. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root image-layout common-tag coverage. `FrameExif::common_tags()` now exposes BitsPerSample as a validated SHORT-array view alongside SamplesPerPixel and the existing Compression, PhotometricInterpretation, Thresholding, and FillOrder root coding/layout tags, and `avutil` now re-exports the helper type from the crate root. Unit tests cover valid parsing plus malformed type, zero count, zero value, and SamplesPerPixel mismatch rejection, and the `avutil_core_models` build-checked fuzz harness asserts the new nonempty/nonzero/count-match invariant for the accessor. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended EXIF root image-layout common-tag coverage for BitsPerSample and crate-root re-export for `FrameExifBitsPerSample`. `frame_side_data_interprets_exif_root_bits_per_sample_tags` covers valid parsing plus malformed type, zero count, zero bit depth, and SamplesPerPixel mismatch rejection, and `avutil_core_models` now build-checks deterministic fixture coverage plus nonempty/nonzero/count-match invariants for the new accessor. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_bits_per_sample_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
