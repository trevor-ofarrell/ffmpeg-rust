...
`avformat-mov-demuxer` remains the current component. It now has a local FATE-runner smoke mapping that executes the focused MOV unit suite through `cargo run -p fate-runner -- run --component avformat-mov-demuxer`, and the ledger records that mapping without marking the component complete. Exact FFmpeg ffprobe output semantics, pinned FFmpeg differential tests, upstream FATE media parity, actual local fuzz execution, additional codec-specific audio extension parsing beyond `wave`/`chan`, and broader MOV subtitle/data sample-entry coverage remain absent.
...
Latest slice: added a local FATE-runner mapping for `avformat-mov-demuxer` that drives `cargo test -p avformat mov::tests`, documented that it is smoke coverage rather than upstream FFmpeg FATE parity, and kept the component below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
