...
`avformat-streamhash-muxer` and `fftools-ffmpeg-streamhash-output` are the latest coherent slice. The constrained `ffmpeg-rs` command path now wires `-f streamhash [-hash <algorithm>] -` stdout output to the Rust `StreamHashMuxer`, preserving stream-index ordering and accumulated packet payload digests per stream while deriving stream-type labels from AVI/MOV metadata or explicit single-stream raw/WAV/YUV4MPEG2/image2 metadata. Exact FFmpeg streamhash output semantics, full generated stream type mapping, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, and broader FFmpeg streamhash behavior are still absent.
...
2. Extend streamhash stream-type mapping beyond the current AVI video and MOV video metadata subset once audio/subtitle/data stream metadata is parsed.
...
Latest slice: tightened constrained `ffmpeg-rs -f streamhash [-hash <algorithm>] -` stdout output so packet labels come from a stream-type map built from available AVI/MOV demuxer metadata or explicit single-stream raw/WAV/YUV4MPEG2/image2 metadata, with direct metadata-map tests, missing-metadata rejection coverage, and AVI streamhash command coverage added while keeping the streamhash components below `complete`.
