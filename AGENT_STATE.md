...
Latest current-loop `avutil-frame` update: Dolby Vision side-data fuzz coverage now mirrors the focused unit contracts for `AV_FRAME_DATA_DOVI_RPU_BUFFER` and `AV_FRAME_DATA_DOVI_METADATA`. The deterministic `avutil_core_models` fixture preserves non-empty and empty raw RPU payloads, checks native Dolby Vision metadata sizing and selected header/mapping/color/display-management values, and rejects malformed metadata length, native offsets, extension-block size, extension-block count, populated extension block level, and color full-range flag through raw parser, deferred side-data accessor, and constructor paths. This remains build-checked fuzz coverage only; pinned-oracle differential tests, upstream FATE media parity, and actual local fuzz execution are still blocked.
...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDolbyVisionRpuBuffer` non-empty and empty raw payload preservation plus `FrameDolbyVisionMetadata` malformed length/offset/size/count/level/color coverage, matching the focused Dolby Vision unit coverage for raw, deferred side-data, and constructor rejection paths.

Update for the current slice: Dolby Vision coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_dolby_vision_metadata_payload`. The deterministic `avutil_core_models` fixture now asserts empty RPU preservation, native metadata payload sizing, selected header/mapping/color/display-management field preservation, empty/short/long metadata rejection, invalid native offset and extension-block-size rejection, negative and oversized extension-block count rejection, zero populated extension-block level rejection, color full-range bound rejection, and non-Dolby lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- `cargo test -p avutil frame_side_data_parses_dolby_vision_payloads`
- `cargo test -p avutil frame_side_data_rejects_malformed_dolby_vision_metadata_payload`
...
- No remaining failing commands in the latest Dolby Vision fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens Dolby Vision side-data fuzz coverage. `FrameDolbyVisionRpuBuffer` and `FrameDolbyVisionMetadata` already have unit coverage for non-empty and empty RPU payloads, native metadata length and substructure sizes, selected RPU header/mapping/color/display-management values, malformed metadata lengths, invalid native offsets and extension-block size, invalid extension-block counts, zero populated extension-block levels, color full-range bounds, and non-Dolby lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser, deferred side-data accessor, and constructor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened Dolby Vision side-data fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the Dolby Vision malformed-input unit branches for non-empty and empty RPU payload preservation, native metadata sizing, selected header/mapping/color/display-management value preservation, empty/short/long metadata rejection, invalid native offset and extension-block-size rejection, negative and oversized extension-block count rejection, zero populated extension-block level rejection, color full-range bound rejection, and non-Dolby lookup absence. Docs and the ledger now call out these Dolby Vision malformed length/offset/size/count/level/color invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_dolby_vision_payloads`, `cargo test -p avutil frame_side_data_rejects_malformed_dolby_vision_metadata_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
