...
Update for the current slice: `avutil_core_models` also build-checks EXIF descriptive common-tag interpretation for ImageDescription, Software, DateTime, Artist, and Copyright plus semantic rejection for malformed known-tag ASCII type/string shapes; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with root descriptive metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports ImageDescription, Software, DateTime, Artist, and Copyright while returning typed `InvalidData` errors for malformed known-tag ASCII type/string shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
