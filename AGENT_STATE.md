...
Update for the current slice: `FrameExif::common_tags()` linked GPS destination coverage now has build-checked fuzz invariants matching more invalid-input paths from the existing GPS destination unit test. The deterministic `avutil_core_models` EXIF fixture already preserved valid GPSDestLatitudeRef, GPSDestLatitude, GPSDestLongitudeRef, GPSDestLongitude, GPSDestBearingRef, GPSDestBearing, GPSDestDistanceRef, and GPSDestDistance parsing plus destination latitude-degree, longitude-minute, bearing-range, and distance-ref-count rejection; it now also covers malformed GPSDestLatitudeRef enum value, GPSDestLongitude count, and GPSDestDistanceRef TIFF type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked GPS destination common-tag fuzz coverage. `FrameExif::common_tags()` already exposed GPSDestLatitudeRef, GPSDestLatitude, GPSDestLongitudeRef, GPSDestLongitude, GPSDestBearingRef, GPSDestBearing, GPSDestDistanceRef, and GPSDestDistance values with unit coverage for malformed latitude ref values, destination coordinate counts/ranges, bearing range, and distance-ref count/type validation. The `avutil_core_models` build-checked fuzz harness now asserts the missing GPSDestLatitudeRef enum-value, GPSDestLongitude count, and GPSDestDistanceRef type paths alongside its existing destination coordinate/bearing range and distance-ref count checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Audit the remaining EXIF common-tag unit tests against `avutil_core_models` and extend the next unmirrored invalid-input fuzz gap.
...
Latest slice: strengthened EXIF linked GPS destination common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching additional malformed cases from `frame_side_data_interprets_exif_gps_destination_tags`: malformed GPSDestLatitudeRef enum value, malformed GPSDestLongitude count, and malformed GPSDestDistanceRef TIFF type, while preserving valid GPSDestLatitudeRef, GPSDestLatitude, GPSDestLongitudeRef, GPSDestLongitude, GPSDestBearingRef, GPSDestBearing, GPSDestDistanceRef, and GPSDestDistance parsing plus existing destination coordinate range, bearing range, and distance-ref count rejection. Docs and the ledger now call out the GPS destination validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_gps_destination_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
