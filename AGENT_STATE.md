...
`fftools-ffmpeg-hash-output`, `fftools-ffmpeg-md5-output`, `fftools-ffmpeg-framehash-output`, and `fftools-ffmpeg-framemd5-output` are now implemented for the constrained Rust-native `ffmpeg-rs` execution path. `ffmpeg-rs` accepts one supported local input to stdout `-f hash -`, defaults to SHA-256, accepts output-scoped `-hash` for Adler-32, CRC-32, MD5, SHA-224, SHA-256, SHA-384, and SHA-512, feeds packet payloads through `avformat::HashMuxer`, and rejects unknown hash algorithm names without invoking FFmpeg at runtime. The dedicated `-f md5 -` muxer routes the same packet payload stream through MD5 while preserving the observable output format name as `md5` and rejecting the generic hash muxer's `-hash` option. `ffmpeg-rs` also accepts constrained stdout `-f framehash -`, defaults that packet-record muxer to SHA-256, accepts output-scoped `-hash` on framehash, and routes `-f framemd5 -` through the same per-packet record path with fixed MD5 while rejecting generic `-hash` on framemd5. The tests cover MOV packet concatenation with default SHA-256, MOV packet concatenation through `-f md5`, raw `pcm_s16le` MD5 output through `-f hash -hash md5`, unknown algorithm rejection, md5 option rejection, framehash SHA-256 and MD5 records, framemd5 records, framemd5 option rejection, and parser grouping for output-scoped `-hash`. The ledger keeps the CLI hash/framehash components at `implemented`, not `complete`, because exact FFmpeg hash/md5/framehash/framemd5 muxer output semantics, pinned-oracle differential tests, FATE coverage, broader command forms, and actual local fuzz execution are still absent.
...
`avformat-framehash-muxer`, `fftools-ffmpeg-framehash-output`, and `fftools-ffmpeg-framemd5-output` are the latest coherent slice. The constrained `ffmpeg-rs` command path now wires `-f framehash [-hash <algorithm>] -` and dedicated `-f framemd5 -` stdout output to the Rust `FrameHashMuxer`, preserving packet-level stream/timestamp/duration/size/digest records while rejecting generic `-hash` algorithm selection on dedicated framemd5. Exact FFmpeg framehash/framemd5 muxer output semantics, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, and full FFmpeg hash/framehash/framemd5 behavior are still absent.

## Next 3 Concrete Actions

1. Add pinned-oracle differential coverage for constrained `ffmpeg-rs -f hash [-hash <algorithm>] -`, `-f md5 -`, `-f framehash [-hash <algorithm>] -`, and `-f framemd5 -` once the FFmpeg 8.1.1 oracle binary is available.
2. Continue hash parity by adding a constrained `streamhash` path or tightening framehash/framemd5 output-field compatibility with tests.
...
Latest slice: added the Rust `FrameHashMuxer`, wired constrained `ffmpeg-rs -f framehash [-hash <algorithm>] -` and `-f framemd5 -` stdout output, extended packet-muxer fuzz build coverage, and updated docs/ledger/state while keeping the framehash components below `complete`.
