...
`avutil-rational` is the latest completed slice. Checked arithmetic, range rejection, unit coverage, and build-checked `avutil_core_models` fuzz coverage have been added, but broad FFmpeg rational parity remains incomplete without pinned-oracle vectors for the wider rational API surface.

## Next 3 Concrete Actions

1. Continue priority-1 avutil completeness with timebase edge-case parity or the next implemented primitive whose ledger records missing behavior.
...
Latest slice: strengthened `avutil::Rational` with checked add, subtract, negate, multiply, divide, reciprocal, and comparison coverage; extended `avutil_core_models` fuzz invariants for arithmetic round trips; and updated the ledger/docs while keeping `avutil-rational` below `complete`.
