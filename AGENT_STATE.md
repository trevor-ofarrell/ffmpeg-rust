...
Update for the current slice: `FrameExif::common_tags()` root BitsPerSample coverage now has build-checked fuzz invariants matching the remaining invalid-input paths from the existing root bits-per-sample unit test. The deterministic `avutil_core_models` EXIF fixture already preserved valid SamplesPerPixel and three-value BitsPerSample parsing plus zero-depth and SamplesPerPixel mismatch rejection; it now also covers malformed BitsPerSample TIFF type and empty BitsPerSample count cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root BitsPerSample common-tag fuzz coverage. `FrameExif::common_tags()` already exposed valid SamplesPerPixel plus three-value BitsPerSample parsing with unit coverage for malformed BitsPerSample type, empty BitsPerSample count, zero sample depth, and SamplesPerPixel mismatch validation. The `avutil_core_models` build-checked fuzz harness now asserts the missing malformed type and empty-count paths alongside its existing zero-depth and mismatch checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root BitsPerSample common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching the remaining malformed cases from `frame_side_data_interprets_exif_root_bits_per_sample_tags`: malformed BitsPerSample TIFF type and empty BitsPerSample count, while preserving valid SamplesPerPixel plus three-value BitsPerSample parsing and the existing zero-depth and SamplesPerPixel mismatch rejection checks. Docs and the ledger now call out the root BitsPerSample validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_root_bits_per_sample_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
