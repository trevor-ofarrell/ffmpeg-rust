...
Latest `avutil-buffer` nullable realloc slice: added `BufferRef::realloc(&mut Option<BufferRef>, len)` so Rust can model `av_buffer_realloc(&buf, size)` when `buf` is `NULL`, while reusing the existing resize path for non-null refs and preserving no-mutation failure behavior. The pinned `buffer_oracle` harness now validates the `av_buffer_realloc(NULL, 4)` return/status row, and `avutil_core_models` fuzz-smokes nullable realloc allocation, prefix preservation, zero-fill growth, and failed NULL realloc invariants. `avutil-buffer` remains `differential_pass`, not `complete`, because broader ABI/lifetime closure, FATE disposition, and final completion documentation remain pending.

Latest validation commands for the `avutil-buffer` nullable realloc slice: `cargo fmt --all`, `CARGO_TARGET_DIR=target-codex cargo check -p avutil`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --lib buffer_ref_realloc_handles_nullable_c_api_shape`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --test buffer_oracle -- --ignored --nocapture`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --mappings tests\\differential\\mappings.txt --component avutil-buffer --target oracle-libavutil-buffer --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, WSL `CARGO_TARGET_DIR=target-wsl cargo run -p fate-runner -- run --component avutil-buffer`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo check --manifest-path fuzz\\Cargo.toml`, `CARGO_TARGET_DIR=target-codex cargo clippy -p avutil -p fate-runner --all-targets --all-features -- -D warnings`, `CARGO_TARGET_DIR=target-codex\\fuzz-check cargo clippy --manifest-path fuzz\\Cargo.toml -- -D warnings`, and WSL `CARGO_TARGET_DIR=target-wsl-fuzz cargo fuzz run avutil_core_models -- -runs=1` passed. Windows `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --component avutil-buffer` ran the selected buffer tests successfully, then failed when Windows Application Control blocked a rebuilt `error_oracle` test binary with `os error 4551`; the equivalent WSL component mapping passed.
