...
2. Extend MOV subtitle/data parsing into remaining timed metadata/text variants or add pinned-oracle differential tests for the newly typed `tx3g`/`stxt`/`urim` paths when an FFmpeg 8.1.1 oracle binary is available.
...
Latest slice: extended MOV text/data sample-entry parsing with `stxt` simple text entries, `urim` URI metadata entries, optional `mett` `btrt`/`txtC` child boxes, malformed full-box/string/duplicate-child tests, and focused MOV unit coverage. MOV stays below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
