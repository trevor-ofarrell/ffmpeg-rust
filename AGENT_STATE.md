...
Update for the current slice: `FrameExif::common_tags()` now validates GPS direction and bearing rational ranges before returning the selected common-tag view. GPSTrack, GPSImgDirection, and GPSDestBearing must be compass bearings below 360 degrees in addition to the existing GPS coordinate and timestamp range checks. Unit tests and the `avutil_core_models` build-checked fuzz fixture cover invalid source track, image direction, and destination bearing values, and the fuzz harness now asserts accepted generated common-tag direction rationals stay below 360 degrees. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` GPS direction validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count, shape, coordinate, and timestamp enforcement and additionally rejects GPSTrack, GPSImgDirection, and GPSDestBearing rationals greater than or equal to 360 degrees. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: tightened `FrameExif::common_tags()` GPS direction validation so GPSTrack, GPSImgDirection, and GPSDestBearing rationals must be below 360 degrees in addition to the existing EXIF/GPS fixed-count, shape, coordinate, and timestamp checks. Unit tests and the `avutil_core_models` fuzz fixture now cover invalid source track, image direction, and destination bearing values, while the random fuzz path asserts accepted generated direction values remain below 360 degrees. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_motion_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_destination_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
