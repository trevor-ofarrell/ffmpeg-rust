...
Update for the current slice: `FrameExif::common_tags()` linked GPS processing/error coverage now has build-checked fuzz invariants matching the existing GPS processing/error unit test. The deterministic `avutil_core_models` EXIF fixture now preserves valid GPSProcessingMethod, GPSAreaInformation, GPSDifferential, and GPSHPositioningError parsing and rejects malformed GPSProcessingMethod type, GPSAreaInformation type, GPSDifferential count/value, and GPSHPositioningError type/denominator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked GPS processing/error common-tag fuzz coverage. `FrameExif::common_tags()` already exposed GPSProcessingMethod, GPSAreaInformation, GPSDifferential, and GPSHPositioningError values with unit coverage for malformed UNDEFINED payload types, differential count/value, and positioning-error type/denominator validation. The `avutil_core_models` build-checked fuzz harness now carries a deterministic GPS processing/error fixture and asserts those valid and invalid paths. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
