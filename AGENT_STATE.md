...
Latest parity status: strict completion is now 4/96 components (about `4%`). `avutil-error`, `avutil-rational`, `avutil-timebase`, and `avutil-hash` are the current `complete` rows for the default-native profile. Intermediate measurable parity now includes 3/96 non-complete status rows: `repo-runtime-guard`, `fftools-version`, and `avutil-channel-layout` are `differential_pass`/validated evidence rows. Current ledger status counts are 88 `implemented`, 1 `scaffolded`, 3 `differential_pass`, and 4 `complete`.
...
Latest documentation update: the repository-level `README.md` now reports strict parity completion as about 4% (`4/96`), with 3 additional rows currently carrying non-complete validation status, and still reports practical engineering progress at about 2% of a complete FFmpeg 8.1.1 default-native rewrite.
