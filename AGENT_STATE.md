...
Latest `avutil-pixel-format` / rawvideo update: `PixelFormatDescriptor` now stores exact `Rational` average bits per pixel, and the shared pixel format model includes FFmpeg's 9-bit planar YUV 4:2:0/4:2:2/4:4:4 formats: `yuv420p9le`, `yuv420p9be`, `yuv422p9le`, `yuv422p9be`, `yuv444p9le`, and `yuv444p9be`, in addition to the existing 10-bit and 12-bit planar YUV families. The slice was checked against pinned FFmpeg 8.1.1 `libavutil/pixfmt.h` and `libavutil/pixdesc.c`: upstream defines `yuv420p9*` as 13.5 bpp (`27/2`) with log2 chroma `(1,1)`, `yuv422p9*` as 18 bpp with `(1,0)`, `yuv444p9*` as 27 bpp with `(0,0)`, and all 9/10/12-bit planar YUV variants use two stored bytes per component sample. Rust now exposes the new `PixelFormat` variants and names, exact rational descriptor metadata, two-byte-per-sample frame-size and plane-splitting math, `VideoFrame` line sizing, rawvideo decode/demux/mux packet sizing, constrained `ffmpeg-rs -f rawvideo ... -pix_fmt yuv420p9le -f null -` execution, and affected fuzz-harness invariants. This remains below `complete` because full `AVPixFmtDescriptor` parity, pinned oracle differential vectors, upstream FATE parity, pixel conversion, hardware formats, and actual fuzz execution are still absent.
...
- Current rational-bpp / 9-bit planar YUV `avutil-pixel-format` / rawvideo slice:
  - `cargo fmt --all`
  - `cargo fmt --all -- --check`
  ...
  - `$env:CARGO_TARGET_DIR='target-fftools-gray32-test'; cargo test -p fftools runs_rawvideo_yuv420p9le_to_null_stdout`
...
- Current rational-bpp / 9-bit planar YUV `avutil-pixel-format` / rawvideo slice: no remaining code/test assertion failures and no fresh Windows Application Control blocks during final validation. The first workspace clippy run failed on a now-redundant `i32::from(bits_per_pixel)` conversion after the descriptor tuple inferred an `i32`; removing that conversion fixed clippy. Focused avutil/avcodec/avformat/fftools tests, fuzz package check/clippy, workspace clippy, local FATE-runner `run --changed`, workspace library tests, formatting, and `git diff --check` passed. `git diff --check` reported CRLF warnings only; Cargo commands continued to report the usual `could not canonicalize path C:\Users\trevo` warning. Oracle differentials, upstream FATE media coverage, full `AVPixFmtDescriptor` parity, and actual fuzz execution remain incomplete.
...
`avutil-pixel-format` remains the current focus, with linked rawvideo decoder, demuxer, muxer, constrained `ffmpeg-rs` input parsing, and fuzz-harness invariant coverage. The latest concrete change makes average descriptor bpp exact rational metadata and adds FFmpeg's 9-bit planar YUV families for 4:2:0, 4:2:2, and 4:4:4 as three two-byte-per-sample planes, including exact `27/2` descriptor bpp for `yuv420p9*`. It does not claim conversion support, full `AVPixFmtDescriptor` parity, full `ffmpeg -pix_fmts` inventory, pinned oracle parity, upstream FATE parity, or actual fuzz execution.
...
1. Continue priority-1 format-model work by choosing the next rawvideo-facing FFmpeg 8.1.1 pixel-format family from pinned inventory/source notes, now that fractional average descriptor bpp is representable.
...
- Current rational-bpp / 9-bit planar YUV validation has no remaining code/test assertion failures and did not hit fresh Windows Application Control blocks. `git diff --check` reported CRLF warnings only; Cargo commands reported only the usual `could not canonicalize path C:\Users\trevo` warning. The remaining blockers are oracle differentials, upstream FATE media parity, full `AVPixFmtDescriptor` parity, pixel conversion, hardware formats, and actual fuzz execution.
...
Latest slice: added exact rational descriptor bpp and FFmpeg 8.1.1 9-bit planar YUV support to the current shared pixel/rawvideo model. `PixelFormatDescriptor::bits_per_pixel` is now a `Rational`, with `bits_per_pixel_integer()` for callers that still need whole-byte packed checks, and `PixelFormat` now reports `yuv420p9le`/`yuv420p9be`, `yuv422p9le`/`yuv422p9be`, and `yuv444p9le`/`yuv444p9be` in the inventory with YUV descriptor metadata, 9-bit component depth, exact `27/2`, `18/1`, or `27/1` average descriptor bpp, three planar payload planes, no alpha or float flag, and the same 4:2:0/4:2:2/4:4:4 chroma validation as the matching 8-bit/10-bit/12-bit families. `VideoFrame`, `RawVideoDecoder`, `RawVideoDemuxer`, `RawVideoMuxer`, constrained `ffmpeg-rs` rawvideo-to-null execution, and the affected fuzz harnesses now exercise the family, including decoder plane/line-size coverage, demux/mux frame-size coverage for all six names, `yuv420p9le` CLI coverage, and rational bpp fuzz-build invariants. Validation passed with focused avutil/avcodec/avformat/fftools tests, fuzz package check/clippy, workspace format check, workspace clippy, local FATE-runner changed mappings, workspace library tests, and `git diff --check` with CRLF warnings only. The affected components remain `implemented`, not `complete`, because oracle differentials, upstream FATE media coverage, full pixel inventory coverage, full `AVPixFmtDescriptor` parity, pixel conversion, hardware formats, and actual fuzz execution are still absent.
