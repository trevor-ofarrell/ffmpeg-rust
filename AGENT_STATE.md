...
Latest slice: added build-checked cargo-fuzz target `avformat_basic_muxers` plus seed corpora, updated the WAV, raw PCM s16le, rawvideo, and yuv4mpegpipe muxer ledger fuzz target fields, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
