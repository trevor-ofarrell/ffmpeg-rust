...
Current avutil-frame coverage also now includes deterministic build-checked `FrameS12mTimecode` malformed-length and invalid-count coverage, matching the focused S12M unit coverage for empty/short/long payload rejection and native count validation.

Update for the current slice: S12M timecode coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_s12m_timecode_payload`. The deterministic `avutil_core_models` fixture now asserts empty constructor rejection, long payload rejection, empty side-data payload rejection, and native count `4`/`u32::MAX` rejection through both raw and deferred side-data paths. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens S12M timecode fuzz coverage. `FrameS12mTimecode` already has unit coverage for valid two-entry payloads, raw unused-slot preservation, malformed empty/short/long payloads, invalid constructor counts, invalid native counts, and non-S12M lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors the remaining malformed-input branches, including empty constructor rejection, long payload rejection, empty side-data payload rejection, and native count `4`/`u32::MAX` rejection through both raw and deferred side-data paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened S12M timecode fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the remaining S12M malformed-input unit branches for empty constructor rejection, long payload rejection, empty side-data payload rejection, and invalid native counts `4` and `u32::MAX` through both raw and deferred side-data paths. Docs and the ledger now call out these S12M malformed-length and invalid-count invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_s12m_timecode_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_s12m_timecode_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
