...
Update for the current slice: `FrameExif::common_tags()` now validates DateTime, DateTimeOriginal, DateTimeDigitized, OffsetTime, OffsetTimeOriginal, OffsetTimeDigitized, and GPSDateStamp numeric ranges after the existing fixed-count and ASCII-shape checks. Accepted date tags keep month/day fields within simple `1..=12` and `1..=31` ranges, DateTime clock fields stay within a 24-hour clock, and offset-time fields stay within `23:59`. Unit tests and the `avutil_core_models` build-checked fuzz fixture cover out-of-range root DateTime, linked EXIF DateTimeOriginal/DateTimeDigitized, offset time, and GPSDateStamp values, and the fuzz harness asserts accepted generated common-tag date/time strings preserve those ranges. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` date/time validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count, ASCII-shape, coordinate, timestamp, and direction enforcement and additionally rejects DateTime-family month/day/time fields, OffsetTime-family hour/minute fields, and GPSDateStamp month/day fields outside the current simple accepted ranges. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: tightened `FrameExif::common_tags()` DateTime, OffsetTime, and GPSDateStamp validation so accepted strings must stay within simple month/day/time ranges after the existing fixed-count and ASCII-shape checks. Unit tests and the `avutil_core_models` fuzz fixture now cover invalid root DateTime hours, linked EXIF DateTimeOriginal months, DateTimeDigitized minutes, OffsetTime hours/minutes, and GPSDateStamp days, while the random fuzz path asserts accepted generated common-tag date/time strings preserve those ranges. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_common_exif_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_exposure_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_altitude_time_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_offset_time_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_descriptive_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
