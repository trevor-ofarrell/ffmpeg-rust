...
Update for the current slice: `FrameExif::common_tags()` camera-characterization and rendering/scene EXIF coverage now has build-checked fuzz invariants matching the raw-value details already asserted by the unit tests. The deterministic `avutil_core_models` EXIF fixtures now preserve focal-plane X/Y rational denominators, the raw focal-plane resolution-unit value, and raw enum values for ColorSpace, SensingMethod, FileSource, SceneType, CustomRendered, ExposureMode, SceneCaptureType, GainControl, Contrast, Saturation, Sharpness, and SubjectDistanceRange. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF camera-characterization and rendering/scene value-preservation fuzz coverage. `FrameExif::common_tags()` already exposed typed focal-plane rationals, focal-plane resolution units, and rendering/scene enums, and the unit tests already asserted denominator/raw-value details. The `avutil_core_models` build-checked fuzz harness now mirrors focal-plane X/Y rational denominators, raw focal-plane resolution-unit value, and raw ColorSpace/SensingMethod/FileSource/SceneType/CustomRendered/ExposureMode/SceneCaptureType/GainControl/Contrast/Saturation/Sharpness/SubjectDistanceRange enum values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Continue auditing the remaining EXIF common-tag unit tests against `avutil_core_models` and extend the next unmirrored invalid-input or raw-value preservation fuzz gap.
...
Latest slice: strengthened EXIF raw-value preservation fuzz coverage for camera-characterization and rendering/scene common tags. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_interprets_exif_camera_characterization_tags` and `frame_side_data_interprets_exif_rendering_scene_tags`: focal-plane X/Y rational denominator preservation, raw focal-plane resolution-unit preservation, and raw enum preservation for ColorSpace, SensingMethod, FileSource, SceneType, CustomRendered, ExposureMode, SceneCaptureType, GainControl, Contrast, Saturation, Sharpness, and SubjectDistanceRange. Docs and the ledger now call out these value-preservation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_camera_characterization_tags`, `cargo test -p avutil frame_side_data_interprets_exif_rendering_scene_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
