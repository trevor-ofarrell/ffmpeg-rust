...
`avformat-mov-demuxer` now parses version-0 `AudioSampleEntry` metadata for known MOV/MP4 audio sample-entry tags. The parsed model captures version, revision, vendor, channel count, sample size, compression ID, packet size, 16.16 sample rate, and child boxes, with invalid truncated entries and malformed child boxes rejected as typed errors. `ffprobe-rs` now surfaces constrained MOV audio stream `sample_rate`, `channels`, and `bits_per_sample` fields from that Rust parser and maps `mp4a` to AAC names in the limited local codec-name table. The ledger keeps the affected MOV and ffprobe entries at `implemented`, not `complete`, because pinned-oracle differential tests, upstream FATE coverage, broader AudioSampleEntry versions, subtitle/data sample entries, decoder-derived audio fields, and actual local fuzz execution are still absent.
...
`avformat-mov-demuxer`, `fftools-ffprobe-stream-codec-fields`, and `fftools-ffprobe-audio-stream-fields` are the latest coherent slice. The MOV demuxer now preserves `mdia/hdlr` handler types and parses version-0 AudioSampleEntry fields for known audio sample-entry tags; `ffprobe-rs` uses known MOV handler types for `codec_type` and emits constrained MOV audio `sample_rate`, `channels`, and `bits_per_sample` from parsed sample-entry metadata. Exact FFmpeg ffprobe output semantics, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, AudioSampleEntry version 1/2 extensions, and broader MOV subtitle/data sample-entry parsing remain absent.
...
2. Extend MOV sample-entry parsing beyond version-0 audio into AudioSampleEntry version 1/2 extensions and subtitle/data sample-entry metadata.
...
Latest slice: parsed MOV version-0 AudioSampleEntry metadata for known audio sample-entry tags, surfaced constrained ffprobe MOV audio `sample_rate`/`channels`/`bits_per_sample` fields, added valid and malformed audio sample-entry tests, and kept the affected components below `complete` because oracle/FATE/fuzz parity remains missing.
