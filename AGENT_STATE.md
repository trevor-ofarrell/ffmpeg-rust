...
Current avutil-frame coverage also now includes deterministic build-checked `FrameFilmGrainParams` valid AV1/H.274 payload parsing plus malformed length/type/dimension, AOM count/boolean/shift/offset, H.274 model/blending/present/count/model-value/interval, and absent-component coverage, matching the focused film grain parameters unit coverage for raw and deferred side-data rejection paths.

Update for the current slice: Film grain parameters coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_film_grain_params_payload`. The deterministic `avutil_core_models` fixture now asserts valid AV1 and H.274 parsing, `FrameFilmGrainParamsType` raw-value/constant mapping, empty/short/long payload rejection, invalid top-level type and dimension rejection, invalid AOM count/boolean/shift/offset rejection, invalid H.274 model/blending/present/count/model-value/interval rejection, absent-component count rejection, and non-film-grain lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens film grain parameters fuzz coverage. `FrameFilmGrainParams` already has unit coverage for valid AV1 and H.274 payloads, common header preservation, AOM point/count/shift/flag fields, selected H.274 interval/model fields, malformed length/type/dimension payloads, invalid AOM count/boolean/shift/offset fields, invalid H.274 model/blending/present/count/model-value/interval fields, absent-component count rejection, and non-film-grain lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser and deferred side-data accessor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened film grain parameters fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the film-grain malformed-input unit branches for valid AV1/H.274 parsing, raw-value/constant mapping, empty/short/long payload rejection, invalid top-level type/dimension rejection, invalid AOM count/boolean/shift/offset rejection, invalid H.274 model/blending/present/count/model-value/interval rejection, absent-component count rejection, and non-film-grain lookup absence. Docs and the ledger now call out these film grain malformed length/type/dimension/AOM/H.274 invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_film_grain_params_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_film_grain_params_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
