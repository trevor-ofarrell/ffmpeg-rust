...
`fftools-ffprobe-stream-coded-dimensions` is the current focus; the latest slice reports constrained `coded_width` and `coded_height` values in `ffprobe-rs -show_streams` output for MOV/MP4 and AVI streams whose demuxer metadata already has dimensions.
...
2. Add more image2 or AVI option parity such as image2 pattern type/glob handling, timestamp modes, or deeper AVI stream metadata.
...
Latest slice: add constrained coded-dimension reporting to `ffprobe-rs`. The stream report now renders `coded_width` and `coded_height` in default and JSON output for MOV/MP4 and AVI video streams by mirroring parsed dimensions in the current model; fixtures prove raw MOV, H.264 MOV, and raw AVI output. This is not full upstream coded/display dimension parity because cropping and codec-parser-derived dimensions are not modeled yet, and it still lacks pinned-oracle differential/FATE coverage.
