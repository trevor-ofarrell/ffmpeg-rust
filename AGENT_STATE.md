...
`avformat-mov-demuxer`, `fftools-ffprobe-stream-codec-fields`, and `fftools-ffmpeg-streamhash-output` are the latest coherent slice. The MOV demuxer now preserves `mdia/hdlr` handler types; `ffprobe-rs` uses known MOV handler types for `codec_type`, and constrained `ffmpeg-rs -f streamhash -` uses them for MOV stream labels before falling back to video sample-entry/dimension metadata. Exact FFmpeg streamhash and ffprobe output semantics, full generated stream type mapping, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, and broader MOV audio/subtitle/data parsing remain absent.
...
2. Extend MOV sample-entry parsing for real audio/subtitle/data metadata beyond handler-level classification.
...
Latest slice: preserved MOV `hdlr` handler types in `MovTrackInfo`, used known handler types for ffprobe `codec_type` and MOV streamhash labels, added MOV audio-handler ffprobe/streamhash coverage, and kept the affected components below `complete` because oracle/FATE/fuzz parity remains missing.
