...
Latest slice: added explicit audio line-size constructors for the current `avutil-frame` packed-sample model. `AudioFrame` can now retain padded per-plane storage through `new_with_line_sizes`, `new_with_buffer_refs_and_line_sizes`, `new_with_channel_layout_and_line_sizes`, and `new_with_channel_layout_and_buffer_refs_and_line_sizes`, while `planes()` exposes only visible sample bytes and `set_plane_visible_data()` preserves trailing storage padding during copy-on-write mutation. Unit and fuzz-build coverage prove valid padded storage, invalid line-size rejection, strict tight-buffer constructors, clone detachment, and channel-layout constructor behavior. Validation passed with `cargo run -p fate-runner -- run --component avutil-frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, and `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
