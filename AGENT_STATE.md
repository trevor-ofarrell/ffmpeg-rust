...
Update for the current slice: `FrameExif::common_tags()` now validates GPS one-character reference tags with their fixed count-two ASCII shape before enum interpretation. The common GPS, acquisition, motion, and destination fixtures now cover invalid count handling for GPSLatitudeRef, GPSStatus, GPSTrackRef, and GPSDestDistanceRef in unit tests and the `avutil_core_models` build-checked fuzz target. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` GPS reference validation in the selected common-tag view. `FrameExif::common_tags()` now requires GPSLatitudeRef/GPSLongitudeRef, GPSStatus, GPSMeasureMode, GPSSpeedRef, GPSTrackRef, GPSImgDirectionRef, GPSDestLatitudeRef/GPSDestLongitudeRef, GPSDestBearingRef, and GPSDestDistanceRef to use fixed count-two ASCII entries before enum interpretation. DateTime, OffsetTime, and GPSDateStamp fixed-count validation remains in place. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: tightened `FrameExif::common_tags()` GPS one-character reference validation so latitude/longitude refs, status, measure mode, speed ref, direction refs, and destination refs require fixed count-two ASCII entries before enum interpretation. Unit tests and the `avutil_core_models` fuzz fixture now cover valid reference parsing plus invalid count handling across common GPS, acquisition, motion, and destination fixtures. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_common_exif_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_acquisition_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_motion_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_destination_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
