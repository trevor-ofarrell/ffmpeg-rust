...
This slice extends EXIF root strip/position common-tag coverage. `FrameExif::common_tags()` now exposes RowsPerStrip as a positive SHORT/LONG value and XPosition/YPosition as single unsigned RATIONAL values. Unit tests cover valid parsing plus malformed type, count, zero-value, and zero-denominator rejection, and the `avutil_core_models` build-checked fuzz harness asserts positive rows-per-strip and nonzero rational-denominator invariants for the new root tags. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
