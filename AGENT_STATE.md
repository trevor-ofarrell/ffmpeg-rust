...
Update for the current slice: malformed EXIF frame side-data payload rejection now has broader build-checked fuzz invariants matching `frame_side_data_rejects_malformed_exif_payload`. The deterministic `avutil_core_models` EXIF fixture now rejects empty and truncated payloads, bad TIFF markers, invalid first/next IFD offsets, missing counts, oversized and truncated IFD tables, invalid TIFF entry types, out-of-line value ranges, linked pointer type/count/offset errors, linked/root IFD loops, invalid TIFF type IDs, and checks the raw parser, `FrameSideData::new_exif`, and deferred `side_data.exif()` error paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens malformed EXIF payload fuzz coverage. `FrameExif` already rejected malformed TIFF headers, directory offsets, IFD tables, entry types, value ranges, pointer fields, pointer loops, and next-IFD loops in the unit test. The `avutil_core_models` build-checked fuzz harness now mirrors those deterministic invalid-input assertions and verifies the same invalid data fails through the raw parser, `FrameSideData::new_exif`, and deferred `side_data.exif()` paths. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened malformed EXIF payload fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_rejects_malformed_exif_payload`: empty and truncated payloads, bad TIFF markers, invalid first/next IFD offsets, missing counts, oversized/truncated IFD tables, invalid TIFF entry types, out-of-line value ranges, linked pointer type/count/offset errors, linked/root IFD loops, invalid TIFF type IDs, and raw parser/new_exif/deferred side-data error paths. Docs and the ledger now call out these malformed-payload shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_rejects_malformed_exif_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
