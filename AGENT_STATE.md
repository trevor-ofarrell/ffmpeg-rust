...
Latest `avutil-packet` differential slice: added explicit `av_packet_clone` parity evidence on top of the existing packet/frame side-data bridge coverage. `Packet::clone` is now unit-tested as the Rust equivalent of `av_packet_clone`: it preserves packet fields, shares the payload and `opaque_ref` references, keeps side-data payloads independently mutable, and detaches data on write. `crates/avutil/tests/packet_oracle.rs` now emits an `av_packet_clone` row and compares it against the Rust packet clone model through the pinned libavcodec/libswresample/libavutil helper. `avutil_core_models` now also build-checks packet clone lifecycle invariants alongside ref/move/copy-props invariants. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
...
Latest validation commands for the `av_packet_clone` oracle slice: `cargo fmt --all`, `cargo fmt --all -- --check`, `cargo test -p avutil packet_clone_matches_ref_from_shape`, `cargo test -p avutil packet`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, `cargo clippy --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check -- -D warnings`, WSL `cargo fuzz run avutil_core_models -- -runs=1`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, and `git diff --check` pass. One packet oracle rerun exceeded its initial timeout during validation and passed when rerun with a longer timeout; `git diff --check` reports CRLF conversion warnings only.
