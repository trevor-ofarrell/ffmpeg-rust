...
Update for the current slice: `FrameExif::common_tags()` now validates EXIF ASCII date/time value shapes before returning the selected common-tag view. DateTime, DateTimeOriginal, and DateTimeDigitized must be fixed 20-byte ASCII fields matching `YYYY:MM:DD HH:MM:SS`; OffsetTime, OffsetTimeOriginal, and OffsetTimeDigitized must be fixed 7-byte ASCII fields matching `[+-]HH:MM`; and GPSDateStamp must be a fixed 11-byte ASCII field matching `YYYY:MM:DD`. Unit tests and the `avutil_core_models` build-checked fuzz target cover invalid root DateTime, linked EXIF DateTimeOriginal/DateTimeDigitized, linked EXIF OffsetTime, and linked GPSDateStamp shapes. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` date/time validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count ASCII enforcement and additionally rejects DateTime/DateTimeOriginal/DateTimeDigitized values that do not match `YYYY:MM:DD HH:MM:SS`, OffsetTime/OffsetTimeOriginal/OffsetTimeDigitized values that do not match `[+-]HH:MM`, and GPSDateStamp values that do not match `YYYY:MM:DD`. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
