...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed line-size metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane ownership, BufferRef-backed frame side-data ownership with typed known-kind mapping, explicit set-style replacement/coalescing, and per-record metadata, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because the complete FFmpeg frame side-data kind inventory, custom alignment/stride, writable plane mutation, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership/shape gap, likely custom alignment/stride metadata or writable plane mutation semantics, with invalid-input and sharing tests.
...
Latest slice: added explicit `Frame::set_side_data*` replacement helpers, mutable typed side-data lookup, and duplicate coalescing for records that resolve to the same `FrameSideDataKind`. Unit tests now prove append-on-absent behavior, alias-based replacement, duplicate removal, replacement metadata mutation, retained order for unrelated side data, and old `BufferRef` release timing after removed records drop; `avutil_core_models` now build-checks replacement/coalescing alongside typed-kind mapping, metadata, removal, retained BufferRef storage sharing, padding exclusion, frame plane ownership, and hw context invariants. Validation passed with formatting, focused `cargo test -p avutil frame` execution, fuzz-target check/clippy, workspace clippy, `cargo test -p avutil --lib --no-run`, local `avutil-frame` FATE-runner execution, and FATE-runner changed dry-run. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
