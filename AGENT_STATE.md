...
Update for the current slice: `FrameExif::common_tags()` root TIFF coding-tag coverage now has build-checked fuzz invariants matching the existing unit tests for malformed `Compression` and `PhotometricInterpretation` shapes. The deterministic `avutil_core_models` EXIF root coding fixture now covers valid parsing, malformed Compression type/count/value-zero cases, and malformed PhotometricInterpretation type/count cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_root_coding_tags`
...
- No remaining failing commands in the latest EXIF root coding fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF root coding common-tag fuzz coverage. `FrameExif::common_tags()` already exposed `Compression` as a nonzero raw SHORT and `PhotometricInterpretation` as a raw SHORT, with unit coverage for malformed type/count/value shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: Compression wrong type, wrong count, zero value, and PhotometricInterpretation wrong type/count. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root coding common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_root_coding_tags`: malformed Compression type, malformed Compression count, zero Compression value, malformed PhotometricInterpretation type, and malformed PhotometricInterpretation count. Docs and the ledger now call out the coding-tag type/count/value validation shape explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_root_coding_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
