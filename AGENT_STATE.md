...
Update for the current slice: `FrameExif::common_tags()` now also reports camera-characterization EXIF metadata for SpectralSensitivity, OECF, FlashEnergy, SpatialFrequencyResponse, FocalPlaneXResolution, FocalPlaneYResolution, FocalPlaneResolutionUnit, and CFAPattern. OECF, SpatialFrequencyResponse, and CFAPattern are preserved as strict UNDEFINED byte payloads, while spectral sensitivity, flash energy, and focal-plane resolution fields use the existing ASCII, RATIONAL, and resolution-unit validation paths. `avutil_core_models` build-checks the new fixture alongside the exposure, sensitivity, APEX exposure, environment, camera/lens, gamma/composite, optics/subject, version/timing/comment, and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF camera-characterization metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports SpectralSensitivity, OECF, FlashEnergy, SpatialFrequencyResponse, FocalPlaneXResolution, FocalPlaneYResolution, FocalPlaneResolutionUnit, and CFAPattern while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended `FrameExif::common_tags()` with linked EXIF camera-characterization metadata for SpectralSensitivity, OECF, FlashEnergy, SpatialFrequencyResponse, FocalPlaneXResolution, FocalPlaneYResolution, FocalPlaneResolutionUnit, and CFAPattern. Unit tests and `avutil_core_models` fuzz fixtures cover the new fields plus strict ASCII/UNDEFINED/RATIONAL/SHORT type validation, rational count-one validation, defined resolution-unit validation, and malformed CFA type rejection. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_camera_characterization_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
