...
- `cargo test -p avutil frame_side_data_properties_drive_multi_and_removal_helpers`
...
- No remaining failing commands in the latest frame `ref_from` replacement fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens frame `ref_from` replacement fuzz coverage. `Frame` already has a unit test proving `ref_from` releases an existing destination frame's external plane owner, copies PTS, shares source plane/side-data/hw-context references, and leaves the source frame live. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened frame `ref_from` replacement fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_ref_from_shares_references_and_replaces_destination`: `Frame::ref_from` releases an existing destination frame's external plane owner, copies PTS, shares source plane/side-data/hw-context references, and leaves the source frame live. Docs and the ledger now call out these `ref_from` ownership invariants explicitly. Validation passed with `cargo test -p avutil frame_ref_from_shares_references_and_replaces_destination`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
