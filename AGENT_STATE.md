...
`fftools-ffmpeg-rawvideo-framecrc-null` is the current focus; the next slice is broader `ffmpeg-rs` command execution coverage or deeper MOV timeline support.

## Next 3 Concrete Actions

1. Extend `ffmpeg-rs` command execution to another already-implemented demuxer such as yuv4mpegpipe or image2 to framecrc/null.
...
Latest slice: extend constrained `ffmpeg-rs` execution to explicit `rawvideo` inputs for stdout `-f framecrc -` or `-f null -`, requiring explicit `-pix_fmt`, `-s`, and `-r`, using the Rust rawvideo demuxer and Rust muxers with in-process unit coverage.
