...
`avformat-hash-muxer` now supports SHA-224 and SHA-256 packet-data hashing in addition to Adler-32, IEEE CRC-32, and MD5. `HashMuxerReport` carries a variable-width `HashDigest`, formats MD5/SHA-224/SHA-256 digest lines as lowercase hex, preserves packet/byte accounting, and the `avformat_packet_muxers` fuzz target now build-checks SHA-224/SHA-256 digest stability alongside Adler-32/CRC-32/MD5. The ledger still keeps `avformat-hash-muxer` at `implemented`, not `complete`, because CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned-oracle differential tests, FATE coverage, remaining SHA variants, and actual local fuzz execution are still absent.

`avutil-hash` now includes Rust-native SHA-224 and SHA-256 support alongside Adler-32, IEEE CRC-32, and MD5 helpers. `Sha224` and `Sha256` support one-shot and streaming updates across block boundaries, `digest_to_hex` formats lowercase digest output for byte digests, and `avutil_core_models` now build-checks SHA-224/SHA-256 streaming equivalence and hex invariants in addition to Adler-32/CRC-32/MD5 streaming equivalence. The ledger still keeps `avutil-hash` at `implemented`, not `complete`, because pinned-oracle differential vectors, FATE coverage, actual local fuzz execution, additional SHA variants, and full FFmpeg hash/framehash behavior are still absent.
...
`avutil-hash` and `avformat-hash-muxer` are the latest coherent slice. SHA-224 one-shot/streaming support was added by sharing the SHA-256 compression core with the SHA-224 initial vector/truncated digest, then wired into the internal hash muxer beside Adler-32, IEEE CRC-32, MD5, and SHA-256. Standard-vector tests and build-checked `avutil_core_models`/`avformat_packet_muxers` coverage now include SHA-224. CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, remaining SHA variants, and full FFmpeg hash/framehash behavior are still absent.

## Next 3 Concrete Actions

1. Continue hash parity by adding the next missing SHA digest variant or wiring the internal hash muxer into constrained CLI hash output with tests.
...
Latest slice: added Rust-native SHA-224 one-shot/streaming helpers, wired SHA-224 into `avformat::HashMuxer`, extended hash fuzz coverage, and updated docs/ledger/state while keeping `avutil-hash` and `avformat-hash-muxer` below `complete`.
