...
Update for the current slice: `FrameExif::common_tags()` now rejects degenerate SubjectArea circle and rectangle shapes after the existing count/type checks. Point coordinates are still accepted as-is, while circle diameter and rectangle width/height must be nonzero. Unit tests and the `avutil_core_models` build-checked fuzz fixture cover zero diameter, width, and height rejection. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF GPS DMS boundary coverage. Unit tests now prove latitude and longitude accept exact 90/180 degree boundaries only when minutes and seconds are zero, reject composite DMS values that exceed those boundaries, and reject a fractional GPSTimeStamp value whose total reaches 24 hours even though each field is individually in range. The `avutil_core_models` fuzz harness now asserts per-field and composite DMS range invariants for GPS coordinates, destination coordinates, GPSTimeStamp, and direction rationals. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next typed side-data payload schema or remaining per-buffer permission-model gap, with invalid-input and sharing tests.
...
Latest slice: strengthened EXIF GPS DMS boundary coverage. Unit tests now cover exact max-coordinate acceptance, latitude/longitude composite overflow rejection, and fractional GPSTimeStamp composite overflow rejection; `avutil_core_models` now asserts full GPS coordinate/time/direction range invariants instead of denominator-only checks. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_common_exif_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gps_altitude_time_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, and `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`.
