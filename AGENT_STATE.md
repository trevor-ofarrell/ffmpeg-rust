...
`avformat-streamhash-muxer` and `fftools-ffmpeg-streamhash-output` are the latest coherent slice. The constrained `ffmpeg-rs` command path now wires `-f streamhash [-hash <algorithm>] -` stdout output to the Rust `StreamHashMuxer`, preserving stream-index ordering, accumulated packet payload digests per stream, and constrained audio/video stream-type labels. Exact FFmpeg streamhash output semantics, generated stream type mapping from real stream metadata, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, and broader FFmpeg streamhash behavior are still absent.
...
2. Tighten hash-family output-field compatibility, including deriving streamhash stream-type labels from demuxer stream metadata instead of command-path assumptions.
...
Latest slice: added the Rust `StreamHashMuxer`, wired constrained `ffmpeg-rs -f streamhash [-hash <algorithm>] -` stdout output, extended packet-muxer fuzz build coverage, and updated docs/ledger/state while keeping the streamhash components below `complete`.
