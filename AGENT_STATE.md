...
Update for the current slice: `FrameExif::common_tags()` now interprets root TIFF Thresholding as a typed 1/2/3 enum, and the crate root re-exports that helper type. Unit tests cover valid randomized-process parsing plus malformed type, count, and value rejection, and the `avutil_core_models` build-checked fuzz fixture covers the deterministic valid and invalid Thresholding path. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root image-layout common-tag coverage. `FrameExif::common_tags()` now exposes Thresholding as a typed TIFF 1/2/3 enum alongside the existing Compression, PhotometricInterpretation, and FillOrder root coding/layout tags, and `avutil` now re-exports the helper type from the crate root. Unit tests cover valid parsing plus malformed type/count/value rejection, and the `avutil_core_models` build-checked fuzz harness asserts the Thresholding 1..=3 invariant for the new accessor. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
