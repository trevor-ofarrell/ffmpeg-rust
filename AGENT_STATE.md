...
Latest `avutil-packet` differential slice: added `PacketSideDataKind::ffmpeg_value()` and extended `crates/avutil/tests/packet_oracle.rs`, the ignored pinned libavcodec/libavutil `AVPacket` oracle harness, so it now validates `AV_PKT_DATA_NB` plus every public `AV_PKT_DATA_*` numeric enum value. The same oracle continues to validate core lifecycle/property behavior plus payload and side-data API behavior: `av_packet_alloc` defaults, `av_packet_rescale_ts` with known and unknown timestamps, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `av_packet_from_data`, `av_grow_packet`, `av_shrink_packet`, `av_packet_make_refcounted`, `av_packet_make_writable`, `AV_PKT_FLAG_KEY|CORRUPT`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data` replacement/append behavior, duplicate `av_packet_new_side_data` replacement behavior, missing side-data lookup size reset, `opaque` address copying, payload preservation, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because complete side-data payload oracle vectors, upstream FATE coverage or a documented FATE exception, remaining packet media-integration vectors, and broader differential media integration remain pending.
