...
`fftools-ffmpeg-image2-start-number` is the current focus; the next slice is broader `ffmpeg-rs` output muxer coverage or deeper image2 option parity.
...
2. Add more image2 option parity such as pattern type/glob handling or timestamp modes.
...
Latest slice: extend constrained `ffmpeg-rs` image2 execution with file-scoped `-start_number` on image2 inputs and image2 outputs, so sequence discovery can start at a nonzero frame number and generated output patterns can start at a nonzero filename number.
