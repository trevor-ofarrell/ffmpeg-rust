...
Update for the current slice: `FrameExif::common_tags()` now also reports Gamma, CompositeImage, SourceImageNumberOfCompositeImage, and SourceExposureTimesOfCompositeImage with strict RATIONAL, SHORT enum/array, and UNDEFINED type validation. `avutil_core_models` build-checks the new EXIF gamma/composite fixture alongside the camera/lens, optics/subject, version/timing/comment, and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF gamma and composite-image metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports Gamma, CompositeImage, SourceImageNumberOfCompositeImage, and SourceExposureTimesOfCompositeImage while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended `FrameExif::common_tags()` with linked EXIF gamma/composite metadata for Gamma, CompositeImage, SourceImageNumberOfCompositeImage, and SourceExposureTimesOfCompositeImage. Unit tests and `avutil_core_models` fuzz invariants cover the new fields, strict one-RATIONAL Gamma validation, CompositeImage enum validation, exact two-SHORT source-image number validation, and UNDEFINED source-exposure payload preservation/type rejection. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_gamma_composite_tags`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
