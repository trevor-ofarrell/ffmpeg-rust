...
Latest `avutil-channel-layout` update: parser-facing channel ID resolution now has explicit coverage for FFmpeg's `strtol` no-conversion/signed-zero edges. `AMBIx`, `AMBI+`, and `AMBI-0tail` resolve as AMBI0 because the pinned `AMBI` branch ignores the end pointer after range-checking the parsed value, while `AMBI-1` still rejects; `USR-0` resolves as raw ID 0 because the pinned `USR` branch requires full consumption and a nonnegative result, while signed-only, negative nonzero, or trailing-junk direct `USR` forms still reject. Layout parsing and string lookup accept these parser-facing spellings without relaxing strict canonical generated-name parsing. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run/execution, formatting, and diff checks passed; `git diff --check` reported CRLF warnings only. The component remains `implemented`, not `complete`, because byte-preserving non-UTF-8 custom-name parity, oracle vectors, full `ffmpeg -layouts` inventory comparison, upstream FATE parity, and actual fuzz execution remain absent.
...
- Current `avutil-channel-layout` parser-facing AMBI no-conversion/signed-zero channel-string slice:
...
- Current `avutil-channel-layout` parser-facing AMBI no-conversion/signed-zero channel-string slice: no remaining failing commands. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run/execution, formatting, and `git diff --check` all passed; `git diff --check` reported CRLF warnings only.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change strengthens parser-facing `ChannelId::from_ffmpeg_string` and layout parser coverage for pinned FFmpeg `strtol` no-conversion and signed-zero behavior: `AMBIx`/`AMBI+`/`AMBI-0tail` resolve to AMBI0, `USR-0` resolves to raw ID 0, and nonzero negative or trailing-junk `USR` forms remain rejected. This builds on bounded base-0 `USR` IDs and prefix-parsed `AMBI` IDs without relaxing strict canonical generated-name parsing. It deliberately does not claim byte-preserving FFmpeg custom-name parity, oracle-vector parity, full `ffmpeg -layouts` inventory coverage, upstream FATE parity, or actual fuzz execution.
...
- Latest `avutil-channel-layout` parser-facing lookup coverage now accepts bare `USR` as raw channel ID 0, signed-zero `USR-0`, bounded FFmpeg base-0 `USR` forms, channel-list trailing-separator `USR+`, prefix-parsed `AMBI` forms, and AMBI no-conversion forms such as `AMBIx`/`AMBI+` in layout parsing and string lookup while keeping canonical generated names strict. Remaining blockers are byte-preserving non-UTF-8 custom-name parity if required by the selected API surface, oracle-vector calibration, full `ffmpeg -layouts` inventory comparison, deeper `AV_CHANNEL_ORDER_AMBISONIC` semantics beyond the current bounded lookup/retype surface, upstream FATE parity, and actual fuzz execution.
...
Latest slice: added explicit parser-facing coverage for FFmpeg `strtol` no-conversion and signed-zero channel strings. `ChannelId::from_ffmpeg_string` now has tests and fuzz fixtures for `AMBIx`, `AMBI+`, and `AMBI-0tail` resolving to AMBI0, `AMBI-1` rejecting, `USR-0` resolving to raw ID 0, and `USR-0tail` rejecting; `ChannelId::from_canonical_name` remains strict. Channel-list parsing now accepts forms such as `AMBIx`, `AMBIx+USR-0`, and `USR-0` through the same parser-facing lookup while preserving canonical decimal generated names. Validation passed with focused avutil channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run and execution, formatting, and `git diff --check` with CRLF warnings only.
