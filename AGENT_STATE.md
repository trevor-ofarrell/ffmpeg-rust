...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `TFL`/top-front-left and `TFR`/top-front-right native mask bits plus the `5.1.2` and `5.1.2(back)` named layouts from `channel_layout_map`, alongside the prior `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. The implementation preserves FFmpeg's map shape where `7.1(wide)` uses the back-speaker wide mask, `7.1(wide-side)` uses the side-speaker wide mask, `5.1.2` uses side surrounds plus top-front speakers, and `5.1.2(back)` uses back surrounds plus top-front speakers. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new channels/layouts, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern "AV_CH_LAYOUT_7POINT1|AV_CH_LAYOUT_5POINT1POINT2|AV_CH_LAYOUT_OCTAGONAL|TOP_FRONT|WIDE"` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern '"7\.1|\"5\.1\.2|\"octagonal'` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures were observed while adding `TFL`/`TFR`, `5.1.2`, and `5.1.2(back)`; focused unit, fuzz-build, clippy, local FATE, changed-path local mapping, and audio-regression checks passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, broader height-channel layout coverage after the first `5.1.2` entries, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `TFL`/`TFR` native mask bits plus the `5.1.2` and `5.1.2(back)` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving the upstream naming-to-mask mapping, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entries after `5.1.2(back)`, starting with `octagonal`/`cube` if their native-channel bits are already modeled or adding the smallest missing bits they require.
2. Add unit tests, fuzz-generator coverage, ledger updates, and local FATE mapping validation for that source-backed follow-up layout slice.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, and `TFR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, and `5.1.2(back)` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no broader height-channel layout coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
