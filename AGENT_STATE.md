...
Update for the current slice: `FrameExif::common_tags()` root TIFF `Make` and `Model` handling now has dedicated validation coverage as single ASCII strings alongside the existing root descriptive/image-layout tags. Unit tests cover valid inline parsing plus malformed Make type, missing Model ASCII terminator, and multiple-string Model rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid Make/Model paths plus ASCII accessor invariants. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` already exposed `Make` and `Model`; they now have dedicated root-tag validation coverage as single ASCII strings alongside the existing root image-layout, document/page, software, date/time, artist, host-computer, predictor, and copyright tags. Unit tests cover valid inline parsing plus malformed Make type, missing Model ASCII terminator, and multiple-string Model rejection, and the `avutil_core_models` build-checked fuzz harness asserts deterministic valid and invalid paths plus ASCII accessor invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root descriptive/image-layout common-tag coverage for `Make` and `Model`. `frame_side_data_interprets_exif_root_camera_identity_tags` covers valid inline ASCII parsing plus malformed Make type, missing Model NUL terminator, and multiple-string Model rejection; `avutil_core_models` now build-checks deterministic valid and invalid Make/Model fixture paths plus ASCII invariants for both accessors. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_camera_identity_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
