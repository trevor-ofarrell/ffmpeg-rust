...
Update for the current slice: `FrameExif::common_tags()` now also reports PhotographicSensitivity, SensitivityType, StandardOutputSensitivity, RecommendedExposureIndex, ISOSpeed, ISOSpeedLatitudeyyy, and ISOSpeedLatitudezzz with strict SHORT/LONG type, count-one, and defined sensitivity-type validation. `avutil_core_models` build-checks the new EXIF sensitivity fixture alongside the exposure, APEX exposure, environment, camera/lens, gamma/composite, optics/subject, version/timing/comment, and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF sensitivity metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports PhotographicSensitivity, SensitivityType, StandardOutputSensitivity, RecommendedExposureIndex, ISOSpeed, ISOSpeedLatitudeyyy, and ISOSpeedLatitudezzz while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes and reserved sensitivity-type values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
