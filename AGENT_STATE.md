...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed line-size metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane ownership, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because side data, custom alignment/stride, writable plane mutation, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership gap, likely side-data references, custom alignment/stride metadata, or writable plane mutation semantics, with invalid-input and sharing tests.
...
Latest slice: added BufferRef-backed plane ownership to `VideoFrame` and `AudioFrame` while preserving visible byte snapshots through `planes()`. Unit tests now prove visible-length validation, padding exclusion, retained plane owner sharing, cloned-frame retention, and final external-owner release timing for video and audio planes; `avutil_core_models` now build-checks plane BufferRef snapshots, retained owners, storage sharing, and padding exclusion. Validation passed with formatting, focused frame tests, fuzz-target check/clippy, the local `avutil-frame` mapping, changed-component dry-run and execution, workspace clippy, workspace tests, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
