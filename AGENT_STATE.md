...
Current avutil-frame coverage also now includes deterministic build-checked `FrameRegionsOfInterest` malformed-length, invalid self-size, invalid qoffset, and empty aggregate coverage, matching the focused ROI unit coverage for raw and deferred side-data rejection paths.

Update for the current slice: Regions of Interest coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_regions_of_interest_payload`. The deterministic `avutil_core_models` fixture now asserts empty/short/long/trailing-byte payload rejection, invalid `self_size` rejection through raw and deferred side-data paths, invalid qoffset constructor and raw-payload rejection, empty aggregate rejection, and non-ROI lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens Regions of Interest fuzz coverage. `FrameRegionsOfInterest` already has unit coverage for valid one- and two-region payloads, raw rectangle and rational qoffset preservation, malformed empty/short/long/trailing-byte payloads, invalid self-size, invalid qoffset constructor and raw payload values, empty aggregate rejection, and non-ROI lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser and deferred side-data accessor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened Regions of Interest fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the ROI malformed-input unit branches for empty/short/long/trailing-byte payload rejection, invalid `self_size` rejection through raw and deferred side-data paths, invalid qoffset constructor and raw-payload rejection, and empty aggregate rejection. Docs and the ledger now call out these ROI malformed length/self-size/qoffset invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_regions_of_interest_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_regions_of_interest_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
