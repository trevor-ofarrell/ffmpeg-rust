...
Latest slice: added packet Dynamic HDR10+ side-data parsing. `PacketDynamicHdr10Plus` models `AV_PKT_DATA_DYNAMIC_HDR10_PLUS` as the pinned native `AVDynamicHDRPlus` envelope by reusing the existing frame-side HDR10+ parser, preserving raw bytes and exposing country/application/window, color-transform, and peak-luminance accessors. `SideData::new_dynamic_hdr10_plus` and deferred `dynamic_hdr10_plus()` lookup reject malformed lengths, invalid ITU-T T.35 country/application/window values, invalid nested overlap options, invalid nested counters/flags, and invalid peak-luminance grid flags/counts, and return `None` for non-HDR10+ kinds. Focused packet unit tests cover FFmpeg constant name, constructor/deferred accessor behavior, frame-wrapper access, byte preservation, non-matching kind behavior, malformed lengths, invalid header/window values, invalid nested overlap option, and deferred accessor errors; `avutil_core_models` now build-checks the same invariants. Validation passed with `cargo fmt --all`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy -p avutil --all-targets -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet --dry-run`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\xtask.exe quick`, and `git diff --check`.
