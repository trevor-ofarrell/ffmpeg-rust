...
2. Extend MOV subtitle/data parsing into remaining timed metadata/text variants or add pinned-oracle differential tests for the newly typed `tx3g`/`stpp`/`stxt`/`metx`/`urim` paths when an FFmpeg 8.1.1 oracle binary is available.
...
Latest slice: extended MOV XML subtitle/metadata sample-entry parsing with typed optional `stpp`/`metx` `btrt` boxes, preserved `metx` child boxes, duplicate/truncated bitrate rejection tests, and focused MOV unit coverage. MOV stays below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
