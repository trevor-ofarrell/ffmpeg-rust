...
`avutil-frame` now retains an optional `BufferRef`-backed hardware frames context, mirroring the `AVFrame.hw_frames_ctx` ownership shape at the Rust type level. `Frame` clone, take, replace, and drop paths preserve the underlying `BufferRef` sharing and release timing, including external opaque owner release once the final frame/context reference is gone. The `avutil_core_models` fuzz harness now build-checks hw-frames-context attachment, sharing, typed opaque lookup, take, clone-retention, and release invariants. This is still a Rust ownership model only; hardware device/frame context internals, actual hardware formats, frame plane buffer ownership, and pinned-oracle/FATE parity remain absent.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed line-size metadata, optional channel-layout metadata for audio, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because frame plane BufferRef ownership, side data, custom alignment/stride, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership gap, likely plane BufferRef-backed frame data or side-data references, with invalid-input and sharing tests.
...
Latest slice: added optional `BufferRef`-backed `hw_frames_context` retention to `Frame`. Unit tests now prove context clone sharing, typed opaque lookup through a frame-held context, take semantics, replacement release, and final-release timing; `avutil_core_models` now build-checks hw-frames-context attachment, clone retention, take, typed opaque lookup, and release invariants. Validation passed with formatting, focused frame tests, fuzz-target check/clippy, the local `avutil-frame` mapping, changed-component dry-run and execution, workspace clippy, workspace tests, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
