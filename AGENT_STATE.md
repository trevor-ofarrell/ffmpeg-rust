...
Update for the current slice: `FrameExif::common_tags()` root TIFF `Copyright` handling now has dedicated validation coverage as a single ASCII string alongside the existing root descriptive/image-layout tags. Unit tests cover valid inline parsing plus malformed type, missing ASCII terminator, and multiple-string rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid Copyright paths plus the ASCII accessor invariant. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` already exposed `Copyright`; it now has dedicated root-tag validation coverage as a single ASCII string alongside the existing root image-layout, document/page, software, date/time, artist, host-computer, and predictor tags. Unit tests cover valid inline parsing plus malformed type, missing ASCII terminator, and multiple-string rejection, and the `avutil_core_models` build-checked fuzz harness asserts deterministic valid and invalid paths plus the ASCII accessor invariant. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root descriptive/image-layout common-tag coverage for `Copyright`. `frame_side_data_interprets_exif_root_copyright_tag` covers valid inline ASCII parsing plus malformed type, missing NUL terminator, and multiple-string rejection; `avutil_core_models` now build-checks deterministic valid and invalid Copyright fixture paths plus an ASCII invariant for the accessor. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_copyright_tag`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
