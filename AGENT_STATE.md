...
`fftools-ffprobe-stream-start-time` is the current focus; the latest slice reports constrained zero-origin `start_pts` and `start_time` values in `ffprobe-rs -show_streams` output.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as MOV sample-entry compressor name/details or more AVI stream header fields.
...
Latest slice: add constrained stream start timestamp reporting to `ffprobe-rs`. The stream report now renders `start_pts=0` and `start_time=0.000000` in default and JSON output for non-empty MOV/MP4 and AVI streams under the current zero-origin demuxer model; fixtures prove raw MOV, H.264 MOV, and raw AVI output. This is not full upstream timeline parity and still lacks pinned-oracle differential/FATE coverage.
