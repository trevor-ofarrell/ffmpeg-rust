...
- `cargo test -p avutil frame_side_data_validates_kind_and_supports_remove_and_take`
...
- No remaining failing commands in the latest side-data remove/take fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens side-data remove/take fuzz coverage. `Frame` already has unit tests for invalid side-data kind rejection, unknown-name removal, ReplayGain alias removal, metadata preservation, and `take_side_data`. The `avutil_core_models` build-checked fuzz harness now mirrors the remove-by-name, remove-by-alias, push-back, take, metadata, and BufferRef storage-sharing assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened side-data remove/take fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_validates_kind_and_supports_remove_and_take`: unknown side-data insertion/removal, ReplayGain insertion through `BufferRef`, metadata preservation, typed lookup storage sharing, remove-by-name, remove-by-alias through `replay_gain`, missing-name removal absence, push-back of a removed record, `take_side_data`, and metadata/BufferRef preservation after the take. Docs and the ledger now call out these remove/take invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_validates_kind_and_supports_remove_and_take`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
