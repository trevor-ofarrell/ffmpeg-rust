...
Current avutil-frame coverage also now includes deterministic build-checked exact native `FrameContentLightMetadata` byte-layout and raw boundary-value coverage, matching the focused content-light unit coverage for `MaxCLL`, `MaxFALL`, and `u32::MAX`/zero raw-value preservation.

Update for the current slice: content-light coverage now has a build-checked fuzz invariant matching the native byte-layout and raw-value branch in `frame_side_data_parses_content_light_metadata_payload`. The deterministic `avutil_core_models` fixture now asserts the exact 8-byte native `AVContentLightMetadata` layout for `MaxCLL=1000`/`MaxFALL=400`, parser round trip, side-data payload preservation, and raw `MaxCLL=u32::MAX`/`MaxFALL=0` preservation. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens content-light metadata fuzz coverage. `FrameContentLightMetadata` already has unit coverage for the exact native 8-byte layout and raw unsigned field preservation. The `avutil_core_models` build-checked fuzz harness now mirrors that branch, including native byte layout, parser round trip, side-data payload preservation, and raw `MaxCLL=u32::MAX`/`MaxFALL=0` preservation. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened content-light native-layout fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_parses_content_light_metadata_payload`, proving the exact native 8-byte `AVContentLightMetadata` layout for `MaxCLL=1000`/`MaxFALL=400`, parser round trip, side-data payload preservation, and raw `MaxCLL=u32::MAX`/`MaxFALL=0` preservation. Docs and the ledger now call out this content-light native-layout invariant explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_content_light_metadata_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
