...
Fuzz harness coverage is the latest focus. Initial cargo-fuzz targets now cover avutil byteio/bitreader/bitwriter invariants, metadata dictionary and AVOption-like value handling, probe registry scoring, WAV, yuv4mpegpipe, raw PCM s16le, rawvideo, constrained AVI, constrained MOV/MP4, image2 path/sequence handling, null/hash/framecrc packet muxer invariants, and FFmpeg-style option parser invariants, but actual local `cargo fuzz run` execution is blocked because the `cargo-fuzz` subcommand is not installed.
...
Latest slice: added build-checked cargo-fuzz target `avformat_packet_muxers` plus seed corpora, updated the null/hash/framecrc muxer ledger fuzz target fields, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
