...
`avformat-mov-demuxer` remains the current component. It now structures direct audio `esds`, `btrt`, `damr`, required `ac-3` `dac3`, required `ec-3` `dec3`, required `Opus` `dOps`, required `fLaC` `dfLa`, and required `alac` Apple Lossless specific boxes in both direct and `wave/alac` compatibility forms, rejects malformed direct descriptor/bitrate/AMR-specific/AC-3-specific/E-AC-3-specific/Opus-specific/FLAC-specific/ALAC-specific payloads, and still has a local FATE-runner smoke mapping that executes the focused MOV unit suite through `cargo run -p fate-runner -- run --component avformat-mov-demuxer`. Exact FFmpeg ffprobe output semantics, pinned FFmpeg differential tests, upstream FATE media parity, actual local fuzz execution, additional codec-specific audio extension parsing beyond direct `esds`/`btrt`/`damr`/`dac3`/`dec3`/`dOps`/`dfLa`/`alac`/`wave`/`chan`, and broader MOV subtitle/data sample-entry coverage remain absent.
...
2. Extend MOV sample-entry parsing into additional codec-specific audio extension atoms beyond direct `esds`/`btrt`/`damr`/`dac3`/`dec3`/`dOps`/`dfLa`/`alac`/`wave`/`chan` and broader subtitle/data sample-entry variants.
...
Latest slice: parsed required MOV `alac` Apple Lossless specific boxes into `MovAlacSpecificBox`, supported both direct AudioSampleEntry `alac` boxes and older `wave/frma` + nested `wave/alac` compatibility cookies, derived effective ALAC sample rate/channel count/bit depth from the codec config, added malformed ALAC payload rejection coverage, updated MOV compatibility notes, and kept the component below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
