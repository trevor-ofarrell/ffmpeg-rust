...
1. Continue priority-1 primitive work by adding the next unblocked shared `avutil` compatibility vector, most likely another packet side-data payload parser such as quality stats, fallback-track, or producer-reference-time once its FFmpeg 8.1.1 layout is verified.
