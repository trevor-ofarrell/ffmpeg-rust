...
1. Continue priority-1 primitive work by adding the next unblocked shared `avutil` compatibility vector, most likely another packet side-data payload parser such as Dolby Vision config, encryption side-data, EXIF, ambient viewing environment, or stereo3d once its FFmpeg 8.1.1 layout is verified.
