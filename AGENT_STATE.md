...
`fftools-ffprobe-mov-handler-name-tag` is the current focus; the latest slice parses MOV/MP4 `mdia/hdlr` handler names and renders them as `handler_name` stream tags in `ffprobe-rs -show_streams` output.
...
Latest slice: add constrained MOV/MP4 handler-name stream tags. The MOV demuxer now parses `mdia/hdlr` name payloads as UTF-8, rejects malformed names with typed invalid-data errors, stores valid names as `handler_name` track metadata, and `ffprobe-rs` renders that metadata in JSON stream tags for local MOV/MP4 fixtures. This still lacks pinned-oracle differential/FATE/fuzz coverage.
