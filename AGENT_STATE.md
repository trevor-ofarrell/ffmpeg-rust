...
Update for the current slice: `FrameExif::common_tags()` root/common EXIF GPS coordinate coverage now has build-checked fuzz invariants matching the existing common-tag unit test. The deterministic `avutil_core_models` EXIF fixture now preserves valid GPSLatitude/GPSLongitude tuples, accepts the inclusive 90-degree latitude and 180-degree longitude boundary tuples, rejects composite-coordinate overflow at those boundaries, rejects malformed GPS reference bytes, and mirrors root Orientation count rejection. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF common GPS coordinate/ref fuzz coverage. `FrameExif::common_tags()` already exposed GPSLatitude/GPSLongitude tuples and unit coverage for valid coordinate boundaries, composite-coordinate overflow, GPS reference validation, and root Orientation count validation. The `avutil_core_models` build-checked fuzz harness now mirrors those valid and invalid paths. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Continue auditing the remaining EXIF common-tag unit tests against `avutil_core_models` and extend the next unmirrored invalid-input fuzz gap.
