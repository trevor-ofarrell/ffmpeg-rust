...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `5.1.4`, `7.1.2`, and `7.1.4` named layouts from `channel_layout_map`, alongside the prior `TBL`/top-back-left, `TBR`/top-back-right, `octagonal`, `cube`, `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `TFL`/top-front-left, `TFR`/top-front-right, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. The implementation preserves FFmpeg's map shape where `5.1.4` uses `AV_CH_LAYOUT_5POINT1POINT4_BACK`, `7.1.2` uses `AV_CH_LAYOUT_7POINT1POINT2`, and `7.1.4` uses `AV_CH_LAYOUT_7POINT1POINT4_BACK`; the adjacent `7.2.3` entry remains pending because it requires `TBC`/top-back-center and `LFE2`/low-frequency-2 channel coverage. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new layouts, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CH_LAYOUT_5POINT1POINT4_BACK|AV_CH_LAYOUT_7POINT1POINT2|AV_CH_LAYOUT_7POINT1POINT4_BACK|AV_CH_LAYOUT_7POINT2POINT3|AV_CH_LOW_FREQUENCY_2|AV_CHAN_LOW_FREQUENCY_2' -Context 0,3` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern '"5\.1\.4|"7\.1\.2|"7\.1\.4|"7\.2\.3|LOW_FREQUENCY_2|LFE2' -Context 0,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `5.1.4`, `7.1.2`, and `7.1.4`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, and `avformat` audio regression passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, `7.2.3` and broader height-channel layout coverage after `7.1.4`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `5.1.4`, `7.1.2`, and `7.1.4` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving the upstream naming-to-mask mapping, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It intentionally leaves `7.2.3` pending until `TBC` and `LFE2` are modeled, and does not claim full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entry after `7.1.4`, starting with the missing `TBC`/top-back-center and `LFE2`/low-frequency-2 channel bits needed for `7.2.3`.
2. Add unit tests, fuzz-generator coverage, ledger updates, and local FATE mapping validation for that source-backed `7.2.3` follow-up slice if the channel-bit source check is straightforward.
3. Continue the next unblocked priority-1 infrastructure gap, likely more channel-layout inventory coverage or another small avutil primitive with local tests.

## Known Blockers

- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFR`, `TBL`, and `TBR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, and `7.1.4` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no `7.2.3`/`TBC`/`LFE2` coverage, no broader height-channel layout coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added the next source-checked native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CH_LAYOUT_5POINT1POINT4_BACK`, `AV_CH_LAYOUT_7POINT1POINT2`, `AV_CH_LAYOUT_7POINT1POINT4_BACK`, and the public layout names `5.1.4`, `7.1.2`, and `7.1.4`; the same check confirmed the adjacent `7.2.3` entry depends on `AV_CHAN_TOP_BACK_CENTER` and `AV_CHAN_LOW_FREQUENCY_2`, so it remains deferred. The Rust model now exposes those three named layouts while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover channel names/masks, layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local FATE mapping, changed-path local mappings, `avformat` audio tests, format checks, and `git diff --check` passed. The component remains `implemented`, not `complete`, because full oracle inventory parity, broader height-channel layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
