...
`avutil-logging` now has a stronger flag-aware primitive. `LogLevel` exposes FFmpeg-style numeric severity values and level names; `LogFlags` stores and truncates the known public `av_log` flag bits; `LogTimestamp` carries Unix-microsecond timestamps for deterministic UTC time/datetime rendering; `LogRecord` has deterministic no-newline formatting with configurable print-level and timestamp prefixes; and `Logger` now carries configured flags in addition to enablement checks, acceptance returns, quiet suppression, clear/take buffer control, formatted record rendering, per-call callback dispatch, installed callback dispatch, process-global shared state helpers, and deterministic `AV_LOG_SKIP_REPEATED`-style consecutive duplicate compression with pending or materialized repeat summaries. The ledger still keeps `avutil-logging` at `implemented`, not `complete`, because byte-identical `av_log` formatting, C ABI callback parity, real system-clock capture, local-time formatting parity, color handling, CLI repeat/time/datetime/stderr integration, oracle differential tests, and FATE coverage remain incomplete.
...
`avutil-logging` is the current focus for this slice. The concrete change is a mutex-backed process-global `Logger` primitive that shares level, flags, callbacks, logging, flushing, formatting, clearing, and record draining across callers while reusing the existing filtering and repeat-suppression semantics. It does not claim byte-identical `av_log`, CLI stderr, C ABI callback parity, real system-clock capture, local-time parity, color, pinned oracle parity, or upstream FATE parity yet.
...
1. Continue priority-1 primitive work by tightening the next unblocked avutil surface, likely logging color/global-callback/system-clock/CLI stderr parity, deeper bit I/O vectors, or option API/CLI integration, unless a pinned-oracle scaffold becomes available first.
