...
Update for the current slice: `FrameExif::common_tags()` capture-setting coverage now has build-checked fuzz invariants matching the existing unit test for malformed capture-setting tags. The deterministic `avutil_core_models` EXIF fixture now covers valid ExposureProgram, MeteringMode, LightSource, Flash, WhiteBalance, DigitalZoomRatio, and FocalLengthIn35mmFilm parsing plus malformed ExposureProgram enum value, LightSource enum value, DigitalZoomRatio type, and FocalLengthIn35mmFilm count cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF capture-setting common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ExposureProgram, MeteringMode, LightSource, Flash, WhiteBalance, DigitalZoomRatio, and FocalLengthIn35mmFilm with unit coverage for malformed enum, type, and count shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: ExposureProgram undefined enum value, LightSource undefined enum value, DigitalZoomRatio wrong TIFF type, and FocalLengthIn35mmFilm wrong count. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
