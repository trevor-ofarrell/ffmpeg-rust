...
Latest slice: added top-level `FrameData` and `Frame` data-plane writability dispatch for `is_writable`, `make_writable`, and visible-plane mutation. Unit tests now prove video and audio frame wrappers detach shared/readonly plane buffers before mutation, cloned frames keep their original payloads, side-data and `hw_frames_context` sharing are not changed by data-plane make-writable, and visible snapshots stay synchronized. `avutil_core_models` now build-checks top-level Frame/FrameData writable dispatch alongside custom video line sizes, frame plane ownership, side-data, and hw context invariants. Validation passed with formatting, focused `cargo test -p avutil frame`, `cargo test -p avutil --lib --no-run`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, focused fuzz-target clippy, workspace clippy, local `avutil-frame` FATE-runner smoke execution, FATE-runner changed dry-run, format checks, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
