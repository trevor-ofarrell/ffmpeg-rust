...
Update for the current slice: SEI unregistered frame side-data coverage now has broader build-checked fuzz invariants matching `frame_side_data_parses_sei_unregistered_payload` and `frame_side_data_rejects_malformed_sei_unregistered_payload`. The deterministic `avutil_core_models` fixture now preserves constructed UUID+user-data bytes, preserves UUID-only records as empty user data, confirms non-SEI lookups return absent payloads, and rejects short payloads through both `FrameSeiUnregistered::parse` and deferred `side_data.sei_unregistered()` error paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens SEI unregistered side-data fuzz coverage. `FrameSeiUnregistered` already accepts UUID+user-data records, UUID-only records, and non-SEI absence while rejecting short payloads in unit tests. The `avutil_core_models` build-checked fuzz harness now mirrors those deterministic assertions and verifies both the raw parser and deferred `side_data.sei_unregistered()` error path. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened SEI unregistered fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_parses_sei_unregistered_payload` and `frame_side_data_rejects_malformed_sei_unregistered_payload`: constructed UUID+user-data byte preservation, UUID-only empty user-data preservation, non-SEI lookup absence, and short-payload rejection through the raw parser and deferred side-data accessor. Docs and the ledger now call out these SEI invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_sei_unregistered_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_sei_unregistered_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
