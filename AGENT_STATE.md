...
Latest `avutil-channel-layout` update: parser-facing channel ID resolution now also covers FFmpeg's bare `USR` no-conversion edge from `strtol`, mapping it to raw channel ID 0 while strict canonical name parsing remains unchanged. Layout parsing and string lookup accept `USR`, channel-list trailing-separator `USR+`, base-0 raw forms such as `USR0x2d`, `USR055`, `USR+45`, and `USR0`, plus prefix-parsed ambisonic forms such as `AMBI0x1tail`; generated names remain canonical decimal `AMBI<n>`/`USR<raw>`, and malformed signed-only, negative, or trailing-junk direct `USR` forms still reject. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run/execution, formatting, and diff checks passed; `git diff --check` reported CRLF warnings only. The component remains `implemented`, not `complete`, because byte-preserving non-UTF-8 custom-name parity, oracle vectors, full `ffmpeg -layouts` inventory comparison, upstream FATE parity, and actual fuzz execution remain absent.
...
- Current `avutil-channel-layout` parser-facing bare-USR channel-string slice:
...
- Current `avutil-channel-layout` parser-facing bare-USR channel-string slice: no remaining failing commands. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run/execution, formatting, and `git diff --check` all passed; `git diff --check` reported CRLF warnings only.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change extends parser-facing `ChannelId::from_ffmpeg_string` coverage for FFmpeg's bare `USR` no-conversion edge, keeping direct malformed `USR+` and `USRx` channel IDs rejected while allowing `USR+` through channel-list trailing-separator parsing. This builds on bounded base-0 `USR` IDs and prefix-parsed `AMBI` IDs without relaxing strict canonical generated-name parsing. It deliberately does not claim byte-preserving FFmpeg custom-name parity, oracle-vector parity, full `ffmpeg -layouts` inventory coverage, upstream FATE parity, or actual fuzz execution.
...
- Latest `avutil-channel-layout` parser-facing lookup coverage now accepts bare `USR` as raw channel ID 0, bounded FFmpeg base-0 `USR` forms, channel-list trailing-separator `USR+`, and prefix-parsed `AMBI` forms in layout parsing and string lookup while keeping canonical generated names strict. Remaining blockers are byte-preserving non-UTF-8 custom-name parity if required by the selected API surface, oracle-vector calibration, full `ffmpeg -layouts` inventory comparison, deeper `AV_CHANNEL_ORDER_AMBISONIC` semantics beyond the current bounded lookup/retype surface, upstream FATE parity, and actual fuzz execution.
...
Latest slice: added bare-`USR` coverage to parser-facing FFmpeg channel-string parsing. `ChannelId::from_ffmpeg_string` now models the bounded `av_channel_from_string` numeric behavior for exact native names, `UNK`/`UNSD`, bare `USR` as raw ID 0, base-0 full-consumption `USR` raw IDs, and base-0 prefix-parsed `AMBI` IDs; `ChannelId::from_canonical_name` remains strict. Channel-list parsing and string lookup now accept forms such as `USR`, `USR+`, `USR0x2d+USR056`, `USR0`, `USR055`, `USR+45`, and `AMBI0x1tail`, while signed-only, negative, or trailing-junk direct `USR` names reject. Unit tests and deterministic `avutil_core_models` fuzz fixtures cover the new parser/lookup forms. Validation passed with focused avutil channel-layout tests, fuzz-package check/clippy, avutil clippy, local component FATE, changed-path FATE dry-run and execution, formatting, and `git diff --check` with CRLF warnings only.
