...
`avutil-buffer` is the current focus. The latest slice adds pinned libavutil `AVBufferPool` overlap coverage on top of the existing `AVBufferRef` oracle rows: custom allocator bytes are preserved, returned buffers are reused without clearing, spare buffers release on pool uninit, and outstanding buffers release after uninit only when their final reference drops. The component remains `differential_pass`, not `complete`, because per-buffer opaque/pool-free callback modeling, broader ABI/lifetime parity, FATE disposition, and final completion documentation still need closure.
...
1. Continue `avutil-buffer` toward strict completion by modeling or explicitly bounding per-buffer opaque/pool-free callback behavior and documenting the FATE disposition for this low-level helper surface.
...
- `avutil-buffer` now has pinned libavutil `AVBufferRef` and bounded `AVBufferPool` differential evidence and is marked `differential_pass`, but not complete. Remaining blockers are per-buffer opaque/pool-free callback modeling, broader ABI/lifetime closure, FATE disposition, and final completion documentation.
