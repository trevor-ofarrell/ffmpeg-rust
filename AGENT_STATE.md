...
Update for the current slice: `hw_frames_context` replacement coverage now has broader build-checked fuzz invariants matching `frame_hw_context_replacement_releases_unshared_context`. The deterministic `avutil_core_models` fixture now covers first context attachment without release, replacement releasing the first external opaque owner, and clearing the context releasing the second owner while preserving release ordering. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- `cargo test -p avutil frame_hw_context_replacement_releases_unshared_context`
...
- No remaining failing commands in the latest hw_frames_context replacement-release fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens `hw_frames_context` replacement-release fuzz coverage. `Frame` already has a unit test for attaching, replacing, and clearing unshared hardware-frame contexts with external opaque release callbacks. The `avutil_core_models` build-checked fuzz harness now mirrors the no-premature-release, first-context replacement release, and second-context clear release assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened `hw_frames_context` replacement-release fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_hw_context_replacement_releases_unshared_context`: first context attachment does not release, replacement releases the first external opaque owner, and clearing releases the second owner in order. Docs and the ledger now call out these hardware context replacement-release invariants explicitly. Validation passed with `cargo test -p avutil frame_hw_context_replacement_releases_unshared_context`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
