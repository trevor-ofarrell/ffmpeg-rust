...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `TBC`/top-back-center and `LFE2`/low-frequency-2 native mask bits plus the `7.2.3` named layout from `channel_layout_map`, alongside the prior `5.1.4`, `7.1.2`, `7.1.4`, `TBL`/top-back-left, `TBR`/top-back-right, `octagonal`, `cube`, `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `TFL`/top-front-left, `TFR`/top-front-right, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. The implementation preserves FFmpeg's map shape where `7.2.3` uses `AV_CH_LAYOUT_7POINT2POINT3`, adding `TBC` and `LFE2` to the existing `7.1.2` shape. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new channels/layout, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CHAN_TOP_BACK_CENTER|AV_CH_TOP_BACK_CENTER|AV_CHAN_LOW_FREQUENCY_2|AV_CH_LOW_FREQUENCY_2|AV_CH_LAYOUT_7POINT2POINT3|AV_CHANNEL_LAYOUT_7POINT2POINT3' -Context 2,3` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern 'TOP_BACK_CENTER|LOW_FREQUENCY_2|TBC|LFE2|"7\.2\.3"' -Context 2,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `TBC`, `LFE2`, and `7.2.3`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, and `avformat` audio regression passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, broader height-channel layout coverage after `7.2.3`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `TBC`/top-back-center and `LFE2`/low-frequency-2 channel bits plus the `7.2.3` standard native-mask layout from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving the upstream naming-to-mask mapping, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entries after `7.2.3`, starting with `9.1.4` if it only requires currently modeled `FLC`/`FRC` plus the existing `7.1.4` shape.
2. Source-check whether `9.1.6`, `hexadecagonal`, `binaural`, or stereo-downmix require additional channel bits, then choose the smallest unblocked follow-up layout slice.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFR`, `TBL`, `TBC`, `TBR`, and `LFE2`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, `7.1.4`, and `7.2.3` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no `9.1.4` and later height layout coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
