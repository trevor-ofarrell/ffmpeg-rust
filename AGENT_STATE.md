...
`fftools-ffprobe-avi-bits-per-raw-sample` is the current focus; the latest slice adds the AVI `bits_per_raw_sample` stream field to constrained `ffprobe-rs -show_streams` reports from parsed BITMAPINFOHEADER bit counts.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as MOV sample-entry compressor/depth fields or more AVI stream header fields.
...
Latest slice: add AVI `bits_per_raw_sample` reporting to `ffprobe-rs` for constrained AVI paths. Stream reports now carry the parsed BITMAPINFOHEADER bit count; the current AVI fixture proves a 24-bit DIB stream as `bits_per_raw_sample=24` in JSON output.
