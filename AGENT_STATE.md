...
Parser fuzz harness coverage is the latest focus. Initial cargo-fuzz targets now cover avutil byteio/bitreader/bitwriter invariants plus WAV, yuv4mpegpipe, raw PCM s16le, and rawvideo demuxer invariants, but actual local `cargo fuzz run` execution is blocked because the `cargo-fuzz` subcommand is not installed.
...
2. Add fuzz targets for the next untrusted parser surfaces, starting with AVI chunk parsing or MOV box/sample-table parsing.
...
Latest slice: added build-checked cargo-fuzz targets `avformat_pcm_s16le` and `avformat_rawvideo` plus seed corpora, updated the raw PCM and rawvideo demuxer ledger fuzz target fields, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
