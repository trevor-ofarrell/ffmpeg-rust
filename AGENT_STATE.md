...
Current avutil-frame coverage also now includes deterministic build-checked zero-rectangle `FrameVideoHintType::Constant` side-data round trips, matching the focused video-hint unit coverage for empty constant hints.

Update for the current slice: video-hint coverage now has a build-checked fuzz invariant matching the empty `FrameVideoHintType::Constant` branch in `frame_side_data_parses_video_hint_payload`. The deterministic `avutil_core_models` fixture now asserts exact header-sized serialization, `AV_VIDEO_HINT_TYPE_CONSTANT` name mapping, empty-state reporting, zero rectangle count, parser round trip, and `FrameSideData::new_video_hint` payload preservation for the zero-rectangle Constant form. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens video-hint fuzz coverage. `FrameVideoHint` already has unit coverage for both changed hints with rectangles and zero-rectangle Constant hints. The `avutil_core_models` build-checked fuzz harness now mirrors the empty Constant branch, including exact header-sized serialization, FFmpeg constant-name mapping, empty-state reporting, parser round trip, and `FrameSideData::new_video_hint` payload preservation. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
