...
`avutil-hash` is the latest coherent slice. MD5 one-shot/streaming helpers and lowercase hex formatting have been added beside Adler-32 and IEEE CRC-32, with standard vectors, block-boundary streaming tests, and build-checked `avutil_core_models` coverage. Pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, SHA variants, and full FFmpeg hash/framehash behavior are still absent.

## Next 3 Concrete Actions

1. Continue priority-1 avutil completeness by wiring the new MD5 digest helper into the packet hash muxer or adding the next missing checksum/hash variant with tests.
...
Latest slice: strengthened `avutil::hash` with Rust-native MD5 one-shot and streaming helpers, lowercase digest hex formatting, standard-vector/block-boundary tests, fuzz-target coverage, and docs/ledger updates while keeping `avutil-hash` below `complete`.
