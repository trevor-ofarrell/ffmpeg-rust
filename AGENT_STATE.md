...
`avutil-buffer` is the current focus. The latest slice adds pinned libavutil `av_buffer_pool_init(size, NULL)` default-allocator coverage: the first buffer is writable with NULL pool opaque, returned storage is reused without clearing, and reused default-pool refs still report NULL pool opaque. The component remains `differential_pass`, not `complete`, because broader ABI/lifetime parity, FATE disposition, and final completion documentation still need closure.
...
Latest slice: added `av_buffer_pool_init(size, NULL)` default-allocator coverage. `crates/avutil/tests/buffer_oracle.rs` now compares the pinned FFmpeg 8.1.1 rows for a default pool: first-buffer status, NULL pool opaque, no-clear reuse after unref, and NULL pool opaque on reuse. `crates/avutil/src/buffer.rs` and `fuzz/fuzz_targets/avutil_core_models.rs` assert the same Rust `BufferPool::new` invariant. The docs/ledger record the added evidence, and `avutil-buffer` remains `differential_pass`, not complete, because broader ABI/lifetime closure and FATE disposition remain pending.
