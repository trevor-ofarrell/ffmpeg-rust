...
Latest slice: expanded the shared `SampleFormat` model to the FFmpeg-native packed and planar `u8`/`s16`/`s32`/`flt`/`dbl`/`s64` set and proved `AudioFrame` handles non-`s16` packed and planar sample storage with exact and padded line sizes. Unit and fuzz-build coverage now exercise all current sample-format names, byte widths, plane counts, whole-frame sizing, per-plane visible sizing, invalid planar plane shapes, per-plane copy-on-write detachment, and explicit channel-layout construction. Validation passed with `cargo test -p avutil samplefmt`, `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-sample-format`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
