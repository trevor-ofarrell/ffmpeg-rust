...
`fftools-ffprobe-mov-avcc-stream-fields` is the current focus; the latest slice reports parsed MOV `avcC` `is_avc` and `nal_length_size` in constrained `ffprobe-rs -show_streams` output.
...
Latest slice: add MOV `avcC` stream-field reporting to `ffprobe-rs`. The stream report now surfaces `is_avc=true` and `nal_length_size=4` from parsed `avcC`; the existing avc1 VisualSampleEntry fixture proves both fields in JSON output.
