...
Update for the current slice: `FrameExif::common_tags()` now also reports GPSDestLatitudeRef, GPSDestLatitude, GPSDestLongitudeRef, GPSDestLongitude, GPSDestBearingRef, GPSDestBearing, GPSDestDistanceRef, and GPSDestDistance with strict ASCII/RATIONAL type, count, denominator, and reference-value validation. `avutil_core_models` build-checks the new GPS destination fixture and denominator invariants; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with GPS destination latitude, longitude, bearing, and distance metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports GPSDestLatitudeRef, GPSDestLatitude, GPSDestLongitudeRef, GPSDestLongitude, GPSDestBearingRef, GPSDestBearing, GPSDestDistanceRef, and GPSDestDistance while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
