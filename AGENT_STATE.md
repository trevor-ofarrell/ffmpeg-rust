...
Latest `avutil-channel-layout` update: `Channel` now exposes FFmpeg 8.1.1 native `FLC`/front-left-of-center and `FRC`/front-right-of-center mask support, and `ChannelLayout` now exposes source-checked `6.0`, `6.0(front)`, and `hexagonal` layouts alongside the prior `BC`/back-center, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, mono/stereo/quad/5.1/5.1(side)/7.1 subset. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new channels/layouts, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures were observed while adding `FLC`, `FRC`, `6.0`, `6.0(front)`, and `hexagonal`; the focused channel-layout unit test passed inside the sandbox with `--target-dir target-codex`.
  - Full `ffmpeg -layouts` oracle inventory comparison, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `FLC`/`FRC` native channels and `6.0`, `6.0(front)`, and `hexagonal` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, extends exact name/mask/parser tests, and expands the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entries after `hexagonal`, likely `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, and `7.0(front)`, including whether to add LFE-backed seven-channel defaults before wider 7.x layouts.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, and `BC`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, and `7.1` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added source-checked front-left/right-of-center channel support and three more standard native layouts to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CHAN_FRONT_LEFT_OF_CENTER`, `AV_CHAN_FRONT_RIGHT_OF_CENTER`, `AV_CH_FRONT_LEFT_OF_CENTER`, `AV_CH_FRONT_RIGHT_OF_CENTER`, `AV_CH_LAYOUT_6POINT0`, `AV_CH_LAYOUT_6POINT0_FRONT`, `AV_CH_LAYOUT_HEXAGONAL`, the public channel names `FLC`/`FRC`, the public layout names `6.0`, `6.0(front)`, and `hexagonal`, plus `av_channel_layout_default` first-map-entry behavior. The Rust model now exposes those channel masks and named layouts while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, and 7.1. Unit tests cover channel short names/masks, layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. The component remains `implemented`, not `complete`, because full oracle inventory parity, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
