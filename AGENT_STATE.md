...
`fftools-ffprobe-mov-format-tags` is the current focus; the latest slice proves MOV movie-level `udta/meta/ilst` metadata tags flow through the Rust demuxer into `ffprobe-rs -show_format` default and JSON output.
...
2. Broaden MOV format-tag command coverage beyond the current UTF-8 title atom once pinned-oracle vectors or more targeted fixtures are available.
...
Latest slice: add command-path coverage for MOV format metadata tags. `ffprobe-rs` now has default and JSON `-show_format` tests proving a UTF-8 movie title from `udta/meta/ilst` is rendered as a format tag while stream and packet sections stay absent unless requested. This still lacks pinned-oracle differential/FATE/fuzz coverage.
