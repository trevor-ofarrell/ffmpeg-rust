...
Latest slice: added typed opaque owner lookup for external opaque readonly `BufferRef` storage. Unit tests now cover successful typed lookup, mismatched-type lookup returning none, lookup preservation on shared originals, lookup removal from detached copies, release after the last original reference, unique detach release on `make_mut()`, resize detach waiting for shared originals, invalid visible-length rejection without invoking release, and source immutability; `avutil_core_models` now build-checks external opaque lookup, release timing, non-writability, resize detach, prefix preservation, zeroed growth/padding, and shared-reference invariants. The final validation pass ran formatting, focused buffer tests, fuzz-target check/clippy, local `avutil-buffer` mapping, changed-component dry-run and execution, workspace clippy, workspace tests, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
