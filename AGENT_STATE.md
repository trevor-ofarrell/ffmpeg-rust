...
Latest slice: added `BufferPoolCallbacks` and `BufferPool::with_callbacks` for custom exact-size allocation and permanent storage release. Unit tests now cover callback allocation/reuse/release, releasing outstanding pool-owned storage after pool drop, bad allocator-shape rejection with release, and manual recycle ownership transfer from callback-owned buffers into a pool; `avutil_core_models` now build-checks custom pool callback allocation/release and bad-shape rejection. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
