...
Latest `avutil-packet` differential slice: added replacement-style `Packet::add_side_data` and extended `crates/avutil/tests/packet_oracle.rs`, the ignored pinned libavcodec/libavutil `AVPacket` oracle harness, so it now validates core lifecycle/property behavior plus side-data API behavior. The oracle covers `av_packet_alloc` defaults, `av_packet_rescale_ts` with known and unknown timestamps, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `AV_PKT_FLAG_KEY|CORRUPT`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data` replacement/append behavior, duplicate `av_packet_new_side_data` replacement behavior, missing side-data lookup size reset, `opaque` address copying, payload preservation, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because remaining AVPacket from-data/grow/shrink/make-refcounted/make-writable behavior, complete side-data oracle vectors, upstream FATE coverage or a documented FATE exception, and broader differential media integration remain pending.
