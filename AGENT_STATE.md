...
Latest `avutil-frame` update: `FrameSkipSamples` now parses `AV_FRAME_DATA_SKIP_SAMPLES` side-data payloads as exactly 10 bytes matching FFmpeg's shared packet/frame skip-samples layout: little-endian start/end sample counts plus start/end reason bytes where `0` is padding silence and `1` is convergence. It rejects malformed lengths and invalid reason bytes with `InvalidData`, has a constructor that serializes the exact payload shape, and is covered by focused unit tests plus the build-checked `avutil_core_models` fuzz target. `FrameActiveFormatDescription` parses `AV_FRAME_DATA_AFD`, and `FrameSeiUnregistered` parses `AV_FRAME_DATA_SEI_UNREGISTERED`. Other typed FFmpeg frame side-data payload schemas remain pending.

Current typed frame side-data payload coverage includes AFD, skip samples, and SEI unregistered. Remaining frame side-data payload schemas are not counted as complete until they have exact shape parsing, invalid-input tests, and fuzz-build invariants.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed, explicit custom, and checked aligned line-size metadata, descriptor-style sample-format metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane and side-data ownership, the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_*` side-data inventory, descriptor/property metadata, remove-by-property helpers, explicit set-style side-data replacement/coalescing, typed `AV_FRAME_DATA_AFD`, `AV_FRAME_DATA_SKIP_SAMPLES`, and `AV_FRAME_DATA_SEI_UNREGISTERED` payload parsers, per-record metadata, side-data/hw-context writable checks and copy-on-write detachment helpers, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage. It remains below `complete` because typed FFmpeg side-data payload schemas beyond AFD, skip samples, and SEI unregistered, complete FFmpeg alignment policy, sample-data conversion routines, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next typed side-data payload schema, likely the audio service type payload, or a per-buffer permission-model gap, with invalid-input and sharing tests.
...
Latest slice: added `FrameSkipSamples`, `FrameSkipSamplesReason`, `FrameSideData::new_skip_samples`, and `FrameSideData::skip_samples()` for the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_SKIP_SAMPLES` payload shape: exactly 10 bytes containing little-endian start/end skip counts followed by start/end reason bytes. Malformed lengths and invalid reason values are rejected with `InvalidData`, non-skip side data returns `None`, and the `avutil_core_models` fuzz harness now build-checks valid, invalid, and random skip-samples parsing invariants. Validation passed with `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, and `git diff --check`.
