...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed line-size metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane ownership, BufferRef-backed frame side-data ownership with per-record metadata, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because typed FFmpeg frame side-data kinds, side-data replacement policy, custom alignment/stride, writable plane mutation, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership/shape gap, likely typed frame side-data kind modeling, custom alignment/stride metadata, or writable plane mutation semantics, with invalid-input and sharing tests.
...
Latest slice: added BufferRef-backed `FrameSideData` payload ownership with validated kind strings, per-side-data `Dictionary` metadata, `Frame` add/remove/take helpers, clone sharing, and final external-owner release timing. Unit tests now prove side-data payload/metadata retention, invalid kind rejection, removal, take semantics, and clone-shared release timing; `avutil_core_models` now build-checks frame side-data validation, metadata, removal, retained BufferRef storage sharing, and padding exclusion alongside the existing frame plane and hw context invariants. Validation passed with formatting, focused frame tests, fuzz-target check/clippy, the local `avutil-frame` mapping, changed-component dry-run and execution, workspace clippy, `cargo test --workspace --all-features --exclude fftools`, and `git diff --check`. `cargo test --workspace --all-features` still reaches the unchanged `fftools` test binary and is blocked by Windows Application Control (`os error 4551`). The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
