...
`avutil-options` now records known public AVOption flag bits and rejects mutation of read-only options. `OptionFlags` truncates unknown bits, `OptionDefinition` stores flags alongside type/default/help metadata, and `OptionSet` rejects parsed and typed writes to READONLY definitions without changing the current value. The `avutil_metadata_options` fuzz target now build-checks generated option flags, unknown-bit truncation, and read-only mutation rejection. The ledger still keeps `avutil-options` at `implemented`, not `complete`, because CLI option ordering, child classes, ranges with units, pinned-oracle differential vectors, FATE coverage, and actual local fuzz execution are still absent.
...
`avutil-options` is the latest coherent slice. Known public AVOption flag bits, flag truncation, read-only mutation rejection, unit coverage, and build-checked fuzz coverage have been added, but CLI option ordering, child classes, ranges with units, pinned FFmpeg differential tests, FATE coverage, and actual local fuzz execution are still absent.

## Next 3 Concrete Actions

1. Continue priority-1 avutil completeness with the next implemented primitive whose ledger records missing behavior, likely option ranges/units/child classes or checksum helper gaps.
...
Latest slice: strengthened `avutil::OptionSet` with public AVOption-style flags, read-only mutation rejection, unit coverage, fuzz-target coverage, and docs/ledger updates while keeping `avutil-options` below `complete`.
