...
Update for the current slice: `FrameExif::common_tags()` now interprets root TIFF FillOrder as a typed 1/2 enum, and the crate root re-exports the root EXIF coding/layout helper types. Unit tests cover valid least-significant-bit-first parsing plus malformed type, count, and value rejection, and the `avutil_core_models` build-checked fuzz fixture covers the deterministic valid and invalid FillOrder path. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root image-layout common-tag coverage. `FrameExif::common_tags()` now exposes FillOrder as a typed TIFF 1/2 enum alongside the existing Compression and PhotometricInterpretation root coding tags, and `avutil` now re-exports those root helper types from the crate root. Unit tests cover valid parsing plus malformed type/count/value rejection, and the `avutil_core_models` build-checked fuzz harness asserts the FillOrder 1..=2 invariant for the new accessor. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended EXIF root image-layout common-tag coverage for FillOrder and crate-root re-exports for the root EXIF coding/layout helper types. `frame_side_data_interprets_exif_root_fill_order_tags` covers valid parsing plus malformed type/count/value rejection, and `avutil_core_models` now build-checks deterministic fixture coverage plus the 1..=2 FillOrder invariant for the new accessor. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_fill_order_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
