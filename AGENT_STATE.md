...
Update for the current slice: `FrameExif::common_tags()` now exposes root TIFF `Predictor` as a nonzero raw SHORT value alongside the existing root descriptive/image-layout tags. Unit tests cover valid inline parsing plus malformed type, malformed count, and zero-value rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid Predictor paths plus the nonzero accessor invariant. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` now exposes `Predictor` as a nonzero raw SHORT value alongside the existing root image-layout, document/page, software, date/time, artist, host-computer, and copyright tags. Unit tests cover valid inline parsing plus malformed type, malformed count, and zero-value rejection, and the `avutil_core_models` build-checked fuzz harness asserts the deterministic valid and invalid paths plus the nonzero accessor invariant. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
