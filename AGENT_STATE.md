...
`avutil-packet` is the current focus for this slice. The concrete change is BufferRef-backed packet payload lifecycle support: packet refs share payload storage while copying side data, writable payload mutation detaches shared storage, move-ref transfers modeled fields and resets the source, and unref resets/release current packet state.
