...
Update for the current slice: side-data property-helper coverage now has broader build-checked fuzz invariants matching `frame_side_data_properties_drive_multi_and_removal_helpers`. The deterministic `avutil_core_models` fixture now covers descriptor/property flag lookup, multi-instance reporting, unknown side-data property emptiness, deterministic GLOBAL then SIZE_DEPENDENT/MULTI removal order, and empty-property no-op removal. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- No remaining failing commands in the latest side-data property-helper fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens side-data property-helper fuzz coverage. `Frame` already has a unit test for descriptor/property lookup, multi-instance reporting, property-driven removal order, unknown extension property emptiness, and empty-property no-op removal. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened side-data property-helper fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_properties_drive_multi_and_removal_helpers`: descriptor/property flag lookup, multi-instance reporting, unknown side-data property emptiness, deterministic removal order for GLOBAL then SIZE_DEPENDENT/MULTI filters, and no-op empty-property removal. Docs and the ledger now call out these property-helper invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_properties_drive_multi_and_removal_helpers`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
