...
`fftools-ffprobe-format-program-counts` is the current focus; the latest slice adds zero `nb_programs` and `nb_stream_groups` fields to `ffprobe-rs -show_format` default and JSON output for constrained MOV/MP4 and AVI files.
...
Latest slice: add `ffprobe-rs` zero program and stream-group counters. The report model now carries `nb_programs` and `nb_stream_groups`, constrained MOV/AVI command paths set both to zero, and default plus JSON `-show_format` tests prove the fields while keeping stream and packet sections absent unless requested. This still lacks pinned-oracle differential/FATE/fuzz coverage.
