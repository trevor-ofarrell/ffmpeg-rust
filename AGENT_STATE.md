...
Latest slice: added fallible `BufferRef` resize/reallocation semantics. Unit tests now cover shrink/grow behavior, zeroed growth and padding, same-shape padding zeroing, overflow rejection without mutation, shared detach, readonly callback-owned detach release, shared callback-owned release timing, and pool-owned resize detaching the original storage back to the pool; `avutil_core_models` now build-checks resize prefix preservation, zeroed resize growth/padding, same-shape padding zeroing, overflow non-mutation, and shared detach invariants. The final validation pass ran formatting, local `avutil-buffer` mapping, changed-component mappings, workspace clippy, workspace tests, fuzz-target check/clippy, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
