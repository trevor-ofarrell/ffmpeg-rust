...
Update for the current slice: `FrameExif::common_tags()` root colorimetry coverage now has build-checked fuzz invariants matching the invalid-input paths from the existing root colorimetry unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid WhitePoint, PrimaryChromaticities, YCbCrCoefficients, and ReferenceBlackWhite rational-array parsing plus malformed WhitePoint count, PrimaryChromaticities type, YCbCrCoefficients denominator, and ReferenceBlackWhite count cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root colorimetry common-tag fuzz coverage. `FrameExif::common_tags()` already exposed WhitePoint, PrimaryChromaticities, YCbCrCoefficients, and ReferenceBlackWhite with unit coverage for malformed WhitePoint count, PrimaryChromaticities type, YCbCrCoefficients denominator, and ReferenceBlackWhite count validation. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths while preserving the valid root colorimetry fixture values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
