...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `TFC`/top-front-center, `WL`/wide-left, `WR`/wide-right, and `hexadecagonal` from `channel_layout_map`, mapped to `AV_CH_LAYOUT_HEXADECAGONAL` as `octagonal` plus `WL`/`WR` and top-front/top-back height speakers. This builds on prior `TSL`/top-side-left, `TSR`/top-side-right, `9.1.6`, `9.1.4`, `TBC`/top-back-center, `LFE2`/low-frequency-2, `7.2.3`, `5.1.4`, `7.1.2`, `7.1.4`, `TBL`/top-back-left, `TBR`/top-back-right, `octagonal`, `cube`, `FLC`/front-left-of-center, `FRC`/front-right-of-center, `BC`/back-center, `TFL`/top-front-left, `TFR`/top-front-right, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new channels/layout, canonical `FL+FR`-style string, mask/name/channel-expression round trips, default lookups, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, binaural/downmix/22.2 layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CHAN_TOP_CENTER|AV_CHAN_TOP_FRONT_CENTER|AV_CHAN_WIDE_LEFT|AV_CHAN_WIDE_RIGHT|AV_CH_TOP_CENTER|AV_CH_TOP_FRONT_CENTER|AV_CH_WIDE_LEFT|AV_CH_WIDE_RIGHT|AV_CH_LAYOUT_HEXADECAGONAL' -Context 3,4` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern 'TOP_CENTER|TOP_FRONT_CENTER|WIDE_LEFT|WIDE_RIGHT|"TC"|"TFC"|"WL"|"WR"|"hexadecagonal"' -Context 3,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `TFC`, `WL`, `WR`, and `hexadecagonal`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, and `avformat` audio regression passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, binaural/downmix/22.2 layout coverage after `hexadecagonal`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `TFC`/top-front-center, `WL`/wide-left, `WR`/wide-right, and `hexadecagonal` from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving its `AV_CH_LAYOUT_HEXADECAGONAL` mapping as `octagonal` plus wide and top-front/top-back height speakers, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, binaural/downmix/22.2 inventory, custom/ambisonic orders, upstream FATE parity, or actual fuzz execution.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entries after `hexadecagonal`, starting with `binaural` and stereo-downmix channel families.
2. Source-check whether `22.2` can be modeled incrementally or requires a larger channel-family slice.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFC`, `TFR`, `TBL`, `TBC`, `TBR`, `WL`, `WR`, `LFE2`, `TSL`, and `TSR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, `7.1.4`, `7.2.3`, `9.1.4`, `9.1.6`, and `hexadecagonal` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no binaural/downmix/22.2 coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added source-checked `TFC`, `WL`, `WR`, and `hexadecagonal` native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CHAN_TOP_FRONT_CENTER`, `AV_CHAN_WIDE_LEFT`, `AV_CHAN_WIDE_RIGHT`, their native mask macros, `AV_CH_LAYOUT_HEXADECAGONAL`, `AV_CHANNEL_LAYOUT_HEXADECAGONAL`, the public short names `TFC`/`WL`/`WR`, and the `channel_layout_map` name `hexadecagonal`. The Rust model now exposes `hexadecagonal` as `octagonal` plus wide and top-front/top-back height speakers, while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover channel names/masks, layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local FATE mapping, changed-path local mappings, `avformat` audio tests, format checks, and `git diff --check` passed. The component remains `implemented`, not `complete`, because full oracle inventory parity, binaural/downmix/22.2 layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, upstream FATE, and actual fuzz execution remain absent.
