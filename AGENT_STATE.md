...
Update for the current slice: `FrameExif::common_tags()` linked GPS acquisition coverage now has build-checked fuzz invariants matching more invalid-input paths from the existing GPS acquisition unit test. The deterministic `avutil_core_models` EXIF fixture already preserved valid GPSSatellites, GPSStatus, GPSMeasureMode, and GPSDOP parsing plus GPSStatus count rejection; it now also covers malformed GPSStatus enum value, GPSMeasureMode count, and GPSDOP TIFF type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked GPS acquisition common-tag fuzz coverage. `FrameExif::common_tags()` already exposed GPSSatellites, GPSStatus, GPSMeasureMode, and GPSDOP values with unit coverage for malformed GPSStatus enum values and count, GPSMeasureMode count, and GPSDOP type validation. The `avutil_core_models` build-checked fuzz harness now asserts the missing GPSStatus enum-value, GPSMeasureMode count, and GPSDOP type paths alongside its existing GPSStatus count check. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF linked GPS acquisition common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching additional malformed cases from `frame_side_data_interprets_exif_gps_acquisition_tags`: malformed GPSStatus enum value, malformed GPSMeasureMode count, and malformed GPSDOP TIFF type, while preserving valid GPSSatellites, GPSStatus, GPSMeasureMode, and GPSDOP parsing plus the existing GPSStatus count rejection. Docs and the ledger now call out the GPS acquisition validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_gps_acquisition_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
