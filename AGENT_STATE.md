...
Latest current-loop `avutil-frame` update: ambient viewing environment side-data fuzz coverage now mirrors the focused unit contracts for `AV_FRAME_DATA_AMBIENT_VIEWING_ENVIRONMENT`. The deterministic `avutil_core_models` fixture preserves the 24-byte native payload size, ambient illuminance, CIE 1931 x/y chromaticity rationals, zero-valued default payloads, and non-ambient lookup absence, and rejects malformed length, zero denominators, negative illuminance/chromaticity, and out-of-range chromaticity through raw parser and deferred side-data accessor paths. This remains build-checked fuzz coverage only; pinned-oracle differential tests, upstream FATE media parity, and actual local fuzz execution are still blocked.
...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDolbyVisionRpuBuffer` non-empty and empty raw payload preservation plus `FrameDolbyVisionMetadata` malformed length/offset/size/count/level/color coverage, `FrameDynamicHdrVivid` nested maximum-RGB/tone-mapping/three-spline/color-saturation preservation plus malformed length/system/window/tone-mapping/three-spline/color-saturation coverage, and `FrameAmbientViewingEnvironment` illuminance/chromaticity preservation plus malformed length/denominator/sign/range coverage, matching the focused side-data unit coverage for raw/deferred side-data paths and constructor rejection paths where the component exposes a typed constructor.

Update for the current slice: ambient viewing coverage now has build-checked fuzz invariants matching the malformed-input branches in `frame_side_data_rejects_malformed_ambient_viewing_environment_payload`. The deterministic `avutil_core_models` fixture now asserts native payload sizing, selected illuminance/chromaticity field preservation, zero-valued default payload acceptance, empty/short/long payload rejection, zero-denominator rejection, negative illuminance/chromaticity rejection, out-of-range chromaticity rejection, typed-constructor rejection for invalid chromaticity, and non-ambient lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens ambient viewing environment side-data fuzz coverage. `FrameAmbientViewingEnvironment` already has unit coverage for native length, illuminance, CIE 1931 x/y chromaticity, zero-valued default payloads, malformed lengths, zero denominators, negative illuminance/chromaticity, out-of-range chromaticity, typed-constructor rejection, and non-ambient lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser and deferred side-data accessor paths, with typed-constructor rejection covered directly. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened ambient viewing environment side-data fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the ambient viewing malformed-input unit branches for native payload sizing, selected illuminance/chromaticity field preservation, zero-valued default payload acceptance, empty/short/long payload rejection, zero-denominator rejection, negative illuminance/chromaticity rejection, out-of-range chromaticity rejection, typed-constructor rejection, and non-ambient lookup absence. Docs and the ledger now call out these malformed length/denominator/sign/range invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_ambient_viewing_environment_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_ambient_viewing_environment_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
