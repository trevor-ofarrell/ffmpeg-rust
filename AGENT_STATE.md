...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes the next source-checked FFmpeg 8.1.1 `channel_layout_map` entries `quad(side)`, `3.1`, `5.0`, `5.0(side)`, and `4.1`, alongside the prior `BC`/back-center, `2.1`, `3.0`, `3.0(back)`, `4.0`, mono/stereo/quad/5.1/5.1(side)/7.1 subset. `ChannelLayout::default_for_count` now follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new names, canonical `FL+FR`-style strings, masks, default lookups, name/mask/channel-expression round trips, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures were observed while adding `quad(side)`, `3.1`, `5.0`, `5.0(side)`, and `4.1`; the focused channel-layout unit test passed inside the sandbox with `--target-dir target-codex`.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `quad(side)`, `3.1`, `5.0`, `5.0(side)`, and `4.1` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, extends exact name/mask/parser/default-count tests, and expands the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entries after the current 5.1 subset, likely `6.0`, `6.0(front)`, and `hexagonal`, including any missing channel-position mask bits.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `BC`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, and `7.1` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no downmix-layout coverage, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added the next source-checked standard-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CH_LAYOUT_2_2`, `AV_CH_LAYOUT_3POINT1`, `AV_CH_LAYOUT_5POINT0_BACK`, `AV_CH_LAYOUT_5POINT0`, `AV_CH_LAYOUT_4POINT1`, the public map names `quad(side)`, `3.1`, `5.0`, `5.0(side)`, and `4.1`, plus `av_channel_layout_default` first-map-entry behavior. The Rust model now exposes those named layouts and modeled defaults for counts 1, 2, 3, 4, 5, 6, and 8: mono, stereo, 2.1, 4.0, 5.0, 5.1, and 7.1. Unit tests cover layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator and explicit five-channel count generation. The component remains `implemented`, not `complete`, because full oracle inventory parity, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
