...
Latest slice: added an explicit empty/unreferenced `FrameData` state plus `Frame::empty`, `Default`, `unref`, `ref_from`, and `move_ref_from`. Unit tests now prove `unref` clears PTS/data/side-data/hw context and releases owned BufferRefs, `ref_from` replaces the destination while sharing source plane/side-data/hw-context refs, and `move_ref_from` transfers references while leaving the source empty and releasing the old destination. `avutil_core_models` now build-checks empty/unref/ref_from/move_ref_from behavior alongside custom video line sizes, frame plane ownership, writable dispatch, side-data, and hw context invariants. The changed `FrameData` enum required avcodec decoder test matches to account for `FrameData::Empty`, and `tests/fate/mappings.txt` now has local rawvideo and pcm_s16le decoder smoke mappings so changed-path dry-runs remain runnable. Validation passed with `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame` through `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --component avcodec-rawvideo`, `cargo run -p fate-runner -- run --component avcodec-pcm-s16le`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
