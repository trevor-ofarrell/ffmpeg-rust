...
This slice tightens the `FrameSideData::data_mut()` ownership contract. Direct side-data mutation is now covered by a unit test and the `avutil_core_models` fuzz harness to prove shared or readonly payload storage detaches before mutation, metadata survives, cloned/original buffers remain unchanged, and external readonly release callbacks still fire only after the final original reference drops. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
