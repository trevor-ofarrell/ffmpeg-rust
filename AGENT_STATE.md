...
Update for the current slice: `FrameExif::common_tags()` interoperability related-image coverage now has build-checked fuzz invariants matching the existing unit test for malformed linked interoperability tags. The deterministic `avutil_core_models` EXIF interoperability fixture now covers valid InteroperabilityVersion, RelatedImageFileFormat, RelatedImageWidth, and RelatedImageLength parsing plus malformed version type/count/digit, related-image format type/NUL, related-image width type/count/zero, and related-image length zero cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF interoperability related-image common-tag fuzz coverage. `FrameExif::common_tags()` already exposed InteroperabilityVersion as fixed digit-coded UNDEFINED bytes, RelatedImageFileFormat as ASCII, and RelatedImageWidth/RelatedImageLength as positive SHORT/LONG values, with unit coverage for malformed type/count/value/NUL shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: version wrong type, version wrong count, version nondigit byte, related-image format wrong type, related-image format missing NUL, related-image width wrong count, related-image width wrong type, related-image width zero, and related-image length zero. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF interoperability related-image common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_interoperability_related_image_tags`: malformed InteroperabilityVersion type/count/digit, malformed RelatedImageFileFormat type/NUL, malformed RelatedImageWidth type/count/zero, and zero RelatedImageLength. Docs and the ledger now call out the interoperability version, related-image format, and related-image dimension validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_interoperability_related_image_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
