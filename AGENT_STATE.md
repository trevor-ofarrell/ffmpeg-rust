...
`avformat-mov-demuxer` remains the current component. It now structures direct audio `esds` child boxes, rejects malformed direct descriptors, and still has a local FATE-runner smoke mapping that executes the focused MOV unit suite through `cargo run -p fate-runner -- run --component avformat-mov-demuxer`. Exact FFmpeg ffprobe output semantics, pinned FFmpeg differential tests, upstream FATE media parity, actual local fuzz execution, additional codec-specific audio extension parsing beyond direct `esds`/`wave`/`chan`, and broader MOV subtitle/data sample-entry coverage remain absent.
...
2. Extend MOV sample-entry parsing into additional codec-specific audio extension atoms beyond direct `esds`/`wave`/`chan` and broader subtitle/data sample-entry variants.
...
Latest slice: parsed direct MOV audio `esds` child boxes into `MovElementaryStreamDescriptor`, added malformed direct-descriptor rejection coverage, updated MOV compatibility notes, and kept the component below `complete` because pinned oracle, upstream FATE, and actual fuzz parity remain missing.
