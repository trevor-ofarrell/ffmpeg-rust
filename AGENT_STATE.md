...
- `cargo test -p avutil frame_set_side_data_replaces_alias_duplicates_and_releases_removed_records`
...
- No remaining failing commands in the latest side-data replacement-release fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens side-data replacement-release fuzz coverage. `Frame` already has unit tests for set-style side-data append, alias duplicate coalescing, replacement ordering, metadata mutation, and external payload release timing. The `avutil_core_models` build-checked fuzz harness now mirrors the replacement, no-premature-release, removed-record release, and retained-record release assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened side-data replacement-release fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_set_side_data_replaces_alias_duplicates_and_releases_removed_records`: empty replacement returns for appended side data, `Display Matrix`/`display_matrix` alias duplicate coalescing, replacement payload ordering, metadata mutation after replacement, no premature external payload release, removed payload release after dropping removed records, and retained replacement/ReplayGain payload release after dropping the owning frame. Docs and the ledger now call out these replacement-release invariants explicitly. Validation passed with `cargo test -p avutil frame_set_side_data_replaces_alias_duplicates_and_releases_removed_records`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
