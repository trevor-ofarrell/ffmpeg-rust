...
This slice tightens `AV_FRAME_DATA_EXIF` SubjectArea validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count, ASCII-shape, coordinate, timestamp, direction, SubSecTime, date, and dimension enforcement and additionally rejects zero circle diameters and zero rectangle width/height values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
