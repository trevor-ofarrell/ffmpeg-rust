...
`fftools-ffprobe-count-frames` is the current focus; the latest slice accepts `-count_frames` and renders constrained `nb_read_frames` in both default and JSON MOV/MP4 and AVI `ffprobe-rs -show_streams` output.
...
Latest slice: add constrained ffprobe frame-count reporting. `ffprobe-rs` now accepts `-count_frames`, uses already parsed MOV sample counts and AVI stream lengths for the current one-frame-per-sample models, renders `nb_read_frames` on stream reports, and keeps packet/format sections absent unless requested; the new tests cover both default and JSON output shapes for MOV and AVI. This is not a general decoder-backed frame counter and still lacks pinned-oracle differential/FATE/fuzz coverage.
