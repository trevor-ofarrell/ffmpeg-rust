...
Update for the current slice: frame `make_writable` data-plane coverage now has broader build-checked fuzz invariants matching `frame_make_writable_detaches_video_payload_refs`. The deterministic `avutil_core_models` fixture now covers detaching a cloned readonly video plane on `Frame::make_writable`, preserving visible bytes, keeping the clone and original source buffer unchanged after visible-plane mutation, and leaving side-data plus `hw_frames_context` references shared because `make_writable` is only the data-plane operation. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- `cargo test -p avutil frame_make_writable_detaches_video_payload_refs`
...
- No remaining failing commands in the latest frame `make_writable` data-plane fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens frame `make_writable` data-plane fuzz coverage. `Frame` already has a unit test proving `make_writable` detaches cloned readonly video plane storage, preserves visible bytes, isolates subsequent visible-plane mutation from the clone and source buffer, and intentionally leaves side-data plus `hw_frames_context` references shared. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened frame `make_writable` data-plane fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_make_writable_detaches_video_payload_refs`: `Frame::make_writable` detaches cloned readonly video plane storage, preserves visible bytes, isolates subsequent visible-plane mutation from the clone and source buffer, and leaves side-data plus `hw_frames_context` references shared. Docs and the ledger now call out these `make_writable` data-only ownership invariants explicitly. Validation passed with `cargo test -p avutil frame_make_writable_detaches_video_payload_refs`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
