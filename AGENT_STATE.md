...
Update for the current slice: `FrameExif::common_tags()` root image-layout coverage now has build-checked fuzz invariants matching the invalid-input paths from the existing root image-layout unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid SamplesPerPixel, PlanarConfiguration, YCbCrSubSampling, and YCbCrPositioning parsing plus malformed SamplesPerPixel type/count/zero, PlanarConfiguration count/value, YCbCrSubSampling type/count/zero, and YCbCrPositioning type/value cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root image-layout common-tag fuzz coverage. `FrameExif::common_tags()` already exposed SamplesPerPixel, PlanarConfiguration, YCbCrSubSampling, and YCbCrPositioning with unit coverage for malformed SamplesPerPixel type/count/zero, PlanarConfiguration count/value, YCbCrSubSampling type/count/zero, and YCbCrPositioning type/value validation. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths while preserving the valid root image-layout fixture values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root image-layout common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching `frame_side_data_interprets_exif_root_image_layout_tags`: malformed SamplesPerPixel type/count/zero, malformed PlanarConfiguration count/value, malformed YCbCrSubSampling type/count/zero, and malformed YCbCrPositioning type/value, while preserving valid SamplesPerPixel, PlanarConfiguration, YCbCrSubSampling, and YCbCrPositioning values. Docs and the ledger now call out the root image-layout validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_root_image_layout_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
