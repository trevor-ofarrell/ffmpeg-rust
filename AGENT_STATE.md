...
`avutil-frame` now retains BufferRef-backed ownership for video/audio frame planes, frame side-data payloads, and the optional hardware frames context. `VideoFrame` and `AudioFrame` expose visible byte snapshots through the existing `planes()` API while retaining underlying plane owners through `plane_buffers()`, validate plane count and visible length, exclude padding from visible snapshots, and keep external plane owners alive until the final frame clone drops. `FrameSideData` carries an initial typed `FrameSideDataKind`, BufferRef payload, and per-record `Dictionary` metadata; known-kind aliases normalize common FFmpeg side-data names while unknown extension names are preserved. `Frame` clone, typed lookup, remove, take, replace, and drop paths preserve side-data and `hw_frames_context` BufferRef sharing and release timing, including external opaque owner release once the final frame/context reference is gone. The `avutil_core_models` fuzz harness now build-checks frame plane BufferRef ownership, frame side-data typed-kind mapping, validation/removal/metadata/storage sharing, padding exclusion, hw-frames-context attachment, sharing, typed opaque lookup, take, clone-retention, and release invariants. This is still a Rust ownership model only; complete FFmpeg frame side-data kind inventory, side-data replacement policy, hardware device/frame context internals, actual hardware formats, writable frame plane mutation, custom alignment/stride behavior, and pinned-oracle/FATE parity remain absent.
...
- `cargo test -p avutil frame` compiled the rebuilt avutil unit-test binary but Windows Application Control blocked launching `target\debug\deps\avutil-c501250f1d03cde5.exe` with `os error 4551` on two attempts. The slice is therefore validated with `cargo test -p avutil --lib --no-run`, focused fuzz-target check/clippy, workspace clippy, and FATE-runner dry-runs, but the new unit tests could not execute locally in this environment yet.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed line-size metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane ownership, BufferRef-backed frame side-data ownership with typed known-kind mapping and per-record metadata, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because the complete FFmpeg frame side-data kind inventory, side-data replacement policy, custom alignment/stride, writable plane mutation, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent. The current rebuilt `avutil` unit-test executable is also blocked by Windows Application Control in this environment, so the new unit tests are compiled but not yet locally executed.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership/shape gap, likely side-data replacement policy, custom alignment/stride metadata, or writable plane mutation semantics, with invalid-input and sharing tests.
...
Latest slice: added `FrameSideDataKind`, an initial typed side-data known-kind layer with aliases for common FFmpeg frame side-data names, typed `Frame` lookup/removal/addition helpers, and unknown extension-name preservation. Unit tests were added for known-kind alias mapping, canonical names, unknown preservation, typed lookup/removal, and invalid-name rejection; `avutil_core_models` now build-checks typed frame side-data kind selection, metadata, removal, retained BufferRef storage sharing, and padding exclusion alongside the existing frame plane and hw context invariants. Validation passed with formatting, fuzz-target check/clippy, workspace clippy, `cargo test -p avutil --lib --no-run`, and FATE-runner dry-runs. `cargo test -p avutil frame` compiled but Windows Application Control blocked launching the rebuilt avutil unit-test executable (`os error 4551`), so the new focused tests are not yet locally executed. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
