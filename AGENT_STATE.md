...
1. Continue priority-1 format-model work by investigating the next rawvideo-facing `PixelFormat` gap from pinned FFmpeg 8.1.1 inventory, likely the high-bit-depth planar YUV family (`yuv420p9*`, `yuv420p10*`, `yuv422p10*`, and nearby simple variants) before claiming any color conversion or component-layout parity.
