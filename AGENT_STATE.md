...
Update for the current slice: `FrameExif::common_tags()` now exposes root TIFF `DocumentName`, `PageName`, and `PageNumber` values, preserving the ASCII strings and fixed two-SHORT page-number pair through typed accessors. Unit tests cover valid inline and out-of-line parsing plus malformed type, missing ASCII terminator, and invalid PageNumber count rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid document/page paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root image-layout/descriptive common-tag coverage. `FrameExif::common_tags()` now exposes `DocumentName` and `PageName` as single ASCII strings and `PageNumber` as a fixed two-SHORT array alongside the existing root image-layout and coding tags. Unit tests cover valid inline/out-of-line parsing plus malformed type, missing ASCII terminator, and invalid PageNumber count rejection, and the `avutil_core_models` build-checked fuzz harness asserts the new deterministic valid and invalid paths. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended EXIF root image-layout/descriptive common-tag coverage for `DocumentName`, `PageName`, and `PageNumber`. `frame_side_data_interprets_exif_root_document_page_tags` covers valid inline `DocumentName`, out-of-line `PageName`, and fixed two-SHORT `PageNumber` parsing plus malformed type, missing ASCII terminator, and invalid PageNumber count/type rejection; `avutil_core_models` now build-checks deterministic valid and invalid document/page fixture paths plus ASCII invariants for the new accessors. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_document_page_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
