...
Latest current-loop `repo-runtime-guard` update: `xtask guard-runtime` now enforces the no-FFmpeg-runtime-linkage policy. The guard scans Cargo dependency sections for known FFmpeg wrapper crates, scans runtime implementation crates for wrapper tokens or literal `ffmpeg`/`ffprobe`/`ffplay` process spawns, intentionally leaves oracle/FATE/test/fuzz/xtask tooling outside the runtime-shell-out ban, and is wired into `xtask quick`, `xtask changed`, `xtask full`, and local FATE-runner changed-path selection. This is a policy/infrastructure guard only; it does not add media parity, oracle snapshots, upstream FATE samples, or actual fuzz execution.
...
`repo-runtime-guard` is the current focus for this slice. It is implemented as `cargo run -p xtask -- guard-runtime`, with unit coverage for allowed local project dependencies, forbidden dependency names, forbidden `package =` renames, forbidden runtime wrapper imports, forbidden literal runtime `ffmpeg` process spawns, and allowed `ffmpeg-rs`/`ffprobe-rs` binary-name strings. The guard is also wired into `xtask quick`, `xtask changed`, `xtask full`, `tests/fate/mappings.txt`, and `fate-runner` changed-path selection for `xtask/` edits.

This slice does not mark media parity complete. It reduces the risk of violating the non-negotiable no-runtime-FFmpeg rule while the next loop returns to the highest-priority incomplete parity work. The broader goal remains blocked on missing pinned-oracle snapshots, upstream FATE media mappings/samples, actual local fuzz execution, and many incomplete FFmpeg surfaces.

## Next 3 Concrete Actions

1. Return to `avutil-frame` or the next priority-1 incomplete primitive and extend the next unmirrored invalid-input, raw-value preservation, or differential-test gap.
...
Latest slice: added a runtime FFmpeg linkage guard. `xtask guard-runtime` now rejects known FFmpeg wrapper dependencies, package-renamed FFmpeg wrapper crates, runtime wrapper import tokens, and literal `ffmpeg`/`ffprobe`/`ffplay` process spawns from implementation crates, while allowing oracle/FATE/test/fuzz/xtask tooling to invoke configured oracle binaries. The guard runs from `xtask quick`, `xtask changed`, `xtask full`, and the local FATE-runner mapping `repo-runtime-guard|local-runtime-guard`; `fate-runner` changed-path selection now maps `xtask/` edits to `repo-runtime-guard`. Validation passed with `cargo fmt --all`, `cargo test -p xtask`, `cargo run -p xtask -- guard-runtime`, `cargo test -p fate-runner default_mappings_cover_runtime_guard_selection`, `cargo clippy -p xtask -p fate-runner --all-targets -- -D warnings`, `cargo run -p fate-runner -- run --component repo-runtime-guard`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, `cargo run -p xtask -- quick`, and `cargo clippy --workspace --all-targets --all-features -- -D warnings`.
