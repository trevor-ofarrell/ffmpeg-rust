...
Update for the current slice: `FrameExif::common_tags()` root TIFF `XResolution` and `YResolution` handling now has dedicated validation coverage as single unsigned RATIONAL values alongside the existing root descriptive/image-layout tags. Unit tests cover valid out-of-line rational parsing plus malformed XResolution type, malformed XResolution count, zero XResolution denominator, malformed YResolution count, and zero YResolution denominator, and the `avutil_core_models` build-checked fuzz fixture covers the same deterministic valid and invalid paths plus denominator invariants for both accessors. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo test -p avutil frame_side_data_interprets_exif_root_resolution_tags`
...
- No remaining failing commands in the latest EXIF root resolution slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` already exposed XResolution and YResolution as selected root rational tags; they now have dedicated root-tag validation coverage as single unsigned RATIONAL values alongside the existing root image-layout, document/page, camera-identity, orientation/resolution-unit, descriptive ASCII, host-computer, predictor, and copyright tags. Unit tests cover valid out-of-line parsing plus malformed XResolution type/count/denominator and malformed YResolution count/denominator, and the `avutil_core_models` build-checked fuzz harness asserts deterministic valid and invalid paths plus denominator invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root resolution common-tag coverage for `XResolution` and `YResolution`. `frame_side_data_interprets_exif_root_resolution_tags` covers valid out-of-line RATIONAL parsing plus malformed XResolution type, malformed XResolution count, zero XResolution denominator, malformed YResolution count, and zero YResolution denominator; `avutil_core_models` now build-checks the same deterministic valid and invalid fixture paths plus denominator invariants for both accessors. The first focused run failed because the YResolution bad-count fixture caused parse-time range rejection before common-tag validation; the fixture now uses count 0 and the focused test passes. Validation passed with `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo test -p avutil frame_side_data_interprets_exif_root_resolution_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --component avutil-frame`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --changed --dry-run`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
