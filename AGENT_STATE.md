...
Update for the current slice: `FrameExif::common_tags()` optics/subject coverage now has build-checked fuzz invariants matching the existing unit test for malformed subject-area and optics tags. The deterministic `avutil_core_models` EXIF fixture now covers valid CompressedBitsPerPixel, MaxApertureValue, SubjectDistance, rectangle/point/circle SubjectArea, SubjectLocation, and ExposureIndex parsing plus malformed SubjectArea zero-size, count, and type cases, malformed SubjectLocation count, and malformed ExposureIndex denominator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_optics_subject_tags`
...
- No remaining failing commands in the latest EXIF optics/subject fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF optics/subject common-tag fuzz coverage. `FrameExif::common_tags()` already exposed CompressedBitsPerPixel, MaxApertureValue, SubjectDistance, SubjectArea, SubjectLocation, and ExposureIndex with unit coverage for rectangle, point, and circle SubjectArea variants plus malformed count, type, zero-size, and rational-denominator shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic paths too: point and circle SubjectArea acceptance, SubjectArea wrong count, SubjectArea wrong TIFF type, SubjectLocation wrong count, and ExposureIndex zero denominator, alongside the existing rectangle zero-width, zero-height, and circle zero-diameter checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF optics/subject common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching `frame_side_data_interprets_exif_optics_subject_tags`: valid point and circle SubjectArea variants, malformed SubjectArea count, malformed SubjectArea TIFF type, malformed SubjectLocation count, and malformed ExposureIndex denominator, in addition to the existing malformed rectangle width, rectangle height, and circle diameter checks. Docs and the ledger now call out the optics/subject SubjectArea, SubjectLocation, and rational-denominator validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_optics_subject_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
