...
`fftools-ffmpeg-image2-file-output` is the current focus; the next slice is image2 option parity or broader `ffmpeg-rs` output muxer coverage.

## Next 3 Concrete Actions

1. Add image2 `start_number` input/output option support and tests.
2. Add another constrained file-output muxer path or wire image2 output from decoded image-compatible inputs once a codec path exists.
...
Latest slice: extend constrained `ffmpeg-rs` execution from explicit image2 single-file and contiguous numbered-sequence inputs to local `-f image2` file or numbered-pattern output, using the Rust `Image2Muxer` with generated-path preflight, create-new output semantics, and in-process unit coverage.
