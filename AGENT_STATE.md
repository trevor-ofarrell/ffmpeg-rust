...
Latest `avutil-packet` differential slice: added Rust-native `packet_pack_dictionary` and `packet_unpack_dictionary` helpers for the `av_packet_pack_dictionary` / `av_packet_unpack_dictionary` string-metadata wire format, with unit coverage for ordered pack/unpack, empty dictionaries, malformed payload rejection, and non-UTF-8 rejection. `crates/avutil/tests/packet_oracle.rs` now validates empty dictionary packing, nonempty dictionary packing, unpacking, and duplicate-key unpack overwrite/key-spelling behavior against pinned libavcodec/libavutil. The same oracle continues to validate fixed/comment-defined/raw side-data payload layouts, `AV_PKT_DATA_NB`, every public `AV_PKT_DATA_*` numeric enum value, all 41 public `av_packet_side_data_name()` display strings, core lifecycle/property behavior, payload API behavior, and side-data API behavior: `av_packet_alloc` defaults, `av_packet_rescale_ts`, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `av_packet_from_data`, `av_grow_packet`, `av_shrink_packet`, `av_packet_make_refcounted`, `av_packet_make_writable`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data`, missing side-data lookup size reset, `opaque` address copying, payload preservation, flags, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
...
Latest validation commands: `cargo fmt --all`, `cargo fmt --all -- --check`, `cargo test -p avutil packet_dictionary`, `cargo test -p avutil packet`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, `cargo clippy --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check -- -D warnings`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, and `git diff --check` pass for the latest packet dictionary pack/unpack oracle slice. `git diff --check` reports CRLF conversion warnings only.
