...
1. Continue priority-1 primitive work by adding the next unblocked shared `avutil` compatibility vector, most likely another packet side-data payload parser such as AFD, S12M timecode, quality stats, or fallback-track once its FFmpeg 8.1.1 layout is verified.
...
- Windows Application Control intermittently blocks freshly built child executables and separate integration-test executables. During recent packet slices it blocked focused `avutil` and `fftools` unit-test executables in multiple target directories; in the latest WebVTT slice the first direct `avutil` packet test launches were blocked after rebuild, but subsequent local FATE, direct focused test, changed FATE mappings, and `cargo run -p xtask -- quick` all passed. The current ffprobe MOV command-path coverage is kept in the `fftools` unit-test binary instead of a process-spawn integration test.

## Summary Of Latest Commit Or Changes

Latest slice: added typed packet side-data payload parsers for WebVTT cue identifier and settings data. `PacketWebVttIdentifier` and `PacketWebVttSettings` model `AV_PKT_DATA_WEBVTT_IDENTIFIER` and `AV_PKT_DATA_WEBVTT_SETTINGS` as raw single-line cue payload bytes, preserve invalid UTF-8 bytes while exposing fallible UTF-8 views, reject empty/NUL/line-terminator payloads, and additionally reject `-->` in identifiers. `SideData` can construct and defer-parse both payload types only when the side-data kind matches. Focused packet unit tests cover raw byte preservation, UTF-8 and non-UTF-8 views, constructors/accessors, non-matching kind behavior, malformed line payloads, and deferred accessor errors; `avutil_core_models` now build-checks the same invariants. Validation passed with `cargo fmt --all`, `cargo test -p avutil packet`, `cargo clippy -p avutil --all-targets -- -D warnings`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `git diff --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-packet --dry-run`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `cargo run -p xtask -- quick`. Initial direct focused test launches hit Windows Application Control on the rebuilt `avutil` test binary, but reruns passed.
