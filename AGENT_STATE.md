...
`avutil-frame` now retains BufferRef-backed ownership for video/audio frame planes, frame side-data payloads, and the optional hardware frames context. `FrameData` has an explicit empty/unreferenced state, and `Frame` supports empty/default construction, `unref`, `ref_from`, and `move_ref_from` for clearing, sharing, or moving current frame references while preserving BufferRef release timing. `VideoFrame` and `AudioFrame` expose visible byte snapshots through the existing `planes()` API while retaining underlying plane owners through `plane_buffers()`, validate plane count and visible length, exclude padding from visible snapshots, report writability, detach shared/readonly plane storage through BufferRef copy-on-write, support visible-plane mutation, and keep external plane owners alive until the final frame clone drops. `FrameData` and `Frame` now dispatch current data-plane writability checks, copy-on-write detachment, and visible-plane mutation across audio/video payloads, while `FrameSideData` and `Frame` expose explicit side-data, hw-context, and combined all-reference writable checks plus copy-on-write detachment helpers. `VideoFrame` also accepts explicit per-plane line sizes for the current pixel-format subset, retains full strided storage in `plane_buffers()`, preserves row padding during visible-plane writes, and packs visible rows for `planes()`. `FrameSideData` carries an initial typed `FrameSideDataKind`, BufferRef payload, per-record `Dictionary` metadata, payload writability checks, and writable data access; known-kind aliases normalize common FFmpeg side-data names while unknown extension names are preserved. `Frame` clone, typed lookup, explicit side-data replacement with duplicate coalescing, remove, take, replace, make-writable, and drop paths preserve side-data and `hw_frames_context` BufferRef sharing and release timing, including external opaque owner release once the final frame/context reference is gone. The `avutil_core_models` fuzz harness now build-checks frame plane BufferRef ownership, custom video line-size handling, writable plane detachment/mutation, top-level Frame/FrameData writable dispatch, full-reference side-data/hw-context writable detachment, empty/unref/ref_from/move_ref_from behavior, frame side-data typed-kind mapping, validation/replacement/removal/metadata/storage sharing, padding exclusion, hw-frames-context attachment, sharing, typed opaque lookup, take, clone-retention, and release invariants. This is still a Rust ownership model only; complete FFmpeg frame side-data kind inventory, hardware device/frame context internals, actual hardware formats, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, broader custom alignment policy, custom audio stride handling, and pinned-oracle/FATE parity remain absent.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed and explicit custom video line-size metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane ownership with visible-plane copy-on-write mutation, top-level Frame/FrameData data-plane writable dispatch, empty/unref/ref_from/move_ref_from reference behavior, BufferRef-backed frame side-data ownership with typed known-kind mapping, explicit set-style replacement/coalescing, per-record metadata, side-data/hw-context writable checks and copy-on-write detachment helpers, optional `BufferRef`-backed `hw_frames_context` ownership, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage, but remains below `complete` because the complete FFmpeg frame side-data kind inventory, broader alignment policies, custom audio stride handling, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` toward the next AVFrame ownership/shape gap, likely broader alignment policy, custom audio stride handling, or a deeper AVFrame side-data kind inventory, with invalid-input and sharing tests.
...
Latest slice: added explicit side-data and `hw_frames_context` writability APIs for the current `avutil-frame` BufferRef ownership model. `FrameSideData` now reports payload writability, detaches shared/readonly payloads through `make_writable`, and exposes `data_mut()` for copy-on-write payload mutation. `Frame` now reports side-data writability, reports optional hw-context writability, can make side-data or hw-context buffers writable independently, and can check/detach all current frame references through `all_references_are_writable()` and `make_all_references_writable()` while keeping `Frame::make_writable()` scoped to data planes. Unit and fuzz-build coverage prove shared/readonly plane, side-data, and hw-context buffers detach from clones without mutating original storage. Validation passed with `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
