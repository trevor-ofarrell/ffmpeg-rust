...
Update for the current slice: `avutil_core_models` also build-checks EXIF exposure common-tag interpretation for ExposureTime, FNumber, ExposureBiasValue, FocalLength, PixelXDimension, PixelYDimension, and DateTimeDigitized plus semantic rejection for malformed known-tag shapes; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF exposure metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports DateTimeDigitized, ExposureTime, FNumber, ExposureBiasValue, FocalLength, PixelXDimension, and PixelYDimension while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
