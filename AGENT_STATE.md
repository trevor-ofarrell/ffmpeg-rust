...
`fftools-ffprobe-format-size` is the current focus; the latest slice threads local input byte length into `ffprobe-rs -show_format` default and JSON output for constrained MOV/MP4 and AVI files.
...
2. Add more format-level ffprobe fields with careful oracle-backed semantics, such as bit_rate, once the exact rounding/unknown-value behavior is pinned.
...
Latest slice: add `ffprobe-rs` format size reporting. The report model now carries local input byte length, MOV/AVI command paths fill it from the read input bytes, and default plus JSON `-show_format` tests prove exact fixture sizes while keeping stream and packet sections absent unless requested. This still lacks pinned-oracle differential/FATE/fuzz coverage.
