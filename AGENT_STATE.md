...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern "AV_CH_LAYOUT_6POINT1|AV_CH_LAYOUT_7POINT0"` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern '"6\.1"|"7\.0"'` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures were observed while adding `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, and `7.0(front)`; the focused channel-layout unit test passed inside the sandbox with `--target-dir target-codex`.
  - Full `ffmpeg -layouts` oracle inventory comparison, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, and `7.0(front)` standard native-mask layouts from pinned FFmpeg 8.1.1 `channel_layout_map`, updates seven-channel default selection to `6.1`, extends exact name/mask/parser/default-count tests, and expands the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entries after `7.0(front)`, likely `7.1(wide)`, `7.1(wide-side)`, and the first height-channel layouts, including whether to add missing top-channel mask bits or defer height layouts to a separate slice.
...
Latest slice: added the next source-checked seven-channel native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CH_LAYOUT_6POINT1`, `AV_CH_LAYOUT_6POINT1_BACK`, `AV_CH_LAYOUT_6POINT1_FRONT`, `AV_CH_LAYOUT_7POINT0`, `AV_CH_LAYOUT_7POINT0_FRONT`, the public layout names `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, and `7.0(front)`, plus `av_channel_layout_default` first-map-entry behavior. The Rust model now exposes those named layouts and updates modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. The component remains `implemented`, not `complete`, because full oracle inventory parity, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
