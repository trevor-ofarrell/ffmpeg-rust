...
`avutil-buffer` is the current focus. The latest slice adds pinned libavutil `av_buffer_create` writable opaque-data coverage on top of the existing `AVBufferRef` and `AVBufferPool` oracle rows: owned opaque buffers expose `av_buffer_get_opaque`-style data, preserve the owner for unique writable mutation, detach shared make-writable copies without carrying the opaque, and release the original opaque plus bytes when the owner drops. The component remains `differential_pass`, not `complete`, because broader ABI/lifetime parity, FATE disposition, and final completion documentation still need closure.
