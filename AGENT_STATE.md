...
Update for the current slice: `FrameExif::common_tags()` now also reports ShutterSpeedValue, ApertureValue, and BrightnessValue with strict SRATIONAL/RATIONAL type, count, and zero-denominator validation. `avutil_core_models` build-checks the new EXIF APEX exposure fixture alongside the exposure, environment, camera/lens, gamma/composite, optics/subject, version/timing/comment, and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_apex_exposure_tags`
...
- No remaining failing commands in the latest EXIF APEX exposure side-data slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF APEX exposure metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports ShutterSpeedValue, ApertureValue, and BrightnessValue while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes and zero rational denominators. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
