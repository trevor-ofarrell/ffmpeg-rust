...
Latest current-loop `avutil-frame` update: video hint side-data fuzz coverage now mirrors the focused unit contracts for `AV_FRAME_DATA_VIDEO_HINT`. The deterministic `avutil_core_models` fixture preserves native header and rectangle sizes, two-rectangle Changed hints, zero-rectangle Constant hints, rectangle coordinates, FFmpeg constant-name mapping, and non-video-hint lookup absence, and rejects malformed length, rectangle offset, rectangle size, hint type, rectangle count, trailing bytes, zero-width/height rectangles, and coordinate overflows through raw parser and deferred side-data accessor paths. This remains build-checked fuzz coverage only; pinned-oracle differential tests, upstream FATE media parity, and actual local fuzz execution are still blocked.
...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDolbyVisionRpuBuffer` non-empty and empty raw payload preservation plus `FrameDolbyVisionMetadata` malformed length/offset/size/count/level/color coverage, `FrameDynamicHdrVivid` nested maximum-RGB/tone-mapping/three-spline/color-saturation preservation plus malformed length/system/window/tone-mapping/three-spline/color-saturation coverage, `FrameAmbientViewingEnvironment` illuminance/chromaticity preservation plus malformed length/denominator/sign/range coverage, and `FrameVideoHint` Changed/Constant payload preservation plus malformed length/offset/size/type/count/rectangle/overflow coverage, matching the focused side-data unit coverage for raw/deferred side-data paths and constructor rejection paths where the component exposes a typed constructor.

Update for the current slice: video hint coverage now has build-checked fuzz invariants matching the malformed-input branches in `frame_side_data_rejects_malformed_video_hint_payload`. The deterministic `avutil_core_models` fixture now asserts native header and rectangle sizing, two-rectangle Changed payload preservation, zero-rectangle Constant payload acceptance, FFmpeg constant-name mapping, empty/short/trailing payload rejection, rectangle offset/size rejection, invalid hint-type rejection, count/length mismatch rejection, zero-width/height rejection, coordinate-overflow rejection, typed constructor rejection for invalid rectangles, and non-video-hint lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens video hint side-data fuzz coverage. `FrameVideoHint` already has unit coverage for native header and rectangle lengths, two-rectangle Changed hints, zero-rectangle Constant hints, malformed lengths, native offset/size validation, invalid hint types, count/length mismatches, zero-sized rectangles, coordinate overflows, typed-constructor rejection for invalid rectangles, and non-video-hint lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser and deferred side-data accessor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened video hint side-data fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the video hint malformed-input unit branches for native header/rectangle sizing, two-rectangle Changed payload preservation, zero-rectangle Constant payload acceptance, FFmpeg constant-name mapping, empty/short/trailing payload rejection, rectangle offset/size rejection, invalid hint-type rejection, count/length mismatch rejection, zero-width/height rejection, coordinate-overflow rejection, typed-constructor rejection for invalid rectangles, and non-video-hint lookup absence. Docs and the ledger now call out these malformed length/offset/size/type/count/rectangle/overflow invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_video_hint_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_video_hint_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
