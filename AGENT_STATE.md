...
Update for the current slice: `FrameExif::common_tags()` now also reports Temperature, Humidity, Pressure, WaterDepth, Acceleration, and CameraElevationAngle with strict SRATIONAL/RATIONAL type, count, and zero-denominator validation. `avutil_core_models` build-checks the new EXIF environment fixture alongside the camera/lens, gamma/composite, optics/subject, version/timing/comment, and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_environment_tags`
...
- No remaining failing commands in the latest EXIF environment side-data slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
