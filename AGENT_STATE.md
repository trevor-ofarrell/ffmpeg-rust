...
Update for the current slice: `FrameExif::common_tags()` now also reports CompressedBitsPerPixel, MaxApertureValue, SubjectDistance, SubjectArea, SubjectLocation, and ExposureIndex with strict RATIONAL and SHORT type/count validation, variable 2/3/4-value SubjectArea shape handling, and denominator validation. `avutil_core_models` build-checks the new EXIF optics/subject fixture alongside the capture-settings and rendering-scene fixtures; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF optics and subject metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports CompressedBitsPerPixel, MaxApertureValue, SubjectDistance, SubjectArea, SubjectLocation, and ExposureIndex while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
