...
`fftools-ffprobe-stream-r-frame-rate` is the current focus; the latest slice reports constrained `r_frame_rate` values alongside existing `avg_frame_rate` values in `ffprobe-rs -show_streams` output.
...
Latest slice: add constrained `r_frame_rate` reporting to `ffprobe-rs`. The stream report now renders `r_frame_rate` in default and JSON stream output for MOV/MP4 and AVI paths using already parsed timing metadata; fixtures prove raw MOV, H.264 MOV, and raw AVI output. This is not full upstream rate-guessing parity and still lacks pinned-oracle differential/FATE coverage.
