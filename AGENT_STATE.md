...
Update for the current slice: `FrameExif::common_tags()` sensitivity coverage now has build-checked fuzz invariants matching the existing unit test for malformed sensitivity tags. The deterministic `avutil_core_models` EXIF fixture now covers valid PhotographicSensitivity, SensitivityType, StandardOutputSensitivity, RecommendedExposureIndex, ISOSpeed, ISOSpeedLatitudeYYY, and ISOSpeedLatitudeZZZ parsing plus malformed PhotographicSensitivity count, SensitivityType enum value, and StandardOutputSensitivity type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- No remaining failing commands in the latest EXIF sensitivity fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF sensitivity common-tag fuzz coverage. `FrameExif::common_tags()` already exposed PhotographicSensitivity, SensitivityType, StandardOutputSensitivity, RecommendedExposureIndex, ISOSpeed, ISOSpeedLatitudeYYY, and ISOSpeedLatitudeZZZ with unit coverage for malformed count/type/enum-value shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: PhotographicSensitivity wrong count, SensitivityType undefined value, and StandardOutputSensitivity wrong type. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
