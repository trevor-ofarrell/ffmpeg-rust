...
`fate-runner` now has local smoke mappings for the current `fftools-option-parser`, `fftools-basic-io`, and `fftools-ffprobe-*` changed-path selections. `tests/fate/mappings.txt` maps the option parser to `cargo test -p fftools --lib option_parser`, basic I/O planning to `cargo test -p fftools --lib io_plan`, and each current ffprobe ledger component to the shared `cargo test -p fftools --lib ffprobe` path. A new `fate-runner` unit test loads the real ledger and default mapping file to prove those current fftools smoke selections all have runnable mappings. These are local smoke checks only; they do not count as upstream FFmpeg FATE media parity.
...
- `cargo run -p fate-runner -- run --changed --dry-run` previously selected changed `fftools-option-parser`, `fftools-basic-io`, and `fftools-ffprobe-*` ledger components for the loglevel slice, then failed with the expected no-runnable-FATE-mapping error because `tests/fate/mappings.txt` only contained local runner and MOV smoke mappings. This is now resolved for option-parser, basic-I/O, and current ffprobe selections by local smoke mappings; `fftools-ffmpeg-*` source selections still need equivalent local smoke mappings in a future slice.
...
`fate-runner` is the current focus for this slice. It now has local smoke mappings and a regression test for current `fftools-option-parser`, `fftools-basic-io`, and `fftools-ffprobe-*` changed-path selections. The component remains below `complete` because upstream FFmpeg FATE samples, media mappings, and broad component command mappings are still absent.

## Next 3 Concrete Actions

1. Add local `tests/fate/mappings.txt` smoke mappings for current `fftools-ffmpeg-*` changed-path selections so `crates/fftools/src/ffmpeg.rs` changes can run through `cargo run -p fate-runner -- run --changed` without reporting unmapped local components.
2. Add pinned-oracle differential coverage for constrained `ffmpeg-rs -f hash [-hash <algorithm>] -`, `-f md5 -`, `-f framehash [-hash <algorithm>] -`, `-f framemd5 -`, and `-f streamhash [-hash <algorithm>] -` once the FFmpeg 8.1.1 oracle binary is available.
...
- Upstream FATE samples and media target mappings are not configured. `tests/fate/mappings.txt` currently contains local `fate-runner`, MOV demuxer, fftools option-parser, fftools I/O-plan, and shared ffprobe unit-test smoke mappings only; current `fftools-ffmpeg-*` changed-path selections still need local smoke rows. The runner has `mappings`, `--check-prereqs`, `--samples`, `--oracle-ffmpeg`, and `--dry-run` support for future mappings.
...
Latest slice: added local FATE-runner smoke mappings for `fftools-option-parser`, `fftools-basic-io`, and every current `fftools-ffprobe-*` ledger component, plus a runner regression test that loads the real ledger and default mapping file to prove those changed-path selections have mappings. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
