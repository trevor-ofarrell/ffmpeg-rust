...
Update for the current slice: `FrameExif::common_tags()` now validates ImageWidth, ImageLength, PixelXDimension, and PixelYDimension as positive nonzero dimensions after the existing SHORT/LONG type and count checks. Unit tests and the `avutil_core_models` build-checked fuzz fixture cover zero root and linked EXIF dimension values, and the fuzz harness asserts accepted generated common-tag dimensions are nonzero. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice tightens `AV_FRAME_DATA_EXIF` dimension validation in the selected common-tag view. `FrameExif::common_tags()` now keeps the existing fixed-count, ASCII-shape, coordinate, timestamp, date/time range, direction, and SubSecTime enforcement and additionally rejects zero ImageWidth, ImageLength, PixelXDimension, and PixelYDimension values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
