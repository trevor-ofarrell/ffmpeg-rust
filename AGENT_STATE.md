...
Latest validation commands: `cargo fmt --all -- --check`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo test -p avutil packet`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo clippy -p fate-runner --all-targets --all-features -- -D warnings`, and `git diff --check` pass for the latest packet side-data API oracle slice. `git diff --check` reports CRLF conversion warnings only.
