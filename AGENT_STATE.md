...
Latest slice: added storage identity and visible/padded data pointer probes for `BufferRef` and `BufferSlice`. Unit tests now distinguish shared refs/slices from equal independent byte buffers, verify visible and padded pointer helpers, and prove copy-on-write detach changes storage identity while original slices remain tied to the old storage; `avutil_core_models` now build-checks storage identity, slice pointer offsets, and detach identity invariants. Validation passed with formatting, focused buffer tests, fuzz-target check/clippy, the local `avutil-buffer` mapping, changed-component dry-run and execution, workspace clippy, workspace tests, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
