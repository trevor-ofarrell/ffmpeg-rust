...
`fftools-ffprobe-mov-extradata-size` is the current focus; the latest slice reports non-empty MOV sample-entry extra-data lengths as `extradata_size` in constrained `ffprobe-rs -show_streams` output.
...
Latest slice: add MOV `extradata_size` reporting to `ffprobe-rs`. The stream report now surfaces non-empty `stsd` sample-entry extra-data length from `MovDemuxer`; raw and avc1 VisualSampleEntry fixtures prove the field in default and JSON output.
