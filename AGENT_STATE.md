...
Update for the current slice: `FrameExif::common_tags()` rendering/scene coverage now has build-checked fuzz invariants matching the existing unit test for malformed rendering and scene tags. The deterministic `avutil_core_models` EXIF fixture now covers valid ColorSpace, SensingMethod, FileSource, SceneType, CustomRendered, ExposureMode, SceneCaptureType, GainControl, Contrast, Saturation, Sharpness, and SubjectDistanceRange parsing plus malformed ColorSpace enum value, FileSource TIFF type, SceneCaptureType count, and SubjectDistanceRange enum value cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF rendering/scene common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ColorSpace, SensingMethod, FileSource, SceneType, CustomRendered, ExposureMode, SceneCaptureType, GainControl, Contrast, Saturation, Sharpness, and SubjectDistanceRange with unit coverage for malformed enum, type, and count shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: ColorSpace undefined enum value, FileSource wrong TIFF type, SceneCaptureType wrong count, and SubjectDistanceRange undefined enum value. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
