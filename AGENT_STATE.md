...
Update for the current slice: `FrameExif::common_tags()` camera-characterization coverage now has build-checked fuzz invariants matching the existing unit test for malformed camera-characterization tags. The deterministic `avutil_core_models` EXIF fixture now covers valid SpectralSensitivity, OECF, FlashEnergy, SpatialFrequencyResponse, FocalPlaneXResolution, FocalPlaneYResolution, FocalPlaneResolutionUnit, and CFAPattern parsing plus malformed SpectralSensitivity type, OECF type, FlashEnergy count, SpatialFrequencyResponse type, FocalPlaneResolutionUnit enum value, and CFAPattern type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- No remaining failing commands in the latest EXIF camera-characterization fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF camera-characterization common-tag fuzz coverage. `FrameExif::common_tags()` already exposed SpectralSensitivity, OECF, FlashEnergy, SpatialFrequencyResponse, FocalPlaneXResolution, FocalPlaneYResolution, FocalPlaneResolutionUnit, and CFAPattern with unit coverage for malformed type/count/enum-value shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: SpectralSensitivity wrong type, OECF wrong type, FlashEnergy wrong count, SpatialFrequencyResponse wrong type, FocalPlaneResolutionUnit undefined value, and CFAPattern wrong type. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF camera-characterization common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_camera_characterization_tags`: malformed SpectralSensitivity type, malformed OECF type, malformed FlashEnergy count, malformed SpatialFrequencyResponse type, malformed FocalPlaneResolutionUnit enum value, and malformed CFAPattern type. Docs and the ledger now call out the camera-characterization ASCII/UNDEFINED/RATIONAL/SHORT validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_camera_characterization_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
