...
Latest `avutil-pixel-format` / rawvideo update: the shared pixel format model now includes FFmpeg's 10-bit and 12-bit planar YUV 4:2:0/4:2:2/4:4:4 formats: `yuv420p10le`, `yuv420p10be`, `yuv422p10le`, `yuv422p10be`, `yuv444p10le`, `yuv444p10be`, `yuv420p12le`, `yuv420p12be`, `yuv422p12le`, `yuv422p12be`, `yuv444p12le`, and `yuv444p12be`. The slice was checked against pinned FFmpeg 8.1.1 `libavutil/pixfmt.h` and `libavutil/pixdesc.c`: upstream uses the same 4:2:0/4:2:2/4:4:4 chroma geometry as the 8-bit planar YUV families, 10- or 12-bit component depth, 15/20/30 or 18/24/36 average descriptor bpp, and two stored bytes per sample. Rust now exposes the new `PixelFormat` variants and names, descriptor metadata, two-byte-per-sample frame-size and plane-splitting math, `VideoFrame` line sizing, rawvideo decode/demux/mux packet sizing, constrained `ffmpeg-rs -f rawvideo ... -pix_fmt yuv420p10le -f null -` execution, and affected fuzz-harness invariants. This remains below `complete` because 9-bit 4:2:0 is deferred until the descriptor can represent FFmpeg's fractional 13.5 bpp exactly, and full `AVPixFmtDescriptor` parity, pinned oracle differential vectors, upstream FATE parity, pixel conversion, hardware formats, and actual fuzz execution are still absent.
...
- Current 10-bit/12-bit planar YUV `avutil-pixel-format` / rawvideo slice:
  - `cargo fmt --all`
  - `cargo fmt --all -- --check`
  ...
  - `$env:CARGO_TARGET_DIR='target-fftools-gray32-test'; cargo test -p fftools runs_rawvideo_yuv420p10le_to_null_stdout`
...
- Current 10-bit/12-bit planar YUV `avutil-pixel-format` / rawvideo slice: no remaining code/test assertion failures and no fresh Windows Application Control blocks during final validation. Focused avutil/avcodec/avformat/fftools tests, fuzz package check/clippy, workspace clippy, local FATE-runner `run --changed`, workspace library tests, formatting, and `git diff --check` passed. `git diff --check` reported CRLF warnings only; Cargo commands continued to report the usual `could not canonicalize path C:\Users\trevo` warning. The 9-bit 4:2:0 family remains blocked by the current integer average-bpp descriptor model, and oracle differentials, upstream FATE media coverage, and actual fuzz execution remain incomplete.
...
`avutil-pixel-format` remains the current focus, with linked rawvideo decoder, demuxer, muxer, constrained `ffmpeg-rs` input parsing, and fuzz-harness invariant coverage. The latest concrete change adds FFmpeg's 10-bit and 12-bit planar YUV families for 4:2:0, 4:2:2, and 4:4:4 as three two-byte-per-sample planes with pinned descriptor bpp and chroma geometry. It does not claim 9-bit 4:2:0 support, conversion support, full `AVPixFmtDescriptor` parity, full `ffmpeg -pix_fmts` inventory, pinned oracle parity, upstream FATE parity, or actual fuzz execution.
...
1. Continue priority-1 format-model work by either extending `PixelFormatDescriptor` to represent fractional average bpp before adding `yuv420p9*`, or choose the next rawvideo-facing FFmpeg 8.1.1 pixel-format family whose descriptor fits the current model without inventing parity.
...
3. Keep local FATE-runner changed-path coverage aligned with ledger selections, and document/standardize the accepted target-directory workaround for Windows Application Control blocks without treating policy-blocked fresh executable launches as test failures.

## Known Blockers

- Current 10-bit/12-bit planar YUV validation has no remaining code/test assertion failures and did not hit fresh Windows Application Control blocks. `git diff --check` reported CRLF warnings only; Cargo commands reported only the usual `could not canonicalize path C:\Users\trevo` warning. The 9-bit 4:2:0 family is intentionally deferred because FFmpeg's descriptor reports 13.5 average bits per pixel and the current Rust descriptor stores that field as an integer.
...
Latest slice: added FFmpeg 8.1.1 10-bit and 12-bit planar YUV support to the current shared pixel/rawvideo model. `PixelFormat` now reports `yuv420p10le`/`yuv420p10be`, `yuv422p10le`/`yuv422p10be`, `yuv444p10le`/`yuv444p10be`, `yuv420p12le`/`yuv420p12be`, `yuv422p12le`/`yuv422p12be`, and `yuv444p12le`/`yuv444p12be` in the inventory with YUV descriptor metadata, 10- or 12-bit component depth, 15/20/30 or 18/24/36 average descriptor bpp, three planar payload planes, no alpha or float flag, and the same 4:2:0/4:2:2/4:4:4 chroma validation as the 8-bit families. `VideoFrame`, `RawVideoDecoder`, `RawVideoDemuxer`, `RawVideoMuxer`, constrained `ffmpeg-rs` rawvideo-to-null execution, and the affected fuzz harnesses now exercise the family, including decoder plane/line-size coverage, demux/mux frame-size coverage for all 12 names, `yuv420p10le` CLI coverage, and fuzz build invariants. Validation passed with focused avutil/avcodec/avformat/fftools tests, fuzz package check/clippy, workspace format check, workspace clippy, local FATE-runner changed mappings, workspace library tests, and `git diff --check` with CRLF warnings only. The affected components remain `implemented`, not `complete`, because 9-bit 4:2:0 descriptor support, oracle differentials, upstream FATE media coverage, full pixel inventory coverage, full `AVPixFmtDescriptor` parity, and actual fuzz execution are still absent.
