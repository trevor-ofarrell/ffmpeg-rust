...
Current avutil-frame coverage also now includes deterministic build-checked `FrameIccProfile` one-record tag-table, malformed declared-size, truncated tag-table, and invalid metadata-name coverage, matching the focused ICC profile unit coverage for bounded tag tables and `name` metadata validation.

Update for the current slice: ICC profile coverage now has build-checked fuzz invariants matching the tag-table and malformed-input branches in `frame_side_data_parses_icc_profile_payload` and `frame_side_data_rejects_malformed_icc_profile_payload`. The deterministic `avutil_core_models` fixture now asserts a valid one-record ICC tag table, declared-size preservation, raw profile preservation, missing-name preservation, declared-size mismatch rejection, truncated tag-table rejection, and NUL-containing `name` metadata rejection. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens ICC profile fuzz coverage. `FrameIccProfile` already has unit coverage for the minimal profile header, valid one-record tag tables, malformed declared sizes, missing `acsp` signatures, truncated tag tables, invalid `name` metadata, and non-ICC lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors the unmirrored tag-table and malformed-input branches, including one-record tag-table preservation, declared-size mismatch rejection, truncated tag-table rejection, and NUL-containing metadata-name rejection. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened ICC profile tag-table fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the ICC unit-test branches for one-record tag-table preservation, declared-size preservation, raw profile preservation, missing-name preservation, declared-size mismatch rejection, truncated tag-table rejection, and NUL-containing metadata-name rejection. Docs and the ledger now call out these ICC tag-table and metadata-name invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_icc_profile_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_icc_profile_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
