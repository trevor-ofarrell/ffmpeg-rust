...
1. Continue `avutil-pixel-format` inventory from pinned FFmpeg 8.1.1 after the packed AYUV64 family.
2. Pick the next small rawvideo-facing pixel-format family with source-checked descriptors and add the same local unit, CLI, rawvideo, and fuzz-build invariant coverage.
3. Add pinned-oracle differential coverage for constrained pixel-format inventory and rawvideo packet sizing once the FFmpeg 8.1.1 oracle binary is available.
