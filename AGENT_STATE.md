...
Update for the current slice: `avutil_core_models` also build-checks EXIF common-tag interpretation for selected root, linked EXIF, linked GPS, and linked interoperability metadata plus semantic rejection for malformed known-tag shapes; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with selected common EXIF/GPS/interoperability tag interpretation. `FrameExif::common_tags()` now reports root Make/Model/ImageWidth/ImageLength/Orientation/XResolution/YResolution/ResolutionUnit, linked EXIF ExifVersion/DateTimeOriginal, linked GPS GPSVersionID/GPSLatitudeRef/GPSLatitude/GPSLongitudeRef/GPSLongitude, and linked interoperability index values, while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: added `FrameExifCommonTags`, selected EXIF/GPS/interoperability semantic enums, `FrameExifEntry::undefined_values()`, and `FrameExif::common_tags()` for a strict common camera-metadata subset. Unit tests and `avutil_core_models` fuzz invariants cover Make/Model, dimensions, orientation, resolution, ExifVersion, DateTimeOriginal, GPS version/coordinate refs/coordinates, interoperability index, and semantic invalid-input rejection. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame_side_data_interprets_common_exif_tags`, `cargo test -p avutil frame_side_data_decodes_exif_entry_values`, `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
