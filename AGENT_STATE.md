...
Latest `avutil-hash` parity slice: added Rust-native RIPEMD-128, RIPEMD-160, RIPEMD-256, and RIPEMD-320 helpers, wired them through hash/framehash/streamhash-capable muxer dispatch and constrained `ffmpeg-rs -hash ripe-md-160` parsing, extended the ignored pinned-libavutil `av_hash_*` differential harness, and build-checked both avutil and avformat hash fuzz models. The differential row still passes locally through the pinned WSL libavutil oracle. `avutil-hash` remains `differential_pass`, not `complete`, because full Rust-side libavutil generic hash API parity remains pending.

Latest validation commands: `cargo test -p avutil hash --quiet`, `cargo test -p avformat hash --quiet`, `cargo test -p fftools hash --quiet`, `cargo test -p avutil --test hash_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-hash --target oracle-libavutil-hash --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-hash`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, `cargo clippy -p avutil -p avformat -p fftools --all-targets --all-features -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check -- -D warnings`, `cargo fmt --all -- --check`, and `git diff --check` all pass for the current RIPEMD slice. `git diff --check` reports CRLF conversion warnings only.
...
1. Add Rust-side generic hash API semantics for `avutil-hash` (`av_hash_names`/allocation/name/size/reset/update/final bin/hex/b64 equivalents), then rerun the pinned `av_hash_*` oracle.
...
- `avutil-hash` now passes the pinned libavutil `av_hash_*` oracle for the modeled algorithms including Murmur3, RIPEMD128/160/256/320, SHA512/224, and SHA512/256, but remains incomplete because full Rust-side generic hash API parity is still pending.
