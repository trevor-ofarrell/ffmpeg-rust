...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CHAN_STEREO_LEFT|AV_CHAN_STEREO_RIGHT|AV_CHAN_BINAURAL_LEFT|AV_CHAN_BINAURAL_RIGHT|AV_CH_STEREO_LEFT|AV_CH_STEREO_RIGHT|AV_CH_BINAURAL_LEFT|AV_CH_BINAURAL_RIGHT|AV_CH_LAYOUT_BINAURAL|AV_CH_LAYOUT_STEREO_DOWNMIX|AV_CHANNEL_LAYOUT_BINAURAL|AV_CHANNEL_LAYOUT_STEREO_DOWNMIX|AV_CH_LAYOUT_22POINT2|AV_CHANNEL_LAYOUT_22POINT2|AV_CHAN_TOP_CENTER|AV_CHAN_BOTTOM_FRONT_CENTER' -Context 3,4` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern 'BINAURAL|STEREO_LEFT|STEREO_RIGHT|BINAURAL_LEFT|BINAURAL_RIGHT|"downmix"|"stereo downmix"|"binaural"|"22\.2"|"DL"|"DR"|"BL"|"BR"' -Context 3,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `DL`, `DR`, `BIL`, `BIR`, `binaural`, and `downmix`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, `avformat` audio regression, format checks, and `git diff --check` passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, `22.2` layout coverage after `downmix`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
1. Inspect the next pinned FFmpeg `channel_layout.c` map entry after `downmix`, starting with `22.2` and its bottom/top-center/surround channel families.
...
- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFC`, `TFR`, `TBL`, `TBC`, `TBR`, `DL`, `DR`, `WL`, `WR`, `LFE2`, `TSL`, `TSR`, `BIL`, and `BIR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, `7.1.4`, `7.2.3`, `9.1.4`, `9.1.6`, `hexadecagonal`, `binaural`, and `downmix` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no `22.2` coverage, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no upstream FATE parity, and no actual cargo-fuzz execution.
...
Latest slice: added source-checked `DL`, `DR`, `BIL`, `BIR`, `binaural`, and `downmix` native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CHAN_STEREO_LEFT`, `AV_CHAN_STEREO_RIGHT`, `AV_CHAN_BINAURAL_LEFT`, `AV_CHAN_BINAURAL_RIGHT`, their native mask macros, `AV_CH_LAYOUT_BINAURAL`, `AV_CH_LAYOUT_STEREO_DOWNMIX`, `AV_CHANNEL_LAYOUT_BINAURAL`, `AV_CHANNEL_LAYOUT_STEREO_DOWNMIX`, the public short names `DL`/`DR`/`BIL`/`BIR`, and the `channel_layout_map` names `binaural` and `downmix`. The Rust model now exposes `binaural` and `downmix` as distinct native two-channel layouts while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover channel names/masks, layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local FATE mapping, changed-path local mappings, `avformat` audio tests, format checks, and `git diff --check` passed. The component remains `implemented`, not `complete`, because full oracle inventory parity, `22.2` layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, upstream FATE, and actual fuzz execution remain absent.
