...
Update for the current slice: `FrameExif::common_tags()` version/timing/comment coverage now has build-checked fuzz invariants matching the remaining invalid-input paths from the existing unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid ComponentsConfiguration, MakerNote, UserComment, SubSecTime, SubSecTimeOriginal, SubSecTimeDigitized, FlashpixVersion, RelatedSoundFile, and PixelXDimension parsing plus malformed ComponentsConfiguration count/value, MakerNote type, SubSecTime terminator/digit, SubSecTimeOriginal digit, SubSecTimeDigitized digit, FlashpixVersion count/digit, and RelatedSoundFile count cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_version_timing_comment_tags`
...
- No remaining failing commands in the latest EXIF version/timing/comment fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF version/timing/comment common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ComponentsConfiguration, MakerNote, UserComment, SubSecTime, SubSecTimeOriginal, SubSecTimeDigitized, FlashpixVersion, RelatedSoundFile, and PixelXDimension with unit coverage for malformed component counts/values, MakerNote type, fractional-second string shapes, FlashpixVersion shapes, and RelatedSoundFile counts. The `avutil_core_models` build-checked fuzz harness now asserts the missing deterministic paths too: ComponentsConfiguration wrong count, MakerNote wrong TIFF type, SubSecTime terminator rejection, RelatedSoundFile wrong count, and valid PixelXDimension preservation alongside the existing value and digit checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Extend `avutil-frame` through the next selected EXIF common-tag fuzz gap, likely camera-lens or gamma-composite invalid-input coverage.
...
Latest slice: strengthened EXIF version/timing/comment common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching the remaining invalid-input cases in `frame_side_data_interprets_exif_version_timing_comment_tags`: malformed ComponentsConfiguration count, malformed MakerNote TIFF type, malformed SubSecTime terminator, malformed RelatedSoundFile count, and valid PixelXDimension preservation, alongside the existing component-value, fractional-second digit, and FlashpixVersion checks. Docs and the ledger now call out the version/timing/comment validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_version_timing_comment_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
