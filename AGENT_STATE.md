...
`fftools-ffprobe-stream-codec-fields` is the current focus; the latest slice adds initial codec identity and average-frame-rate fields to constrained MOV/AVI stream reports.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as MOV pixel aspect ratio and color side information.
...
Latest slice: add initial stream codec identity reporting to `ffprobe-rs` for constrained MOV/MP4 and AVI paths. Stream reports now carry `codec_name`, `codec_type`, `codec_tag_string`, numeric `codec_tag`, and `avg_frame_rate` when derivable from Rust demuxer metadata, with focused tests covering default, MOV JSON, and AVI JSON render paths.
