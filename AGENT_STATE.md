...
Latest `avutil-hash` parity slice: added Rust-native MurmurHash3 x64 128-bit with FFmpeg's default seed, wired it through hash/framehash/streamhash-capable muxer dispatch and constrained `ffmpeg-rs -hash murmur3` parsing, extended the ignored pinned-libavutil `av_hash_*` differential harness, and build-checked both avutil and avformat hash fuzz models. The differential row still passes locally through the pinned WSL libavutil oracle. `avutil-hash` remains `differential_pass`, not `complete`, because full libavutil generic hash API parity plus RIPEMD128/160/256/320 remain pending.

Latest validation commands: `cargo test -p avutil hash --quiet`, `cargo test -p avformat hash --quiet`, `cargo test -p fftools hash --quiet`, `cargo test -p avutil --test hash_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-hash --target oracle-libavutil-hash --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-hash`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, `cargo clippy -p avutil -p avformat -p fftools --all-targets --all-features -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check -- -D warnings`, `cargo fmt --all -- --check`, and `git diff --check` all pass. `git diff --check` reports CRLF conversion warnings only.
...
Latest documentation update: the repository-level `README.md` now reports strict parity completion as about 3% (`3/96`), with 4 additional rows currently carrying non-complete validation status, and still reports practical engineering progress at about 2% of a complete FFmpeg 8.1.1 default-native rewrite.
...
`avutil-hash` is the current non-parser strict-parity candidate. The latest slice added FFmpeg-exposed SHA512/224 and SHA512/256 coverage, but the row remains below `complete` until the remaining libavutil hash algorithms and generic API semantics are covered.
...
1. Expand `avutil-hash` with libavutil's remaining RIPEMD128/160/256/320 algorithms plus generic hash API semantics, then rerun the pinned `av_hash_*` oracle.
...
- `avutil-hash` now passes the pinned libavutil `av_hash_*` oracle for the modeled algorithms including Murmur3, SHA512/224, and SHA512/256, but remains incomplete because RIPEMD128/160/256/320 and full generic hash API parity are still pending.
...
- `fftools-version` and `avutil-channel-layout` have executed pinned-oracle differential rows and remain marked `differential_pass`, not `complete`; `avutil-error` has advanced to `complete`.
