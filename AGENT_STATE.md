...
Latest `avutil-packet` differential slice: added Rust-native `PacketSideDataList` for standalone `AVPacketSideData` array semantics, plus `SideData::data_mut()` for safe payload initialization. Unit tests now cover `av_packet_side_data_new`/`av_packet_side_data_add`-style first matching type replacement, first-match lookup, `av_packet_side_data_remove`-style last matching entry removal via swap-with-last, miss preservation, and clear/free reset. `crates/avutil/tests/packet_oracle.rs` now validates standalone `av_packet_side_data_new`, `av_packet_side_data_add`, `av_packet_side_data_get`, `av_packet_side_data_remove`, and `av_packet_side_data_free` rows against pinned libavcodec/libavutil. The same oracle continues to validate dictionary pack/unpack, fixed/comment-defined/raw side-data payload layouts, `AV_PKT_DATA_NB`, every public `AV_PKT_DATA_*` numeric enum value, all 41 public `av_packet_side_data_name()` display strings, core lifecycle/property behavior, payload API behavior, and packet-owned side-data API behavior: `av_packet_alloc` defaults, `av_packet_rescale_ts`, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `av_packet_from_data`, `av_grow_packet`, `av_shrink_packet`, `av_packet_make_refcounted`, `av_packet_make_writable`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data`, missing side-data lookup size reset, `opaque` address copying, payload preservation, flags, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
...
Latest validation commands: `cargo fmt --all`, `cargo fmt --all -- --check`, `cargo test -p avutil packet_side_data_list`, `cargo test -p avutil packet`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, `cargo clippy --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check -- -D warnings`, WSL `cargo fuzz run avutil_core_models -- -runs=1`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, and `git diff --check` pass for the latest standalone packet side-data array oracle slice. `git diff --check` reports CRLF conversion warnings only.
...
1. Continue the post-10% loop by selecting the highest-priority incomplete component, likely `avutil-bitwriter`, `avutil-options`, `avutil-logging`, `fftools-version`, or `avutil-channel-layout`.
2. Add or strengthen pinned oracle/FATE/fuzz evidence for that selected component before changing status.
