...
Latest slice: added build-checked cargo-fuzz target `avcodec_basic_decoders` plus seed corpora, updated the rawvideo and pcm_s16le decoder ledger fuzz target fields, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
