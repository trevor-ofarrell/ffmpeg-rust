...
Latest current-loop `avutil-frame` update: HDR Vivid side-data fuzz coverage now mirrors the focused unit contracts for `AV_FRAME_DATA_DYNAMIC_HDR_VIVID`. The deterministic `avutil_core_models` fixture preserves native payload size, system start code, window count, maximum RGB fields, two tone-mapping parameter records, base parameters, two three-spline records, color-saturation gains, and non-HDR lookup absence, and rejects malformed length, system start code, window count, tone-mapping flag/count, tone-mapping base-enable flag, three-spline flag/count/mode, and color-saturation flag/count through raw parser, deferred side-data accessor, and constructor paths. This remains build-checked fuzz coverage only; pinned-oracle differential tests, upstream FATE media parity, and actual local fuzz execution are still blocked.
...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDolbyVisionRpuBuffer` non-empty and empty raw payload preservation plus `FrameDolbyVisionMetadata` malformed length/offset/size/count/level/color coverage, and `FrameDynamicHdrVivid` nested maximum-RGB/tone-mapping/three-spline/color-saturation preservation plus malformed length/system/window/tone-mapping/three-spline/color-saturation coverage, matching the focused side-data unit coverage for raw, deferred side-data, and constructor rejection paths.

Update for the current slice: HDR Vivid coverage now has build-checked fuzz invariants matching the malformed-input branches in `frame_side_data_rejects_malformed_dynamic_hdr_vivid_payload`. The deterministic `avutil_core_models` fixture now asserts native payload sizing, selected maximum-RGB/tone-mapping/base-parameter/three-spline/color-saturation field preservation, empty/short/long payload rejection, invalid system start code rejection, invalid window count rejection, invalid tone-mapping flag/count rejection, invalid tone-mapping base-enable rejection, invalid three-spline flag/count/mode rejection, invalid color-saturation flag/count rejection, and non-HDR lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
- `cargo test -p avutil frame_side_data_parses_dynamic_hdr_vivid_payload`
- `cargo test -p avutil frame_side_data_rejects_malformed_dynamic_hdr_vivid_payload`
...
- No remaining failing commands in the latest HDR Vivid fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
`avutil-frame` is the current focus for this slice. It is implemented with unit tests, PTS storage, typed audio/video payload shells, tightly packed, explicit custom, and checked aligned line-size metadata, descriptor-style sample-format metadata, optional channel-layout metadata for audio, BufferRef-backed frame plane and side-data ownership, the pinned FFmpeg 8.1.1 `AV_FRAME_DATA_*` side-data inventory, descriptor/property metadata, remove-by-property helpers, remove-by-name/alias and take-side-data helpers, explicit set-style side-data replacement/coalescing with external payload release timing coverage, typed `AV_FRAME_DATA_PANSCAN`, `AV_FRAME_DATA_A53_CC`, `AV_FRAME_DATA_STEREO3D`, `AV_FRAME_DATA_DISPLAYMATRIX`, `AV_FRAME_DATA_MATRIXENCODING`, `AV_FRAME_DATA_DOWNMIX_INFO`, `AV_FRAME_DATA_REPLAYGAIN`, `AV_FRAME_DATA_AFD`, `AV_FRAME_DATA_MOTION_VECTORS`, `AV_FRAME_DATA_SKIP_SAMPLES`, `AV_FRAME_DATA_AUDIO_SERVICE_TYPE`, `AV_FRAME_DATA_MASTERING_DISPLAY_METADATA`, `AV_FRAME_DATA_GOP_TIMECODE`, `AV_FRAME_DATA_SPHERICAL`, `AV_FRAME_DATA_CONTENT_LIGHT_LEVEL`, `AV_FRAME_DATA_ICC_PROFILE`, `AV_FRAME_DATA_S12M_TIMECODE`, `AV_FRAME_DATA_DYNAMIC_HDR_PLUS`, `AV_FRAME_DATA_REGIONS_OF_INTEREST`, `AV_FRAME_DATA_VIDEO_ENC_PARAMS`, `AV_FRAME_DATA_FILM_GRAIN_PARAMS`, `AV_FRAME_DATA_DETECTION_BBOXES`, `AV_FRAME_DATA_DOVI_RPU_BUFFER`, `AV_FRAME_DATA_DOVI_METADATA`, `AV_FRAME_DATA_DYNAMIC_HDR_VIVID`, `AV_FRAME_DATA_AMBIENT_VIEWING_ENVIRONMENT`, `AV_FRAME_DATA_VIDEO_HINT`, `AV_FRAME_DATA_LCEVC`, `AV_FRAME_DATA_VIEW_ID`, `AV_FRAME_DATA_3D_REFERENCE_DISPLAYS`, `AV_FRAME_DATA_EXIF`, and `AV_FRAME_DATA_SEI_UNREGISTERED` payload parsers or preserved raw payload views, per-record metadata, side-data/hw-context writable checks and copy-on-write detachment helpers, optional `BufferRef`-backed `hw_frames_context` ownership including replacement release ordering, build-checked fuzz invariants, and local FATE-runner smoke mapping coverage. It remains below `complete` because full EXIF/GPS/interoperability tag coverage beyond the selected common-tag view, deeper HDR10+/Dolby Vision/HDR Vivid color transform semantics, ROI encoder constraint handling, codec-specific video encoding parameter quantizer semantics, film grain synthesis/selection behavior, detection-model semantics beyond the native envelope, complete FFmpeg alignment policy, sample-data conversion routines, deeper AVFrame per-buffer permission flags beyond the current BufferRef-backed writable/readonly model, hardware device/frame context internals, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

This slice strengthens HDR Vivid side-data fuzz coverage. `FrameDynamicHdrVivid` already has unit coverage for native length, `system_start_code`, window count, selected maximum-RGB/tone-mapping/base-parameter/three-spline/color-saturation fields, malformed length, malformed system/window values, malformed nested tone-mapping and three-spline values, malformed color-saturation values, and non-HDR lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser, deferred side-data accessor, and constructor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.

## Next 3 Concrete Actions

1. Continue auditing remaining `avutil-frame` unit tests against `avutil_core_models` and extend the next unmirrored invalid-input or raw-value preservation fuzz gap.
...
Latest slice: strengthened HDR Vivid side-data fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the HDR Vivid malformed-input unit branches for native payload sizing, selected maximum-RGB/tone-mapping/base-parameter/three-spline/color-saturation field preservation, empty/short/long payload rejection, invalid system start code rejection, invalid window count rejection, invalid tone-mapping flag/count rejection, invalid tone-mapping base-enable rejection, invalid three-spline flag/count/mode rejection, invalid color-saturation flag/count rejection, and non-HDR lookup absence. Docs and the ledger now call out these HDR Vivid malformed length/system/window/tone-mapping/three-spline/color-saturation invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_dynamic_hdr_vivid_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_dynamic_hdr_vivid_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
