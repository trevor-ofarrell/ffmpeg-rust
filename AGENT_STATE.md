...
1. Continue priority-1 format-model work by adding the next narrow `PixelFormat` inventory slice needed by rawvideo parity, likely high-bit-depth grayscale formats such as `gray9le`/`gray9be`, `gray10le`/`gray10be`, `gray12le`/`gray12be`, and `gray14le`/`gray14be` after re-checking pinned FFmpeg 8.1.1 headers/descriptors.
