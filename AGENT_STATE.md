...
Update for the current slice: `FrameExif::common_tags()` root Orientation/ResolutionUnit and GPSDifferential EXIF coverage now has build-checked fuzz invariants matching exact raw values already asserted by unit tests. The deterministic `avutil_core_models` common EXIF fixtures now preserve Orientation raw value `6`, ResolutionUnit raw value `2`, and GPSDifferential raw value `1` in addition to the existing malformed count/value/type coverage. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root and GPS raw-value preservation fuzz coverage. `FrameExif::common_tags()` already exposed typed root Orientation, root ResolutionUnit, and GPSDifferential values, and the unit tests already asserted their exact raw values. The `avutil_core_models` build-checked fuzz harness now mirrors Orientation raw value `6`, ResolutionUnit raw value `2`, and GPSDifferential raw value `1` while preserving the existing malformed count/value/type checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
