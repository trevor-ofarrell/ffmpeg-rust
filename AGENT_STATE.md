...
Update for the current slice: `FrameExif::common_tags()` exposure and APEX exposure coverage now has build-checked fuzz invariants matching the existing unit tests for malformed exposure tags. The deterministic `avutil_core_models` EXIF fixtures now cover valid ExposureTime, FNumber, ExposureBiasValue, FocalLength, PixelXDimension, PixelYDimension, DateTimeDigitized, ShutterSpeedValue, ApertureValue, and BrightnessValue parsing plus malformed ExposureTime type, PixelXDimension/PixelYDimension value, PixelYDimension count, DateTimeDigitized count/layout/minute/calendar shapes, and APEX shutter/aperture/brightness count/type/denominator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF exposure and APEX exposure common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ExposureTime, FNumber, ExposureBiasValue, FocalLength, PixelXDimension, PixelYDimension, DateTimeDigitized, ShutterSpeedValue, ApertureValue, and BrightnessValue with unit coverage for malformed type/count/value/denominator/date shapes. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too: ExposureTime wrong type, PixelXDimension zero, PixelYDimension zero, PixelYDimension wrong count, DateTimeDigitized wrong count/layout/minute/calendar values with leap-day acceptance, ShutterSpeedValue wrong count, ApertureValue wrong type, and BrightnessValue zero denominator. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF exposure and APEX exposure common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic invalid paths matching `frame_side_data_interprets_exif_exposure_tags` and `frame_side_data_interprets_exif_apex_exposure_tags`: malformed ExposureTime type, PixelXDimension/PixelYDimension zero, PixelYDimension count, DateTimeDigitized count/layout/minute/calendar values with leap-day acceptance, ShutterSpeedValue count, ApertureValue type, and BrightnessValue denominator. Docs and the ledger now call out the exposure/APEX rational, dimension, and DateTimeDigitized validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_exposure_tags`, `cargo test -p avutil frame_side_data_interprets_exif_apex_exposure_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
