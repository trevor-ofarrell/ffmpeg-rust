...
This slice extends EXIF root colorimetry common-tag coverage. `FrameExif::common_tags()` now exposes WhitePoint, PrimaryChromaticities, YCbCrCoefficients, and ReferenceBlackWhite as fixed-count unsigned RATIONAL arrays. Unit tests cover valid parsing plus malformed count, type, and zero-denominator rejection, and the `avutil_core_models` build-checked fuzz harness asserts denominator invariants for the new root colorimetry arrays. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
