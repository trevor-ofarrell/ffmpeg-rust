...
Update for the current slice: `FrameExif::common_tags()` environment coverage now has build-checked fuzz invariants matching the invalid-input paths from the existing unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid Temperature, Humidity, Pressure, WaterDepth, Acceleration, and CameraElevationAngle parsing plus malformed Temperature count, Humidity TIFF type, and WaterDepth denominator cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_interprets_exif_environment_tags`
...
- No remaining failing commands in the latest EXIF environment fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF environment common-tag fuzz coverage. `FrameExif::common_tags()` already exposed Temperature, Humidity, Pressure, WaterDepth, Acceleration, and CameraElevationAngle with unit coverage for malformed Temperature counts, Humidity type shapes, and WaterDepth denominator validation. The `avutil_core_models` build-checked fuzz harness now asserts those deterministic invalid paths too while preserving the valid signed and unsigned rational environment fixture values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
