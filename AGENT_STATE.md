...
- `cargo test -p avutil frame_data_make_writable_detaches_audio_payload_refs`
...
- No remaining failing commands in the latest frame data audio `make_writable` fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens frame data audio `make_writable` fuzz coverage. `Frame` already has a unit test proving `FrameData::make_writable` detaches cloned readonly packed s16 audio plane storage, isolates subsequent visible-plane mutation from the clone and source buffer, and preserves the clone's bytes. The `avutil_core_models` build-checked fuzz harness now mirrors those assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened frame data audio `make_writable` fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_data_make_writable_detaches_audio_payload_refs`: `FrameData::make_writable` detaches cloned readonly packed s16 audio plane storage, preserves and rewrites visible bytes, and isolates subsequent visible-plane mutation from the clone and source buffer. Docs and the ledger now call out this audio payload detachment invariant explicitly. Validation passed with `cargo test -p avutil frame_data_make_writable_detaches_audio_payload_refs`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
