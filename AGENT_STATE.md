...
Update for the current slice: 3D reference displays frame side-data coverage now has broader build-checked fuzz invariants matching `frame_side_data_parses_three_d_reference_displays_payload` and `frame_side_data_rejects_malformed_three_d_reference_displays_payload`. The deterministic `avutil_core_models` fixture now preserves two-display payloads, native header length and entry-offset invariants, indexed display lookups, individual display and aggregate round trips, non-3D-reference lookup absence, no-display and too-many-display constructor rejection, short display-entry rejection, and malformed precision, flag, count, native offset, entry size, trailing-byte, and truncated-payload rejection through deferred `side_data.three_d_reference_displays()` paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- `cargo test -p avutil frame_side_data_parses_three_d_reference_displays_payload`
- `cargo test -p avutil frame_side_data_rejects_malformed_three_d_reference_displays_payload`
...
- No remaining failing commands in the latest 3D reference displays fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens 3D reference displays side-data fuzz coverage. `FrameThreeDReferenceDisplays` already validates the native allocation envelope and preserves display records in unit tests. The `avutil_core_models` build-checked fuzz harness now mirrors the two-display payload, native header and entry metadata, indexed display lookup, raw round trips, constructor rejection, and malformed deferred side-data accessor assertions. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened 3D reference displays fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_parses_three_d_reference_displays_payload` and `frame_side_data_rejects_malformed_three_d_reference_displays_payload`: two-display payload preservation, native header length and entry-offset invariants, indexed display lookup, individual display and aggregate round trips, non-3D-reference lookup absence, no-display and too-many-display constructor rejection, short display-entry rejection, and malformed precision, flag, count, offset, entry-size, trailing-byte, and truncated-payload rejection through deferred side-data access. Docs and the ledger now call out these 3D reference display invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_three_d_reference_displays_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_three_d_reference_displays_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
