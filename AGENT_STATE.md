...
Update for the current slice: `FrameExif::common_tags()` now exposes root TIFF NewSubfileType through `FrameExifNewSubfileType` and legacy SubfileType through `FrameExifSubfileType`, validating the known NewSubfileType bit mask and SubfileType 1/2/3 enum values. The crate root re-exports both helper types. Unit tests cover valid reduced-resolution/multi-page parsing plus malformed type, unknown flag bit, invalid count, and invalid enum value rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid subfile-type paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends EXIF root image-layout common-tag coverage. `FrameExif::common_tags()` now exposes NewSubfileType as known bitflags and legacy SubfileType as a typed 1/2/3 enum alongside the existing root image-layout and coding tags, and `avutil` now re-exports both helper types from the crate root. Unit tests cover valid parsing plus malformed type, unknown flag bit, invalid count, and invalid enum value rejection, and the `avutil_core_models` build-checked fuzz harness asserts the new flag-mask and enum invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: extended EXIF root image-layout common-tag coverage for NewSubfileType/SubfileType and crate-root re-exports for `FrameExifNewSubfileType` and `FrameExifSubfileType`. `frame_side_data_interprets_exif_root_subfile_type_tags` covers valid parsing plus malformed type, unknown NewSubfileType flag bits, invalid SubfileType count, and invalid SubfileType value rejection, and `avutil_core_models` now build-checks deterministic fixture coverage plus flag-mask/enum invariants for the new accessors. Validation passed with `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame_side_data_interprets_exif_root_subfile_type_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `$env:CARGO_TARGET_DIR='target-codex'; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --component avutil-frame`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-codex'; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
