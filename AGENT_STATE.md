...
Latest `avutil-packet` differential slice: added packet/frame side-data bridge parity for the pinned libavcodec global side-data map. `PacketSideDataKind` now maps the 11 FFmpeg-supported packet/frame side-data pairs, `SideData` can convert from/to `FrameSideData`, and `PacketSideDataList` can add frame side data with FFmpeg-style replacement behavior. `FrameSideDataFlags` and `Frame::add_side_data_with_flags` now model the `UNIQUE`, `REPLACE`, and `NEW_REF` insertion flags needed by `av_packet_side_data_to_frame`, including the duplicate non-MULTI no-replace path returning `ENOMEM`. Unit tests and the `avutil_core_models` fuzz harness cover mapped conversions, replacement, duplicate rejection, and unmapped `EINVAL` errors. `crates/avutil/tests/packet_oracle.rs` now validates `av_packet_side_data_from_frame`, `av_packet_side_data_to_frame`, duplicate frame-side insertion, `AV_FRAME_SIDE_DATA_FLAG_REPLACE`, and unmapped bridge errors against pinned libavcodec/libswresample/libavutil. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
