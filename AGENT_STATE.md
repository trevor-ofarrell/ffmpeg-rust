...
Update for the current slice: `FrameExif::common_tags()` linked GPS motion coverage now has build-checked fuzz invariants matching more invalid-input paths from the existing GPS motion unit test. The deterministic `avutil_core_models` EXIF fixture already preserved valid GPSSpeedRef, GPSSpeed, GPSTrackRef, GPSTrack, GPSImgDirectionRef, GPSImgDirection, and GPSMapDatum parsing plus GPSTrackRef count and track/image-direction range rejection; it now also covers malformed GPSSpeedRef enum value, GPSImgDirection count, and GPSMapDatum TIFF type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked GPS motion common-tag fuzz coverage. `FrameExif::common_tags()` already exposed GPSSpeedRef, GPSSpeed, GPSTrackRef, GPSTrack, GPSImgDirectionRef, GPSImgDirection, and GPSMapDatum values with unit coverage for malformed GPSSpeedRef values, GPSTrackRef count, GPSImgDirection count/range, GPSTrack range, and GPSMapDatum type validation. The `avutil_core_models` build-checked fuzz harness now asserts the missing GPSSpeedRef enum-value, GPSImgDirection count, and GPSMapDatum type paths alongside its existing GPSTrackRef count and track/image-direction range checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF linked GPS motion common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching additional malformed cases from `frame_side_data_interprets_exif_gps_motion_tags`: malformed GPSSpeedRef enum value, malformed GPSImgDirection count, and malformed GPSMapDatum TIFF type, while preserving valid GPSSpeedRef, GPSSpeed, GPSTrackRef, GPSTrack, GPSImgDirectionRef, GPSImgDirection, and GPSMapDatum parsing plus existing GPSTrackRef count and track/image-direction range rejection. Docs and the ledger now call out the GPS motion validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_gps_motion_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
