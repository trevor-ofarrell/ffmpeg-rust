...
- `cargo fmt --all`
- `cargo test --target-dir target-avutil-timebase-test -p avutil packet_side_data_parses_encryption`
- `cargo test --target-dir target-avutil-timebase-test -p avutil packet_side_data_rejects_malformed_encryption`
...
Latest slice: added packet `AV_PKT_DATA_ENCRYPTION_INFO` and `AV_PKT_DATA_ENCRYPTION_INIT_INFO` parsing. `PacketEncryptionInfo` models FFmpeg's big-endian encryption-info side-data envelope with scheme, crypt/skip byte blocks, key ID, IV, subsample clear/protected byte counts, parsed length, and trailing-byte preservation. `PacketEncryptionInitInfo` models the counted big-endian init-info list with system IDs, fixed-size key IDs, init data, parsed length, and trailing-byte preservation, while rejecting nonzero key counts with zero key-id size as an unsafe invalid input shape. Focused packet unit tests cover constructor/deferred accessor behavior for both side-data kinds, raw byte preservation, field access, parsed/trailing length behavior, empty init-info lists, non-matching kind behavior, truncated payloads, oversized declared lengths, zero-sized keyed init-info records, and deferred accessor errors; `avutil_core_models` now build-checks the same invariants. Validation passed with `cargo fmt --all`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet_side_data_parses_encryption`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet_side_data_rejects_malformed_encryption`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy -p avutil --all-targets -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed`, and `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\xtask.exe quick`. The component remains `implemented`, not `complete`.
