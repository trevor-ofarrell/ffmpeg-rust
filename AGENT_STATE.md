...
`avformat-hash-muxer` now supports SHA-256 packet-data hashing in addition to Adler-32, IEEE CRC-32, and MD5. `HashMuxerReport` carries a variable-width `HashDigest`, formats MD5 and SHA-256 digest lines as lowercase hex, preserves packet/byte accounting, and the `avformat_packet_muxers` fuzz target now build-checks SHA-256 digest stability alongside Adler-32/CRC-32/MD5. The ledger still keeps `avformat-hash-muxer` at `implemented`, not `complete`, because CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned-oracle differential tests, FATE coverage, remaining SHA variants, and actual local fuzz execution are still absent.

`avutil-hash` now includes Rust-native SHA-256 support alongside Adler-32, IEEE CRC-32, and MD5 helpers. `Sha256` supports one-shot and streaming updates across block boundaries, `digest_to_hex` formats lowercase digest output for byte digests, and `avutil_core_models` now build-checks SHA-256 streaming equivalence and hex invariants in addition to Adler-32/CRC-32/MD5 streaming equivalence. The ledger still keeps `avutil-hash` at `implemented`, not `complete`, because pinned-oracle differential vectors, FATE coverage, actual local fuzz execution, additional SHA variants, and full FFmpeg hash/framehash behavior are still absent.
...
`avformat-hash-muxer` is the latest coherent slice. SHA-256 packet-data hashing is wired into the internal hash muxer beside Adler-32, IEEE CRC-32, and MD5, with standard-vector report tests and build-checked `avformat_packet_muxers` coverage. CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, remaining SHA variants, and full FFmpeg hash/framehash behavior are still absent.
...
Latest slice: wired Rust-native SHA-256 into `avformat::HashMuxer`, added report-line tests and packet-muxer fuzz coverage, and updated docs/ledger/state while keeping `avformat-hash-muxer` below `complete`.
