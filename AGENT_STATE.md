...
`fftools-ffprobe-stream-nb-frames` is the current focus; the latest slice strengthens default and JSON tests for `ffprobe-rs -show_streams` `nb_frames` output on constrained MOV/MP4 and AVI files.
...
Latest slice: strengthen `ffprobe-rs` stream `nb_frames` coverage. Existing MOV sample-count and AVI stream-length values are now asserted in default and JSON `-show_streams` paths, including alongside `-count_frames` and `-count_packets`, and a ledger component records the limited one-frame-per-sample fixture semantics. This still lacks pinned-oracle differential/FATE/fuzz coverage.
