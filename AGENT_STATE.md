...
Latest validation commands for the `av_init_packet` oracle slice: `cargo fmt --all`, `cargo fmt --all -- --check`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --lib packet_legacy_init_resets_fields_but_preserves_payload`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --lib packet`, `CARGO_TARGET_DIR=target-codex cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --component avutil-packet`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --changed`, and `git diff --check` pass for the latest packet legacy-init oracle slice. `git diff --check` reports CRLF conversion warnings only. A filtered non-`--lib` unit run proved the target test passed but exited nonzero after Windows Application Control blocked unrelated freshly built integration-test executables with the same filter; the equivalent `--lib` run passed cleanly.
...
`avutil-packet` is the current focus. The latest slice adds explicit pinned `av_init_packet()` oracle coverage for deterministic field-reset behavior: payload data/size are preserved, while PTS/DTS, position, duration, stream index, flags, side data, opaque metadata, `opaque_ref`, and `time_base` are reset to the observed FFmpeg 8.1.1 shape.
...
1. Continue `avutil-packet` toward strict completion by deciding the upstream FATE disposition and adding any required media-integration oracle vectors.
2. If packet completion remains blocked, move to the next highest-priority incomplete infrastructure row such as `avutil-options`, `avutil-logging`, `fftools-version`, or `avutil-channel-layout`.
...
- `fftools-version` and `avutil-channel-layout` have executed pinned-oracle differential rows and remain marked `differential_pass`, not `complete`. The 10% milestone is now reached; next candidates for strict completion are `avutil-bitwriter`, `avutil-options`, `avutil-logging`, `fftools-version`, or continued channel-layout closure, depending on which can honestly satisfy the full completion rules.
