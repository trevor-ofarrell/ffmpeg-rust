...
Latest `avutil-packet` differential slice: `PacketSideDataKind::ffmpeg_side_data_name()` now exposes pinned `av_packet_side_data_name()` display strings for all 41 public `AV_PKT_DATA_*` kinds, and `crates/avutil/tests/packet_oracle.rs` validates those strings against the ignored pinned libavcodec/libavutil `AVPacket` oracle harness. The same oracle continues to validate `AV_PKT_DATA_NB`, every public `AV_PKT_DATA_*` numeric enum value, public side-data payload layout rows for FFmpeg C struct and enum payloads (`AVReplayGain`, `AVCPBProperties`, `AVProducerReferenceTime`, `AVRTCPSenderReport`, `AVDOVIDecoderConfigurationRecord`, and `enum AVAudioServiceType`), core lifecycle/property behavior, payload API behavior, and side-data API behavior: `av_packet_alloc` defaults, `av_packet_rescale_ts` with known and unknown timestamps, `av_packet_copy_props`, `av_packet_ref`, `av_packet_move_ref`, `av_packet_unref`, `av_packet_from_data`, `av_grow_packet`, `av_shrink_packet`, `av_packet_make_refcounted`, `av_packet_make_writable`, `AV_PKT_FLAG_KEY|CORRUPT`, `AV_PKT_DATA_NEW_EXTRADATA` payload cloning, `av_packet_new_side_data`, `av_packet_get_side_data`, `av_packet_shrink_side_data`, `av_packet_free_side_data`, `av_packet_add_side_data` replacement/append behavior, duplicate `av_packet_new_side_data` replacement behavior, missing side-data lookup size reset, `opaque` address copying, payload preservation, and packet `time_base` fields against the Rust `Packet` model. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because remaining comment-defined/raw side-data payload oracle vectors, upstream FATE coverage or a documented FATE exception, remaining packet media-integration vectors, and broader differential media integration remain pending.
...
Latest validation commands: `third_party\ffmpeg-oracle\build\bin\ffmpeg.cmd -version`, `cargo fmt --all -- --check`, `cargo test -p avutil packet_side_data_kind`, `cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-packet`, `cargo run -p fate-runner -- run --changed`, `cargo check --manifest-path fuzz\Cargo.toml --target-dir target-codex\fuzz-check --quiet`, and `git diff --check` pass for the latest packet side-data display-name oracle slice. `git diff --check` reports CRLF conversion warnings only.
