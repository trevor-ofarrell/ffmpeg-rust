...
Update for the current slice: `FrameExif::common_tags()` now exposes root TIFF `HostComputer` as a single ASCII string alongside the existing root descriptive/image-layout tags. Unit tests cover valid inline parsing plus malformed type, missing ASCII terminator, and multiple-string rejection, and the `avutil_core_models` build-checked fuzz fixture covers deterministic valid and invalid HostComputer paths. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
