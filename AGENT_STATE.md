...
- Current `avutil-channel-layout` slice:
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.h -Pattern 'AV_CHAN_TOP_CENTER|AV_CHAN_BOTTOM_FRONT_CENTER|AV_CHAN_BOTTOM_FRONT_LEFT|AV_CHAN_BOTTOM_FRONT_RIGHT|AV_CH_TOP_CENTER|AV_CH_BOTTOM_FRONT_CENTER|AV_CH_BOTTOM_FRONT_LEFT|AV_CH_BOTTOM_FRONT_RIGHT|AV_CH_LAYOUT_22POINT2|AV_CHANNEL_LAYOUT_22POINT2' -Context 3,4` (source-check only)
  - `Select-String -Path $env:TEMP\ffmpeg-channel-layout-8.1.1.c -Pattern 'TOP_CENTER|BOTTOM_FRONT|"TC"|"BFC"|"BFL"|"BFR"|"22\.2"' -Context 3,4` (source-check only)
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `TC`, `BFC`, `BFL`, `BFR`, and `22.2`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, `avformat` audio regression, format checks, and `git diff --check` passed.
...
1. Commit the coherent `avutil-channel-layout` `22.2` update.
2. Inspect the remaining FFmpeg channel families and default-layout count table before claiming broader `ffmpeg -layouts` inventory parity.
3. Continue the next unblocked priority-1 infrastructure gap with local unit/fuzz/FATE coverage.

## Known Blockers

- `avutil-channel-layout` now covers local FFmpeg-shaped channel names and native mask helpers for the current subset, including `FLC`, `FRC`, `BC`, `TFL`, `TFC`, `TFR`, `TBL`, `TBC`, `TBR`, `TC`, `DL`, `DR`, `WL`, `WR`, `LFE2`, `TSL`, `TSR`, `BFC`, `BFL`, `BFR`, `BIL`, and `BIR`, plus named `mono`, `stereo`, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `5.1`, `5.1(side)`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, `octagonal`, `cube`, `5.1.4`, `7.1.2`, `7.1.4`, `7.2.3`, `9.1.4`, `9.1.6`, `hexadecagonal`, `binaural`, `downmix`, and `22.2` layouts. It models FFmpeg default-layout count selection for currently covered first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, but still has no full pinned `ffmpeg -layouts` inventory comparison, no remaining default-layout count table coverage, no custom/native/ambisonic order semantics, no upstream FATE parity, and no actual cargo-fuzz execution.
