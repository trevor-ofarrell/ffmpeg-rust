...
Latest slice: added build-checked cargo-fuzz target `avformat_avi_muxer` plus seed corpora, updated the AVI muxer ledger fuzz target field, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
