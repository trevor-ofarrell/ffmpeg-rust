...
`avutil-hash` now includes Rust-native MD5 support alongside the existing Adler-32 and IEEE CRC-32 helpers. `Md5` supports one-shot and streaming updates across block boundaries, `digest_to_hex` formats lowercase digest output, and `avutil_core_models` now build-checks MD5 streaming equivalence and hex invariants in addition to Adler-32/CRC-32 streaming equivalence. The ledger still keeps `avutil-hash` at `implemented`, not `complete`, because pinned-oracle differential vectors, FATE coverage, actual local fuzz execution, SHA variants, and full FFmpeg hash/framehash behavior are still absent.
...
`avformat-hash-muxer` is the latest coherent slice. MD5 packet-data hashing has been added beside Adler-32 and IEEE CRC-32 using variable-width `HashDigest` reporting, with unit tests and build-checked `avformat_packet_muxers` coverage. CLI hash muxer wiring, exact FFmpeg hash muxer output semantics, pinned FFmpeg differential tests, FATE coverage, SHA variants, and actual local fuzz execution are still absent.

## Next 3 Concrete Actions

1. Continue hash-muxer parity by adding the next missing SHA digest variant or wiring the internal hash muxer into constrained CLI execution with tests.
...
Latest slice: strengthened `avformat::HashMuxer` with MD5 packet-data hashing, variable-width digest reporting, build-checked fuzz-target coverage, and docs/ledger updates while keeping `avformat-hash-muxer` below `complete`.
