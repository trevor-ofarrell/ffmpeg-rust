...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDynamicHdrPlus` malformed-length, header/count/flag, overlap-option, and peak-grid coverage, matching the focused Dynamic HDR10+ unit coverage for raw and deferred side-data rejection paths.

Update for the current slice: Dynamic HDR10+ coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_dynamic_hdr_plus_payload`. The deterministic `avutil_core_models` fixture now asserts empty/short/long side-data payload rejection, invalid ITU-T T.35 country/application/window count values, invalid color-transform count/flag values, invalid overlap option rejection, and invalid targeted peak-luminance grid rejection through raw and deferred side-data paths. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens Dynamic HDR10+ fuzz coverage. `FrameDynamicHdrPlus` already has unit coverage for a valid native envelope, selected field access, malformed empty/short/long payloads, invalid ITU-T T.35 country/application/window count values, invalid color-transform count/flag values, invalid overlap option rejection, invalid targeted peak-luminance grid rejection, and non-Dynamic-HDR10+ lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches, including raw constructor rejection and deferred side-data accessor rejection. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened Dynamic HDR10+ fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the Dynamic HDR10+ malformed-input unit branches for empty/short/long payload rejection, invalid ITU-T T.35 country/application/window count values, invalid color-transform count/flag values, invalid overlap option rejection, and invalid targeted peak-luminance grid rejection through raw and deferred side-data paths. Docs and the ledger now call out these Dynamic HDR10+ malformed length/header/count/flag/overlap/grid invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_dynamic_hdr_plus_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_dynamic_hdr_plus_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
