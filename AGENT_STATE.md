...
2. Add fuzz targets for the next untrusted parser surfaces, starting with image2 path/sequence parsing.
...
Latest slice: added build-checked cargo-fuzz target `fftools_option_parser` plus seed corpora, updated the option parser ledger fuzz target field, and documented that actual local fuzz execution remains blocked until `cargo-fuzz` is installed.
