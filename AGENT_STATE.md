...
Latest validation commands: `cargo test -p avutil bitwriter`, `cargo test -p avutil --test bitwriter_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-bitwriter --target oracle-libavcodec-put-bits --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-bitwriter`, WSL `cargo fuzz run avutil_bitreader -- -runs=1`, `cargo test -p fate-runner changed_selection_maps_bitwriter_oracle_test_to_component`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo fmt --all -- --check`, `cargo run -p fate-runner -- run --changed`, `cargo clippy -p fate-runner --all-targets --all-features -- -D warnings`, and `git diff --check` pass for the bitwriter completion slice. `git diff --check` reports CRLF conversion warnings only.
