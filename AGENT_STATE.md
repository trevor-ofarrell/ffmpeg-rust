...
Update for the current slice: `FrameExifEntry` typed EXIF value semantics now have broader build-checked fuzz invariants matching the unit fixture. The deterministic `avutil_core_models` EXIF value-semantics fixture now preserves ASCII, BYTE, SHORT, LONG, signed BYTE/SHORT/LONG, unsigned/signed RATIONAL, FLOAT, and out-of-line DOUBLE values, and it rejects malformed ASCII terminators plus zero-denominator rationals through typed errors. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF typed-entry value-semantics fuzz coverage. `FrameExifEntry` already exposed endian-aware typed accessors for ASCII, byte, signed byte, short/long, signed short/long, unsigned/signed rational, float, and double values, and the unit test already asserted representative values plus invalid ASCII and zero-denominator rational rejection. The `avutil_core_models` build-checked fuzz harness now mirrors those deterministic value and invalid-input assertions in the value-semantics fixture. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF typed-entry value-semantics fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching `frame_side_data_decodes_exif_entry_values`: ASCII string decoding, BYTE values, SHORT/LONG values, signed BYTE/SHORT/LONG values, unsigned/signed RATIONAL values, FLOAT values, out-of-line DOUBLE values, malformed ASCII terminator rejection, and zero-denominator rational rejection. Docs and the ledger now call out these value-semantics shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_decodes_exif_entry_values`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
