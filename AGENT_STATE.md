...
1. Continue priority-1 primitive work by adding the next unblocked shared `avutil` compatibility vector, most likely another packet side-data payload parser such as Dolby Vision config, encryption side-data, EXIF, or ambient viewing environment once its FFmpeg 8.1.1 layout is verified.
...
- Windows Application Control intermittently blocks freshly built child executables and separate integration-test executables. During recent packet slices it blocked focused `avutil` and `fftools` unit-test executables in multiple target directories; `target-avutil-opaque-ref-test` and `target-avutil-timebase-test` have launched the same focused packet tests successfully, and the current ReplayGain slice validated through `target-avutil-timebase-test`. The current ffprobe MOV command-path coverage is kept in the `fftools` unit-test binary instead of a process-spawn integration test.

## Summary Of Latest Commit Or Changes

Latest slice: added packet stereo3d side-data parsing. `PacketStereo3d` models `AV_PKT_DATA_STEREO3D` as the FFmpeg 8.1.1 native `AVStereo3D` packet payload, preserving stereo type, invert flags, view, primary eye, baseline, and rational disparity/field-of-view metadata through the existing frame-side native envelope. `SideData::new_stereo3d` and deferred `stereo3d()` lookup preserve exact payload bytes, reject malformed short/long payloads plus invalid enum/flag/rational values, and return `None` for non-stereo3d kinds. Focused packet unit tests cover native byte layout, FFmpeg constant names, unset rational sentinels, constructor/deferred accessor behavior, non-matching kind behavior, malformed lengths, invalid type/flag/view/primary-eye values, invalid rational values, and deferred accessor errors; `avutil_core_models` now build-checks the same invariants. Validation passed with `cargo fmt --all`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy -p avutil --all-targets -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet --dry-run`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed --dry-run`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\xtask.exe quick`, and `git diff --check`.
