...
Update for the current slice: `avutil_core_models` also build-checks EXIF typed entry value accessors for ASCII, byte, unsigned/signed short and long, and unsigned/signed rational values, including malformed ASCII rejection; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
Latest slice: added `FrameExifRational`, `FrameExifSignedRational`, endian-bearing `FrameExifEntry` values, and typed EXIF entry accessors for ASCII, BYTE, unsigned/signed SHORT and LONG, and unsigned/signed rational values. Unit tests and `avutil_core_models` fuzz invariants cover little-endian and big-endian value decoding, malformed ASCII accessor errors, and zero-denominator rational accessor errors. Validation passed with `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame_side_data_decodes_exif_entry_values`, `cargo test -p avutil frame`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
