...
Latest slice: added borrowed static readonly `BufferRef` storage. Unit tests now cover static pointer retention before detach, detach-on-mutation, detach-on-resize with zeroed padding, shared static references remaining untouched, and invalid visible-length rejection; `avutil_core_models` now build-checks static readonly pointer retention, non-writability, resize detach, prefix preservation, zeroed growth/padding, and shared-reference invariants. The final validation pass ran formatting, local `avutil-buffer` mapping, changed-component mappings, workspace clippy, workspace tests, fuzz-target check/clippy, and `git diff --check`. The mappings execute local unit-test filters only and do not count as upstream FFmpeg FATE parity.
