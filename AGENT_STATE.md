...
`fftools-ffprobe-mov-visual-side-fields` is the current focus; the latest slice adds MOV visual sample-entry aspect-ratio and color fields to constrained `ffprobe-rs -show_streams` reports.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as MOV sample-entry compressor/depth fields or AVI bit depth and stream handler details.
...
Latest slice: add MOV visual side-field reporting to `ffprobe-rs` for constrained MOV/MP4 paths. Stream reports now carry `sample_aspect_ratio`, `display_aspect_ratio`, `color_range`, `color_space`, `color_transfer`, and `color_primaries` when derivable from parsed `pasp` and `colr` VisualSampleEntry child boxes, with an `avc1` fixture test covering JSON output.
