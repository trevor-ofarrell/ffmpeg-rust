...
`fftools-ffprobe-count-packets` is the current focus; the latest slice strengthens coverage for `-count_packets` by proving `nb_read_packets` in both default and JSON constrained MOV/MP4 and AVI `ffprobe-rs -show_streams` output.
...
Latest slice: strengthen constrained ffprobe packet-count coverage. `ffprobe-rs` already accepts `-count_packets`, collects packets through the Rust MOV/MP4 or AVI demuxer when requested, renders `nb_read_packets` on stream reports, and keeps the packets section absent unless `-show_packets` is also present; the new tests cover both default and JSON output shapes for MOV and AVI. This still lacks pinned-oracle differential/FATE/fuzz coverage.
