...
Update for the current slice: `avutil_core_models` also build-checks EXIF linked IFD pointer discovery for EXIF/GPS/interoperability chains plus malformed pointer rejection; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
