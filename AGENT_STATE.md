...
Update for the current slice: `FrameExif::common_tags()` descriptive-string coverage now has build-checked fuzz invariants matching the remaining invalid-input path from the existing descriptive unit test. The deterministic `avutil_core_models` EXIF fixture now covers valid ImageDescription, Software, DateTime, Artist, and Copyright parsing plus malformed Software TIFF type alongside the existing ImageDescription terminator, Software non-ASCII, Artist multiple-string, and DateTime count/layout/hour cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
- No remaining failing commands in the latest EXIF descriptive-string fuzz-coverage slice. `git diff --check` exited successfully with CRLF line-ending warnings only.
...
This slice strengthens EXIF descriptive-string common-tag fuzz coverage. `FrameExif::common_tags()` already exposed ImageDescription, Software, DateTime, Artist, and Copyright with unit coverage for malformed Software TIFF type, DateTime count/layout/hour validation, and separate ASCII-shape rejection. The `avutil_core_models` build-checked fuzz harness now asserts the missing Software TIFF type rejection while preserving the valid descriptive string fixture values. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF descriptive-string common-tag fuzz coverage. `avutil_core_models` now build-checks the deterministic path matching `frame_side_data_interprets_exif_descriptive_tags` for malformed Software TIFF type while preserving valid ImageDescription, Software, DateTime, Artist, and Copyright values alongside existing descriptive ASCII and DateTime rejection checks. Docs and the ledger now call out the descriptive validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_descriptive_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
