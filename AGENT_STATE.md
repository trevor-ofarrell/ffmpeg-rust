...
Latest `avutil-packet` differential slice: extended the pinned libavcodec `AVPacket` oracle rows to explicitly cover `av_new_packet()` allocation behavior. The oracle exposed that visible payload bytes are unspecified, so the new row compares only the deterministic fields: return code, size, zeroed `AV_INPUT_BUFFER_PADDING_SIZE` tail padding, and writable refcounted buffer state. `PORTING_LEDGER.toml` keeps `avutil-packet` at `implemented`, not `complete`, because upstream FATE coverage or a documented FATE exception, remaining ABI/media-integration oracle vectors, and broader differential media integration remain pending.
...
Latest validation commands for the `av_new_packet` oracle slice: `third_party\ffmpeg-oracle\build\bin\ffmpeg.cmd -version` verified the local pinned oracle, `cargo fmt --all`, `cargo fmt --all -- --check`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil packet_from_data_and_zeroed_constructors_add_input_padding`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil --test packet_oracle -- --ignored --nocapture`, `CARGO_TARGET_DIR=target-codex cargo test -p avutil packet`, `CARGO_TARGET_DIR=target-codex cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-packet --target oracle-libavcodec-packet-core --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --component avutil-packet`, `CARGO_TARGET_DIR=target-codex cargo run -p fate-runner -- run --changed`, and `git diff --check` pass for the latest packet allocation oracle slice. `git diff --check` reports CRLF conversion warnings only. The first default-target oracle rerun was blocked by Windows Application Control (`os error 4551`) after compiling; the same checks passed through `target-codex`.
...
- Current `avutil-packet` av_new_packet oracle slice:
  - `cmd /c third_party\ffmpeg-oracle\build\bin\ffmpeg.cmd -version`
  - `cmd /c cargo fmt --all`
  - `cmd /c cargo fmt --all -- --check`
  - `cmd /c "set CARGO_TARGET_DIR=target-codex&& cargo test -p avutil packet_from_data_and_zeroed_constructors_add_input_padding"`
  - `cmd /c "set CARGO_TARGET_DIR=target-codex&& cargo test -p avutil --test packet_oracle -- --ignored --nocapture"`
  - `cmd /c "set CARGO_TARGET_DIR=target-codex&& cargo test -p avutil packet"`
...
`avutil-packet` is the current focus. The latest slice adds explicit pinned `av_new_packet()` oracle coverage for deterministic allocation behavior: return code, packet size, zeroed FFmpeg input padding, and writable refcounted buffer state, while recording that visible payload bytes are unspecified and therefore not a valid byte-for-byte assertion target.
