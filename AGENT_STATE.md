...
`fftools-ffprobe-stream-duration` is the current focus; the latest slice strengthens `duration_ts`/`duration` assertions for `ffprobe-rs -show_streams` default and JSON output on constrained MOV/MP4 and AVI files.
...
Latest slice: strengthen `ffprobe-rs` stream duration coverage. Existing MOV media-duration and AVI stream-length values are now asserted in default and JSON `-show_streams` paths, and a ledger component records the constrained container-derived semantics. This still lacks pinned-oracle differential/FATE/fuzz coverage.
