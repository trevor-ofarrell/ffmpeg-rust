...
1. Continue priority-1 primitive work by adding the next unblocked shared `avutil` compatibility vector, most likely another packet side-data payload parser such as Dolby Vision config, encryption side-data, EXIF, ambient viewing environment, stereo3d, or replaygain once its FFmpeg 8.1.1 layout is verified.
...
- Windows Application Control intermittently blocks freshly built child executables and separate integration-test executables. During recent packet slices it blocked focused `avutil` and `fftools` unit-test executables in multiple target directories; in the current audio-service-type packet slice, default/debug, release, `target-codex`, and `target-avutil-audio-service-test` harness launches were blocked with `os error 4551`, but `target-avutil-opaque-ref-test` and `target-avutil-timebase-test` launched the same focused packet tests successfully. The current ffprobe MOV command-path coverage is kept in the `fftools` unit-test binary instead of a process-spawn integration test.

## Summary Of Latest Commit Or Changes

Latest slice: added packet audio-service-type side-data parsing. `PacketAudioServiceType` models `AV_PKT_DATA_AUDIO_SERVICE_TYPE` as the FFmpeg 8.1.1 four-byte native `AVAudioServiceType` enum payload, preserving all public raw values 0..=8 with exact `AV_AUDIO_SERVICE_TYPE_*` constant names. `SideData::new_audio_service_type` and deferred `audio_service_type()` lookup preserve exact enum bytes, reject malformed short/long payloads and unknown raw values, and return `None` for non-audio-service kinds. Focused packet unit tests cover enum value/constant round trips, native byte layout, constructor/deferred accessor behavior, non-matching kind behavior, malformed lengths, invalid raw values, and deferred accessor errors; `avutil_core_models` now build-checks the same invariants. Validation passed with `cargo fmt --all`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo clippy -p avutil --all-targets -- -D warnings`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-packet --dry-run`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo test -p avutil packet --no-run`, `cargo test --target-dir target-avutil-opaque-ref-test -p avutil packet`, `cargo test --target-dir target-avutil-timebase-test -p avutil packet`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --component avutil-packet`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\fate-runner.exe run --changed`, `$env:CARGO_TARGET_DIR='target-avutil-timebase-test'; .\target\debug\xtask.exe quick`, and `git diff --check`.
