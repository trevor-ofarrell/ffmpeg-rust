...
Update for the current slice: `FrameExif::common_tags()` root descriptive ASCII handling now has focused malformed-shape coverage for `ImageDescription`, `Software`, and `Artist` alongside the existing root image-layout/descriptive tags. Unit tests cover missing NUL termination, non-ASCII bytes, and multiple-string ASCII payloads, and the `avutil_core_models` build-checked fuzz fixture covers the same deterministic invalid paths plus ASCII accessor invariants for ImageDescription, Software, and Artist. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF root descriptive/image-layout common-tag coverage. `FrameExif::common_tags()` already exposed ImageDescription, Software, and Artist as selected root ASCII tags; they now have dedicated malformed-shape validation coverage alongside the existing root image-layout, document/page, camera-identity, orientation/resolution, host-computer, predictor, and copyright tags. Unit tests cover missing NUL termination, non-ASCII bytes, and multiple-string ASCII payloads, and the `avutil_core_models` build-checked fuzz harness asserts deterministic invalid paths plus ASCII accessor invariants. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF root descriptive ASCII common-tag coverage for `ImageDescription`, `Software`, and `Artist`. `frame_side_data_rejects_exif_descriptive_ascii_shapes` covers missing NUL termination, non-ASCII bytes, and multiple-string ASCII payloads; `avutil_core_models` now build-checks the same deterministic invalid fixture paths plus ASCII accessor invariants for ImageDescription, Software, and Artist. Validation passed with `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo test -p avutil frame_side_data_rejects_exif_descriptive_ascii_shapes`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --component avutil-frame`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --changed --dry-run`, `Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue; cargo run -p fate-runner -- run --changed`, and `git diff --check`.
