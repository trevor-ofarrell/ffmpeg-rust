...
Update for the current slice: `FrameExif::common_tags()` now also reports ExposureProgram, MeteringMode, LightSource, Flash, WhiteBalance, DigitalZoomRatio, and FocalLengthIn35mmFilm with strict SHORT/RATIONAL type, count, denominator, and defined-value validation where the EXIF tag uses an enumerated domain. `avutil_core_models` build-checks the new EXIF capture-settings fixture and denominator invariants; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with linked EXIF capture-setting metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports ExposureProgram, MeteringMode, LightSource, Flash, WhiteBalance, DigitalZoomRatio, and FocalLengthIn35mmFilm while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
