...
- `cargo run -p fate-runner -- run --changed --dry-run` selected the changed `fftools-option-parser`, `fftools-basic-io`, and `fftools-ffprobe-*` ledger components for this loglevel slice, then failed with the expected no-runnable-FATE-mapping error because `tests/fate/mappings.txt` still only contains local runner and MOV smoke mappings. The focused fftools unit tests, fuzz harness build/clippy checks, workspace clippy, and full workspace tests pass.
...
`fftools-option-parser` is the current component for this slice. It now preserves compound loglevel directives in parser/planning state and keeps output-scoped `-hash`, input/output option ordering, stream specifier option names, and strict unknown-option rejection. The component remains below `complete` because broad FFmpeg option coverage, byte-identical CLI diagnostics, pinned-oracle differential tests, upstream FATE coverage, real stderr logging behavior, and actual local fuzz execution remain absent.

## Next 3 Concrete Actions

1. Add local `tests/fate/mappings.txt` smoke mappings for `fftools-option-parser`, `fftools-basic-io`, and the shared `fftools-ffprobe-*` unit-test path so `cargo run -p fate-runner -- run --changed` can validate fftools-only source changes without reporting unmapped local components.
...
- Upstream FATE samples and media target mappings are not configured. `tests/fate/mappings.txt` currently contains local `fate-runner` and MOV demuxer unit-test smoke mappings only, so changed fftools components still report missing local runnable mappings until those smoke rows are added. The runner has `mappings`, `--check-prereqs`, `--samples`, `--oracle-ffmpeg`, and `--dry-run` support for future mappings.
...
Latest slice: extended `fftools` loglevel parsing so `-loglevel`/`-v` accept the current FFmpeg-style compound repeat/level/time/datetime flag directives as well as numeric and named levels. `CliLogConfig` now preserves log flags for `IoPlan` and the ffprobe parser, and the `fftools_option_parser` fuzz target treats those directives as valid parser inputs. Byte-identical stderr logging, oracle differentials, and upstream FATE coverage remain missing.
