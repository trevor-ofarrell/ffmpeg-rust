...
Current avutil-frame coverage also now includes deterministic build-checked `FrameDetectionBboxes` valid one-box and zero-box payload parsing plus malformed length/offset/size/count/classify-count coverage, matching the focused detection bounding boxes unit coverage for raw, deferred side-data, and constructor rejection paths.

Update for the current slice: Detection bounding boxes coverage now has build-checked fuzz invariants matching the remaining malformed-input branches in `frame_side_data_rejects_malformed_detection_bboxes_payload`. The deterministic `avutil_core_models` fixture now asserts native header and record sizing, source/raw-label length preservation, one-box geometry/label/confidence/classification preservation, zero-box parsing, empty/short/long payload rejection, invalid bbox offset and bbox size rejection, count-derived length mismatch rejection, invalid classify_count rejection, and non-detection lookup absence. `avutil-frame` remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
This slice strengthens detection bounding boxes fuzz coverage. `FrameDetectionBboxes` already has unit coverage for valid multi-box and zero-box payloads, source and label byte preservation, geometry/confidence fields, malformed header/record length payloads, invalid native bbox offset and bbox size fields, count-derived length mismatches, invalid per-box classify_count fields, and non-detection lookup absence. The `avutil_core_models` build-checked fuzz harness now mirrors those malformed-input branches through raw parser, deferred side-data accessor, and constructor paths. The focus component remains below `complete` because pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened detection bounding boxes fuzz coverage. `avutil_core_models` now build-checks deterministic assertions matching the detection-bboxes malformed-input unit branches for valid one-box and zero-box parsing, native header/record sizing, source/label/confidence preservation, empty/short/long payload rejection, invalid bbox offset and bbox size rejection, count-derived length mismatch rejection, invalid classify_count rejection, and non-detection lookup absence. Docs and the ledger now call out these detection bounding boxes malformed length/offset/size/count/classify-count invariants explicitly. Validation passed with `cargo test -p avutil frame_side_data_parses_detection_bboxes_payload`, `cargo test -p avutil frame_side_data_rejects_malformed_detection_bboxes_payload`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
