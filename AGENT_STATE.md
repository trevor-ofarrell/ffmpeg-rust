...
`avutil-buffer` is the current focus. The latest slice adds pinned libavutil `AVBufferRef.data`/`size` offset-reference coverage on top of the existing `AVBufferRef` and `AVBufferPool` oracle rows: offset refs preserve shared storage identity and visible pointer deltas, shared offset refs detach on make-writable/realloc, and same-buffer replace preserves the source ref's offset data view. The component remains `differential_pass`, not `complete`, because broader ABI/lifetime parity, FATE disposition, and final completion documentation still need closure.
...
- `avutil-buffer` now has pinned libavutil `AVBufferRef` and bounded `AVBufferPool` differential evidence, including nullable replace/unref rows and offset `data`/`size` subrange refs, and is marked `differential_pass`, but not complete. Remaining blockers are broader ABI/lifetime closure, FATE disposition, and final completion documentation.
