...
`fftools-ffprobe-mov-codec-profile-level` is the current focus; the latest slice adds MOV codec profile and level fields to constrained `ffprobe-rs -show_streams` reports when `avcC` or `hvcC` data is parsed.

## Next 3 Concrete Actions

1. Extend ffprobe stream metadata further where already parsed, such as AVI bit depth and stream handler details or MOV sample-entry compressor/depth fields.
...
Latest slice: add MOV codec profile and level reporting to `ffprobe-rs` for constrained MOV/MP4 paths. Stream reports now carry `profile` and `level` when derivable from parsed `avcC` or `hvcC` decoder configuration; the current `avc1` fixture proves `profile=High` and `level=31` in JSON output.
