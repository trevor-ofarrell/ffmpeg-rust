...
`avformat-mov-demuxer` remains the current focus; the next slice is additional metadata value classes beyond text, genre, int8 metadata, and track/disc pairs or MOV CLI wiring.

## Next 3 Concrete Actions

1. Add remaining MOV metadata value classes such as freeform atoms or cover art.
...
Latest slice: parse MOV one-byte integer/boolean metadata atoms such as `cpil`, `pcst`, `pgap`, `rtng`, `stik`, `tves`, and `tvsn` while rejecting short payloads.
