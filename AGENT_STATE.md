...
Latest parity status: strict completion is now 9/96 components (about `9%`). `avutil-error`, `avutil-rational`, `avutil-timebase`, `avutil-byteio`, `repo-runtime-guard`, `oracle-inventory`, `avutil-hash`, `avutil-sample-format`, and `avutil-dict` are the current `complete` rows for the default-native profile. Intermediate measurable parity still includes 2/96 non-complete status rows: `fftools-version` and `avutil-channel-layout` are `differential_pass`/validated evidence rows. Current ledger status counts are 84 `implemented`, 1 `scaffolded`, 2 `differential_pass`, and 9 `complete`.
...
Latest validation commands: `cargo test -p avutil byteio`, `cargo test -p avutil --test byteio_oracle -- --ignored --nocapture`, `cargo run -p fate-runner -- run --mappings tests\differential\mappings.txt --component avutil-byteio --target oracle-libavutil-byteio --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg.cmd`, `cargo run -p fate-runner -- run --component avutil-byteio`, WSL `cargo fuzz run avutil_byteio -- -runs=1`, `cargo test -p fate-runner changed_selection_maps_byteio_oracle_test_to_component`, `cargo clippy -p fate-runner --all-targets --all-features -- -D warnings`, `cargo clippy -p avutil --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --changed`, `cargo fmt --all -- --check`, and `git diff --check` all pass for the byteio completion slice. `git diff --check` reports CRLF conversion warnings only.
...
1. Add pinned libavutil or source-calibrated differential vectors for the next strict 10% candidate, likely `avutil-bitreader`, `avutil-bitwriter`, `avutil-options`, or `avutil-logging`.
2. For the next 10% candidate, decide and document upstream FATE applicability; add a FATE mapping or a defensible inapplicability note before changing status.
3. Re-run the relevant WSL `cargo fuzz run <target> -- -runs=1` plus focused unit/oracle tests, then mark one more ledger row complete only if all strict completion rules are satisfied.
...
- `avutil-hash`, `avutil-error`, `avutil-rational`, `avutil-timebase`, `avutil-byteio`, `avutil-sample-format`, `avutil-dict`, `repo-runtime-guard`, and `oracle-inventory` are complete for the selected default-native profile; their completion evidence remains valid, and WSL avutil smoke fuzz now also passes.
...
- `fftools-version` and `avutil-channel-layout` have executed pinned-oracle differential rows and remain marked `differential_pass`, not `complete`. The next 10% candidates are now `avutil-bitreader`, `avutil-bitwriter`, `avutil-options`, or `avutil-logging`; one more strict completion is needed to hit the 10% milestone.
