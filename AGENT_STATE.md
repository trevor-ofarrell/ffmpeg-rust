...
Latest `avutil-frame` update: `FrameActiveFormatDescription` now parses `AV_FRAME_DATA_AFD` side-data payloads as exactly one byte with the pinned FFmpeg 8.1.1 `AVActiveFormatDescription` values, rejects bad lengths and invalid enum values with `InvalidData`, and is covered by focused unit tests plus the build-checked `avutil_core_models` fuzz target. `FrameSeiUnregistered` also parses `AV_FRAME_DATA_SEI_UNREGISTERED` payloads as a 16-byte UUID plus trailing user data. Other typed FFmpeg frame side-data payload schemas remain pending.
