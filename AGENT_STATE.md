...
`avutil-packet` is the current focus for this slice. The concrete change is AVPacket-style property copy support: `Packet::copy_props_from` copies modeled timing, byte position, stream index, flags, and side data while preserving the destination payload storage.
