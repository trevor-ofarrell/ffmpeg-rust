...
Update for the current slice: `FrameExif::common_tags()` now also reports GPSAltitudeRef, GPSAltitude, GPSTimeStamp, and GPSDateStamp with strict BYTE/RATIONAL/ASCII type, count, denominator, and altitude-reference validation. `avutil_core_models` build-checks the new GPS altitude/time/date fixture and denominator invariants; no pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice extends `AV_FRAME_DATA_EXIF` with GPS altitude/time/date metadata in the selected common-tag view. `FrameExif::common_tags()` now also reports GPSAltitudeRef, GPSAltitude, GPSTimeStamp, and GPSDateStamp while returning typed `InvalidData` errors for malformed known-tag type/count/value shapes. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
