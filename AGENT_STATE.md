...
Update for the current slice: `FrameExif::common_tags()` linked GPS altitude/time/date coverage now has build-checked fuzz invariants matching more invalid-input paths from the existing GPS altitude/time unit test. The deterministic `avutil_core_models` EXIF fixture already preserved valid GPSAltitudeRef, GPSAltitude, GPSTimeStamp, and GPSDateStamp parsing plus hour/seconds/date count/date layout/date range rejection; it now also covers malformed GPSAltitudeRef enum value, GPSTimeStamp count, GPSTimeStamp composite overflow, and GPSDateStamp TIFF type cases. No pinned FFmpeg frame side-data kind remains wholly untyped, though several payloads still have deeper semantic work pending.
...
This slice strengthens EXIF linked GPS altitude/time/date common-tag fuzz coverage. `FrameExif::common_tags()` already exposed GPSAltitudeRef, GPSAltitude, GPSTimeStamp, and GPSDateStamp values with unit coverage for malformed altitude-ref enum values, timestamp count/range/composite validation, GPSDateStamp type/count/layout/date validation, and leap-day acceptance. The `avutil_core_models` build-checked fuzz harness now asserts the missing altitude-ref, timestamp count, timestamp composite, and GPSDateStamp type paths alongside its existing hour/seconds/date-count/date-layout/date-range checks. The focus component remains below `complete` because full EXIF/GPS/interoperability tag interpretation, pinned-oracle differential tests, upstream FATE coverage, actual local fuzz execution, and broader AVFrame semantics are still absent.
...
Latest slice: strengthened EXIF linked GPS altitude/time/date common-tag fuzz coverage. `avutil_core_models` now build-checks deterministic paths matching additional malformed cases from `frame_side_data_interprets_exif_gps_altitude_time_tags`: malformed GPSAltitudeRef enum value, malformed GPSTimeStamp count, malformed GPSTimeStamp composite overflow, and malformed GPSDateStamp TIFF type, while preserving valid GPSAltitudeRef, GPSAltitude, GPSTimeStamp, and GPSDateStamp values plus existing timestamp hour/seconds and GPSDateStamp count/layout/date/leap-day checks. Docs and the ledger now call out the GPS altitude/time/date validation shapes explicitly. Validation passed with `cargo test -p avutil frame_side_data_interprets_exif_gps_altitude_time_tags`, `cargo check --manifest-path fuzz\Cargo.toml --bin avutil_core_models`, `cargo fmt --all`, `rustfmt fuzz\fuzz_targets\avutil_core_models.rs`, `cargo test -p avutil frame`, `cargo fmt --all -- --check`, `rustfmt --check fuzz\fuzz_targets\avutil_core_models.rs`, `cargo clippy --manifest-path fuzz\Cargo.toml --bin avutil_core_models -- -D warnings`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo run -p fate-runner -- run --component avutil-frame`, `cargo run -p fate-runner -- run --changed --dry-run`, `cargo run -p fate-runner -- run --changed`, and `git diff --check`.
