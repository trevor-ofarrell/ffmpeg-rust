...
`avformat-mov-demuxer` and `fftools-ffprobe-audio-stream-fields` are the latest coherent slice. The MOV demuxer now parses AudioSampleEntry version 0/1/2 fields for known audio sample-entry tags, rejects malformed versioned audio sample-entry metadata with typed errors, and `ffprobe-rs` emits constrained MOV audio `sample_rate`, `channels`, and `bits_per_sample` from parsed sample-entry metadata including version-2 effective channel and bit-depth values. Exact FFmpeg ffprobe output semantics, pinned FFmpeg differential tests, FATE coverage, actual local fuzz execution, codec-specific audio extension parsing, and broader MOV subtitle/data sample-entry parsing remain absent.
...
2. Extend MOV sample-entry parsing into codec-specific audio extension atoms and subtitle/data sample-entry metadata.
...
Latest slice: parsed MOV AudioSampleEntry version 1/2 extension fields in addition to version 0, surfaced version-2 effective ffprobe MOV audio `sample_rate`/`channels`/`bits_per_sample` fields, added valid and malformed versioned audio sample-entry tests, and kept the affected components below `complete` because oracle/FATE/fuzz parity remains missing.
