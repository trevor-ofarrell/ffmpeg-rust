...
Latest `avutil-channel-layout` update: `ChannelLayout` now exposes source-checked FFmpeg 8.1.1 `9.1.4` from `channel_layout_map`, mapped to `AV_CH_LAYOUT_9POINT1POINT4_BACK` as the existing `7.1.4` shape plus `FLC`/front-left-of-center and `FRC`/front-right-of-center, alongside the prior `TBC`/top-back-center, `LFE2`/low-frequency-2, `7.2.3`, `5.1.4`, `7.1.2`, `7.1.4`, `TBL`/top-back-left, `TBR`/top-back-right, `octagonal`, `cube`, `BC`/back-center, `TFL`/top-front-left, `TFR`/top-front-right, `2.1`, `3.0`, `3.0(back)`, `4.0`, `quad(side)`, `3.1`, `5.0`, `5.0(side)`, `4.1`, `6.0`, `6.0(front)`, `hexagonal`, `6.1`, `6.1(back)`, `6.1(front)`, `7.0`, `7.0(front)`, `7.1(wide)`, `7.1(wide-side)`, `5.1.2`, `5.1.2(back)`, and mono/stereo/quad/5.1/5.1(side)/7.1 subset. `ChannelLayout::default_for_count` still follows FFmpeg's first `channel_layout_map` match for modeled 1-, 2-, 3-, 4-, 5-, 6-, 7-, and 8-channel defaults: mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Local unit tests and the shared `avutil_core_models` fuzz harness cover the new layout, canonical `FL+FR`-style string, mask/name/channel-expression round trips, default lookups, and unsupported-expression rejection. The component remains `implemented`, not `complete`, because full `ffmpeg -layouts` inventory parity, `9.1.6` and broader height-channel layout inventory, remaining FFmpeg default-layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE parity, and actual fuzz execution are still absent.
...
- Current `avutil-channel-layout` slice:
  ...
  - No new command failures have been observed while adding `9.1.4`; focused channel-layout unit tests, local FATE mapping, fuzz-package build/clippy, changed-path smoke run, and `avformat` audio regression passed.
  - Full `ffmpeg -layouts` oracle inventory comparison, `9.1.6` and broader height-channel layout coverage after `9.1.4`, remaining FFmpeg default-layout count behavior beyond the modeled first-map entries for counts 1, 2, 3, 4, 5, 6, 7, and 8, custom/native/ambisonic order semantics, downmix layouts, upstream FATE parity, and actual fuzz execution remain blockers rather than completion claims.
...
`avutil-channel-layout` is the active infrastructure focus for this turn. The concrete change adds source-checked `9.1.4` from pinned FFmpeg 8.1.1 `channel_layout_map`, preserving its `AV_CH_LAYOUT_9POINT1POINT4_BACK` mapping as the existing `7.1.4` shape plus `FLC`/`FRC`, extending exact name/mask/parser tests, and expanding the shared `avutil_core_models` fuzz generator. It does not claim full `ffmpeg -layouts` inventory parity, `9.1.6`/broader height-channel layout inventory, custom/ambisonic orders, downmix layout coverage, upstream FATE parity, or actual fuzz execution.
...
1. Inspect and implement the next pinned FFmpeg `channel_layout.c` map entry after `9.1.4`, starting with `9.1.6` and its required `TSL`/`TSR` top-side channels if their native mask bits are straightforward.
2. Source-check whether `hexadecagonal`, `binaural`, or stereo-downmix require additional channel bits or non-native order semantics, then choose the smallest unblocked follow-up layout slice.
...
Latest slice: added source-checked `9.1.4` native-layout coverage to `avutil-channel-layout`. Source checking against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c` confirmed `AV_CH_LAYOUT_9POINT1POINT4_BACK`, `AV_CHANNEL_LAYOUT_9POINT1POINT4_BACK`, and the public `channel_layout_map` name `9.1.4`; the next `9.1.6` entry requires `TSL`/`TSR` top-side channels and remains pending. The Rust model now exposes `9.1.4` as the existing `7.1.4` shape plus `FLC`/`FRC`, while keeping modeled defaults for counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1. Unit tests cover layout names/counts/channels, canonical channel strings, mask/channel lookups, known-layout order, default lookups, parser round trips, and unsupported expressions; `avutil_core_models` now build-checks the expanded layout generator. Focused channel-layout tests, fuzz-package check/clippy, avutil clippy, local FATE mapping, changed-path local mappings, `avformat` audio tests, format checks, and `git diff --check` passed. The component remains `implemented`, not `complete`, because full oracle inventory parity, `9.1.6` and broader height-channel layouts, remaining FFmpeg default layout count behavior, custom/ambisonic orders, downmix layouts, upstream FATE, and actual fuzz execution remain absent.
