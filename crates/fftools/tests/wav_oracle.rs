...
#[test]
#[ignore = "requires pinned FFmpeg 8.1.1 oracle; set FFMPEG_ORACLE or install third_party/ffmpeg-oracle/build/bin/ffmpeg"]
fn wav_pcm_s16le_duplicate_fmt_unsupported_second_is_ignored_for_framecrc() {
    let payload = [0x00, 0x00, 0x01, 0x00];
    let path = write_generated_wav_with_duplicate_fmt_unsupported_second_chunk(
        "generated-pcm-s16le-duplicate-fmt-unsupported-second-framecrc",
        1,
        44_100,
        1,
        48_000,
...
#[test]
#[ignore = "requires pinned FFmpeg 8.1.1 oracle; set FFMPEG_ORACLE or install third_party/ffmpeg-oracle/build/bin/ffmpeg"]
fn wav_missing_padding_after_odd_fmt_chunk_is_rejected_by_ffmpeg() {
    let path = write_generated_missing_padding_after_odd_fmt_chunk_wav(
        "generated-missing-padding-after-odd-fmt-chunk",
    );
...
fn write_generated_wav_with_duplicate_fmt_unsupported_second_chunk(
    label: &str,
    first_channels: u16,
    first_sample_rate: u32,
    second_channels: u16,
    ...
    let path = unique_temp_path(label, "wav");
    let bytes = wav_multiple_fmt_chunks_with_unsupported_second_bytes(
        first_channels,
        first_sample_rate,
        second_channels,
        second_sample_rate,
    ...
    fs::write(&path, bytes).expect(
        "generated duplicate fmt with unsupported second fmt WAV fixture should be writable",
    );
    path
}
...
fn write_generated_missing_padding_after_odd_fmt_chunk_wav(label: &str) -> PathBuf {
    let path = unique_temp_path(label, "wav");
    fs::write(&path, wav_missing_padding_after_odd_fmt_chunk_bytes())
        .expect("generated odd-sized fmt chunk without padding WAV fixture should be writable");
    path
}
...
fn wav_missing_padding_after_odd_fmt_chunk_bytes() -> Vec<u8> {
    let mut body = Vec::new();
    body.extend_from_slice(b"fmt ");
    body.extend_from_slice(&17_u32.to_le_bytes());
    body.extend_from_slice(&[1, 0, 1, 0, 0x44, 0xAC, 0x00, 0x00]);
    body.extend_from_slice(&[0x88, 0x58, 0x01, 0x00, 2, 0, 16, 0, 0]);
    body.extend_from_slice(b"data");
...
fn wav_multiple_fmt_chunks_with_unsupported_second_bytes(
    first_channels: u16,
    first_sample_rate: u32,
    second_channels: u16,
    second_sample_rate: u32,
    ...
    body.extend_from_slice(b"fmt ");
    body.extend_from_slice(&16_u32.to_le_bytes());
    body.extend_from_slice(&second_audio_format.to_le_bytes());
    body.extend_from_slice(&second_channels.to_le_bytes());
    body.extend_from_slice(&second_sample_rate.to_le_bytes());
    body.extend_from_slice(&second_byte_rate.to_le_bytes());
    body.extend_from_slice(&second_block_align.to_le_bytes());
