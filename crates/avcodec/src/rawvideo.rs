...
mod tests {
    ...
    #[test]
    fn decodes_rgba_float_packets_to_single_plane_frames() {
        let rgbaf16 = RawVideoDecoder::new(2, 1, PixelFormat::RgbaF16Le).unwrap();
        let mut packet = Packet::new((0..16).collect(), 0);
        packet.set_pts(Some(112));

        let frame = rgbaf16.decode_packet(&packet).unwrap();

        assert_eq!(rgbaf16.frame_size(), 16);
        assert_eq!(frame.pts(), Some(112));
        match frame.data() {
            FrameData::Video(video) => {
                assert_eq!(video.width(), 2);
                assert_eq!(video.height(), 1);
                assert_eq!(video.pixel_format(), PixelFormat::RgbaF16Le);
                assert_eq!(video.pixel_format_name(), "rgbaf16le");
                assert!(video.pixel_format().has_alpha());
                assert_eq!(video.planes(), &[(0..16).collect::<Vec<_>>()]);
            }
            FrameData::Audio(_) | FrameData::Empty => panic!("expected video frame"),
        }

        let rgbaf32 = RawVideoDecoder::new(1, 1, PixelFormat::RgbaF32Be).unwrap();
        assert_eq!(rgbaf32.frame_size(), 16);
        assert!(rgbaf32
    ...
    #[test]
    fn decodes_rgb96_and_rgba128_packets_to_single_plane_frames() {
        let rgb96 = RawVideoDecoder::new(1, 1, PixelFormat::Rgb96Le).unwrap();
        let mut packet = Packet::new((0..12).collect(), 0);
        packet.set_pts(Some(113));

        let frame = rgb96.decode_packet(&packet).unwrap();

        assert_eq!(rgb96.frame_size(), 12);
        assert_eq!(frame.pts(), Some(113));
        match frame.data() {
            FrameData::Video(video) => {
                assert_eq!(video.width(), 1);
                assert_eq!(video.height(), 1);
                assert_eq!(video.pixel_format(), PixelFormat::Rgb96Le);
                assert_eq!(video.pixel_format_name(), "rgb96le");
                assert!(!video.pixel_format().is_float());
                assert_eq!(video.planes(), &[(0..12).collect::<Vec<_>>()]);
            }
            FrameData::Audio(_) | FrameData::Empty => panic!("expected video frame"),
        }
        ...
        assert_eq!(rgba128.frame_size(), 16);
        assert!(rgba128
            .decode_packet(&Packet::new((0..16).collect(), 0))
            .is_ok());
        assert_eq!(
    ...
    #[test]
    fn decodes_msb_planar_gbr_packets_to_three_planes() {
        for (format, name, pts) in [
            (PixelFormat::Gbrp10MsbLe, "gbrp10msble", 114),
            (PixelFormat::Gbrp10MsbBe, "gbrp10msbbe", 115),
            (PixelFormat::Gbrp12MsbLe, "gbrp12msble", 116),
            (PixelFormat::Gbrp12MsbBe, "gbrp12msbbe", 117),
        ] {
            ...
            match frame.data() {
                FrameData::Video(video) => {
                    ...
                    assert_eq!(video.line_sizes(), &[4, 4, 4]);
                    assert_eq!(
                        video.planes(),
                        &[vec![0, 1, 2, 3], vec![4, 5, 6, 7], vec![8, 9, 10, 11]]
                    );
    ...
    fn validates_dimensions_and_packet_size() {
        ...
        assert!(RawVideoDecoder::new(3, 2, PixelFormat::Yuv444p10MsbLe).is_ok());
        assert!(RawVideoDecoder::new(3, 2, PixelFormat::Yuv444p12MsbBe).is_ok());
