...
fn expected_rows() -> BTreeMap<String, Vec<String>> {
    ...
    let mut rescaled_negative_near_inf_rounding = Packet::new(vec![0x61, 0x62, 0x63], 8);
    rescaled_negative_near_inf_rounding.set_pts(Some(-24));
    rescaled_negative_near_inf_rounding.set_dts(Some(-23));
    rescaled_negative_near_inf_rounding
        .set_duration(24)
        .unwrap();
    rescaled_negative_near_inf_rounding
        .set_pos(Some(655))
        .unwrap();
    rescaled_negative_near_inf_rounding.set_flag(PacketFlags::DISCARD, true);
    rescaled_negative_near_inf_rounding
        .set_time_base(Rational::new(1, 48_000).unwrap())
        .unwrap();
    rescaled_negative_near_inf_rounding
        ...
        .unwrap();
    rows.insert(
        "packet:rescale-negative-near-inf-rounding".to_string(),
    ...
    rows.insert(
        "packet:copy-props-replace-side-padding".to_string(),
        side_data_padding_fields(
            copy_replace_dst.side_data_by_kind_id(&PacketSideDataKind::NewExtradata),
        ),
    );
    rows.insert(
    ...
    rows.insert(
        "packet:ref-duplicate-side-padding".to_string(),
        side_data_padding_fields(
            ref_duplicate_dst.side_data_by_kind_id(&PacketSideDataKind::Palette),
        ),
    );
    ...
    rows.insert(
        "packet:clone-duplicate-side-padding".to_string(),
        side_data_padding_fields(
            cloned_duplicate.side_data_by_kind_id(&PacketSideDataKind::Palette),
        ),
    );
...
fn insert_payload_api_rows(rows: &mut BTreeMap<String, Vec<String>>) {
    ...
    let null_nonzero_data = Packet::from_null_data_with_len(3).unwrap();
    rows.insert(
        "packet:payload-from-data-null-nonzero-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero".to_string(),
        payload_nullable_fields(&null_nonzero_data),
    );

    let null_nonzero_ref_src = Packet::from_null_data_with_len(3).unwrap();
    let mut null_nonzero_ref_dst = Packet::default();
    null_nonzero_ref_dst.ref_from(&null_nonzero_ref_src);
    rows.insert(
        "packet:payload-from-data-null-nonzero-ref-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-ref-src".to_string(),
        payload_nullable_fields(&null_nonzero_ref_src),
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-ref-dst".to_string(),
        payload_nullable_fields(&null_nonzero_ref_dst),
    );

    let null_nonzero_clone_src = Packet::from_null_data_with_len(3).unwrap();
    let null_nonzero_cloned = null_nonzero_clone_src.clone();
    rows.insert(
        "packet:payload-from-data-null-nonzero-clone-src".to_string(),
        payload_nullable_fields(&null_nonzero_clone_src),
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-clone".to_string(),
        payload_nullable_fields(&null_nonzero_cloned),
    );

    let mut null_nonzero_refcounted = Packet::from_null_data_with_len(3).unwrap();
    null_nonzero_refcounted.make_refcounted().unwrap();
    rows.insert(
        "packet:payload-from-data-null-nonzero-make-refcounted-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-make-refcounted".to_string(),
        payload_nullable_fields(&null_nonzero_refcounted),
    );

    let mut null_nonzero_unique_writable = Packet::from_null_data_with_len(3).unwrap();
    null_nonzero_unique_writable.make_writable().unwrap();
    rows.insert(
        "packet:payload-from-data-null-nonzero-unique-make-writable-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-unique-make-writable".to_string(),
        payload_nullable_fields(&null_nonzero_unique_writable),
    );

    let mut null_nonzero_move_src = Packet::from_null_data_with_len(3).unwrap();
    let mut null_nonzero_move_dst = Packet::default();
    null_nonzero_move_dst.move_ref_from(&mut null_nonzero_move_src);
    rows.insert(
        "packet:payload-from-data-null-nonzero-move-dst".to_string(),
        payload_nullable_fields(&null_nonzero_move_dst),
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-move-src".to_string(),
        packet_fields(&null_nonzero_move_src),
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-move-src-payload".to_string(),
        payload_visible_fields(&null_nonzero_move_src),
    );

    let mut null_nonzero_unref = Packet::from_null_data_with_len(3).unwrap();
    null_nonzero_unref.unref();
    rows.insert(
        "packet:payload-from-data-null-nonzero-unref".to_string(),
        packet_fields(&null_nonzero_unref),
    );
    rows.insert(
        "packet:payload-from-data-null-nonzero-unref-payload".to_string(),
        payload_visible_fields(&null_nonzero_unref),
    );
    ...
    let mut from_data_custom_padding = Packet::default();
    let mut from_data_custom_padding_storage = vec![0x5a; 2 + AV_INPUT_BUFFER_PADDING_SIZE];
    from_data_custom_padding_storage[0] = 0x10;
    from_data_custom_padding_storage[1] = 0x20;
    from_data_custom_padding
        .replace_data_from_vec_with_len(from_data_custom_padding_storage, 2)
        .unwrap();
    rows.insert(
        "packet:payload-from-data-custom-padding-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-from-data-custom-padding".to_string(),
    ...
    let mut grow_empty_zero = Packet::default();
    grow_empty_zero.grow_data(0).unwrap();
    rows.insert(
        "packet:payload-grow-empty-zero-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-grow-empty-zero".to_string(),
        payload_prefix_fields(&grow_empty_zero, 0),
    );
    ...
    let mut offset_helper_storage = vec![0xa0, 0xa1, 0xa2, 0x11, 0x22];
    offset_helper_storage.resize(3 + 2 + AV_INPUT_BUFFER_PADDING_SIZE, 0x5a);
    let mut offset_helper = Packet::with_buffer(
        BufferRef::from_vec(offset_helper_storage)
            .into_ref_slice(3, 2)
            .unwrap(),
        0,
    );
    let offset_helper_ptr = offset_helper.data_buffer().as_padded_ptr();
    offset_helper.make_refcounted().unwrap();
    rows.insert(
        "packet:payload-make-refcounted-offset-padding-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-make-refcounted-offset-padding-same-ptr".to_string(),
        vec![
            u8::from(offset_helper.data_buffer().as_padded_ptr() == offset_helper_ptr).to_string(),
        ],
    );
    rows.insert(
        "packet:payload-make-refcounted-offset-padding-offset".to_string(),
        vec![offset_helper.data_buffer().offset().to_string()],
    );
    rows.insert(
        "packet:payload-make-refcounted-offset-padding".to_string(),
        payload_fields(&offset_helper),
    );
    offset_helper.make_writable().unwrap();
    rows.insert(
        "packet:payload-make-writable-offset-padding-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-make-writable-offset-padding-same-ptr".to_string(),
        vec![
            u8::from(offset_helper.data_buffer().as_padded_ptr() == offset_helper_ptr).to_string(),
        ],
    );
    rows.insert(
        "packet:payload-make-writable-offset-padding-offset".to_string(),
        vec![offset_helper.data_buffer().offset().to_string()],
    );
    rows.insert(
        "packet:payload-make-writable-offset-padding".to_string(),
        payload_fields(&offset_helper),
    );
    ...
    let mut grow_offset_storage = vec![0xa0, 0xa1, 0xa2, 0x11, 0x22, 0xcc, 0xdd];
    grow_offset_storage.resize(3 + 4 + AV_INPUT_BUFFER_PADDING_SIZE, 0x5a);
    let mut grow_offset = Packet::with_buffer(
        BufferRef::from_vec(grow_offset_storage)
            .into_ref_slice(3, 2)
            .unwrap(),
        0,
    );
    let grow_offset_ptr = grow_offset.data_buffer().as_padded_ptr();
    grow_offset.grow_data(2).unwrap();
    rows.insert(
        "packet:payload-grow-offset-padding-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:payload-grow-offset-padding-same-ptr".to_string(),
        vec![u8::from(grow_offset.data_buffer().as_padded_ptr() == grow_offset_ptr).to_string()],
    );
    rows.insert(
        "packet:payload-grow-offset-padding-offset".to_string(),
        vec![grow_offset.data_buffer().offset().to_string()],
    );
    rows.insert(
        "packet:payload-grow-offset-padding".to_string(),
...
fn insert_side_data_api_rows(rows: &mut BTreeMap<String, Vec<String>>) {
    ...
    assert!(
        appended.is_none(),
        "nullable nonzero-size new_extradata should append to an empty packet"
    );
    rows.insert(
        "packet:side-add-null-nonzero-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:side-add-null-nonzero".to_string(),
        side_data_summary_fields(&packet),
    );
    rows.insert(
        "packet:side-get-null-nonzero".to_string(),
        packet_side_data_get_nullable_fields(
            packet.side_data_by_kind_id(&PacketSideDataKind::NewExtradata),
        ),
    );
    ...
    let replaced = packet
        .try_add_null_side_data_with_size(PacketSideDataKind::NewExtradata, 1)
        .unwrap()
        .expect("nullable nonzero-size packet side data should replace existing new_extradata");
    assert_eq!(replaced.data(), &[0x31, 0x32]);
    rows.insert(
        "packet:side-add-null-nonzero-replace-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:side-add-null-nonzero-replace".to_string(),
        side_data_summary_fields(&packet),
    );
    rows.insert(
        "packet:side-get-null-nonzero-replace".to_string(),
        packet_side_data_get_nullable_fields(
            packet.side_data_by_kind_id(&PacketSideDataKind::NewExtradata),
        ),
    );
    ...
    let raw_min_added = raw_packet
        .try_add_side_data(padded_side_data(raw_min_kind.clone(), &[0xe1]))
        .unwrap();
    assert!(raw_min_added.is_none());
    rows.insert(
        "packet:side-add-raw-min-type-ret".to_string(),
        vec!["0".to_string()],
    );
    rows.insert(
        "packet:side-add-raw-min-type".to_string(),
        side_data_summary_fields(&raw_packet),
    );
    rows.insert(
        "packet:side-get-raw-min-type-added".to_string(),
        side_data_lookup_fields(raw_packet.side_data_by_kind_id(&raw_min_kind)),
    );
    rows.insert(
        "packet:side-add-raw-min-type-padding".to_string(),
...
fn insert_side_data_array_api_rows(rows: &mut BTreeMap<String, Vec<String>>) {
    ...
    assert!(
        appended.is_none(),
        "nullable nonzero-size new_extradata should append to an empty standalone list"
    );
    rows.insert(
        "packet:array-add-null-nonzero-ret".to_string(),
        vec!["1".to_string()],
    );
    rows.insert(
        "packet:array-add-null-nonzero".to_string(),
        side_data_list_summary_fields(&list),
    );
    rows.insert(
        "packet:array-get-null-nonzero".to_string(),
        side_data_lookup_fields(list.get(&PacketSideDataKind::NewExtradata)),
    );
    ...
    let replaced = list
        .try_add_null_side_data_with_size_and_flags(PacketSideDataKind::NewExtradata, 1, 0)
        .unwrap()
        .expect("nullable nonzero-size standalone side data should replace existing new_extradata");
    assert_eq!(replaced.data(), &[0x31, 0x32]);
    rows.insert(
        "packet:array-add-null-nonzero-replace-ret".to_string(),
        vec!["1".to_string()],
    );
    rows.insert(
        "packet:array-add-null-nonzero-replace".to_string(),
        side_data_list_summary_fields(&list),
    );
    rows.insert(
        "packet:array-get-null-nonzero-replace".to_string(),
        side_data_lookup_fields(list.get(&PacketSideDataKind::NewExtradata)),
    );
    ...
    let removed = raw_type_new_list
        .remove_kind(&raw_type_kind)
        .expect("new raw AV_PKT_DATA_NB standalone side data should be removable");
    assert_eq!(removed.data(), &[0x7a, 0x7b]);
    rows.insert(
        "packet:array-remove-new-raw-type".to_string(),
        side_data_list_summary_fields(&raw_type_new_list),
    );
    rows.insert(
        "packet:array-get-new-raw-type-removed".to_string(),
        side_data_lookup_fields(raw_type_new_list.get(&raw_type_kind)),
    );
    raw_type_new_list.clear();
    rows.insert(
        "packet:array-free-new-raw-type".to_string(),
        side_data_list_summary_fields(&raw_type_new_list),
    );
    ...
    let removed = raw_plus_one_new_list
        .remove_kind(&raw_plus_one_kind)
        .expect("new raw plus-one standalone side data should be removable");
    assert_eq!(removed.data(), &[0x6c, 0x6d]);
    rows.insert(
        "packet:array-remove-new-raw-plus-one-type".to_string(),
        side_data_list_summary_fields(&raw_plus_one_new_list),
    );
    rows.insert(
        "packet:array-get-new-raw-plus-one-type-removed".to_string(),
        side_data_lookup_fields(raw_plus_one_new_list.get(&raw_plus_one_kind)),
    );
    raw_plus_one_new_list.clear();
    rows.insert(
        "packet:array-free-new-raw-plus-one-type".to_string(),
        side_data_list_summary_fields(&raw_plus_one_new_list),
    );
    ...
    let raw_min_kind = PacketSideDataKind::from_ffmpeg_raw_value(i32::MIN);
    let raw_min_added = raw_list
        .try_add_side_data(padded_side_data(raw_min_kind.clone(), &[0xe1]))
        .unwrap();
    assert!(raw_min_added.is_none());
    rows.insert(
        "packet:array-add-raw-min-type-ret".to_string(),
        vec!["1".to_string()],
    );
    rows.insert(
        "packet:array-add-raw-min-type".to_string(),
        side_data_list_summary_fields(&raw_list),
    );
    rows.insert(
        "packet:array-get-raw-min-type-added".to_string(),
        side_data_lookup_fields(raw_list.get(&raw_min_kind)),
    );
    rows.insert(
        "packet:array-add-raw-min-type-padding".to_string(),
    ...
    raw_negative_new_list
        ...
        .copy_from_slice(&[0xd1, 0xd2]);
    rows.insert(
        "packet:array-new-raw-negative-type-ret".to_string(),
        vec!["1".to_string()],
    );
    rows.insert(
        "packet:array-new-raw-negative-type".to_string(),
        side_data_list_summary_fields(&raw_negative_new_list),
    );
    rows.insert(
        "packet:array-get-new-raw-negative-type".to_string(),
        side_data_lookup_fields(raw_negative_new_list.get(&raw_negative_kind)),
    );
    rows.insert(
        "packet:array-new-raw-negative-type-padding".to_string(),
        side_data_padding_fields(raw_negative_new_list.get(&raw_negative_kind)),
    );
    let removed = raw_negative_new_list
        .remove_kind(&raw_negative_kind)
        .expect("new raw negative standalone side data should be removable");
    assert_eq!(removed.data(), &[0xd1, 0xd2]);
    rows.insert(
        "packet:array-remove-new-raw-negative-type".to_string(),
        side_data_list_summary_fields(&raw_negative_new_list),
    );
    rows.insert(
        "packet:array-get-new-raw-negative-type-removed".to_string(),
        side_data_lookup_fields(raw_negative_new_list.get(&raw_negative_kind)),
    );
    raw_negative_new_list.clear();
    rows.insert(
        "packet:array-free-new-raw-negative-type".to_string(),
        side_data_list_summary_fields(&raw_negative_new_list),
    );
...
fn packet_side_data_get_nullable_fields(side_data: Option<&SideData>) -> Vec<String> {
    match side_data {
        Some(side_data) => vec![
            u8::from(!side_data.is_data_ptr_null()).to_string(),
            side_data.len().to_string(),
            hex_or_dash(side_data.data()),
        ],
        None => vec!["0".to_string(), "0".to_string(), "-".to_string()],
...
static AVPacket *packet_with_offset_grow_capacity(void) {
    AVPacket *pkt = new_packet();
    uint8_t *data = av_malloc(3 + 4 + AV_INPUT_BUFFER_PADDING_SIZE);
    fail_if(!data, "offset grow packet allocation failed");
    data[0] = 0xa0;
    data[1] = 0xa1;
    data[2] = 0xa2;
    data[3] = 0x11;
    ...
    pkt->buf = av_buffer_create(
        ...
        NULL,
        0);
    fail_if(!pkt->buf, "offset grow packet buffer creation failed");
    pkt->data = data + 3;
    pkt->size = 2;
    return pkt;
}
...
static void exercise_side_data_api(void) {
    ...
    pkt = new_packet();
    ret = av_packet_add_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, NULL, 1);
    printf("packet:side-add-null-nonzero-ret|%d\n", ret);
    print_side_data_summary("packet:side-add-null-nonzero", pkt);
    print_side_data_lookup("packet:side-get-null-nonzero", pkt,
                           AV_PKT_DATA_NEW_EXTRADATA);
    av_packet_free(&pkt);
    ...
    pkt = new_packet();
    sd = av_packet_new_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, 2);
    fail_if(!sd, "av_packet_new_side_data NULL nonzero replace seed failed");
    sd[0] = 0x31;
    sd[1] = 0x32;
    ret = av_packet_add_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, NULL, 1);
    printf("packet:side-add-null-nonzero-replace-ret|%d\n", ret);
    print_side_data_summary("packet:side-add-null-nonzero-replace", pkt);
    print_side_data_lookup("packet:side-get-null-nonzero-replace", pkt,
                           AV_PKT_DATA_NEW_EXTRADATA);
    av_packet_free(&pkt);
    ...
    owned = av_mallocz(1 + AV_INPUT_BUFFER_PADDING_SIZE);
    fail_if(!owned, "av_mallocz INT_MIN raw packet side data failed");
    owned[0] = 0xe1;
    ret = av_packet_add_side_data(pkt, (enum AVPacketSideDataType)INT_MIN,
                                  owned, 1);
    if (ret < 0)
        av_free(owned);
    printf("packet:side-add-raw-min-type-ret|%d\n", ret);
    fail_if(ret < 0, "av_packet_add_side_data INT_MIN raw type failed");
    print_side_data_summary("packet:side-add-raw-min-type", pkt);
    print_side_data_lookup("packet:side-get-raw-min-type-added", pkt,
...
static void exercise_side_data_array_api(void) {
    ...
    entry = av_packet_side_data_add(&sd, &nb_sd, AV_PKT_DATA_NEW_EXTRADATA,
                                    NULL, 1, 0);
    printf("packet:array-add-null-nonzero-ret|%d\n", entry != NULL);
    print_side_data_array_summary("packet:array-add-null-nonzero", sd, nb_sd);
    print_side_data_array_lookup("packet:array-get-null-nonzero", sd, nb_sd,
                                 AV_PKT_DATA_NEW_EXTRADATA);
    av_packet_side_data_free(&sd, &nb_sd);
    ...
    entry = av_packet_side_data_new(&sd, &nb_sd, AV_PKT_DATA_NEW_EXTRADATA,
                                    2, 0);
    fail_if(!entry, "av_packet_side_data_new NULL nonzero replace seed failed");
    entry->data[0] = 0x31;
    entry->data[1] = 0x32;
    entry = av_packet_side_data_add(&sd, &nb_sd, AV_PKT_DATA_NEW_EXTRADATA,
                                    NULL, 1, 0);
    printf("packet:array-add-null-nonzero-replace-ret|%d\n", entry != NULL);
    print_side_data_array_summary("packet:array-add-null-nonzero-replace",
                                  sd, nb_sd);
    ...
    owned = av_mallocz(1 + AV_INPUT_BUFFER_PADDING_SIZE);
    fail_if(!owned, "av_mallocz array INT_MIN raw type failed");
    owned[0] = 0xe1;
    entry = av_packet_side_data_add(&sd, &nb_sd,
                                    (enum AVPacketSideDataType)INT_MIN,
                                    owned, 1, 0);
    printf("packet:array-add-raw-min-type-ret|%d\n", entry != NULL);
    if (!entry)
        av_free(owned);
    fail_if(!entry, "av_packet_side_data_add INT_MIN raw type failed");
    print_side_data_array_summary("packet:array-add-raw-min-type",
                                  sd, nb_sd);
    print_side_data_array_lookup("packet:array-get-raw-min-type-added",
                                 sd, nb_sd,
                                 (enum AVPacketSideDataType)INT_MIN);
    ...
    print_side_data_array_padding("packet:array-new-raw-type-padding",
                                  ...
                                  raw_type_new_nb_sd,
                                  (enum AVPacketSideDataType)AV_PKT_DATA_NB);
    av_packet_side_data_remove(raw_type_new_sd, &raw_type_new_nb_sd,
                               (enum AVPacketSideDataType)AV_PKT_DATA_NB);
    print_side_data_array_summary("packet:array-remove-new-raw-type",
                                  raw_type_new_sd,
                                  raw_type_new_nb_sd);
    print_side_data_array_lookup("packet:array-get-new-raw-type-removed",
                                 raw_type_new_sd,
                                 raw_type_new_nb_sd,
                                 (enum AVPacketSideDataType)AV_PKT_DATA_NB);
    av_packet_side_data_free(&raw_type_new_sd,
                             &raw_type_new_nb_sd);
    print_side_data_array_summary("packet:array-free-new-raw-type",
                                  raw_type_new_sd,
                                  raw_type_new_nb_sd);
    ...
    print_side_data_array_summary(
        ...
        raw_plus_one_new_sd, raw_plus_one_new_nb_sd);
    print_side_data_array_lookup(
        "packet:array-get-new-raw-plus-one-type-removed",
        raw_plus_one_new_sd, raw_plus_one_new_nb_sd,
        (enum AVPacketSideDataType)(AV_PKT_DATA_NB + 1));
    av_packet_side_data_free(&raw_plus_one_new_sd,
                             &raw_plus_one_new_nb_sd);
    print_side_data_array_summary(
        "packet:array-free-new-raw-plus-one-type",
        raw_plus_one_new_sd, raw_plus_one_new_nb_sd);

    AVPacketSideData *raw_negative_new_sd = NULL;
    int raw_negative_new_nb_sd = 0;
    entry = av_packet_side_data_new(&raw_negative_new_sd,
                                    &raw_negative_new_nb_sd,
                                    (enum AVPacketSideDataType)-1,
                                    2, 0);
    printf("packet:array-new-raw-negative-type-ret|%d\n", entry != NULL);
    fail_if(!entry, "av_packet_side_data_new negative raw type failed");
    entry->data[0] = 0xd1;
    entry->data[1] = 0xd2;
    print_side_data_array_summary("packet:array-new-raw-negative-type",
                                  raw_negative_new_sd,
                                  raw_negative_new_nb_sd);
    print_side_data_array_lookup("packet:array-get-new-raw-negative-type",
                                 raw_negative_new_sd,
                                 raw_negative_new_nb_sd,
                                 (enum AVPacketSideDataType)-1);
    print_side_data_array_padding("packet:array-new-raw-negative-type-padding",
                                  raw_negative_new_sd,
                                  raw_negative_new_nb_sd,
                                  (enum AVPacketSideDataType)-1);
    av_packet_side_data_remove(raw_negative_new_sd,
                               &raw_negative_new_nb_sd,
                               (enum AVPacketSideDataType)-1);
    print_side_data_array_summary(
        ...
        raw_negative_new_nb_sd);
    print_side_data_array_lookup(
        "packet:array-get-new-raw-negative-type-removed",
        raw_negative_new_sd,
        raw_negative_new_nb_sd,
        (enum AVPacketSideDataType)-1);
    av_packet_side_data_free(&raw_negative_new_sd,
                             &raw_negative_new_nb_sd);
    print_side_data_array_summary(
...
static void exercise_payload_api(void) {
    ...
    pkt = new_packet();
    ret = av_packet_from_data(pkt, NULL, 3);
    printf("packet:payload-from-data-null-nonzero-ret|%d\n", ret);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero payload failed");
    print_payload_nullable("packet:payload-from-data-null-nonzero", pkt);
    av_packet_free(&pkt);

    AVPacket *null_nonzero_ref_src = new_packet();
    ret = av_packet_from_data(null_nonzero_ref_src, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero ref source failed");
    AVPacket *null_nonzero_ref_dst = new_packet();
    ret = av_packet_ref(null_nonzero_ref_dst, null_nonzero_ref_src);
    printf("packet:payload-from-data-null-nonzero-ref-ret|%d\n", ret);
    fail_if(ret < 0, "av_packet_ref NULL nonzero payload failed");
    print_payload_nullable("packet:payload-from-data-null-nonzero-ref-src",
                           null_nonzero_ref_src);
    print_payload_nullable("packet:payload-from-data-null-nonzero-ref-dst",
                           null_nonzero_ref_dst);
    av_packet_free(&null_nonzero_ref_dst);
    av_packet_free(&null_nonzero_ref_src);

    AVPacket *null_nonzero_clone_src = new_packet();
    ret = av_packet_from_data(null_nonzero_clone_src, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero clone source failed");
    AVPacket *null_nonzero_cloned = av_packet_clone(null_nonzero_clone_src);
    fail_if(!null_nonzero_cloned, "av_packet_clone NULL nonzero payload failed");
    print_payload_nullable("packet:payload-from-data-null-nonzero-clone-src",
                           null_nonzero_clone_src);
    print_payload_nullable("packet:payload-from-data-null-nonzero-clone",
                           null_nonzero_cloned);
    av_packet_free(&null_nonzero_cloned);
    av_packet_free(&null_nonzero_clone_src);

    pkt = new_packet();
    ret = av_packet_from_data(pkt, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero make_refcounted failed");
    ret = av_packet_make_refcounted(pkt);
    printf("packet:payload-from-data-null-nonzero-make-refcounted-ret|%d\n",
           ret);
    fail_if(ret < 0, "av_packet_make_refcounted NULL nonzero payload failed");
    print_payload_nullable("packet:payload-from-data-null-nonzero-make-refcounted",
                           pkt);
    av_packet_free(&pkt);

    pkt = new_packet();
    ret = av_packet_from_data(pkt, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero unique writable failed");
    ret = av_packet_make_writable(pkt);
    printf("packet:payload-from-data-null-nonzero-unique-make-writable-ret|%d\n",
           ret);
    fail_if(ret < 0, "av_packet_make_writable unique NULL nonzero payload failed");
    print_payload_nullable(
        "packet:payload-from-data-null-nonzero-unique-make-writable",
        pkt);
    av_packet_free(&pkt);

    AVPacket *null_nonzero_move_src = new_packet();
    ret = av_packet_from_data(null_nonzero_move_src, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero move source failed");
    AVPacket *null_nonzero_move_dst = new_packet();
    av_packet_move_ref(null_nonzero_move_dst, null_nonzero_move_src);
    print_payload_nullable("packet:payload-from-data-null-nonzero-move-dst",
                           null_nonzero_move_dst);
    print_packet("packet:payload-from-data-null-nonzero-move-src",
                 null_nonzero_move_src);
    print_payload_visible("packet:payload-from-data-null-nonzero-move-src-payload",
                          null_nonzero_move_src);
    av_packet_free(&null_nonzero_move_dst);
    av_packet_free(&null_nonzero_move_src);

    pkt = new_packet();
    ret = av_packet_from_data(pkt, NULL, 3);
    fail_if(ret < 0, "av_packet_from_data NULL nonzero unref failed");
    av_packet_unref(pkt);
    print_packet("packet:payload-from-data-null-nonzero-unref", pkt);
    print_payload_visible("packet:payload-from-data-null-nonzero-unref-payload",
                          pkt);
    av_packet_free(&pkt);
    ...
    pkt = new_packet();
    uint8_t *custom_padding_owned =
        av_malloc(2 + AV_INPUT_BUFFER_PADDING_SIZE);
    fail_if(!custom_padding_owned,
            "av_malloc payload from-data custom padding failed");
    memset(custom_padding_owned, 0x5a, 2 + AV_INPUT_BUFFER_PADDING_SIZE);
    custom_padding_owned[0] = 0x10;
    custom_padding_owned[1] = 0x20;
    ret = av_packet_from_data(pkt, custom_padding_owned, 2);
    ...
    pkt = new_packet();
    ret = av_grow_packet(pkt, 0);
    printf("packet:payload-grow-empty-zero-ret|%d\n", ret);
    fail_if(ret < 0, "av_grow_packet empty zero payload failed");
    print_payload_prefix("packet:payload-grow-empty-zero", pkt, 0);
    av_packet_free(&pkt);
...
int main(void) {
    ...
    pkt = new_packet();
    fail_if(av_new_packet(pkt, 3) < 0, "av_new_packet negative near-inf rounding rescale failed");
    pkt->data[0] = 0x61;
    pkt->data[1] = 0x62;
    pkt->data[2] = 0x63;
    pkt->pts = -24;
    ...
    print_side_data_summary("packet:copy-props-replace-side",
                            copy_replace_dst);
    print_packet_side_data_padding("packet:copy-props-replace-side-padding",
                                   copy_replace_dst,
                                   AV_PKT_DATA_NEW_EXTRADATA);
    ...
    print_packet_side_data_padding("packet:ref-duplicate-side-padding",
                                   duplicate_ref_dst, AV_PKT_DATA_PALETTE);
    ...
    print_packet_side_data_padding("packet:clone-duplicate-side-padding",
                                   duplicate_cloned, AV_PKT_DATA_PALETTE);
    ...
    AVPacket *offset_helper = packet_with_offset_padding();
    uint8_t *offset_helper_ptr = offset_helper->data;
    int offset_refcounted_ret = av_packet_make_refcounted(offset_helper);
    printf("packet:payload-make-refcounted-offset-padding-ret|%d\n",
           offset_refcounted_ret);
    fail_if(offset_refcounted_ret < 0,
            "av_packet_make_refcounted offset padding failed");
    printf("packet:payload-make-refcounted-offset-padding-same-ptr|%d\n",
           offset_helper->data == offset_helper_ptr);
    printf("packet:payload-make-refcounted-offset-padding-offset|%td\n",
           offset_helper->data - offset_helper->buf->data);
    print_payload("packet:payload-make-refcounted-offset-padding",
                  offset_helper);
    int offset_writable_ret = av_packet_make_writable(offset_helper);
    printf("packet:payload-make-writable-offset-padding-ret|%d\n",
           offset_writable_ret);
    fail_if(offset_writable_ret < 0,
            "av_packet_make_writable offset padding failed");
    printf("packet:payload-make-writable-offset-padding-same-ptr|%d\n",
           offset_helper->data == offset_helper_ptr);
    printf("packet:payload-make-writable-offset-padding-offset|%td\n",
           offset_helper->data - offset_helper->buf->data);
    ...
    AVPacket *grow_offset = packet_with_offset_grow_capacity();
    uint8_t *grow_offset_ptr = grow_offset->data;
    int grow_offset_ret = av_grow_packet(grow_offset, 2);
    printf("packet:payload-grow-offset-padding-ret|%d\n", grow_offset_ret);
    fail_if(grow_offset_ret < 0, "av_grow_packet offset padding failed");
    printf("packet:payload-grow-offset-padding-same-ptr|%d\n",
           grow_offset->data == grow_offset_ptr);
    printf("packet:payload-grow-offset-padding-offset|%td\n",
