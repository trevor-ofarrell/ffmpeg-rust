...
fn upstream_fate_side_data_array_passes() {
    let output = if cfg!(windows) {
        let script = match env::var("FFMPEG_FATE_BUILD_DIR") {
            Ok(build_dir) => {
                ...
                format!(
                    "test -d {0} || {{ echo 'missing FFmpeg FATE build dir: {0}' >&2; exit 66; }}; make -C {0} fate-side_data_array",
            ...
            Err(_) => concat!(
                ...
                "make -C \"$build_dir\" fate-side_data_array"
            )
            .to_string(),
        };
        Command::new("wsl")
            ...
            .output()
            .expect("run upstream FFmpeg fate-side_data_array through WSL")
    } else {
        ...
        Command::new("make")
            .arg("-C")
            .arg(&build_dir)
            .arg("fate-side_data_array")
            .output()
            .unwrap_or_else(|err| {
                panic!(
                    "run upstream FFmpeg fate-side_data_array in `{}`: {err}",
                    build_dir.display()
                )
            })
    };
    ...
    assert!(
        ...
        "upstream FFmpeg fate-side_data_array failed with status {}\nstdout:\n{}\nstderr:\n{}",
        output.status,
