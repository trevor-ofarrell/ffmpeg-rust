...
const PATH_RULES: &[PathRule] = &[
    ...
    PathRule {
        path: "crates/avutil/tests/bitreader_oracle.rs",
        exact_ids: &["avutil-bitreader"],
        id_prefixes: &[],
    },
    PathRule {
    ...
    PathRule {
        path: "crates/avutil/tests/bitwriter_oracle.rs",
        exact_ids: &["avutil-bitwriter"],
        id_prefixes: &[],
    },
    PathRule {
    ...
    PathRule {
        path: "crates/avutil/tests/options_oracle.rs",
        exact_ids: &["avutil-options"],
        id_prefixes: &[],
    },
    PathRule {
    ...
    PathRule {
        ...
        exact_ids: &[
            ...
            "fftools-ffmpeg-yuv4mpegpipe-framecrc-null",
            "avutil-packet",
...
mod tests {
    ...
    fn changed_selection_maps_yuv4mpegpipe_oracle_test_to_covered_components() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "fftools-ffmpeg-yuv4mpegpipe-framecrc-null".to_string(),
                "avutil-packet".to_string(),
    ...
    fn changed_selection_maps_image2_oracle_test_to_covered_components() {
        ...
        let paths = vec!["crates/fftools/tests/image2_oracle.rs".to_string()];
    ...
    #[test]
    fn changed_selection_maps_bitreader_oracle_test_to_component() {
        let component_ids = component_ids_from_ledger(&ledger(&["avutil-bitreader"]));
        let paths = vec!["crates/avutil/tests/bitreader_oracle.rs".to_string()];

        assert_eq!(
            changed_components(&component_ids, &paths),
            vec!["avutil-bitreader".to_string()]
        );
    }

    #[test]
    fn changed_selection_maps_bitwriter_oracle_test_to_component() {
        let component_ids = component_ids_from_ledger(&ledger(&["avutil-bitwriter"]));
        let paths = vec!["crates/avutil/tests/bitwriter_oracle.rs".to_string()];
    ...
    #[test]
    fn changed_selection_maps_options_oracle_test_to_component() {
        let component_ids = component_ids_from_ledger(&ledger(&["avutil-options"]));
        let paths = vec!["crates/avutil/tests/options_oracle.rs".to_string()];

        assert_eq!(
            changed_components(&component_ids, &paths),
            vec!["avutil-options".to_string()]
        );
    }
    ...
    fn differential_mappings_parse_against_current_ledger() {
        ...
        assert!(pairs.contains(&("avutil-packet", "oracle-yuv4mpegpipe-framecrc-records")));
        assert!(pairs.contains(&(
            "fftools-ffmpeg-image2-file-output",
