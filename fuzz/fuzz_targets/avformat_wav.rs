...
fuzz_target!(|data: &[u8]| {
    ...
    let duplicate_fmt_unsupported = wav_with_duplicate_fmt_chunks_with_unsupported_second();
    exercise_wav(&duplicate_fmt_unsupported);
    exercise_duplicate_fmt_with_unsupported_second_fmt_wav(&duplicate_fmt_unsupported);
...
fn exercise_duplicate_fmt_with_unsupported_second_fmt_wav(input: &[u8]) {
    let mut demuxer =
        WavDemuxer::open(input).expect("duplicate fmt WAV with unsupported second fmt should open");
    let info = demuxer.info().clone();
    ...
    let packet = demuxer
        .read_packet()
        .expect("duplicate fmt WAV with unsupported second fmt should yield a packet")
        .expect("duplicate fmt WAV with unsupported second fmt should produce one packet");
...
fn wav_with_duplicate_fmt_chunks_with_unsupported_second() -> Vec<u8> {
    let first_channels: u16 = 1;
    let first_sample_rate: u32 = 44_100;
    let second_channels: u16 = 1;
    let second_sample_rate: u32 = 48_000;
    let block_align = first_channels * 2u16;
    let byte_rate = first_sample_rate * u32::from(block_align);
    let data: &[u8] = &[0x00, 0x00, 0x01, 0x00];
