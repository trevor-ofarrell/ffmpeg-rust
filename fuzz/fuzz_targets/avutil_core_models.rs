...
fn exercise_pixel_and_video_frame(cursor: &mut Cursor<'_>) {
    ...
    match pixel_format.class() {
        PixelFormatClass::Gray => {
            ...
            match pixel_format {
                ...
                PixelFormat::Yaf16Le | PixelFormat::Yaf16Be => {
                    assert_eq!(pixel_format.component_count(), 2);
                    assert_eq!(pixel_format.bits_per_component(), 16);
                    assert_eq!(pixel_format.bits_per_pixel(), bpp(32));
                    assert!(pixel_format.has_alpha());
                    assert!(pixel_format.is_float());
                }
                PixelFormat::Yaf32Le | PixelFormat::Yaf32Be => {
                    assert_eq!(pixel_format.component_count(), 2);
                    assert_eq!(pixel_format.bits_per_component(), 32);
                    assert_eq!(pixel_format.bits_per_pixel(), bpp(64));
                    assert!(pixel_format.has_alpha());
                    assert!(pixel_format.is_float());
                }
                _ => {
        ...
        PixelFormatClass::Xyz => {
            ...
            assert_eq!(pixel_format.bits_per_component(), 12);
            assert_eq!(pixel_format.bits_per_pixel(), bpp(36));
            assert_eq!(pixel_format.log2_chroma(), (0, 0));
            assert!(!pixel_format.has_alpha());
            assert!(!pixel_format.has_chroma_subsampling());
            assert_eq!(pixel_format.packed_bytes_per_pixel(), Some(6));
        }
    ...
    let Ok(plane_sizes) = pixel_format.plane_sizes(width, height) else {
        assert!(
            width == 0
                || height == 0
                || (pixel_format.is_yuv() && pixel_format.has_chroma_subsampling())
        );
        return;
    };
...
fn exercise_packet_and_hashes(cursor: &mut Cursor<'_>) {
    ...
    let mut zero_duration_rescale_packet = Packet::from_data(vec![0xee]).unwrap();
    zero_duration_rescale_packet.set_pts(Some(180_000));
    zero_duration_rescale_packet.set_dts(Some(90_000));
    zero_duration_rescale_packet.set_pos(Some(789)).unwrap();
    zero_duration_rescale_packet
        .set_time_base(rescale_src)
        .unwrap();
    zero_duration_rescale_packet.set_flag(PacketFlags::TRUSTED, true);
    zero_duration_rescale_packet
        .rescale_ts(rescale_src, rescale_dst)
        .unwrap();
    assert_eq!(zero_duration_rescale_packet.pts(), Some(2_000));
    assert_eq!(zero_duration_rescale_packet.dts(), Some(1_000));
    assert_eq!(zero_duration_rescale_packet.duration(), 0);
    assert_eq!(zero_duration_rescale_packet.pos(), Some(789));
    assert_eq!(zero_duration_rescale_packet.time_base(), rescale_src);
    assert!(zero_duration_rescale_packet
        .flags()
    ...
    let opaque_address = usize::from(cursor.next().unwrap_or_default()) + 1;
    let packet_opaque = PacketOpaque::new(opaque_address).unwrap();
    assert_eq!(packet_opaque.address(), opaque_address);
    assert_eq!(packet_opaque.nonzero_address().get(), opaque_address);
    assert_eq!(
        PacketOpaque::from_nonzero(packet_opaque.nonzero_address()),
        packet_opaque
    );
    assert_eq!(
        PacketOpaque::from_address(opaque_address),
        Some(packet_opaque)
    );
    packet.set_opaque(Some(packet_opaque));
    assert_eq!(packet.opaque(), Some(packet_opaque));
    assert_eq!(packet.opaque_address(), Some(opaque_address));
    packet.set_opaque_address(0);
    assert!(packet.opaque().is_none());
    packet.set_opaque_address(opaque_address);
    assert_eq!(packet.opaque(), Some(packet_opaque));
    ...
    packet_fifo.write_move(&mut fifo_move_replace_src).unwrap();
    ...
    assert_eq!(
        ...
        fifo_move_replace_payload.as_slice()
    );
    assert!(fifo_move_replace_dst
        .data_buffer()
        .shares_storage(&fifo_move_replace_storage));
    assert!(!fifo_move_replace_dst
        .data_buffer()
        .shares_storage(&fifo_move_replace_old_storage));
    assert!(fifo_move_replace_dst
        .opaque_ref()
        .unwrap()
        .shares_storage(&fifo_move_replace_opaque));
    assert!(!fifo_move_replace_dst
        .opaque_ref()
        .unwrap()
    ...
    packet_fifo.clear();
    assert!(packet_fifo.is_empty());
    let fifo_empty_peek = packet_fifo.peek(0).unwrap_err();
    assert_eq!(fifo_empty_peek.kind(), AvErrorKind::InvalidArgument);
    assert_eq!(fifo_empty_peek.code(), Some(AvErrorCode::EINVAL));
    ...
    let mut fifo_empty_move_preserve =
        Packet::from_data(vec![cursor.next().unwrap_or_default()]).unwrap();
    fifo_empty_move_preserve
        .push_side_data(SideData::new("skip_samples", vec![0x09, 0x0a]).unwrap());
    fifo_empty_move_preserve.set_opaque_ref(Some(BufferRef::from_vec(vec![0x0b, 0x0c])));
    let fifo_empty_move_payload = fifo_empty_move_preserve.data().to_vec();
    let fifo_empty_move_storage = fifo_empty_move_preserve.data_buffer().clone();
    let fifo_empty_move_opaque = fifo_empty_move_preserve.opaque_ref().unwrap().clone();
    let fifo_empty_move_err = packet_fifo
        .read_move(&mut fifo_empty_move_preserve)
        .unwrap_err();
    assert_eq!(fifo_empty_move_err.kind(), AvErrorKind::InvalidArgument);
    assert_eq!(fifo_empty_move_err.code(), Some(AvErrorCode::EINVAL));
    assert_eq!(
        fifo_empty_move_preserve.data(),
        fifo_empty_move_payload.as_slice()
    );
    assert!(fifo_empty_move_preserve
        .data_buffer()
        .shares_storage(&fifo_empty_move_storage));
    assert!(fifo_empty_move_preserve
        .opaque_ref()
        .unwrap()
        .shares_storage(&fifo_empty_move_opaque));
    assert_eq!(
        fifo_empty_move_preserve
            .side_data_by_kind("skip_samples")
            .unwrap()
    ...
    let mut fifo_empty_ref_preserve =
        Packet::from_data(vec![cursor.next().unwrap_or_default()]).unwrap();
    fifo_empty_ref_preserve
        .push_side_data(SideData::new("skip_samples", vec![0x0d, 0x0e]).unwrap());
    fifo_empty_ref_preserve.set_opaque_ref(Some(BufferRef::from_vec(vec![0x0f, 0x10])));
    let fifo_empty_ref_payload = fifo_empty_ref_preserve.data().to_vec();
    let fifo_empty_ref_storage = fifo_empty_ref_preserve.data_buffer().clone();
    let fifo_empty_ref_opaque = fifo_empty_ref_preserve.opaque_ref().unwrap().clone();
    let fifo_empty_ref_err = packet_fifo
        .read_ref(&mut fifo_empty_ref_preserve)
        .unwrap_err();
    assert_eq!(fifo_empty_ref_err.kind(), AvErrorKind::InvalidArgument);
    assert_eq!(fifo_empty_ref_err.code(), Some(AvErrorCode::EINVAL));
    assert_eq!(
        fifo_empty_ref_preserve.data(),
        fifo_empty_ref_payload.as_slice()
    );
    assert!(fifo_empty_ref_preserve
        .data_buffer()
        .shares_storage(&fifo_empty_ref_storage));
    assert!(fifo_empty_ref_preserve
        .opaque_ref()
        .unwrap()
        .shares_storage(&fifo_empty_ref_opaque));
    assert_eq!(
        fifo_empty_ref_preserve
            .side_data_by_kind("skip_samples")
            .unwrap()
    ...
    duplicate_bridge_list
        ...
        .unwrap();
    assert_eq!(duplicate_bridge_list.len(), 3);
    assert_eq!(
        duplicate_bridge_list.entries()[0].kind_id(),
        &PacketSideDataKind::ReplayGain
    );
    assert_eq!(
        duplicate_bridge_list.entries()[0].data(),
        bridge_replacement.as_slice()
    );
    assert_eq!(
        duplicate_bridge_list.entries()[1].kind_id(),
        &PacketSideDataKind::DisplayMatrix
    );
    assert_eq!(duplicate_bridge_list.entries()[1].data(), &[0x22]);
    assert_eq!(
        duplicate_bridge_list.entries()[2].kind_id(),
        &PacketSideDataKind::ReplayGain
    );
    assert_eq!(duplicate_bridge_list.entries()[2].data(), &[0x33]);

    let mut combined_flag_bridge_list = PacketSideDataList::from_entries(vec![
        SideData::new_with_kind(PacketSideDataKind::ReplayGain, vec![0x11]).unwrap(),
        SideData::new_with_kind(PacketSideDataKind::DisplayMatrix, vec![0x22]).unwrap(),
        SideData::new_with_kind(PacketSideDataKind::ReplayGain, vec![0x33]).unwrap(),
    ]);
    combined_flag_bridge_list
        .add_from_frame_side_data_with_flags(
            &FrameSideData::new_with_kind(
                FrameSideDataKind::ReplayGain,
                bridge_replacement.clone(),
        ...
        .unwrap();
    assert_eq!(combined_flag_bridge_list.len(), 3);
    assert_eq!(
        combined_flag_bridge_list.entries()[0].kind_id(),
        &PacketSideDataKind::ReplayGain
    );
    assert_eq!(
        combined_flag_bridge_list.entries()[0].data(),
        bridge_replacement.as_slice()
    );
    assert_eq!(
        combined_flag_bridge_list.entries()[1].kind_id(),
        &PacketSideDataKind::DisplayMatrix
    );
    assert_eq!(combined_flag_bridge_list.entries()[1].data(), &[0x22]);
    assert_eq!(
        combined_flag_bridge_list.entries()[2].kind_id(),
        &PacketSideDataKind::ReplayGain
    );
    assert_eq!(combined_flag_bridge_list.entries()[2].data(), &[0x33]);
    ...
    let mut combined_flag_bridge_frame = Frame::default();
    combined_flag_bridge_frame
        .add_side_data_with_flags(
            FrameSideData::new_with_kind(FrameSideDataKind::ReplayGain, vec![0x11]).unwrap(),
            FrameSideDataFlags::EMPTY,
        )
    ...
    SideData::new_with_kind(PacketSideDataKind::ReplayGain, bridge_replacement.clone())
        .unwrap()
        .add_to_frame(
            &mut combined_flag_bridge_frame,
            FrameSideDataFlags::UNIQUE.union(FrameSideDataFlags::REPLACE),
        )
        .unwrap();
    assert_eq!(combined_flag_bridge_frame.side_data().len(), 2);
    assert_eq!(
        combined_flag_bridge_frame.side_data()[0].kind_id(),
        &FrameSideDataKind::DisplayMatrix
    );
    assert_eq!(combined_flag_bridge_frame.side_data()[0].data(), &[0x22]);
    assert_eq!(
        combined_flag_bridge_frame.side_data()[1].kind_id(),
        &FrameSideDataKind::ReplayGain
    );
    assert_eq!(
        combined_flag_bridge_frame.side_data()[1].data(),
        bridge_replacement.as_slice()
    );
    ...
    assert!(packet_ref.opaque().is_none());
    ...
    assert!(moved_packet
        ...
        .unwrap()
        .shares_storage(packet.opaque_ref().unwrap()));
    assert_eq!(moved_packet.opaque(), packet.opaque());
    moved_packet.unref();
    ...
    assert!(moved_packet.opaque().is_none());
    ...
    let taken_opaque_address = props_packet.take_opaque().unwrap();
    assert_eq!(taken_opaque_address, packet_opaque);
    assert!(props_packet.opaque().is_none());
    assert_eq!(packet.opaque(), Some(packet_opaque));
...
fn exercise_fixtures() {
    ...
    for (name, alias, format, bits) in [
        ("gray9le", "y9le", PixelFormat::Gray9Le, 9),
        ("gray9be", "y9be", PixelFormat::Gray9Be, 9),
        ("gray10le", "y10le", PixelFormat::Gray10Le, 10),
        ("gray10be", "y10be", PixelFormat::Gray10Be, 10),
        ("gray12le", "y12le", PixelFormat::Gray12Le, 12),
        ("gray12be", "y12be", PixelFormat::Gray12Be, 12),
        ("gray14le", "y14le", PixelFormat::Gray14Le, 14),
        ("gray14be", "y14be", PixelFormat::Gray14Be, 14),
    ] {
    ...
    assert_eq!(PixelFormat::Yaf32Le.frame_size(2, 2).unwrap(), 32);
    ...
    assert_eq!(PixelFormat::Gbrp10MsbLe.frame_size(2, 2).unwrap(), 24);
    assert_eq!(
        PixelFormat::Gbrp10MsbLe.plane_sizes(2, 2).unwrap(),
        vec![8, 8, 8]
    );
    assert_eq!(PixelFormat::Gbrp10MsbLe.bits_per_component(), 10);
    assert_eq!(PixelFormat::Gbrp10MsbLe.bits_per_pixel(), bpp(30));
    assert!(PixelFormat::Gbrp10MsbLe.is_rgb());
    assert!(PixelFormat::Gbrp10MsbLe.is_planar());
    assert!(!PixelFormat::Gbrp10MsbLe.has_alpha());
    assert_eq!(
    ...
    assert_eq!(PixelFormat::Gbrp12Le.frame_size(2, 2).unwrap(), 24);
    assert_eq!(
        PixelFormat::Gbrp12Le.plane_sizes(2, 2).unwrap(),
        vec![8, 8, 8]
    );
    ...
    assert_eq!(PixelFormat::Gbrp12MsbLe.frame_size(2, 2).unwrap(), 24);
    assert_eq!(
        PixelFormat::Gbrp12MsbLe.plane_sizes(2, 2).unwrap(),
        vec![8, 8, 8]
    );
    assert_eq!(PixelFormat::Gbrp12MsbLe.bits_per_component(), 12);
    assert_eq!(PixelFormat::Gbrp12MsbLe.bits_per_pixel(), bpp(36));
    assert!(PixelFormat::Gbrp12MsbLe.is_rgb());
    assert!(PixelFormat::Gbrp12MsbLe.is_planar());
    assert!(!PixelFormat::Gbrp12MsbLe.has_alpha());
    assert_eq!(
    ...
    assert_eq!(PixelFormat::Gbrp14Le.frame_size(2, 2).unwrap(), 24);
    assert_eq!(
        PixelFormat::Gbrp14Le.plane_sizes(2, 2).unwrap(),
        vec![8, 8, 8]
    );
    ...
    assert_eq!(PixelFormat::Gbrp16Le.frame_size(2, 2).unwrap(), 24);
    assert_eq!(
        PixelFormat::Gbrp16Le.plane_sizes(2, 2).unwrap(),
        vec![8, 8, 8]
    );
    ...
    for (name, format, bits_per_component, bits_per_pixel, packed_bytes_per_pixel, frame_size) in [
        ...
        ("v30xbe", PixelFormat::V30xBe, 10, 30, Some(4), 16),
    ] {
        let descriptor = format.descriptor();
        assert_eq!(PixelFormat::from_name(name), Some(format));
        assert_eq!(descriptor.name, name);
    ...
    for (name, format, bits_per_component, bits_per_pixel, packed_bytes_per_pixel, frame_size) in [
        ...
        ("rgbaf32be", PixelFormat::RgbaF32Be, 32, 128, Some(16), 64),
    ] {
        let descriptor = format.descriptor();
        assert_eq!(PixelFormat::from_name(name), Some(format));
        assert_eq!(descriptor.name, name);
    ...
    assert_eq!(
        PixelFormat::from_name("yuv420p14le"),
        Some(PixelFormat::Yuv420p14Le)
    );
    assert_eq!(
        PixelFormat::Yuv420p14Le.plane_sizes(2, 2).unwrap(),
        vec![8, 2, 2]
    );
    assert_eq!(PixelFormat::Yuv420p14Le.bits_per_component(), 14);
    assert_eq!(PixelFormat::Yuv420p14Le.bits_per_pixel(), bpp(21));
    assert_eq!(
        PixelFormat::Yuv422p12Be.plane_sizes(2, 3).unwrap(),
        vec![12, 6, 6]
    );
    assert_eq!(
        PixelFormat::from_name("yuv422p16be"),
        Some(PixelFormat::Yuv422p16Be)
    );
    assert_eq!(
        PixelFormat::Yuv422p16Be.plane_sizes(2, 3).unwrap(),
        vec![12, 6, 6]
    );
    assert_eq!(PixelFormat::Yuv422p16Be.bits_per_component(), 16);
    assert_eq!(PixelFormat::Yuv422p16Be.bits_per_pixel(), bpp(32));
    assert_eq!(
        PixelFormat::Yuv411p.plane_sizes(4, 3).unwrap(),
        vec![12, 3, 3]
    );
    for (name, format, bits_per_pixel, log2_chroma, width, height, expected) in [
        (
            "yuvj420p",
            PixelFormat::YuvJ420p,
            bpp(12),
        ...
        assert_eq!(PixelFormat::from_name(name), Some(format));
        assert_eq!(format.bits_per_pixel(), bits_per_pixel);
    ...
    assert_eq!(
        PixelFormat::from_name("yuv444p10msble"),
        Some(PixelFormat::Yuv444p10MsbLe)
    );
    assert_eq!(PixelFormat::Yuv444p10MsbLe.bits_per_component(), 10);
    assert_eq!(PixelFormat::Yuv444p10MsbLe.bits_per_pixel(), bpp(30));
    assert_eq!(PixelFormat::Yuv444p10MsbLe.log2_chroma(), (0, 0));
    assert_eq!(
        PixelFormat::Yuv444p10MsbLe.plane_sizes(3, 2).unwrap(),
        vec![12, 12, 12]
    );
    assert_eq!(
        PixelFormat::from_name("yuv444p12msbbe"),
        Some(PixelFormat::Yuv444p12MsbBe)
    );
    assert_eq!(PixelFormat::Yuv444p12MsbBe.bits_per_component(), 12);
    assert_eq!(PixelFormat::Yuv444p12MsbBe.bits_per_pixel(), bpp(36));
    assert_eq!(PixelFormat::Yuv444p12MsbBe.log2_chroma(), (0, 0));
    assert_eq!(
    ...
    assert_eq!(
        PixelFormat::from_name("xyz12le"),
        Some(PixelFormat::Xyz12Le)
    );
    assert_eq!(PixelFormat::Xyz12Le.class(), PixelFormatClass::Xyz);
    ...
    let mut rgb_crop_storage = vec![0; 192 * 4];
    for row in 0..4 {
        for column in 0..24 {
            rgb_crop_storage[row * 192 + column] = (row * 16 + column) as u8;
        }
    }
    let mut rgb_aligned_crop = Frame::video(
        VideoFrame::new_with_line_sizes(
            8,
            4,
            PixelFormat::Rgb24,
    ...
    rgb_aligned_crop.set_crop_offsets(1, 0, 1, 1);
    rgb_aligned_crop
        .apply_cropping(FrameCropFlags::NONE)
        .unwrap();
    let FrameData::Video(rgb_aligned_crop_video) = rgb_aligned_crop.data() else {
        panic!("constructed rgb crop frame changed variant");
    };
    ...
    rgb_unaligned_crop.set_crop_offsets(1, 0, 1, 1);
    rgb_unaligned_crop
        .apply_cropping(FrameCropFlags::UNALIGNED)
        .unwrap();
    let FrameData::Video(rgb_unaligned_crop_video) = rgb_unaligned_crop.data() else {
        panic!("constructed rgb crop frame changed variant");
    };
    assert_eq!(rgb_unaligned_crop_video.width(), 6);
    assert_eq!(rgb_unaligned_crop_video.height(), 3);
    assert_eq!(
        &rgb_unaligned_crop_video.planes()[0][..6],
        &[19, 20, 21, 22, 23, 24]
    );
    ...
    let make_planar_alpha_sample_storage =
        |log2_chroma_w: usize, log2_chroma_h: usize, sample_bytes: usize| {
            let mut storage =
                make_planar_yuv_sample_storage(log2_chroma_w, log2_chroma_h, sample_bytes);
            let mut alpha = vec![0; 64 * 4];
            for row in 0..4 {
                for column in 0..8 * sample_bytes {
                    alpha[row * 64 + column] = 0xe0_u8.wrapping_add((row * 16 + column) as u8);
                }
            }
            storage.push(alpha);
            storage
        };
    let make_planar_alpha_storage = |log2_chroma_w: usize, log2_chroma_h: usize| {
        make_planar_alpha_sample_storage(log2_chroma_w, log2_chroma_h, 1)
    };
    ...
    let mut yuv420p_odd_default_crop = Frame::video(
        VideoFrame::new_with_line_sizes(
            8,
            4,
            PixelFormat::Yuv420p,
    ...
    yuv420p_odd_default_crop.set_crop_offsets(1, 0, 1, 1);
    yuv420p_odd_default_crop
        .apply_cropping(FrameCropFlags::NONE)
        .unwrap();
    let FrameData::Video(yuv420p_odd_default_crop_video) = yuv420p_odd_default_crop.data() else {
        panic!("constructed odd yuv420p default crop frame changed variant");
    };
    assert_eq!(yuv420p_odd_default_crop.crop(), FrameCrop::default());
    assert_eq!(yuv420p_odd_default_crop_video.width(), 7);
    assert_eq!(yuv420p_odd_default_crop_video.height(), 3);
    assert_eq!(
        &yuv420p_odd_default_crop_video.planes()[0][..7],
        &[0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26]
    );
    assert_eq!(
        yuv420p_odd_default_crop_video.planes()[1],
        vec![0x80, 0x81, 0x82]
    );
    assert_eq!(
        yuv420p_odd_default_crop_video.planes()[2],
        vec![0xc0, 0xc1, 0xc2]
    );

    let mut yuv420p_odd_unaligned_crop = Frame::video(
        VideoFrame::new_with_line_sizes(
            8,
            4,
            PixelFormat::Yuv420p,
    ...
    yuv420p_odd_unaligned_crop.set_crop_offsets(1, 0, 1, 1);
    ...
    assert_eq!(yuv420p_odd_unaligned_crop.crop(), FrameCrop::default());
    assert_eq!(yuv420p_odd_unaligned_crop_video.width(), 6);
    assert_eq!(yuv420p_odd_unaligned_crop_video.height(), 3);
    assert_eq!(
        &yuv420p_odd_unaligned_crop_video.planes()[0][..6],
        &[0x21, 0x22, 0x23, 0x24, 0x25, 0x26]
    );
    assert_eq!(
        yuv420p_odd_unaligned_crop_video.planes()[1],
        vec![0x80, 0x81, 0x82]
    );
    assert_eq!(
    ...
    for (format, log2_chroma_w, log2_chroma_h) in [
        ...
        let planar_storage = make_planar_alpha_storage(log2_chroma_w, log2_chroma_h);
        let mut planar_default_crop = Frame::video(
            VideoFrame::new_with_line_sizes(
                8,
                4,
        ...
        planar_default_crop
            ...
            .unwrap();
        let FrameData::Video(planar_default_crop_video) = planar_default_crop.data() else {
            panic!("constructed planar alpha default crop frame changed variant");
        };
        assert_eq!(planar_default_crop.crop(), FrameCrop::default());
        assert_eq!(planar_default_crop_video.width(), 7);
        assert_eq!(planar_default_crop_video.height(), 3);
        assert_eq!(planar_default_crop_video.line_sizes(), &[64, 64, 64, 64]);
        assert_eq!(
            &planar_default_crop_video.planes()[0][..7],
            &[0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26]
        );
        ...
        assert_eq!(
            &planar_default_crop_video.planes()[3][..7],
            &[0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6]
        );

        let mut planar_unaligned_crop = Frame::video(
            VideoFrame::new_with_line_sizes(8, 4, format, planar_storage, vec![64, 64, 64, 64])
                .unwrap(),
        );
        planar_unaligned_crop.set_crop_offsets(1, 0, 1, 1);
        planar_unaligned_crop
            ...
            .unwrap();
        let FrameData::Video(planar_unaligned_crop_video) = planar_unaligned_crop.data() else {
            panic!("constructed planar alpha unaligned crop frame changed variant");
        };
        assert_eq!(planar_unaligned_crop.crop(), FrameCrop::default());
        assert_eq!(planar_unaligned_crop_video.width(), 6);
        assert_eq!(planar_unaligned_crop_video.height(), 3);
        assert_eq!(planar_unaligned_crop_video.line_sizes(), &[64, 64, 64, 64]);
        assert_eq!(
            &planar_unaligned_crop_video.planes()[0][..6],
            &[0x21, 0x22, 0x23, 0x24, 0x25, 0x26]
        );
        ...
        assert_eq!(
            &planar_unaligned_crop_video.planes()[3][..6],
    ...
    for (format, log2_chroma_w, log2_chroma_h, sample_bytes) in [
        ...
        let planar_storage =
            make_planar_alpha_sample_storage(log2_chroma_w, log2_chroma_h, sample_bytes);
        let mut planar_default_crop = Frame::video(
            VideoFrame::new_with_line_sizes(
                8,
                4,
        ...
        planar_default_crop.set_crop_offsets(1, 0, 1, 1);
        let planar_before = planar_default_crop.clone();
        let default_error = planar_default_crop
            .apply_cropping(FrameCropFlags::NONE)
            .unwrap_err();
        assert_eq!(
        ...
        assert_eq!(planar_default_crop, planar_before);
        ...
        planar_unaligned_crop
            ...
            .unwrap();
        let FrameData::Video(planar_unaligned_crop_video) = planar_unaligned_crop.data() else {
            panic!("constructed high-bit planar alpha unaligned crop frame changed variant");
        };
        assert_eq!(planar_unaligned_crop.crop(), FrameCrop::default());
        assert_eq!(planar_unaligned_crop_video.width(), 6);
        assert_eq!(planar_unaligned_crop_video.height(), 3);
        ...
        assert_eq!(
            &planar_unaligned_crop_video.planes()[3][..6 * sample_bytes],
            &(0..6 * sample_bytes)
                .map(|column| {
                    0xf0_u8
                        .wrapping_add(sample_bytes as u8)
    ...
    for (format, log2_chroma_w, log2_chroma_h) in [
        ...
        assert_eq!(semiplanar_unaligned_crop.crop(), FrameCrop::default());
        let FrameData::Video(semiplanar_unaligned_video) = semiplanar_unaligned_crop.data() else {
            panic!("constructed high-bit semi-planar unaligned crop frame changed variant");
        };
        assert_eq!(semiplanar_unaligned_video.width(), 6);
        assert_eq!(semiplanar_unaligned_video.height(), 3);
        assert_eq!(
        ...
        let mut semiplanar_even_default_crop = Frame::video(
            VideoFrame::new_with_line_sizes(8, 4, format, storage.clone(), line_sizes.clone())
                .unwrap(),
        );
        semiplanar_even_default_crop.set_crop_offsets(2, 0, 2, 2);
        ...
        semiplanar_even_unaligned_crop
            .apply_cropping(FrameCropFlags::UNALIGNED)
            .unwrap();
        assert_eq!(semiplanar_even_unaligned_crop.crop(), FrameCrop::default());
        let FrameData::Video(semiplanar_even_video) = semiplanar_even_unaligned_crop.data() else {
            panic!("constructed high-bit semi-planar even crop frame changed variant");
        };
        assert_eq!(semiplanar_even_video.width(), 4);
        assert_eq!(semiplanar_even_video.height(), 2);
        assert_eq!(
            semiplanar_even_video.planes()[0].as_slice(),
            plane_visible_sample_bytes(0x10, 2, 2, 4, 2, 2).as_slice()
        );
        assert_eq!(
            semiplanar_even_video.planes()[1].as_slice(),
            plane_visible_sample_bytes(
...
fn channel_layout_from(byte: Option<u8>) -> ChannelLayout {
    match byte.unwrap_or_default() % 40 {
        ...
        16 => ChannelLayout::three_one_two(),
        17 => ChannelLayout::hexagonal(),
        18 => ChannelLayout::six_one(),
        19 => ChannelLayout::six_one_back(),
        20 => ChannelLayout::six_one_front(),
        21 => ChannelLayout::seven_zero(),
        22 => ChannelLayout::seven_zero_front(),
        23 => ChannelLayout::seven_one(),
        24 => ChannelLayout::seven_one_wide(),
        25 => ChannelLayout::seven_one_wide_side(),
        26 => ChannelLayout::five_one_two(),
        27 => ChannelLayout::five_one_two_back(),
        28 => ChannelLayout::octagonal(),
        29 => ChannelLayout::cube(),
        30 => ChannelLayout::five_one_four(),
        31 => ChannelLayout::seven_one_two(),
        32 => ChannelLayout::seven_one_four(),
        33 => ChannelLayout::seven_two_three(),
        34 => ChannelLayout::nine_one_four(),
        35 => ChannelLayout::nine_one_six(),
        36 => ChannelLayout::hexadecagonal(),
        37 => ChannelLayout::binaural(),
        38 => ChannelLayout::downmix(),
        _ => ChannelLayout::twenty_two_two(),
    }
}
...
fn expected_video_line_sizes(pixel_format: PixelFormat, width: usize) -> Vec<usize> {
    match pixel_format {
        ...
        | PixelFormat::Gray14Be => vec![width * 2],
        ...
        PixelFormat::Yaf32Le | PixelFormat::Yaf32Be => vec![width * 8],
        ...
        | PixelFormat::Ayuv64Be
        | PixelFormat::Xv48Le
        | PixelFormat::Xv48Be => vec![width * 8],
        PixelFormat::Rgba
        | PixelFormat::Bgra
        | PixelFormat::Argb
        | PixelFormat::Abgr
...
fn expected_video_plane_shapes(
    ...
    match pixel_format {
        ...
        | PixelFormat::Gray12Be
        | PixelFormat::Gray14Le
        | PixelFormat::Gray14Be => vec![(width * 2, height)],
        ...
        PixelFormat::Yaf32Le | PixelFormat::Yaf32Be => vec![(width * 8, height)],
        ...
        | PixelFormat::Ayuv64Be
        | PixelFormat::Xv48Le
        | PixelFormat::Xv48Be => vec![(width * 8, height)],
        PixelFormat::Rgba
        | PixelFormat::Bgra
        | PixelFormat::Argb
        | PixelFormat::Abgr
