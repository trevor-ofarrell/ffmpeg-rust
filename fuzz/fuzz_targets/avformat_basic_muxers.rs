...
fn exercise_fixtures() {
    ...
    let mut raw_gray16 =
        RawVideoMuxer::new(2, 1, PixelFormat::Gray16Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_gray16
    ...
    let mut raw_gray10 =
        RawVideoMuxer::new(2, 1, PixelFormat::Gray10Le, Rational::new(24, 1).unwrap()).unwrap();
    raw_gray10
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    ...
    let mut raw_gray32 =
        RawVideoMuxer::new(2, 1, PixelFormat::Gray32Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_gray32
    ...
    let mut raw_grayf16 =
        RawVideoMuxer::new(2, 1, PixelFormat::GrayF16Le, Rational::new(24, 1).unwrap()).unwrap();
    raw_grayf16
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    ...
    let mut raw_grayf32 =
        RawVideoMuxer::new(2, 1, PixelFormat::GrayF32Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_grayf32
        .write_packet(&Packet::new((0..8).collect(), 0))
        .unwrap();
    ...
    assert!(raw_gbrp10_msb_demuxer.read_packet().unwrap().is_none());
    ...
    let mut raw_gbrpf16 =
        RawVideoMuxer::new(2, 1, PixelFormat::GbrpF16Le, Rational::new(24, 1).unwrap()).unwrap();
    raw_gbrpf16
        .write_packet(&Packet::new((0..12).collect(), 0))
        .unwrap();
    let raw_gbrpf16_output = raw_gbrpf16.finish();
    let mut raw_gbrpf16_demuxer = RawVideoDemuxer::open(
        &raw_gbrpf16_output,
        2,
        1,
    ...
    .unwrap();
    assert_eq!(
        raw_gbrpf16_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..12).collect::<Vec<_>>()
    );
    assert!(raw_gbrpf16_demuxer.read_packet().unwrap().is_none());

    let mut raw_gbrpf32 =
        RawVideoMuxer::new(1, 1, PixelFormat::GbrpF32Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_gbrpf32
        .write_packet(&Packet::new((0..12).collect(), 0))
        .unwrap();
    let raw_gbrpf32_output = raw_gbrpf32.finish();
    let mut raw_gbrpf32_demuxer = RawVideoDemuxer::open(
        &raw_gbrpf32_output,
        1,
        1,
    ...
    .unwrap();
    assert_eq!(
        raw_gbrpf32_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..12).collect::<Vec<_>>()
    );
    ...
    let mut raw_rgb96 =
        RawVideoMuxer::new(1, 1, PixelFormat::Rgb96Le, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgb96
        .write_packet(&Packet::new((0..12).collect(), 0))
        .unwrap();
    let raw_rgb96_output = raw_rgb96.finish();
    let mut raw_rgb96_demuxer = RawVideoDemuxer::open(
        &raw_rgb96_output,
        1,
        1,
        PixelFormat::Rgb96Le,
    ...
    .unwrap();
    assert_eq!(
        raw_rgb96_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..12).collect::<Vec<_>>()
    );
    assert!(raw_rgb96_demuxer.read_packet().unwrap().is_none());

    let mut raw_rgbaf16 =
        RawVideoMuxer::new(2, 1, PixelFormat::RgbaF16Le, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgbaf16
        .write_packet(&Packet::new((0..16).collect(), 0))
        .unwrap();
    let raw_rgbaf16_output = raw_rgbaf16.finish();
    let mut raw_rgbaf16_demuxer = RawVideoDemuxer::open(
        &raw_rgbaf16_output,
        2,
        1,
    ...
    .unwrap();
    assert_eq!(
        raw_rgbaf16_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..16).collect::<Vec<_>>()
    );
    assert!(raw_rgbaf16_demuxer.read_packet().unwrap().is_none());

    let mut raw_rgbaf32 =
        RawVideoMuxer::new(1, 1, PixelFormat::RgbaF32Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgbaf32
        .write_packet(&Packet::new((0..16).collect(), 0))
        .unwrap();
    let raw_rgbaf32_output = raw_rgbaf32.finish();
    let mut raw_rgbaf32_demuxer = RawVideoDemuxer::open(
        &raw_rgbaf32_output,
        1,
        1,
    ...
    .unwrap();
    assert_eq!(
        raw_rgbaf32_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..16).collect::<Vec<_>>()
    );
    assert!(raw_rgbaf32_demuxer.read_packet().unwrap().is_none());

    let mut raw_rgba128 =
        RawVideoMuxer::new(1, 1, PixelFormat::Rgba128Be, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgba128
        .write_packet(&Packet::new((0..16).collect(), 0))
        .unwrap();
    let raw_rgba128_output = raw_rgba128.finish();
    let mut raw_rgba128_demuxer = RawVideoDemuxer::open(
        &raw_rgba128_output,
        1,
        1,
    ...
    .unwrap();
    assert_eq!(
        raw_rgba128_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..16).collect::<Vec<_>>()
    );
    assert!(raw_rgba128_demuxer.read_packet().unwrap().is_none());
    ...
    let mut raw_rgb8 =
        RawVideoMuxer::new(2, 2, PixelFormat::Rgb8, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgb8
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    let raw_rgb8_output = raw_rgb8.finish();
    let mut raw_rgb8_demuxer = RawVideoDemuxer::open(
        &raw_rgb8_output,
        2,
        2,
        PixelFormat::Rgb8,
    ...
    .unwrap();
    assert_eq!(
        raw_rgb8_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..4).collect::<Vec<_>>()
    );
    assert!(raw_rgb8_demuxer.read_packet().unwrap().is_none());

    let mut raw_pal8 =
        RawVideoMuxer::new(2, 2, PixelFormat::Pal8, Rational::new(24, 1).unwrap()).unwrap();
    raw_pal8
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    let raw_pal8_output = raw_pal8.finish();
    let mut raw_pal8_demuxer = RawVideoDemuxer::open(
        &raw_pal8_output,
        2,
        2,
        PixelFormat::Pal8,
    ...
    .unwrap();
    assert_eq!(
        raw_pal8_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..4).collect::<Vec<_>>()
    );
    assert!(raw_pal8_demuxer.read_packet().unwrap().is_none());

    let mut raw_rgb4 =
        RawVideoMuxer::new(3, 2, PixelFormat::Rgb4, Rational::new(24, 1).unwrap()).unwrap();
    raw_rgb4
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    let raw_rgb4_output = raw_rgb4.finish();
    let mut raw_rgb4_demuxer = RawVideoDemuxer::open(
        &raw_rgb4_output,
        3,
        2,
        PixelFormat::Rgb4,
    ...
    .unwrap();
    assert_eq!(
        raw_rgb4_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..4).collect::<Vec<_>>()
    );
    assert!(raw_rgb4_demuxer.read_packet().unwrap().is_none());

    let mut raw_monow =
        RawVideoMuxer::new(9, 2, PixelFormat::MonoWhite, Rational::new(24, 1).unwrap()).unwrap();
    raw_monow
        .write_packet(&Packet::new((0..4).collect(), 0))
        .unwrap();
    let raw_monow_output = raw_monow.finish();
    let mut raw_monow_demuxer = RawVideoDemuxer::open(
        &raw_monow_output,
        9,
        2,
    ...
    .unwrap();
    assert_eq!(
        raw_monow_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..4).collect::<Vec<_>>()
    );
    assert!(raw_monow_demuxer.read_packet().unwrap().is_none());
    ...
    for (format, payload) in [
        (PixelFormat::Y210Le, (0..16).collect::<Vec<_>>()),
        (PixelFormat::Y212Be, (16..32).collect::<Vec<_>>()),
        (PixelFormat::Y216Le, (32..48).collect::<Vec<_>>()),
    ] {
        let mut raw_yuv_packed =
            RawVideoMuxer::new(2, 2, format, Rational::new(24, 1).unwrap()).unwrap();
        raw_yuv_packed
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_yuv_packed_output = raw_yuv_packed.finish();
        let mut raw_yuv_packed_demuxer = RawVideoDemuxer::open(
            &raw_yuv_packed_output,
            2,
            2,
            format,
            Rational::new(24, 1).unwrap(),
    ...
    for (format, payload) in [
        (PixelFormat::Xyz12Le, (64..70).collect::<Vec<_>>()),
        (PixelFormat::Xyz12Be, (70..76).collect::<Vec<_>>()),
    ] {
        let mut raw_xyz12 =
            RawVideoMuxer::new(1, 1, format, Rational::new(24, 1).unwrap()).unwrap();
        raw_xyz12
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_xyz12_output = raw_xyz12.finish();
        let mut raw_xyz12_demuxer = RawVideoDemuxer::open(
            &raw_xyz12_output,
            1,
            1,
            format,
        ...
        .unwrap();
        assert_eq!(
            raw_xyz12_demuxer.read_packet().unwrap().unwrap().data(),
            payload.as_slice()
        );
        assert!(raw_xyz12_demuxer.read_packet().unwrap().is_none());
    }

    for (format, payload) in [
        (PixelFormat::X2Rgb10Le, (76..80).collect::<Vec<_>>()),
        (PixelFormat::X2Rgb10Be, (80..84).collect::<Vec<_>>()),
        (PixelFormat::X2Bgr10Le, (84..88).collect::<Vec<_>>()),
        (PixelFormat::X2Bgr10Be, (88..92).collect::<Vec<_>>()),
    ] {
        let mut raw_x2rgb10 =
            RawVideoMuxer::new(1, 1, format, Rational::new(24, 1).unwrap()).unwrap();
        raw_x2rgb10
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_x2rgb10_output = raw_x2rgb10.finish();
        let mut raw_x2rgb10_demuxer = RawVideoDemuxer::open(
            &raw_x2rgb10_output,
            1,
            1,
            format,
        ...
        .unwrap();
        assert_eq!(
            raw_x2rgb10_demuxer.read_packet().unwrap().unwrap().data(),
            payload.as_slice()
        );
    ...
    for (format, payload) in [
        ...
        let mut raw_xv = RawVideoMuxer::new(1, 1, format, Rational::new(24, 1).unwrap()).unwrap();
        raw_xv
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_xv_output = raw_xv.finish();
        let mut raw_xv_demuxer =
            RawVideoDemuxer::open(&raw_xv_output, 1, 1, format, Rational::new(24, 1).unwrap())
                .unwrap();
        assert_eq!(
    ...
    let mut raw_yuva420p9 = RawVideoMuxer::new(
        4,
        2,
    ...
    .unwrap();
    raw_yuva420p9
        .write_packet(&Packet::new((0..40).collect(), 0))
        .unwrap();
    let raw_yuva420p9_output = raw_yuva420p9.finish();
    let mut raw_yuva420p9_demuxer = RawVideoDemuxer::open(
        &raw_yuva420p9_output,
        4,
        2,
        PixelFormat::Yuva420p9Le,
        Rational::new(25, 1).unwrap(),
    ...
    .unwrap();
    assert_eq!(
        raw_yuva420p9_demuxer.read_packet().unwrap().unwrap().data(),
        &(0..40).collect::<Vec<_>>()
    );
    assert!(raw_yuva420p9_demuxer.read_packet().unwrap().is_none());

    for (format, width, height, payload) in [
        (PixelFormat::YuvJ420p, 4, 2, (0..12).collect::<Vec<_>>()),
        (PixelFormat::YuvJ422p, 4, 3, (0..24).collect::<Vec<_>>()),
        (PixelFormat::YuvJ411p, 4, 3, (0..18).collect::<Vec<_>>()),
        (PixelFormat::YuvJ440p, 3, 2, (0..12).collect::<Vec<_>>()),
        (PixelFormat::YuvJ444p, 3, 2, (0..18).collect::<Vec<_>>()),
    ] {
        let mut raw_yuvj =
    ...
    for (format, width, height, payload) in [
        (PixelFormat::Yuv420p9Le, 4, 2, (0..24).collect::<Vec<_>>()),
        (PixelFormat::Yuv422p9Be, 4, 3, (0..48).collect::<Vec<_>>()),
        (PixelFormat::Yuv444p9Be, 3, 2, (0..36).collect::<Vec<_>>()),
        (PixelFormat::Yuv420p10Le, 4, 2, (0..24).collect::<Vec<_>>()),
        (PixelFormat::Yuv440p10Le, 3, 2, (0..24).collect::<Vec<_>>()),
        (PixelFormat::Yuv422p12Be, 4, 3, (0..48).collect::<Vec<_>>()),
        (PixelFormat::Yuv440p12Be, 3, 2, (0..24).collect::<Vec<_>>()),
        (PixelFormat::Yuv444p10Be, 3, 2, (0..36).collect::<Vec<_>>()),
        (
        ...
        (PixelFormat::Yuv420p14Le, 4, 2, (0..24).collect::<Vec<_>>()),
        (PixelFormat::Yuv422p16Be, 4, 3, (0..48).collect::<Vec<_>>()),
        (PixelFormat::Yuv444p16Be, 3, 2, (0..36).collect::<Vec<_>>()),
    ] {
        let mut raw_yuv =
            RawVideoMuxer::new(width, height, format, Rational::new(25, 1).unwrap()).unwrap();
        raw_yuv
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_yuv_output = raw_yuv.finish();
        let mut raw_yuv_demuxer = RawVideoDemuxer::open(
            &raw_yuv_output,
            width,
            height,
            format,
        ...
        .unwrap();
        assert_eq!(
            raw_yuv_demuxer.read_packet().unwrap().unwrap().data(),
            payload.as_slice()
        );
        assert!(raw_yuv_demuxer.read_packet().unwrap().is_none());
    }

    for (format, width, height, payload) in [
        (PixelFormat::Vuya, 2, 1, (0..8).collect::<Vec<_>>()),
        (PixelFormat::Vuyx, 2, 1, (8..16).collect::<Vec<_>>()),
        ...
        raw_packed_yuv
            .write_packet(&Packet::new(payload.clone(), 0))
            .unwrap();
        let raw_packed_yuv_output = raw_packed_yuv.finish();
        let mut raw_packed_yuv_demuxer = RawVideoDemuxer::open(
            &raw_packed_yuv_output,
            width,
            height,
            format,
            Rational::new(25, 1).unwrap(),
        ...
        assert!(raw_packed_yuv_demuxer.read_packet().unwrap().is_none());
    }
