...
fn exercise_fixtures() {
    ...
    assert!(yaf16.decode_packet(&Packet::new(vec![0; 8], 0)).is_ok());
    assert_eq!(
        yaf16
            .decode_packet(&Packet::new(vec![0; 7], 0))
            .unwrap_err()
    ...
    assert!(yaf32.decode_packet(&Packet::new(vec![0; 8], 0)).is_ok());
    assert_eq!(
        yaf32
            .decode_packet(&Packet::new(vec![0; 7], 0))
            .unwrap_err()
    ...
    let gbrpf16 = RawVideoDecoder::new(2, 1, PixelFormat::GbrpF16Le).unwrap();
    assert!(gbrpf16
        .decode_packet(&Packet::new((0..12).collect(), 0))
        .is_ok());
    assert_eq!(
    ...
    let uyyvyy411 = RawVideoDecoder::new(4, 2, PixelFormat::Uyyvyy411).unwrap();
    assert!(uyyvyy411
        .decode_packet(&Packet::new(vec![0; 12], 0))
        .is_ok());
    assert_eq!(
        uyyvyy411
    ...
    for (format, width, height, valid_len, invalid_len) in [
        (PixelFormat::Nv16, 4, 3, 24, 23),
        (PixelFormat::Nv20Le, 4, 3, 48, 47),
        (PixelFormat::Nv20Be, 4, 3, 48, 47),
        (PixelFormat::Nv24, 3, 2, 18, 17),
        (PixelFormat::Nv42, 3, 2, 18, 17),
    ] {
        let decoder = RawVideoDecoder::new(width, height, format).unwrap();
        assert!(decoder
            .decode_packet(&Packet::new(vec![0; valid_len], 0))
    ...
    for (format, valid_len, invalid_len) in [
        ...
        (PixelFormat::Rgb96Le, 12, 11),
        (PixelFormat::Rgb96Be, 12, 11),
        (PixelFormat::Rgba128Le, 16, 15),
        (PixelFormat::Rgba128Be, 16, 15),
    ...
    assert!(RawVideoDecoder::new(3, 2, PixelFormat::Y212Be).is_err());
    ...
    let rgb96 = RawVideoDecoder::new(1, 1, PixelFormat::Rgb96Le).unwrap();
    assert!(rgb96.decode_packet(&Packet::new(vec![0; 12], 0)).is_ok());
    assert_eq!(
        rgb96
            .decode_packet(&Packet::new(vec![0; 11], 0))
            .unwrap_err()
    ...
    let rgbaf16 = RawVideoDecoder::new(1, 1, PixelFormat::RgbaF16Le).unwrap();
    assert!(rgbaf16.decode_packet(&Packet::new(vec![0; 8], 0)).is_ok());
    assert_eq!(
        rgbaf16
            .decode_packet(&Packet::new(vec![0; 7], 0))
            .unwrap_err()
