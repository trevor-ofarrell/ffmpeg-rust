...
- `ffprobe-rs -show_format`, `ffprobe-rs -show_streams`, and `ffprobe-rs -show_packets` can auto-detect local seekable MOV/MP4 and constrained AVI files through the shared probe registry, route them to the Rust MOV or AVI demuxer, force those paths with `-f avi`, `-f mov`, or `-f mp4`, then render small default or JSON summaries.
...
- The ffprobe MOV and AVI outputs are deterministic compatibility-oriented summaries, not byte-identical upstream output, and they have no pinned-oracle differential coverage yet.
