...
- `avformat` has an initial MOV/MP4 demuxer path for metadata boxes and simple one-track sample table packet extraction.
...
- MOV/MP4 demuxing is internal only and currently limited to box-bound validation, `ftyp`/movie/track/media metadata, and one populated track using simple `stsd`, `stts`, `stsc`, `stsz`, and `stco`/`co64` tables to extract packet payloads from `mdat`. Multiple populated tracks, `ctts`, `stss`, chunk groups beyond the initial mapping, edit lists, fragments, metadata atoms, codec parameters, probing, CLI execution, differential tests, FATE, and fuzzing are pending.
