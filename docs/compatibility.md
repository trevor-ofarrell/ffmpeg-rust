...
- MOV/MP4 demuxing is internal only and currently limited to box-bound validation, `ftyp`/movie/track/media metadata, generic `stsd` sample-entry records, explicit `mvex`/`moof`, `edts`, and multiple-populated-track unsupported errors, and one populated track using simple `stsd`, `stts`, `ctts`, `stsc`, `stsz`, `stss`, and `stco`/`co64` tables to extract packet payloads from one or more `mdat` boxes with PTS/DTS separation and key flags. Packet extraction for multiple populated tracks, sample description indexes other than 1, edit-list timeline application, actual fragment parsing, metadata atoms, codec-specific sample-entry parsing, probing, CLI execution, differential tests, FATE, and fuzzing are pending.
