...
- `avformat` has an initial MOV/MP4 metadata demuxer path for `ftyp`, `moov/mvhd`, `trak/tkhd`, and `mdia/mdhd` boxes.
...
- MOV/MP4 demuxing is internal only and currently limited to box-bound validation plus `ftyp`, movie header, track header, and media header metadata. Sample tables, packet extraction from `mdat`, edit lists, fragments, metadata atoms, codec parameters, probing, CLI execution, differential tests, FATE, and fuzzing are pending.
