...
- `avformat` has an initial MOV/MP4 demuxer path for metadata boxes, explicit fragmented-layout and multiple-populated-track rejection, simple one-track sample table packet extraction, multi-chunk sample-to-chunk mapping, multiple `mdat` ranges, composition-offset PTS, and sync-sample key flags.
...
- MOV/MP4 demuxing is internal only and currently limited to box-bound validation, `ftyp`/movie/track/media metadata, explicit `mvex`/`moof` and multiple-populated-track unsupported errors, and one populated track using simple `stsd`, `stts`, `ctts`, `stsc`, `stsz`, `stss`, and `stco`/`co64` tables to extract packet payloads from one or more `mdat` boxes with PTS/DTS separation and key flags. Packet extraction for multiple populated tracks, sample description indexes other than 1, edit lists, actual fragment parsing, metadata atoms, codec parameters, probing, CLI execution, differential tests, FATE, and fuzzing are pending.
