...
- `ffmpeg-rs` can execute one local seekable MOV/MP4, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, or yuv4mpegpipe input to stdout with explicit `-f null -` or `-f framecrc -`, using Rust demuxers plus null/framecrc muxers.
...
- CLI execution support is limited to `-version`, one local MOV/MP4, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, or yuv4mpegpipe `ffmpeg-rs` input to stdout `-f null -` or `-f framecrc -`, and a small `ffprobe-rs -show_format`/`-show_streams`/`-show_packets` path for local MOV/MP4 files; most FFmpeg/ffprobe options return unsupported-command errors.
...
- Null muxer support is wired only to the constrained MOV/MP4, WAV, raw PCM, rawvideo, and yuv4mpegpipe `ffmpeg-rs -f null -` execution paths; it does not have broad CLI, differential, or FATE coverage yet.
...
- Framecrc muxer support is wired only to the constrained MOV/MP4, WAV, raw PCM, rawvideo, and yuv4mpegpipe `ffmpeg-rs -f framecrc -` execution paths and is not byte-identical to FFmpeg framecrc output yet.
