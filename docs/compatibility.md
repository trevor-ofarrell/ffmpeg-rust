...
- `ffprobe-rs -show_format`, `ffprobe-rs -show_streams`, and `ffprobe-rs -show_packets` can open local seekable MOV/MP4 files through the Rust MOV probe descriptor and demuxer and constrained local AVI files through RIFF/AVI or `.avi` detection and the Rust AVI demuxer, then render small default or JSON summaries.
...
- AVI demuxing is currently limited to constrained video-only RIFF AVI files with `avih`, `strh`, `strf`, and simple `movi` chunks, with constrained `ffmpeg-rs -i <input.avi> -f null/framecrc -` CLI execution, explicit `-f avi` input support, and initial `ffprobe-rs -show_format`/`-show_streams`/`-show_packets` summary wiring. Audio streams, indexes, OpenDML, palette handling, non-BI_RGB video, interleaving semantics, seeking, broader probing, broader CLI execution, differential tests, FATE, and fuzzing are pending.
