...
- `ffmpeg-rs` can execute one local seekable MOV/MP4, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, yuv4mpegpipe, or explicit single-image image2 input to stdout with explicit `-f null -` or `-f framecrc -`, using Rust demuxers plus null/framecrc muxers.
...
- CLI execution support is limited to `-version`, one local MOV/MP4, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, yuv4mpegpipe, or explicit single-image image2 `ffmpeg-rs` input to stdout `-f null -` or `-f framecrc -`, and a small `ffprobe-rs -show_format`/`-show_streams`/`-show_packets` path for local MOV/MP4 files; most FFmpeg/ffprobe options return unsupported-command errors.
...
- Null muxer support is wired only to the constrained MOV/MP4, WAV, raw PCM, rawvideo, yuv4mpegpipe, and single-image image2 `ffmpeg-rs -f null -` execution paths; it does not have broad CLI, differential, or FATE coverage yet.
...
- Framecrc muxer support is wired only to the constrained MOV/MP4, WAV, raw PCM, rawvideo, yuv4mpegpipe, and single-image image2 `ffmpeg-rs -f framecrc -` execution paths and is not byte-identical to FFmpeg framecrc output yet.
...
- image2 demuxing and muxing are currently limited to caller-provided entries for a single image or contiguous `%d`/`%0Nd` numbered sequence; constrained explicit single-file `ffmpeg-rs -f image2 -framerate <rate> -i <file> -f null/framecrc -` CLI execution is implemented. Numbered-sequence filesystem discovery, filesystem writes, glob patterns, timestamp modes, looping, codec probing, broad CLI execution, differential tests, FATE, and fuzzing are pending.
