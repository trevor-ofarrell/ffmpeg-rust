...
- `ffprobe-rs -show_format` and `ffprobe-rs -show_streams` can open local seekable MOV/MP4 files through the Rust MOV probe descriptor and demuxer, then render a small default or JSON summary.
...
- CLI execution support is limited to `-version` plus a small `ffprobe-rs -show_format`/`-show_streams` path for local MOV/MP4 files; most FFmpeg/ffprobe options return unsupported-command errors.
