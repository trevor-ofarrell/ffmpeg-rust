...
- `ffprobe-rs -show_format`, `ffprobe-rs -show_streams`, and `ffprobe-rs -show_packets` can open local seekable MOV/MP4 files through the Rust MOV probe descriptor and demuxer and constrained local AVI files through RIFF/AVI or `.avi` detection and the Rust AVI demuxer, can force those paths with `-f avi`, `-f mov`, or `-f mp4`, then render small default or JSON summaries.
