...
- `ffmpeg-rs` can execute one local seekable MOV/MP4 input to stdout with explicit `-f null -` or `-f framecrc -`, using the Rust MOV demuxer plus null/framecrc muxers.
...
- CLI execution support is limited to `-version`, one local MOV/MP4 `ffmpeg-rs` input to stdout `-f null -` or `-f framecrc -`, and a small `ffprobe-rs -show_format`/`-show_streams`/`-show_packets` path for local MOV/MP4 files; most FFmpeg/ffprobe options return unsupported-command errors.
...
- Null muxer support is wired only to the constrained MOV/MP4 `ffmpeg-rs -f null -` execution path; it does not have broad CLI, differential, or FATE coverage yet.
...
- Framecrc muxer support is wired only to the constrained MOV/MP4 `ffmpeg-rs -f framecrc -` execution path and is not byte-identical to FFmpeg framecrc output yet.
