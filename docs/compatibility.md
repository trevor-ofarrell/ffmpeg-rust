...
- `ffmpeg-rs` can execute one local seekable MOV/MP4, AVI, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, yuv4mpegpipe, or explicit image2 single-file/numbered-sequence input to stdout with explicit `-f null -` or `-f framecrc -`, using Rust demuxers plus null/framecrc muxers.
...
- `ffprobe-rs -show_format`, `ffprobe-rs -show_streams`, and `ffprobe-rs -show_packets` can auto-detect local seekable MOV/MP4 and constrained AVI files through the shared probe registry, route them to the Rust MOV or AVI demuxer, force those paths with `-f avi`, `-f mov`, or `-f mp4`, then render small default or JSON summaries including initial stream codec name/type/tag, average-frame-rate fields, and MOV visual sample aspect, display aspect, and color fields where the Rust demuxer has enough metadata.
...
- The ffprobe MOV and AVI outputs are deterministic compatibility-oriented summaries, not byte-identical upstream output, and they have no pinned-oracle differential coverage yet. Stream field coverage is limited to the constrained local metadata model, a small known FourCC-to-codec-name map, and MOV visual `pasp`/`colr` side fields already parsed by the Rust MOV demuxer.
...
- Probe support is still an early registry/scoring primitive with hand-written AVI and MOV/MP4 descriptors; no full demuxer probe table has been generated from the pinned oracle yet.
- Null muxer support is wired only to the constrained MOV/MP4, AVI, WAV, raw PCM, rawvideo, yuv4mpegpipe, and image2 single-file/numbered-sequence `ffmpeg-rs -f null -` execution paths; it does not have broad CLI, differential, or FATE coverage yet.
...
- Framecrc muxer support is wired only to the constrained MOV/MP4, AVI, WAV, raw PCM, rawvideo, yuv4mpegpipe, and image2 single-file/numbered-sequence `ffmpeg-rs -f framecrc -` execution paths and is not byte-identical to FFmpeg framecrc output yet.
