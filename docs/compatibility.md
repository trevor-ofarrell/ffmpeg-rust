...
- AVI demuxing is currently limited to constrained video-only RIFF AVI files with `avih`, `strh`, `strf`, and simple `movi` chunks, with constrained `ffmpeg-rs -i <input.avi> -f null/framecrc -` CLI execution and explicit `-f avi` input support. Audio streams, indexes, OpenDML, palette handling, non-BI_RGB video, interleaving semantics, seeking, probing, broader CLI execution, differential tests, FATE, and fuzzing are pending.
