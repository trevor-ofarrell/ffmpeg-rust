...
The newest no-force pseudo-terminal TERM rows run as fresh oracle processes because FFmpeg caches the first default-callback color decision. They prove unset `TERM` produces plain warning output, `TERM=dumb` produces basic ANSI context (`0;39m`), warning (`0;33m`), error (`1;31m`), and fatal/panic (`4;31m`) segments while info stays plain, empty `TERM` produces the same basic warning row, and `TERM=xterm-256color` produces 256-color context, warning, error, fatal, and panic segments while info stays plain.
...
The latest buffer oracle rows add offset custom-pool allocation coverage: a custom `av_buffer_pool_init2()` allocator may return a ref whose visible `data`/`size` point inside a larger backing allocation. FFmpeg preserves that offset range for the first checkout, but after unref/reuse creates the spare ref from the original backing pointer and pool size while preserving pool opaque lookup and delaying release of the full backing bytes until pool uninit.

Prior buffer oracle rows pin readonly custom-pool allocation, default-free opaque owners, shared-readonly custom-owner make-writable, unique readonly make-writable, unique writable/readonly shrink-realloc, and shared writable/read-only shrink-realloc edges. The readonly custom-pool row proves an allocator returning an `AV_BUFFER_FLAG_READONLY` ref yields a readonly first checkout, then later reuses it as writable storage with final release delayed.

`crates/fftools/tests/version_oracle.rs` is an ignored oracle harness for `ffmpeg -version`, `ffprobe -version`, and the `-hide_banner -version` variant. It checks the pinned tool-version prefix and the libav* ABI versions reported by the oracle against the Rust `ffmpeg-rs`/`ffprobe-rs` banner constants, verifies that `-hide_banner -version` preserves the same version surface as `-version`, compares accepted/rejected `-loglevel` directive forms on version requests for both tools, and probes real `ffmpeg` repeated diagnostic stderr with an invalid MJPEG image sequence. The loglevel rows pin case-sensitive names, standalone `repeat`/`level`/`time`/`datetime` flag directives, `repeat`/`+repeat` versus `-repeat` CLI semantics, compound flag+level values, `+error`, known numeric levels, raw numeric thresholds including compound raw-threshold forms, and representative rejected aliases such as `warn` and `ERROR`; the repeated-diagnostic row pins default indented summary emission plus `repeat+error` duplicate preservation. `FFMPEG_ORACLE` points to the pinned `ffmpeg` binary; `ffprobe` is found through `FFPROBE_ORACLE`, a sibling of `FFMPEG_ORACLE`, or the standard `third_party/ffmpeg-oracle/build/bin/ffprobe(.exe)` path. Run it with:
