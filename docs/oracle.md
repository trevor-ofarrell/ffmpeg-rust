...
The latest buffer oracle rows add the shared writable custom-owner shrink-realloc edge: `av_buffer_realloc()` shrinking one reference of a shared `av_buffer_create(..., flags=0)` owner detaches only the destination to ordinary writable reallocatable storage with the truncated visible prefix, clears destination opaque lookup, preserves the writable source owner, and runs the original release callback only after the source is unreffed. The prior shared-readonly shrink rows pin the matching lifecycle for `AV_BUFFER_FLAG_READONLY` sources.
