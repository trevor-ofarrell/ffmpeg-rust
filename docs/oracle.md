...
The latest `avutil_core_models` deterministic logging fixtures include the UTC+05:30 default-callback timestamp row, the context-sensitive default-callback repeat row where identical text from distinct AVClass instances is not coalesced, the default-callback carriage-return prefix reset row, and low-level `av_log_format_line()` / `av_log_format_line2()` carriage-return prefix reset rows. The latest recorded WSL smoke evidence includes 1024 inputs after the UTC+05:30 timestamp addition, a warmed one-input run after adding the context-sensitive repeat fixture, a warmed one-input run after adding the default-callback carriage-return reset fixture, and a dedicated one-input run after adding the low-level format-line carriage-return reset fixture; those sanitizer-backed smokes are useful, but not a sustained fuzz campaign.
