...
The latest buffer oracle rows add the shared-readonly custom-owner make-writable edge: `av_buffer_make_writable()` on one reference of a shared `av_buffer_create(..., AV_BUFFER_FLAG_READONLY)` owner detaches only the destination to ordinary writable copied storage, clears destination opaque lookup, preserves the readonly source owner, and runs the original release callback only after the final source reference is unreffed. Prior rows pin unique readonly make-writable, unique writable/readonly shrink-realloc, and shared writable/read-only shrink-realloc edges.
