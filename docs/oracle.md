...
The latest buffer oracle rows add the zero-length readonly custom-owner make-writable edge: `av_buffer_create(data, 0, free, opaque, AV_BUFFER_FLAG_READONLY)` preserves opaque lookup on the empty readonly ref, and `av_buffer_make_writable()` releases that original owner while detaching to ordinary writable empty storage.
