...
kinds, standalone raw `-1`/`INT_MIN` append, and standalone
...
`packet:side-shrink-raw-negative-type-padding` alongside
...
when shrinking a raw `-1` packet-owned entry to zero. The deterministic
