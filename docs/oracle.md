...
The `avutil-channel-layout` slice was checked against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c`. The pinned header defines `AV_CHAN_BACK_CENTER` after `AV_CHAN_BACK_RIGHT`, maps `AV_CH_BACK_CENTER` to `1ULL << AV_CHAN_BACK_CENTER`, and defines `AV_CH_LAYOUT_2POINT1`, `AV_CH_LAYOUT_SURROUND`, `AV_CH_LAYOUT_2_1`, and `AV_CH_LAYOUT_4POINT0` as stereo plus LFE, stereo plus FC, stereo plus BC, and surround plus BC respectively. The pinned `channel_layout_map` names those layouts `2.1`, `3.0`, `3.0(back)`, and `4.0`, and `av_channel_layout_default` selects the first map entry with the requested channel count. The Rust model exposes those source-checked shapes and now models default counts 1, 2, 3, 4, 6, and 8 as mono, stereo, 2.1, 4.0, 5.1, and 7.1, while leaving custom/ambisonic orders and the remaining `ffmpeg -layouts` inventory for later oracle-backed slices.
