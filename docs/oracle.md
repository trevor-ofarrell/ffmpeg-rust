...
The harness also includes `packet:rescale-mixed` and `packet:rescale-mixed-dts`, proving `av_packet_rescale_ts()` preserves `AV_NOPTS_VALUE` independently for PTS or DTS while rescaling the other timestamp and duration fields, without changing payload bytes, byte position, flags, stream index, or the packet `time_base` field.
