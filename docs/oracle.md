...
The `avutil-channel-layout` slice was checked against pinned FFmpeg 8.1.1 `libavutil/channel_layout.h` and `libavutil/channel_layout.c`. The pinned header defines the currently modeled native channels from front/center/back/side positions through height, wide, downmix, binaural, and 22.2-specific entries including `AV_CHAN_TOP_CENTER`, `AV_CHAN_BOTTOM_FRONT_CENTER`, `AV_CHAN_BOTTOM_FRONT_LEFT`, and `AV_CHAN_BOTTOM_FRONT_RIGHT`; it maps their `AV_CH_*` macros to native mask bits including `1 << 11`, `1 << 38`, `1 << 39`, and `1 << 40` for `TC`, `BFC`, `BFL`, and `BFR`. The pinned source maps those short names to `top center`, `bottom front center`, `bottom front left`, and `bottom front right`, and `channel_layout_map` names the currently modeled layouts from `mono` through `downmix` plus `22.2`. The `22.2` macro is `AV_CH_LAYOUT_9POINT1POINT6 | AV_CH_BACK_CENTER | AV_CH_LOW_FREQUENCY_2 | AV_CH_TOP_FRONT_CENTER | AV_CH_TOP_CENTER | AV_CH_TOP_BACK_CENTER | AV_CH_BOTTOM_FRONT_CENTER | AV_CH_BOTTOM_FRONT_LEFT | AV_CH_BOTTOM_FRONT_RIGHT`, with `AV_CHANNEL_LAYOUT_22POINT2` declaring 24 channels. `av_channel_layout_default` selects the first map entry with the requested channel count. The Rust model exposes those source-checked shapes and models default counts 1, 2, 3, 4, 5, 6, 7, and 8 as mono, stereo, 2.1, 4.0, 5.0, 5.1, 6.1, and 7.1, while leaving broader remaining channel families, custom/native/ambisonic order semantics, and the remaining `ffmpeg -layouts` inventory for later oracle-backed slices.
