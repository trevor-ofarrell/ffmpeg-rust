...
The latest buffer oracle rows add custom-pool replace coverage: `av_buffer_replace()` with a pool-owned custom destination and ordinary source returns the destination's original pool storage immediately for reuse with preserved bytes and pool opaque data, makes the destination an ordinary shared ref to the source with NULL ordinary opaque lookup, and releases only the original pool allocation plus pool owner on final pool uninit.
