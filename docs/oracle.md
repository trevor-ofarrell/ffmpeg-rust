...
The 10-bit planar YUV `av_frame_apply_cropping()` slice was checked against pinned FFmpeg 8.1.1 libavutil through `crates/avutil/tests/frame_oracle.rs`. For `yuv420p10le`, `yuv420p10be`, `yuv422p10le`, `yuv422p10be`, `yuv444p10le`, and `yuv444p10be`, FFmpeg returns `AVERROR_BUG` without mutating the frame for default nonzero-left crop. With `AV_FRAME_CROP_UNALIGNED`, FFmpeg applies exact two-byte-per-sample luma offsets, subsampled chroma offsets derived from the format chroma geometry, floor-shifted visible chroma dimensions, and resets crop fields on success.
