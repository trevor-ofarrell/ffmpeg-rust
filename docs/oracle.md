...
`crates/fftools/tests/image2_oracle.rs` is an ignored oracle harness for the constrained image2 file-output path. It compares a valid one-frame PPM input through Rust `-f image2 -framerate 1 ... -f image2 <file>` against pinned FFmpeg 8.1.1 `-c:v copy -f image2`, proving the current bounded Image2 demuxer, Packet path, and Image2 muxer preserve the input bytes for that case. It is wired into `tests/differential/mappings.txt` as `oracle-image2-file-output` for `fftools-ffmpeg-image2-file-output`, `avformat-image2-demuxer`, `avformat-image2-muxer`, and `avutil-packet`:
