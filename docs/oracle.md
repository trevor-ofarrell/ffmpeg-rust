...
`crates/fftools/tests/image2_oracle.rs` is an ignored oracle harness for the constrained image2 file-output path. It compares a valid one-frame PPM input and a two-frame numbered PPM sequence through Rust `-f image2 -framerate 1 ... -f image2 <file-or-pattern>` against pinned FFmpeg 8.1.1 `-c:v copy -f image2`, proving the current bounded Image2 demuxer, Packet path, and Image2 muxer preserve the input bytes for those cases. It is wired into `tests/differential/mappings.txt` as `oracle-image2-file-output` and `oracle-image2-sequence-file-output` for `fftools-ffmpeg-image2-file-output`, `avformat-image2-demuxer`, `avformat-image2-muxer`, and `avutil-packet`:
