...
The newest local-time rows add `TZ=PST8PDT,M3.2.0,M11.1.0` `PRINT_DATETIME|PRINT_LEVEL` default-callback cases around Pacific spring-forward and fall-back boundaries, proving the bounded POSIX DST formatter in addition to the fixed UTC+2, UTC+5:30, and UTC-8 rows.
