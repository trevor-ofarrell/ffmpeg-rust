...
The latest set-from-string rows add empty-key, unclosed-quote, quote-escape, and empty-pair-separator multi-token coverage: `=7` returns `AVERROR_OPTION_NOT_FOUND` without mutation, `metadata='title` succeeds with `title`, `metadata='\\''x'` stores `\\x`, and `pairs_sep=""` with `threads=7:quality=0.25` returns `EINVAL` without mutating the option state.
...
cargo run -p fate-runner -- run --mappings tests/differential/mappings.txt --component avformat-wav-demuxer --component avformat-framecrc-muxer --component fftools-ffmpeg-wav-framecrc-null --target oracle-wav-duplicate-data-last-empty-framecrc-records --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg
```
