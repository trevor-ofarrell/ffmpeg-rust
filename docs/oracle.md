...
The 10-bit and 12-bit planar YUV rawvideo slice was checked against FFmpeg 8.1.1 `libavutil/pixfmt.h` and `libavutil/pixdesc.c`. The pinned descriptors define `yuv420p10*` as 15 bpp with log2 chroma `(1,1)`, `yuv422p10*` as 20 bpp with `(1,0)`, `yuv444p10*` as 30 bpp with `(0,0)`, `yuv420p12*` as 18 bpp with `(1,1)`, `yuv422p12*` as 24 bpp with `(1,0)`, and `yuv444p12*` as 36 bpp with `(0,0)`, all using two stored bytes per component sample. The 9-bit 4:2:0 family is deferred because FFmpeg documents an average 13.5 bpp descriptor value that the current integer bpp model cannot represent exactly.
