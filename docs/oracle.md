...
The same harness now has a separate bounded `AV_OPT_TYPE_DURATION` class that validates duration string parsing for seconds, `MM:SS`, `HH:MM:SS`, fractional microseconds, `ms`/`us` suffixes, parse `EINVAL`, range `ERANGE`, `av_opt_get` string formatting, typed int/double/q getters, and default `av_opt_query_ranges` rows against pinned FFmpeg 8.1.1.
