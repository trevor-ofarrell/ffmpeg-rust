...
The latest buffer oracle rows add readonly custom-pool allocation coverage: a custom `av_buffer_pool_init2()` allocator that returns an `AV_BUFFER_FLAG_READONLY` ref yields a readonly/non-writable first checkout, returns it to the pool without calling the original free callback, and later reuses it as writable storage while preserving pool opaque lookup and final release. Prior rows pin default-free opaque owners, shared-readonly custom-owner make-writable, unique readonly make-writable, unique writable/readonly shrink-realloc, and shared writable/read-only shrink-realloc edges.
