...
The latest buffer oracle rows add default-free opaque-owner coverage: `av_buffer_create(data, size, free=NULL, opaque, flags)` preserves `av_buffer_get_opaque()` while attached, keeps it across unique writable make-writable, and clears it when shared/readonly make-writable or realloc detaches to ordinary writable storage. Prior rows pin shared-readonly custom-owner make-writable, unique readonly make-writable, unique writable/readonly shrink-realloc, and shared writable/read-only shrink-realloc edges.
