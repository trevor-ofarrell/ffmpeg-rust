...
The same harness now has separate bounded `AV_OPT_TYPE_DURATION` and `AV_OPT_TYPE_IMAGE_SIZE` classes. The duration class validates duration string parsing for seconds, `MM:SS`, `HH:MM:SS`, fractional microseconds, `ms`/`us` suffixes, parse `EINVAL`, range `ERANGE`, `av_opt_get` string formatting, typed int/double/q getters, and default `av_opt_query_ranges` rows against pinned FFmpeg 8.1.1. The image-size class validates default parsing, numeric pair strings, named video-size abbreviations, `none` as `0x0`, typed `av_opt_set_image_size` / `av_opt_get_image_size`, negative typed `EINVAL`, wrong-type `EINVAL`, generic numeric ERANGE/EINVAL behavior, `av_opt_get` string formatting, and default image-size range rows.
