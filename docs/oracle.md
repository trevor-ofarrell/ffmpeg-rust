...
The latest buffer oracle rows add the unique readonly custom-owner shrink-realloc edge: `av_buffer_realloc()` shrinking a unique `av_buffer_create(..., AV_BUFFER_FLAG_READONLY)` owner releases the original owner immediately with the full original bytes, detaches to ordinary writable reallocatable storage with the truncated visible prefix, clears destination opaque lookup, and does not run the original release callback again when the detached ref is unreffed.
