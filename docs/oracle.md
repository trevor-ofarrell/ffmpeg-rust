...
The harness also includes `packet:array-add-capacity-*` and `packet:array-new-capacity-overflow` rows, proving the standalone `AVPacketSideData` array helpers do not apply the same packet-owned `AV_PKT_DATA_NB` ceiling: `av_packet_side_data_add()` accepts an out-of-range raw type and grows the array from 41 to 42 entries, and `av_packet_side_data_new()` for that same raw type replaces the entry while keeping the count at 42. The newest standalone raw-type rows add `packet:array-add-raw-type-*`, `packet:array-add-raw-type-padding`, `packet:array-remove-raw-type`, `packet:array-get-raw-type-removed`, `packet:array-free-raw-type`, `packet:array-remove-new-raw-type`, `packet:array-get-new-raw-type-removed`, `packet:array-free-new-raw-type`, `packet:array-add-raw-plus-one-type-*`, `packet:array-add-raw-plus-one-type-padding`, `packet:array-remove-raw-plus-one-type`, `packet:array-get-raw-plus-one-type-removed`, `packet:array-free-raw-plus-one-type`, `packet:array-remove-new-raw-plus-one-type`, `packet:array-get-new-raw-plus-one-type-removed`, `packet:array-free-new-raw-plus-one-type`, `packet:array-new-raw-type-padding`, `packet:array-new-raw-plus-one-type-padding`, `packet:array-add-raw-negative-type-*`, `packet:array-add-raw-negative-type-padding`, `packet:array-add-raw-min-type-padding`, `packet:array-new-raw-negative-type-padding`, `packet:array-new-raw-min-type-padding`, and `packet:array-new/get/remove-raw-min-type`, proving the same standalone array API accepts raw `AV_PKT_DATA_NB`, `AV_PKT_DATA_NB + 1`, `-1`, and `INT_MIN` values, lookup finds them, raw `AV_PKT_DATA_NB`, `AV_PKT_DATA_NB + 1`, `-1`, and `INT_MIN` add/new-side-data paths preserve or allocate zeroed input padding, transferred and newly allocated raw `AV_PKT_DATA_NB` and raw `AV_PKT_DATA_NB + 1` removals make lookup miss and keep the subsequent free state empty, and removing the `INT_MIN` entry preserves the `-1` entry.
