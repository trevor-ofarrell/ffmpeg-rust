...
The harness also includes `packet:rescale-mixed`, `packet:rescale-mixed-dts`, and `packet:rescale-zero-duration`, proving `av_packet_rescale_ts()` preserves `AV_NOPTS_VALUE` independently for PTS or DTS, rescales valid timestamps while preserving zero duration, and avoids changing payload bytes, byte position, flags, stream index, or the packet `time_base` field.
