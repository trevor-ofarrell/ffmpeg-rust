...
The latest buffer oracle rows add the shared readonly custom-owner shrink-realloc edge: `av_buffer_realloc()` shrinking one reference of a shared `av_buffer_create(..., AV_BUFFER_FLAG_READONLY)` owner detaches only the destination to ordinary writable reallocatable storage with the truncated visible prefix, clears destination opaque lookup, preserves the readonly source owner, and runs the original release callback only after the source is unreffed.
