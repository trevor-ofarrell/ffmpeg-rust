...
The packet bridge rows also cover `av_packet_side_data_from_frame(..., AV_FRAME_SIDE_DATA_FLAG_UNIQUE)` on a duplicate-rich packet side-data array. The pinned FFmpeg 8.1.1 behavior is packet-specific: the first matching mapped packet side-data entry is replaced and later duplicates are preserved.
