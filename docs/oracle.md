...
The harness also includes `packet:rescale-mixed`, proving `av_packet_rescale_ts()` preserves an `AV_NOPTS_VALUE` PTS sentinel while rescaling valid DTS and duration fields, without changing payload bytes, byte position, flags, stream index, or the packet `time_base` field.
