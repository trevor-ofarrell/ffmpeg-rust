...
state. `av_buffer_ref()` preserves the NULL data pointer, opaque lookup, shared
storage, refcount 2, and READONLY non-writability for both refs, and the custom
release callback remains delayed until the final source unref.
