...
The latest buffer oracle rows add the unique writable custom-owner shrink-realloc edge: `av_buffer_realloc()` shrinking a unique `av_buffer_create(..., flags=0)` owner detaches to ordinary writable reallocatable storage with the truncated visible prefix, clears destination opaque lookup, runs the original release callback immediately with the full original bytes, and does not run it again on detached unref. Prior rows pin the matching unique readonly shrink edge and the shared writable/read-only shrink edges where the original owner remains alive until the final source reference is unreffed.
