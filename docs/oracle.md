...
The latest buffer oracle rows add the unique readonly custom-owner realloc edge: `av_buffer_realloc()` on a unique `av_buffer_create(..., AV_BUFFER_FLAG_READONLY)` owner releases the original owner immediately, detaches to ordinary writable reallocatable storage with copied prefix bytes, clears destination opaque lookup, and does not run the original release callback again when the detached ref is unreffed.
