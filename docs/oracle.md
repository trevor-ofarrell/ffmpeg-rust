...
The packet bridge rows also cover `AV_FRAME_SIDE_DATA_FLAG_UNIQUE` in both directions. For `av_packet_side_data_from_frame()` on a duplicate-rich packet side-data array, the pinned FFmpeg 8.1.1 behavior is packet-specific: the first matching mapped packet side-data entry is replaced and later duplicates are preserved. For `av_packet_side_data_to_frame()`, the frame-owned insertion path removes matching frame side data, preserves nonmatching entries, and appends the mapped replacement. The bridge rows also cover `AV_FRAME_SIDE_DATA_FLAG_NEW_REF` in both directions; for the bounded raw packet/frame side-data conversion surface, FFmpeg returns success and produces the same mapped side-data payload shape as the ordinary copied insertion path.
