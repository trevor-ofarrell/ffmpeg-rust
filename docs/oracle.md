...
`av_frame_copy_props`, PTS/packet-DTS/duration/time-base propagation through
ref/move/copy-props rows, `av_frame_get_buffer` for a gray8 video frame and a
packed s16 stereo audio frame, `AV_FRAME_DATA_*` numeric
...
preserved, PTS, packet DTS, duration, and time base are copied, existing
destination side data is retained, source side data is appended as a deep copy,
and destination `hw_frames_ctx` remains unchanged.
