...
The latest `avutil_core_models` deterministic logging fixtures include the UTC+05:30 default-callback timestamp row, the context-sensitive default-callback repeat row where identical text from distinct AVClass instances is not coalesced, and the default-callback carriage-return prefix reset row. The latest recorded WSL smoke evidence includes 1024 inputs after the UTC+05:30 timestamp addition, a warmed one-input run after adding the context-sensitive repeat fixture, and a warmed one-input run after adding the carriage-return reset fixture; those sanitizer-backed smokes are useful, but not a sustained fuzz campaign.
