...
The `avutil-buffer`, `avutil-packet`, and `avutil-logging` harnesses now use
...
The latest `avutil_metadata_options` deterministic fixture includes nested
AVOption child-search cases for `AV_OPT_SEARCH_CHILDREN`: duplicate `threads`
options resolve to the grandchild leaf before the direct child/root, a
grandchild-only option is absent without child search, and the same ordering is
checked for get, set, range, and definition-query paths. A WSL
`CARGO_TARGET_DIR=target-wsl-fuzz ASAN_OPTIONS=detect_leaks=0 cargo fuzz run
avutil_metadata_options -- -runs=64` smoke passed locally after correcting the
...
reported final corpus `376/27Kb`, found no crash, and the tracked corpus
stayed at four files. This strengthens current logging-model smoke evidence,
but it is still not a sustained fuzz campaign.
...
`CARGO_TARGET_DIR=target-wsl-fuzz` and `ASAN_OPTIONS=detect_leaks=0` against a
temporary copy of the four tracked seed files. It reached `DONE`, libFuzzer
reported final corpus `283/20Kb`, found no crash, and the tracked corpus stayed
...
`packet:side-add-raw-negative-type-padding`,
`packet:side-add-raw-min-type-padding`,
`packet:array-add-raw-type-padding`,
`packet:array-add-raw-plus-one-type-padding`,
`packet:array-add-raw-negative-type-padding`,
`packet:array-add-raw-min-type-padding`, and
`packet:array-add-flags-nonzero-padding` rows. These prove packet-owned
positive-size side-data adds under raw `AV_PKT_DATA_NB`, `-1`, and `INT_MIN`
kinds, standalone raw `AV_PKT_DATA_NB`/`AV_PKT_DATA_NB + 1`/`-1`/`INT_MIN`
...
`packet:array-free-new-raw-min-singleton-type` rows. These prove standalone
`av_packet_side_data_new()` accepts a raw `INT_MIN` singleton entry, allocates
zeroed input padding, removes it, misses lookup afterward, and leaves
`av_packet_side_data_free()` with an empty state. The deterministic
`avutil_core_models` fixture mirrors the same singleton lifecycle through
`PacketSideDataList::new_side_data()`, `remove_kind()`, and `clear()`.
...
when shrinking raw exact `AV_PKT_DATA_NB`, out-of-range `AV_PKT_DATA_NB + 1`,
`-1`, or `INT_MIN` packet-owned entries to zero. The deterministic
...
`AV_PKT_DATA_NB`, `AV_PKT_DATA_NB + 1`, and `INT_MIN` standalone
`av_packet_side_data_new()` allocations include the same zeroed padding, and an
...
before pool_free. A later warmed WSL `avutil_core_models` smoke ran 8192 inputs
against a temporary copy of the four tracked seeds with
`CARGO_TARGET_DIR=target-wsl-fuzz` and `ASAN_OPTIONS=detect_leaks=0`. It reached
`DONE`, libFuzzer reported final corpus `449/33Kb`, found no crash, and the
tracked corpus stayed at four files after the scratch corpus was removed. This
...
source packet flags, and unref clears retained raw bits while resetting the
packet. The pinned libavcodec packet oracle emits the matching
`packet:flags-unknown-*` rows, including `packet:flags-unknown-unref`, so this
fixture is evidence strengthening for the Rust model rather than a status
completion claim.
...
The newest exact-size `av_log_format_line2()` rows prove that when `line_size`
equals the full would-write length, FFmpeg still copies only `line_size - 1`
visible bytes plus the C terminator, clears `print_prefix`, and returns the
full would-write length. The NULL-context row uses `[warning] plain`; the
AVClass-context row normalizes the pointer text to `<ptr>` while keeping the
raw return-length boundary from the pinned 64-bit oracle.
...
generated MOV/AVI default-output, JSON, compact, CSV, flat, INI, and XML
...
visible in the source packet. The offset row proves the same behavior when the
packet data pointer starts inside the shared `AVBufferRef`: FFmpeg preserves
the nonzero data offset and prefix bytes before `AVPacket.data`, then zeroes
the new `pkt->data + size` padding window over the truncated tail. It also
covers callback-owned and default-free
opaque-owner `AVBufferRef` shapes: the callback-owned row proves the owner stays
shared and unreleased while either packet keeps a reference, then releases once
with the tail-zeroed backing bytes after final unref, while the default-free
opaque-owner row proves `av_buffer_get_opaque()` identity is preserved on both
shared refs during the aliasing shrink. Pool-owned rows prove the same
...
active ordinary, offset ordinary, callback-owned, readonly-flagged owned, default-free
...
The harness also includes `packet:array-add-capacity-*` and `packet:array-new-capacity-overflow` rows, proving the standalone `AVPacketSideData` array helpers do not apply the same packet-owned `AV_PKT_DATA_NB` ceiling: `av_packet_side_data_add()` accepts an out-of-range raw type and grows the array from 41 to 42 entries, and `av_packet_side_data_new()` for that same raw type replaces the entry while keeping the count at 42. The newest standalone raw-type rows add `packet:array-add-raw-type-*`, `packet:array-add-raw-type-padding`, `packet:array-remove-raw-type`, `packet:array-get-raw-type-removed`, `packet:array-free-raw-type`, `packet:array-remove-new-raw-type`, `packet:array-get-new-raw-type-removed`, `packet:array-free-new-raw-type`, `packet:array-add-raw-plus-one-type-*`, `packet:array-add-raw-plus-one-type-padding`, `packet:array-remove-raw-plus-one-type`, `packet:array-get-raw-plus-one-type-removed`, `packet:array-free-raw-plus-one-type`, `packet:array-remove-new-raw-plus-one-type`, `packet:array-get-new-raw-plus-one-type-removed`, `packet:array-free-new-raw-plus-one-type`, `packet:array-remove-new-raw-negative-type`, `packet:array-get-new-raw-negative-type-removed`, `packet:array-free-new-raw-negative-type`, `packet:array-get-raw-negative-type-after-min-remove`, `packet:array-remove-raw-negative-type`, `packet:array-get-raw-negative-type-removed`, `packet:array-free-raw-negative-type`, `packet:array-new-raw-type-padding`, `packet:array-new-raw-plus-one-type-padding`, `packet:array-add-raw-negative-type-*`, `packet:array-add-raw-negative-type-padding`, `packet:array-add-raw-min-type-padding`, `packet:array-new-raw-negative-type-padding`, `packet:array-new-raw-min-type-padding`, and `packet:array-new/get/remove-raw-min-type`, proving the same standalone array API accepts raw `AV_PKT_DATA_NB`, `AV_PKT_DATA_NB + 1`, `-1`, and `INT_MIN` values, lookup finds them, raw `AV_PKT_DATA_NB`, `AV_PKT_DATA_NB + 1`, `-1`, and `INT_MIN` add/new-side-data paths preserve or allocate zeroed input padding, transferred and newly allocated raw `AV_PKT_DATA_NB`, raw `AV_PKT_DATA_NB + 1`, transferred raw `-1`, and newly allocated raw `-1` removals make lookup miss and keep the subsequent free state empty, and removing the `INT_MIN` entry preserves the `-1` entry.
...
The latest buffer oracle rows add legacy zero-size custom-pool coverage:
`av_buffer_pool_init(0, alloc)` invokes the legacy allocator once with size
zero, returns writable empty refs with NULL pool opaque lookup, reuses the
empty spare without a second allocation, and releases the empty allocation only
...
`crates/fftools/tests/ffprobe_mov_oracle.rs` is an ignored oracle harness for
bounded generated MOV and AVI `ffprobe` paths. It uses pinned FFmpeg 8.1.1 to
create a two-frame rawvideo MOV fixture with edit lists disabled, then compares
Rust `ffprobe-rs` default output against pinned `ffprobe` for selected shared
fields: format counts/name/duration/size/probe score, stream
codec/tag/dimensions/timing/count fields, and packet codec type, stream index,
PTS/DTS, duration, size, sample-offset `pos`, and packet flags. It is wired
into `tests/differential/mappings.txt` as the shared
`oracle-ffprobe-mov-core-fields` target for the mapped `fftools-ffprobe-*`
rows plus `avutil-packet`. The same harness also creates a two-packet BGR24
AVI fixture and maps `oracle-ffprobe-avi-packet-fields` to
...
`pos`, and key flags. Both rows also compare JSON, compact, CSV, flat, INI,
and XML
...
FFmpeg-shaped `<packet .../>` elements for the bounded packet surface.
...
FFMPEG_ORACLE=./third_party/ffmpeg-oracle/build/bin/ffmpeg cargo test -p fftools --test ffprobe_mov_oracle avi_bgr24_ffprobe_packet_fields_match_ffmpeg_oracle -- --ignored
cargo run -p fate-runner -- run --mappings tests/differential/mappings.txt --component avutil-packet --target oracle-ffprobe-avi-packet-fields --oracle-ffmpeg ./third_party/ffmpeg-oracle/build/bin/ffmpeg
```
