...
`crates/fftools/tests/image2_oracle.rs` is an ignored oracle harness for the constrained image2 file-output and framecrc paths. It compares a valid one-frame PPM input and a two-frame numbered PPM sequence through Rust `-f image2 -framerate 1 ... -f image2 <file-or-pattern>` against pinned FFmpeg 8.1.1 `-c:v copy -f image2`, proving the current bounded Image2 demuxer, Packet path, and Image2 muxer preserve the input bytes for those cases. It also compares normalized two-frame PPM sequence `framecrc` rows against pinned FFmpeg `-c:v copy -f framecrc`, proving the current Image2 demuxer, Packet path, and FrameCrc muxer packet records for that bounded sequence. It is wired into `tests/differential/mappings.txt` as `oracle-image2-file-output`, `oracle-image2-sequence-file-output`, and `oracle-image2-sequence-framecrc-records` for the relevant image2, framecrc, CLI, and packet rows:
