...
The `avutil-rational` nearest-helper slice was checked against FFmpeg 8.1.1 `libavutil/rational.h` and `libavutil/rational.c`. The pinned header documents `av_cmp_q`, `av_nearer_q`, and `av_find_nearest_q_idx`; the pinned implementation treats positive and negative zero-denominator rationals as infinities for comparison, treats `0/0` as indeterminate, and scans a `{0,0}`-terminated nearest list while preserving the first candidate on ties. The Rust model uses explicit slices instead of C sentinels and rejects zero-denominator nearest candidates with typed errors.
