...
The harness currently compares Rust constrained `-f rawvideo ... -f rawvideo <file>` output bytes against pinned FFmpeg `-c:v copy -f rawvideo` output for `rgb24` and `gbrp10msble`. It also compares normalized `-f framecrc -`, `-f framehash -`, and `-f framemd5 -` packet rows for `rgb24`, proving the bounded Rust rawvideo path matches FFmpeg packet stream index, DTS, PTS, duration, payload size, framecrc checksum, SHA-256 framehash digest, and MD5 framemd5 digest while ignoring FFmpeg/Rust header comments. If `FFMPEG_ORACLE` is unset and `third_party/ffmpeg-oracle/build/bin/ffmpeg(.exe)` is absent, the ignored tests fail before comparison instead of silently passing.

The same harness is wired into `tests/differential/mappings.txt`, which can be executed through `fate-runner` with `--mappings tests/differential/mappings.txt --oracle-ffmpeg <path> --component fftools-ffmpeg-rawvideo-file-output` for rawvideo file-copy rows, `--component avformat-framecrc-muxer --target oracle-rawvideo-framecrc-records` for the framecrc record row, `--component avformat-framehash-muxer --target oracle-rawvideo-framehash-records` for framehash, or `--component avformat-framehash-muxer --target oracle-rawvideo-framemd5-records` for framemd5. The current `rgb24`, `gbrp10msble`, framecrc, framehash, and framemd5 rows have passed locally through the generated WSL oracle wrapper.
