...
The harness currently compares Rust constrained `-f rawvideo ... -f rawvideo <file>` output bytes against pinned FFmpeg `-c:v copy -f rawvideo` output for `rgb24` and `gbrp10msble`. It also compares normalized `-f framecrc -` packet rows for `rgb24`, proving the bounded Rust rawvideo path matches FFmpeg packet stream index, PTS, DTS, duration, payload size, and framecrc checksum while ignoring FFmpeg/Rust header comments. If `FFMPEG_ORACLE` is unset and `third_party/ffmpeg-oracle/build/bin/ffmpeg(.exe)` is absent, the ignored tests fail before comparison instead of silently passing.

The same harness is wired into `tests/differential/mappings.txt`, which can be executed through `fate-runner` with `--mappings tests/differential/mappings.txt --oracle-ffmpeg <path> --component fftools-ffmpeg-rawvideo-file-output` for rawvideo file-copy rows, or `--component avformat-framecrc-muxer --target oracle-rawvideo-framecrc-records` for the framecrc record row. The current `rgb24`, `gbrp10msble`, and framecrc record rows have passed locally through the generated WSL oracle wrapper.
