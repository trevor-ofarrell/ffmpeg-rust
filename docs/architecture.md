...
CLI compatibility is treated as a first-class surface. The current implementation supports version banners for `ffmpeg-rs` and `ffprobe-rs` plus an internal parser that groups a small known set of FFmpeg-style options onto global scope, the next `-i` input, or the next output filename. `IoPlan` turns parsed input/output URLs into validated file, pipe, or protocol endpoints for command execution. `ffmpeg-rs` has an initial constrained Rust-native execution path for one local MOV/MP4, PCM s16le RIFF/WAVE, or raw `pcm_s16le` input to stdout `-f null -` or `-f framecrc -`, using `MovDemuxer`, `WavDemuxer`, `PcmS16leDemuxer`, `NullMuxer`, and `FrameCrcMuxer` without FFmpeg runtime linkage. `ffprobe-rs` also has the first Rust-native command execution path for local seekable MOV/MP4 files: it probes with the MOV descriptor, opens the `MovDemuxer`, and renders small `-show_format`, `-show_streams`, and `-show_packets` summaries with default and JSON writers. Unsupported command forms still exit non-zero and are recorded as incomplete in the ledger.
