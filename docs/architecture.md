...
CLI compatibility is treated as a first-class surface. The current implementation supports version banners for `ffmpeg-rs` and `ffprobe-rs` plus an internal parser that groups a small known set of FFmpeg-style options onto global scope, the next `-i` input, or the next output filename. `IoPlan` turns parsed input/output URLs into validated file, pipe, or protocol endpoints for command execution. `ffmpeg-rs` has an initial constrained Rust-native execution path for one local MOV/MP4, PCM s16le RIFF/WAVE, raw `pcm_s16le`, explicit `rawvideo`, yuv4mpegpipe, or explicit image2 single-file/numbered-sequence input to stdout `-f null -` or `-f framecrc -`, using `MovDemuxer`, `WavDemuxer`, `PcmS16leDemuxer`, `RawVideoDemuxer`, `Yuv4MpegDemuxer`, `Image2Demuxer`, `NullMuxer`, and `FrameCrcMuxer` without FFmpeg runtime linkage. It can also packet-copy explicit raw `pcm_s16le` input to a local `-f s16le` file through `PcmS16leMuxer`, explicit raw `pcm_s16le` input to a local `-f wav` file through `WavMuxer`, explicit rawvideo input to a local `-f rawvideo` file through `RawVideoMuxer`, explicit raw `yuv420p` input to a local `-f yuv4mpegpipe` file through `Yuv4MpegMuxer`, and explicit image2 input to local `-f image2` file or numbered-pattern outputs through `Image2Muxer`, all with create-new output semantics. The rawvideo path currently requires explicit `-f rawvideo`, `-pix_fmt`, `-s`, and `-r` input options; yuv4mpegpipe is accepted with explicit `-f yuv4mpegpipe` or YUV4MPEG2/.y4m detection; and image2 currently requires explicit `-f image2` plus `-framerate` or `-r` for a single local image file or contiguous `%d`/`%0Nd` numbered sequence discovered in the pattern's parent directory, with file-scoped `-start_number` support on image2 input and output groups. `ffprobe-rs` also has the first Rust-native command execution path for local seekable MOV/MP4 files: it probes with the MOV descriptor, opens the `MovDemuxer`, and renders small `-show_format`, `-show_streams`, and `-show_packets` summaries with default and JSON writers. Unsupported command forms still exit non-zero and are recorded as incomplete in the ledger.
